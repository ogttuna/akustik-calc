...
export function buildSimpleWorkbenchOutputPosture(input: {
  ...
  if (status === "bound") {
    return {
      detail: "DynEcho is intentionally packaging this as a one-sided support value. It is useful for scoping, but it should not be read as a claimed exact readout.",
      label: "Conservative bound",
      tone: "warning"
    };
  }
