...
describe("simple workbench results panel", () => {
  it("keeps the hero stack vertical inside the narrow workbench results rail", () => {
    ...
    expect(source).toContain('onSelectReviewTab(action.id)');
    expect(source).toContain('{ icon: FileText, id: "proposal", label: "Proposal" }');
    expect(source).toContain("Stack visual");
    expect(source).toContain("Open assembly");
    expect(source).toContain('onSelectReviewTab("diagnostics")');
