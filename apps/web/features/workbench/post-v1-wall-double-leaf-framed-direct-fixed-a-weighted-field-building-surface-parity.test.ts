...
const LAB_ALIAS_OUTPUTS = ["Rw", "STC", "C", "Ctr"] as const satisfies readonly RequestedOutputId[];
...
function buildContextFromSnapshot(snapshot: ScenarioSnapshot): AirborneContext {
  ...
  if (!wallTopology) {
    throw new Error("Expected a saved direct-fixed A-weighted field/building wall topology.");
  }
...
function buildScenario(input: {
  ...
  const scenario = evaluateScenario({
    ...
    calculator: "dynamic",
    id: input.id,
    name: `Direct-fixed A-weighted field/building surface ${input.id}`,
    rows: input.rows ?? TWO_BOARD_ROWS,
    savedAtIso: input.source === "saved" ? "2026-06-11T18:00:00.000Z" : undefined,
    source: input.source ?? "current",
    studyMode: "wall",
    targetOutputs: input.targetOutputs ?? A_WEIGHTED_OUTPUTS
  });
...
function buildCards(
  ...
  outputs: readonly RequestedOutputId[] = A_WEIGHTED_OUTPUTS
...
function buildReport(
  ...
  requestedOutputs: readonly RequestedOutputId[] = A_WEIGHTED_OUTPUTS
): string {
  return composeWorkbenchReport({
    ...
    projectName: "Double-Leaf Direct-Fixed A-Weighted Field Building Surface",
...
function expectDirectFixedAWeightedSurface(
...
async function saveDirectFixedAWeightedSnapshot(): Promise<ScenarioSnapshot> {
  ...
  store.setRequestedOutputs([...A_WEIGHTED_OUTPUTS]);
  ...
  if (!savedSnapshot) {
    throw new Error("Expected the workbench store to save the direct-fixed A-weighted snapshot.");
  }
...
describe("post-V1 wall double-leaf/framed direct-fixed A-weighted field/building surface parity", () => {
  it("shows full absorber field Dn,A/DnT,A on cards, status, report, and calculator API", async () => {
    const scenario = buildScenario({
      airborneContext: withFieldContext(DIRECT_FIXED_BASE_CONTEXT),
      id: "direct-fixed-a-weighted-field-full"
    });
    const result = scenario.result;
    const [dnACard, dnTACard] = buildCards(result);

    expectDirectFixedAWeightedSurface(
      result,
    ...
    const status = getTargetOutputStatus({ guideResult: null, output: "DnT,A", result });
    ...
    expect(report).toContain("- DnT,A estimate: 31.0 dB");
    expect(report).toContain("- Dn,A: Area-normalized");
    expect(report).toContain("Resolver value pins: DnT,A 31, Dn,A 28.9");
  ...
  it("keeps partial absorber and saved/server building replay on the Gate AR A-weighted surface", async () => {
    const partial = buildScenario({
      ...
      id: "direct-fixed-a-weighted-building-partial"
    });
    const [dnACard, dnTACard] = buildCards(partial.result);
    ...
    expect(getGateARAirborneBuildingPredictionSurface(partial.result)?.detail).toContain("Dn,A 26.9 dB");
    expect(getGateARAirborneBuildingPredictionSurface(partial.result)?.detail).toContain("DnT,A 29 dB");
    expect(dnACard).toMatchObject({
    ...
    expect(report).toContain(
      "- Airborne building values: Dn,A 26.9 dB and DnT,A 29 dB; source-absent budget +/-9 dB; not measured building evidence yes."
    );

    const savedSnapshot = await saveDirectFixedAWeightedSnapshot();
    expect(savedSnapshot.requestedOutputs).toEqual([...A_WEIGHTED_OUTPUTS]);
    ...
    const savedScenario = buildScenario({
      ...
      id: "direct-fixed-a-weighted-building-saved",
    ...
    expect(parsedServerSnapshot?.requestedOutputs).toEqual([...A_WEIGHTED_OUTPUTS]);
    ...
    const serverScenario = buildScenario({
      ...
      id: "direct-fixed-a-weighted-building-server",
      rows: parsedServerSnapshot?.rows ?? [],
      source: "saved",
      targetOutputs: parsedServerSnapshot?.requestedOutputs ?? A_WEIGHTED_OUTPUTS
    });
    expectDirectFixedAWeightedSurface(
  ...
  it("keeps empty direct-fixed A-only live while base values and boundary rows stay explicit", async () => {
    const emptyField = buildScenario({
      advancedWallInputSurface: { ...DIRECT_FIXED_FULL_ABSORPTIVE_SURFACE, cavities: [] },
      airborneContext: withFieldContext(DIRECT_FIXED_EMPTY_CONTEXT),
      id: "direct-fixed-a-weighted-field-empty"
    }).result;
    const baseFullField = buildScenario({
      airborneContext: withFieldContext(DIRECT_FIXED_BASE_CONTEXT),
      id: "direct-fixed-a-weighted-base-full",
      targetOutputs: BASE_FIELD_BUILDING_OUTPUTS
    }).result;
    const missingOwnership = buildScenario({
      advancedWallInputSurface: null,
      airborneContext: withFieldContext(DIRECT_FIXED_BASE_CONTEXT),
      id: "direct-fixed-a-weighted-missing-ownership"
    }).result;
    const labAliases = buildScenario({
      airborneContext: withFieldContext(DIRECT_FIXED_BASE_CONTEXT),
      id: "direct-fixed-a-weighted-lab-aliases",
      targetOutputs: LAB_ALIAS_OUTPUTS
    }).result;
    const astm = buildScenario({
      airborneContext: withFieldContext(DIRECT_FIXED_BASE_CONTEXT),
      id: "direct-fixed-a-weighted-astm",
      targetOutputs: ASTM_OUTPUTS
    }).result;
    const impact = buildScenario({
      airborneContext: withFieldContext(DIRECT_FIXED_BASE_CONTEXT),
      id: "direct-fixed-a-weighted-impact",
    ...
    expect(buildCards(emptyField)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ output: "Dn,A", value: "24.9 dB" }),
        expect.objectContaining({ output: "DnT,A", value: "27 dB" })
      ])
    );

    expect(baseFullField.supportedTargetOutputs).toEqual([...BASE_FIELD_BUILDING_OUTPUTS]);
    expect(baseFullField.unsupportedTargetOutputs).toEqual([]);
    ...
    expect(missingOwnership.unsupportedTargetOutputs).toEqual([...A_WEIGHTED_OUTPUTS]);
    ...
    expect(getGateIAirborneFieldContextSurface(missingOwnership)).toBeNull();

    expect(labAliases.supportedTargetOutputs).toEqual([]);
    expect(labAliases.unsupportedTargetOutputs).toEqual([...LAB_ALIAS_OUTPUTS]);
    ...
    const nonDirect = buildScenario({
      ...
      id: "direct-fixed-a-weighted-non-direct"
    }).result;
  ...
  it("keeps docs and current-gate runner aligned with the landed surface parity and selected coverage refresh", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(text, path).toContain("Dn,A 28.9");
      expect(text, path).toContain("DnT,A 31");
      expect(text, path).toContain("Dn,A 26.9");
      expect(text, path).toContain("DnT,A 29");
      expect(text, path).toContain("Dn,A 24.9");
      expect(text, path).toContain("DnT,A 27");
