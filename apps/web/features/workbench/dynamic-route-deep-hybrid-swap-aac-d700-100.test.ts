...
describe("dynamic route deep hybrid AAC-D700-100 swap contracts", () => {
  const readSnapshot = buildSnapshotReader();

  for (const cohort of DEEP_HYBRID_AAC_D700_100_SWAP_SCAN_COHORTS) {
    it(
      `finds no silent >=8 dB adjacent-swap jumps for AAC-D700-100 deep-hybrid route cohort ${cohort.label}`,
      () => {
