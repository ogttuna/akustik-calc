...
const SURFACE_PARITY_ACTION = "post_v1_wall_compatible_anchor_delta_lab_metric_companion_surface_parity_plan";
...
const SELECTED_NEXT_ACTION = "post_v1_wall_compatible_anchor_delta_lab_metric_companion_coverage_refresh_plan";
const SELECTED_NEXT_FILE =
  "packages/engine/src/post-v1-wall-compatible-anchor-delta-lab-metric-companion-coverage-refresh-contract.test.ts";
const SELECTED_NEXT_LABEL = "post-V1 wall compatible anchor-delta lab metric companion coverage refresh";
...
function buildScenario(input: {
  ...
  const scenario = evaluateScenario({
    airborneContext: ELEMENT_LAB_CONTEXT,
    calculator: "dynamic",
    id: input.id,
    name: `Compatible anchor-delta lab companion surface ${input.id}`,
...
function buildScenarioFromSnapshot(snapshot: ScenarioSnapshot, id: string): EvaluatedScenario & { result: AssemblyCalculation } {
  const scenario = evaluateScenario({
    ...
    calculator: "dynamic",
    id,
    name: `Compatible anchor-delta lab companion surface ${id}`,
    rows: snapshot.rows,
...
function buildReport(scenario: EvaluatedScenario): string {
  return composeWorkbenchReport({
    ...
    projectName: "Compatible Anchor-Delta Lab Companion Surface",
...
function expectLabCompanionSurface(result: AssemblyCalculation | null | undefined): asserts result is AssemblyCalculation {
...
async function saveCompleteLabSnapshot(): Promise<ScenarioSnapshot> {
  ...
  store.clearRows();
  store.appendRows(withoutIds(PAIRED_EXTERIOR_BOARD_LSF_ROWS));
  ...
  if (!savedSnapshot) {
    throw new Error("Expected the workbench store to save the compatible lab companion snapshot.");
  }
...
describe("post-V1 wall compatible anchor-delta lab metric companion surface parity", () => {
  it("shows paired exterior-board Rw/STC/C/Ctr lab companions on cards, report, and calculator API", async () => {
    const scenario = buildScenario({ id: "compatible-anchor-lab-companion-live" });
    const result = scenario.result;

    expectLabCompanionSurface(result);
    ...
    expect(report).toContain("- Resolver value pins: Rw 59, STC 59, C -1.1, Ctr -6");
    expect(report).toContain(LAB_COMPANION_WARNING_PREFIX);

    expectLabCompanionSurface(await estimateViaApi());
  });

  it("keeps saved replay and server snapshot replay on the lab companion resolver candidate", async () => {
    const savedSnapshot = await saveCompleteLabSnapshot();
    expect(savedSnapshot.rows).toHaveLength(PAIRED_EXTERIOR_BOARD_LSF_ROWS.length);
    ...
    expectLabCompanionSurface(buildScenarioFromSnapshot(savedSnapshot, "compatible-anchor-lab-companion-saved").result);
    ...
    expectLabCompanionSurface(
      buildScenarioFromSnapshot(parsedServerSnapshot as ScenarioSnapshot, "compatible-anchor-lab-companion-server").result
    );
  });

  it("keeps STC-only and one-side exterior-board requests on the lab companion surface", async () => {
    const stcOnly = buildScenario({
      id: "compatible-anchor-lab-companion-stc-only",
      targetOutputs: STC_ONLY_OUTPUT
    });
    expectStcOnlyLabCompanionSurface(stcOnly.result, 59);
    expect(addOutputCardPosture(buildOutputCard({ output: "STC", result: stcOnly.result, studyMode: "wall" }), {
      result: stcOnly.result,
      studyMode: "wall"
    })).toMatchObject({ status: "live", value: "59 dB" });

    const oneSideMixed = buildScenario({
      id: "compatible-anchor-lab-companion-one-side",
      rows: ONE_SIDE_EXTERIOR_BOARD_LSF_ROWS
    });
    expectOneSideLabCompanionSurface(oneSideMixed.result);

    const stcOnlyApiResult = await estimateViaApi({ targetOutputs: STC_ONLY_OUTPUT });
    expectStcOnlyLabCompanionSurface(stcOnlyApiResult, 59);

    expectOneSideLabCompanionSurface(await estimateViaApi({ rows: ONE_SIDE_EXTERIOR_BOARD_LSF_ROWS }));
  });

  it("keeps docs and the current-gate runner aligned with the landed lab companion surface parity", () => {
