...
const OPEN_WEB_ROWS: readonly LayerDraft[] = [
  { floorRole: "base_structure", id: "open-web", materialId: "open_web_steel_floor", thicknessMm: "300" }
];
...
function buildScenario(input: {
  ...
  const scenario = evaluateScenario({
    calculator: "dynamic",
    id: input.id ?? "open-web-raw-bare-surface",
    name: "Open-web raw-bare surface parity",
  ...
  if (!scenario.result) {
    throw new Error("Open-web raw-bare scenario did not evaluate.");
  }
...
function expectRawBareSurfaceResult(result: AssemblyCalculation | null | undefined): asserts result is AssemblyCalculation {
  ...
  expect(isOpenWebRawBareResult(result)).toBe(true);
}
...
async function saveCompleteScenario(): Promise<ScenarioSnapshot> {
  ...
  if (!savedSnapshot) {
    throw new Error("Expected the workbench store to save the raw-bare open-web snapshot.");
  }
...
describe("open-web raw-bare surface parity", () => {
  it("shows the same source-absent formula basis on route labels, cards, dossiers, and Markdown report", () => {
    ...
    expect(cards.Rw).toMatchObject({
      postureLabel: "Raw-bare open-web formula",
      status: "live",
      value: "32 dB"
    });
    expect(cards.Rw.detail).toContain("Raw-bare open-web steel formula corridor");
    expect(cards.Rw.detail).toContain("UBIQ INEX/firestop");
    ...
    expect(laneKind).toBe("open_web_raw_bare");
    expect(getImpactLanePillLabel(laneKind)).toBe("Raw-bare open-web live");
    expect(getImpactLaneHeadline(laneKind)).toBe("Raw-bare open-web steel formula corridor");
    expect(getImpactLaneNarrative(laneKind, false)).toContain("source-absent raw-bare open-web steel carrier formula");

    expect(formatConfidenceProvenanceForImpact({
      basis: result.impact.basis,
      provenance: result.impact.confidence.provenance
    })).toBe("Raw-bare open-web steel formula corridor");
    ...
    expect(basisRows.map((row) => [row.label, formatImpactMetricBasisLabel(row.basis)])).toEqual([
      ["Ln,w", "Raw-bare open-web steel formula corridor"],
      ["CI", "Raw-bare open-web steel formula corridor"],
      ["CI,50-2500", "Raw-bare open-web steel formula corridor"],
      ["Ln,w+CI", "Raw-bare open-web steel formula corridor"]
    ]);
    expect(basisRows[0]?.description).toContain("source-absent raw-bare open-web steel carrier formula lane");

    const validationPosture = describeImpactValidationPosture(result);
    expect(validationPosture).toMatchObject({
      label: "Raw-bare open-web steel formula corridor",
      posture: "estimate"
    });
    expect(validationPosture.detail).toContain("source-absent raw-bare open-web steel formula corridor");
    ...
    expect(corridorDossier.headline).toContain("Raw-bare open-web steel formula corridor");
    expect(corridorDossier.headline).toContain("Source-absent raw-bare open-web steel formula budgets");
    expect(corridorDossier.cards.find((card) => card.label === "Benchmark mode")).toMatchObject({
      value: "Raw-bare open-web steel formula corridor"
    });
    expect(corridorDossier.cards.find((card) => card.label === "Rw error budget")).toMatchObject({
      detail: expect.stringContaining("raw-bare open-web steel formula budget"),
      value: "+/-9.0 dB"
    });
    ...
    const methodDossier = buildSimpleWorkbenchMethodDossier({
      branchDetail: "Dynamic calculator route is evaluating a raw-bare open-web steel floor.",
      ...
      result,
      stackDetail: "1 live row feeds the raw-bare open-web steel floor route.",
    ...
    expect(report).toContain("- Impact lane: Raw-bare open-web steel formula corridor");
    expect(report).toContain("- Impact basis: Raw-bare open-web steel formula corridor");
    expect(report).toContain("- Impact Ln,w: 96.0 dB");
    expect(report).toContain("- Impact CI: +1.8 dB");
    expect(report).toContain("- Impact CI,50-2500: +5.2 dB");
    expect(report).toContain("- Impact Ln,w+CI: 97.8 dB");
    expect(report).toContain("- Impact error budget Rw: 32.0 dB, range 23.0-41.0 dB, tolerance +/-9.0 dB");
    expect(report).toContain("- Impact error budget Ln,w: 96.0 dB, range 84.0-108.0 dB, tolerance +/-12.0 dB");
    expect(report).toContain("- Estimated family: open-web steel raw-bare");
    expect(report).toContain("- Estimated Rw: 32.0 dB");
    expect(report).toContain("- Estimated Ln,w+CI: 97.8 dB");
    expect(report).toContain("- Companion Rw: 32.0 dB");
    expect(report).toContain(`- Companion basis: ${OPEN_WEB_RAW_BARE_BASIS}`);
    expect(report).toContain("- Implemented formula estimate: yes");
  });

  it("keeps local saved replay and server snapshot replay on the same raw-bare open-web formula basis", async () => {
