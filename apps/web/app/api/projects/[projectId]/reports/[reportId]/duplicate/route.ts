import { ServerProjectDuplicateReportRequestSchema } from "@dynecho/shared";
...
type ProjectReportDuplicateRouteContext = {
  params: Promise<{
    projectId: string;
    reportId: string;
  }>;
...
export async function POST(request: Request, context: ProjectReportDuplicateRouteContext) {
  ...
  const parsed = ServerProjectDuplicateReportRequestSchema.safeParse(rawPayload ?? {});

  if (!parsed.success) {
    return NextResponse.json(
      {
        error: "Invalid project report duplicate payload.",
  ...
  try {
    ...
    const updatedProject = await repository.duplicateReport(owner.scope, projectId, reportId, parsed.data);
    const report = updatedProject.reports[updatedProject.reports.length - 1] ?? null;
