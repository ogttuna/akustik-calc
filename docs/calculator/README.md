...
4. If green, continue the selected slice `blocked_source_backed_widening_rerank_v1` from the landed wall-selector rank-4 hold toward rerank closeout selection.
...
- current rerank progress:
  rank-1 `GDMTXA04A`, rank-2 `C11c`, rank-3 raw bare open-box/open-web, and
  rank-4 wall-selector feasibility are now explicitly held, so the active next
  move is rerank closeout selection rather than another comparison
...
- [blocked-source-rank-4-wall-selector-feasibility-contract.test.ts](../../packages/engine/src/blocked-source-rank-4-wall-selector-feasibility-contract.test.ts): executable rank-4 rerank feasibility hold
