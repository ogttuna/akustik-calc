# Checkpoint - Dynamic Airborne Split v2 Gate B Eleventh Carve

Status: Gate B eleventh carve landed. Validate the carve, then decide
...
bounded recursive carve: `applyLinedMassiveMasonryMonotonicFloor`.

## What Changed

- Moved `applyNarrowHeavyDoubleLeafGapCap` from
...
- Injected `DynamicAirborneComposer` into the moved guard and replaced
  its contact-equivalent sibling probe call with
  `composer(equivalentLayers, ...)`.
...
- Updated `tools/dev/run-calculator-current-gate.ts` and
  `coverage-grid-consistency.test.ts` to track the eleventh-carve
  contract.
...
- Targeted eleventh-carve contract + coverage-grid contract:
...
- continue Gate B by moving `applyLinedMassiveMasonryMonotonicFloor`
  into `dynamic-airborne-correction-guards.ts` with
