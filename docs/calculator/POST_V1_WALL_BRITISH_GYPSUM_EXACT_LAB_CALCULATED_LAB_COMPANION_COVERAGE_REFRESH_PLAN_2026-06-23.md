...
The owner opened calculated `STC`, `C`, and `Ctr` companions beside
exact measured `Rw` for British Gypsum `A046005` and `A046006` mixed
lab requests. The refresh should re-probe those pins and protect the
single-output exact `Rw`, field/building alias, impact, nearby-row,
legacy `auto`, and lab-copy boundaries.
...
Re-probe `A046005` and `A046006` mixed lab requests:

- `A046005`: `Rw 55`, `STC 55`, `C -0.6`, `Ctr -5.4`.
- `A046006`: `Rw 58`, `STC 58`, `C -0.6`, `Ctr -5.4`.

Keep single-output `Rw` on the exact measured route. Keep field,
building, impact, nearby British Gypsum rows, legacy `auto`
exact-source promotion, and lab-to-field/building copying closed.
