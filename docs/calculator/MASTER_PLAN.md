...
| 3 | `wall_hostile_input_matrix_with_airborne_cartography_v1` ⭐ next | Wall hostile-input matrix (floor analog: 50+ layer, unknown material, invalid thickness) + engine thickness cross-cutting guardrail + `dynamic-airborne.ts` cartography for the subsequent split slice |
