...
| 5 | `wall_field_continuation_value_pinning_v1` ⭐ next | Every defended wall corridor × lab/field/building × every field output VALUE pinned (extends slice 2's AAC/masonry/CLT field VALUE pinning to every corridor in `dynamic-airborne-wall-selector-trace-matrix`). Surfaces silent accuracy gaps the way the invariants matrix caught masonry flanking inversion. Unlocks the conditional step 6 formula widening (timber stud accuracy gap lives there). |
