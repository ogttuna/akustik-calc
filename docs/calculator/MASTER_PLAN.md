...
| 4 | `dynamic_airborne_split_refactor_v1` ⭐ next | 6630-line `dynamic-airborne.ts` splits into `-family-detection.ts` + `-predictor-scoring.ts` + `-helpers.ts`. Zero behavior change. Guards: hostile-input + invariants + benchmark suites all green pre-move. Unlocks future wall/floor work velocity; C6 ✓ |
