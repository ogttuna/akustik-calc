...
| 4b | `dynamic_airborne_split_refactor_v2` (conditional) | Refactor the remaining `apply*` floor/cap guards to accept the composer as a function parameter, then mechanically carve them into `dynamic-airborne-floor-guards.ts`. Unlocks the final C6 signal. Plan at the tail of `docs/calculator/DYNAMIC_AIRBORNE_CARTOGRAPHY.md`. Conditional on step 5-6 findings — if primary-objective accuracy work wins ROI, this defers again. |
...
| 7b | `wall_corridor_surface_value_pinning_v1` ⭐ next | Close C2 + C3 🟡 corridor-surface gap — extend VALUE-pin discipline from presets to wall selector families (`double_leaf`, `lined_massive_wall`, `aac_boundary`, `g5_sibling`, `heavy_core_trim`, `lab_double_stud`). ~162 new numerical pins across 6 corridors × 3 contexts × 9 outputs. Re-selection from step-7 post-contract after 2026-04-22 post-step-7 ROI analysis — step 8 audits a fully-green grid instead of a documented-deferred partial. Plan doc [SLICE_WALL_CORRIDOR_SURFACE_VALUE_PINNING_PLAN.md](./SLICE_WALL_CORRIDOR_SURFACE_VALUE_PINNING_PLAN.md) |
| 8 | `good_calculator_final_audit_v1` | All six completion signals in §8 hold; executable grid-consistency test passes; post-calculator productization roadmap opens. Runs on a fully-green grid after 7b closes. |
