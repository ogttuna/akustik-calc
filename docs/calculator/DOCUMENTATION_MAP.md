...
Support work policy: docs, tooling, validation, UI/workbench clarity,
source triage, and cleanup can be valid work when they are bounded,
have an exit condition, and protect or unblock calculator behavior.
They should be recorded as support, not as runtime progress, and should
not become broad source crawling, confidence labeling, cosmetic polish,
or process cleanup that displaces the selected calculator slice.
...
Latest high-ROI system strengthening plan:
`docs/calculator/CALCULATOR_HIGH_ROI_SYSTEM_STRENGTHENING_PLAN_2026-06-29.md`.
This user-requested analysis records why DynEcho still fails closed for
too many arbitrary layer combinations, identifies the reusable
calculator primitives that would increase implementation speed, and
locks the recommended sequence from the current `OITC` owner into a
spectral rating backbone, first-class route/input families, a golden
scenario matrix, route-required input questioning, calibrated
triple-leaf runtime, and building/flanking adapters. The latest
strengthening iteration also adds conditional accelerators for a shared
layer physical-property resolver, realistic cartography gap-ledger
selection, and an explicit roof/facade airborne owner once the selected
route/input prerequisites exist. It is a planning guard only; it does
not replace the current selected implementation action.

Current opening sequence drift lock:
`docs/calculator/CALCULATOR_OPENING_SEQUENCE_DRIFT_LOCK_2026-06-29.md`.
This user-requested guard documents the intended opening order from the
landed route-split boundaries and already-landed `IIC`/`AIIC` exact-band
work into the landed `OITC` owner, its coverage refresh, the landed
post-refresh rerank, the landed spectral rating backbone, the landed
route/input surface, the landed golden matrix, and the landed
route-required question engine. Use it to
reject drift into broad source crawling,
confidence-label work, cosmetic UI passes, or support loops that do not
protect or unlock calculator behavior. It constrains reranks and owner
selection but does not replace the current selected implementation plan.

Current checkpoint reconciliation:
`docs/calculator/CHECKPOINT_2026-06-30_ROUTE_REQUIRED_INPUT_QUESTION_ENGINE_HANDOFF.md`.
It records the docs/implementation comparison at the current stop point:
the route-required input question engine has landed, the current gate is
green, and the selected next value-moving owner is roof airborne formula
ownership after input surface V1. The older OITC owner-ready checkpoint
is historical pre-owner evidence.

Latest industry-grade gap analysis and route selection guard:
`docs/calculator/INDUSTRY_GRADE_CALCULATOR_GAP_ANALYSIS_AND_ROUTE_SELECTION_GUARD_2026-06-25.md`.
This user-requested plan records the post-checkpoint market-bar research,
repo-grounded gap analysis, wrong-path exclusions, and recommended
runtime-first candidate sequence. It supported the now-landed rerank
after the advanced-wall current-gate checkpoint and remains historical
selection evidence; it does not replace or change the current selected
...
rows while keeping unsupported-only requests unsupported. Counters:
...
It follows the OITC spectral rating coverage refresh and selects
candidate `post_v1_spectral_rating_backbone_v1`. Counters:
...
It follows the landed OITC spectral rating owner and re-probes selected
candidate `opening.facade_outdoor_indoor_oitc_spectral_rating_owner`.
Complete outdoor-indoor facade/opening requests with explicit
`one_third_octave_80_4000` transmission-loss curves remain supported
for `OITC` through ASTM E1332 from the owned Gate S area-energy
composite transmission-loss curve. Missing OITC band-set context
remains `needs_input`; legacy `third_octave_100_3150` ISO curves,
indoor partition requests, `Rw`, `STC`, `NISR`/`ISR`, indoor `DnT,w`,
and source-report scalar OITC aliases remain blocked. Counters:
...
and uses selected candidate
`opening.facade_outdoor_indoor_oitc_spectral_rating_owner`. Complete
outdoor-indoor facade/opening requests with explicit
...
source-report scalar OITC aliases remain blocked. Counters:
`newCalculableRequestShapes: 1`, `newCalculableTargetOutputs: 1`,
`requiredPhysicalInputsCaptured: 4`, `runtimeBasisPromotions: 1`,
...
`opening.facade_outdoor_indoor_oitc_spectral_rating_owner`. Counters:
...
/
`post_v1_ceiling_multileaf_airborne_plenum_element_lab_formula_owner_landed_runtime_basis_selected_coverage_refresh`.
It opens `Rw 48`, `STC 48`, `C -1.7`, and `Ctr -6.5` for complete
ceiling-only plenum element-lab context and selects next
`post_v1_ceiling_multileaf_airborne_plenum_element_lab_formula_owner_coverage_refresh_plan`
/
`packages/engine/src/post-v1-ceiling-multileaf-airborne-plenum-element-lab-formula-owner-coverage-refresh-contract.test.ts`
/
...
1. The no-runtime ceiling field/building coverage refresh has landed in
   ...
   `DnT,A 38.9`, and `DnT,A,k 36`; moves `runtimeValuesMoved 0`;
   keeps `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
   `frontendImplementationFilesTouched: 0`; and is not a broad source
   crawl.
2. The follow-up runtime-first rerank after that coverage refresh has
   landed. Action:
   ...
   Candidate: `ceiling.multileaf_airborne_plenum_input_boundary_owner`.
   ...
   crawl.
3. The ceiling multileaf airborne plenum input-boundary owner has
   landed. Action:
   ...
   Method:
   `post_v1_ceiling_multileaf_airborne_plenum_input_boundary_missing_physical_inputs`.
   Counters: `newCalculableRequestShapes: 0`,
   `newCalculableTargetOutputs: 0`, `requiredPhysicalInputsCaptured: 5`,
   `runtimeBasisPromotions: 0`, `runtimeValuesMoved 0`,
   `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`,
   `frontendImplementationFilesTouched: 0`, and
   `unsupportedBoundariesProtected: 7`. This is not a broad source
   crawl.
4. The no-runtime coverage refresh for that owner has landed. Action:
   ...
   It re-probes method
   `post_v1_ceiling_multileaf_airborne_plenum_input_boundary_missing_physical_inputs`
   for `ceilingLeafGrouping`, `ceilingLeafSurfaceMassKgM2`,
   `ceilingCavityOrPlenumDepthMm`,
   `ceilingAbsorberThicknessAndFlowResistivity`, and
   `ceilingSupportCouplingOrHangerClass`. Counters:
   `coverageRefreshContractFilesTouched: 1`,
   `newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
   `requiredPhysicalInputsCaptured: 0`, `runtimeBasisPromotions: 0`,
   `runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
   `sourceRowsImported: 0`, `frontendImplementationFilesTouched: 0`,
   and `unsupportedBoundariesProtected: 0`. This is not a broad source
   crawl.
5. The runtime-first rerank after that coverage refresh has landed.
   Action:
...
6. Current selected next is the ceiling multileaf airborne plenum
   element-lab formula owner. Action:
   `post_v1_ceiling_multileaf_airborne_plenum_element_lab_formula_owner_plan`.
   File:
   `packages/engine/src/post-v1-ceiling-multileaf-airborne-plenum-element-lab-formula-owner-contract.test.ts`.
   Plan:
...
Scope refinement:
the landed ceiling field/building coverage refresh was a support
protection slice with no runtime movement. The landed rerank also moved
no runtime values; it selected a behavior-moving input-boundary owner,
and that owner and its coverage refresh have now landed. The current
selected rerank should choose the next formula, accuracy, input-surface,
or boundary move without broad support drift.
...
and selects candidate
`ceiling.single_leaf_field_building_context_adapter_owner` with
`estimatedNextCalculableRequestShapes: 2`,
`estimatedNextCalculableTargetOutputs: 6`,
...
and `ceiling.single_leaf_airborne_building_prediction_adapter`.
Complete ceiling-only field/building requests now publish field and
building values on route `ceiling`; missing context, impact, ASTM, and
OITC boundaries remain blocked. Counters:
...
This turns the strategic ROI streams into readiness gates, candidate
families, no-go conditions, and step-by-step rerank/owner sequencing for
the current selected runtime-first rerank. It supports the selected next
plan but does not replace it or authorize a runtime owner before the
rerank contract lands.

Route input effectiveness indicator plan:
`docs/calculator/WORKBENCH_ROUTE_INPUT_EFFECTIVENESS_INDICATOR_PLAN_2026-06-19.md`.
This is a user-requested calculator-integrity/UI-clarity plan with its
route, material, and layer-row slices implemented for marking whether
visible workbench route inputs, route-critical material fields, metadata
non-inputs, and layer material/role/thickness controls are used, needed,
inactive, or defaulted for the selected route and output set. It
supports route-required input capture and basis integrity. It does not
replace the selected runtime implementation action.

Latest workbench calculator usability/input-dependency bug hunt:
`docs/calculator/WORKBENCH_CALCULATOR_USABILITY_AND_INPUT_DEPENDENCY_BUG_HUNT_2026-06-22.md`.
This is a user-requested non-runtime support record for route-input
visibility, material-property wording, raw route-id copy leaks, scenario
warning copy, Gate AY/opening-leak surface copy, and report-assistant
missing-input language. It documents UI/report/assistant copy fixes and
focused web validations only. It does not change formula ownership,
...
Latest Workbench V2 route-input validation and needs-input bug hunt:
`docs/calculator/WORKBENCH_V2_ROUTE_INPUT_VALIDATION_AND_NEEDS_INPUT_BUG_HUNT_2026-06-24.md`.
This is a user-requested support record with targeted diagnosis
verification for the route-input states seen on 2026-06-24: `Panel
width` highlighted while paired `Panel height` is not, raw
`toppingOrFloatingLayer` / `impactFieldContext` needs-input copy, and
raw `ratings.field.partitionAreaM2` validation JSON. It records
Workbench/API presentation gaps, confirmed result-shape/error exposure
mechanics, remaining live-capture uncertainties, and safe fix candidates
only. It does not change formulas, runtime values, route ownership,
source rows, `needs_input` / `unsupported` semantics, or the current
selected runtime/coverage-refresh chain.

Latest Workbench V2 route-input validation and needs-input fix plan:
`docs/calculator/WORKBENCH_V2_ROUTE_INPUT_VALIDATION_AND_NEEDS_INPUT_FIX_PLAN_2026-06-24.md`.
This turns the verified bug hunt into the landed scoped support fix. It
separates broad shared fixes from narrow route/surface fixes: shared
missing-input presentation, value-aware area target resolution, stale
ready-result suppression, structured estimate/impact-only error
normalization, and positive field-rating metadata / critical
tiny-geometry handling. The plan records the implementation execution
log, touched code/test surfaces, three rounds of expanded targeted
validation, and the final focused checks: web targeted `7 files / 68
tests`, engine targeted `3 files / 20 tests`, `apps/web` typecheck, and
`git diff --check` passed. It explicitly forbids formula retunes,
source-row imports, schema loosening for zero area, silent defaults,
lab-to-field/building aliases, and hiding `needs_input`. It does not
change formulas, runtime values, route ownership, source rows, or the
current selected runtime/coverage-refresh chain.
...
added for British Gypsum/Gyproc, Knauf, QuietRock, Tecsound, ROCKWOOL,
Stora Enso CLT, H+H Celcon, and REGUPOL materials. It also records the
direct-calculation contract proving those material IDs resolve into
existing dynamic airborne and heavy floating-floor impact routes. This
is not the active selected calculator slice, not a source-row import,
not a broad source crawl, not a formula retune, and not permission to
copy manufacturer acoustic ratings into runtime outputs. It does not
replace the current selected implementation action named at the top of
this file.

Latest user-requested Excel impact-product candidate rescreen handoff:
`docs/calculator/EXCEL_IMPACT_PRODUCT_CANDIDATE_RESCREEN_HANDOFF_2026-06-24.md`.
This records the follow-up Excel review after impact product catalog
matcher hardening. It documents the fail-closed criteria now used for
official impact product rows, the two Getzner AFM29 exact-system rows
opened for `160 mm` reinforced concrete plus `172 kg/m2` and
`197 kg/m2` screed stacks, and the DAMTEC / Isolgomma / DANOSA / Forbo
/ generic candidates that remain parked. It is not a broad import and
does not replace the current selected implementation action named at
the top of this file. Counters:
`impactExactSystemRowsAdded: 2`, `sourceRowsImported: 2`,
`newCalculableRequestShapes: 2`, `newCalculableTargetOutputs: 4`,
`runtimeBasisPromotions: 2`, `runtimeValuesMoved 4`,
`runtimeFormulaRetunes: 0`, and `frontendImplementationFilesTouched: 0`.

Latest checkpoint reconciliation:
`docs/calculator/CHECKPOINT_2026-06-30_ROUTE_REQUIRED_INPUT_QUESTION_ENGINE_HANDOFF.md`.
This records the current docs/implementation comparison after the OITC
owner sequence, spectral backbone, route/input surface, golden matrix,
and route-required input question engine landed. The selected next owner
is the roof airborne formula owner after input surface V1.
...
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, `frontendImplementationFilesTouched: 0`, and
`unsupportedBoundariesProtected: 6`. Selected next:
`post_v1_opening_facade_outdoor_indoor_oitc_metric_schema_and_adapter_bridge_coverage_refresh_plan`
/
`packages/engine/src/post-v1-opening-facade-outdoor-indoor-oitc-metric-schema-and-adapter-bridge-coverage-refresh-contract.test.ts`.
...
It keeps the landed indoor field/building adapter values pinned and
selects the fresh runtime-first rerank. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
`runtimeBasisPromotions: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
...
Complete indoor partition door/window/facade field/building runtime
values are now owned for `field_between_rooms` and
`building_prediction`; `newCalculableTargetOutputs: 6`,
`runtimeValuesMoved 22`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
Outdoor-indoor/OITC, missing frequency input, broad facade prediction,
and impact aliases remain blocked. Selected next:
`post_v1_opening_facade_door_window_indoor_field_building_adapter_coverage_refresh_plan`
/
`packages/engine/src/post-v1-opening-facade-door-window-indoor-field-building-adapter-coverage-refresh-contract.test.ts`.
...
`estimatedNextRuntimeValuesMoved: 22` and no support-only loop.
...
It is not a broad source crawl and records
...
It opens complete indoor door/window/facade field/building lab
companions `Rw 38.2` and `STC 39` while keeping field/building facade,
...
The refresh keeps legacy generic and explicit complete opening/leak lab
`Rw 38.2` and `STC 39` live, keeps
`openingFrequencyBandsOrRatingBasis` as a precise single-field
...
It selects candidate
`opening.facade_door_window_frequency_input_boundary_owner`
as the next current owner. Counters: `candidateCount: 8`,
...
Previous broad checkpoint reconciliation:
...
2026-06-20 `pnpm check`, records the cavity-depth numeric-sensitivity
owner handoff, and keeps the 2026-06-19 physical-input sensitivity
checkpoint as the focused owner-chain validation reference. The owner,
its coverage refresh, the post-refresh rerank, the leaf surface-mass
owner, the leaf surface-mass coverage refresh, and the post-refresh
rerank, the frequency-backbone owner, the frequency-backbone coverage
refresh, the explicit surface-mass leaf scope opener and coverage
refresh, and the single-leaf explicit surface-mass unknown-material
scope opener, coverage refresh, field-context adapter owner,
field-context adapter coverage refresh, building-context adapter owner,
building-context adapter coverage refresh, building `DnT,A,k`
characteristic adapter owner, single-leaf building `DnT,A,k` coverage
refresh, double-leaf/framed building `DnT,A,k` owner and coverage
refresh, compatible anchor-delta building `DnT,A,k` owner and coverage
refresh, and direct-fixed double-leaf/framed context absorptive-cavity
building `DnT,A,k` owner and coverage refresh, local-substitution
building `DnT,A,k` owner and coverage refresh, opening/leak building
`DnT,A,k` owner and coverage refresh, and opening/leak building
apparent `Dn,w` / `Dn,A` companion owner and coverage refresh have now
landed; the runtime-first route-family rerank after that opening/leak
building apparent companion coverage refresh and the current-gate stale
metric/basis reconciliation have now landed too. The British Gypsum
exact lab needs-input boundary fix and the runtime-first rerank after
the current-gate stale metric/basis reconciliation also landed after
that reconciliation. The British Gypsum exact lab field/building
adapter owner and coverage refresh have now landed too. The British
Gypsum exact lab building `DnT,A,k` owner and coverage refresh, and the
British Gypsum exact lab calculated lab companion owner and coverage
refresh, and the runtime-first rerank after that coverage refresh have
now landed too. The wall user-material formula required input-surface
owner and coverage refresh have now landed too. The runtime-first
rerank after that coverage refresh and the wall user-material
double-leaf building leaf-mass boundary owner and coverage refresh have
now landed too. The runtime-first rerank after that coverage refresh
and the wall user-material double-leaf building lab companion
target-output independence owner and coverage refresh have now landed
too. The runtime-first rerank after that coverage refresh, the wall
Gate AR direct-curve building lab companion target-output independence
owner, its coverage refresh, the runtime-first rerank after that
coverage refresh, and the advanced-wall source-absent field/building
lab-companion target-output independence owner and coverage refresh
have now landed too. The current selected next is the fresh
runtime-first rerank after the advanced-wall current-gate checkpoint.
...
Complete built-in gypsum/rockwool/gypsum Gate AR building requests now
keep lab companions `Rw 44`, `STC 44`, `C -1.3`, and `Ctr -6.4`
from the owned direct curve while field/building values stay `R'w 38`,
`Dn,w 39`, `Dn,A 37.8`, `DnT,w 41`, `DnT,A 39.8`, and
`DnT,A,k 36.7`. Counters: `runtimeValuesMoved 40`,
`newCalculableRequestShapes: 3`, `newCalculableTargetOutputs: 3`,
`runtimeBasisPromotions: 3`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
...
Historical selected next at that checkpoint, now landed:
`post_v1_runtime_first_route_family_rerank_after_advanced_wall_current_gate_checkpoint_plan`
/
`packages/engine/src/post-v1-runtime-first-route-family-rerank-after-advanced-wall-current-gate-checkpoint-contract.test.ts`
/
...
It follows the Gate AR post-refresh rerank in the active current chain
and uses selected candidate
`wall.advanced_wall_source_absent_field_building_lab_companion_target_output_independence_owner`.
Complete source-absent advanced-wall field/building contexts now
publish Gate AY lab-only companions `Rw 65`, `STC 65`, `C -1.1`, and
`Ctr -6.4`; mixed requests keep field `R'w 63`, building `DnT,w 66`,
and lab companions basis-separated. Counters:
...
Previous landed runtime/accuracy owner:
...
Complete positive-mass user/project double-leaf/framed building
requests now keep `Rw 46`, `STC 46`, `C -1`, and `Ctr -6.1`
target-output independent while field/building values stay on Gate AR
and support remains request-scoped. Counters:
...
It follows
`post_v1_wall_user_material_double_leaf_building_lab_companion_target_output_independence_owner_landed_runtime_accuracy_selected_coverage_refresh`,
re-probes `wall.user_material_double_leaf_building_lab_companion_target_output_independence_owner`,
and keeps `Rw 46`, `STC 46`, `C -1`, `Ctr -6.1`, `R'w 40`,
`Dn,w 41`, `Dn,A 39.5`, `DnT,w 43`, `DnT,A 41.9`, and
`DnT,A,k 39` pinned with `runtimeValuesMoved 0`. Counters:
`coverageRefreshContractFilesTouched: 1`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
`frontendImplementationFilesTouched: 0`. This is not a broad source
crawl.
...
requests with missing or zero side-leaf mass now stay behind Gate S
`needs_input` before Gate AR can publish false field/building or lab
companion values. Complete positive-mass double-leaf/framed building
requests remain calculable through Gate AR. Counters:
`falseGateARBuildingRequestShapesBlocked: 2`,
`targetOutputsMovedToNeedsInput: 10`,
`requiredPhysicalInputsCaptured: 1`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
`frontendImplementationFilesTouched: 0`.
...
Complete source-absent user-material wall stacks remain calculable
through owned formula routes when the physical inputs are explicit.
...
`wall.user_material_formula_required_input_surface_owner`.
Complete source-absent user-material wall stacks remain calculable
through owned single-leaf and double-leaf/framed formula routes when
the physical inputs are explicit. Zero-mass user/project leaf requests
now park the requested wall airborne outputs behind
`needs_input` for
`layer.surfaceMassKgM2_or_materialCatalog.densityKgM3_and_thicknessMm`
instead of publishing fallback values. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
`newCalculableTargetOutputs: 8`, `requiredPhysicalInputsCaptured: 8`,
`runtimeBasisPromotions: 2`, `runtimeValuesMoved 8`,
`zeroMassFallbackTargetOutputsParked: 9`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
This is not a broad source crawl.
...
Mixed BG exact lab `Rw + STC + C + Ctr` requests now keep exact
measured `Rw` and calculate companion metrics from the selected dynamic
transmission-loss curve and rating adapters. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
...
and uses selected candidate
`wall.british_gypsum_exact_lab_building_dntak_characteristic_adapter_owner`.
The owner opens exact BG `A046005` / `A046006` building `DnT,A,k`
through the exact lab `Rw` direct curve, Gate AR `DnT,A`,
receiving-room volume, partition area with the 7 m2 minimum, and
`T0=0.5 s`. Counters: `newCalculableLayerTemplates: 0`,
...
and uses selected candidate
`wall.british_gypsum_exact_lab_a_weighted_field_building_adapter_owner`.
The owner opens `Dn,A` and `DnT,A` companions for exact BG `A046005` /
`A046006` field/building requests: `A046005` returns `Dn,A 48.0` and
`DnT,A 49.1`; `A046006` returns `Dn,A 51.0` and `DnT,A 52.1`.
...
The refresh protects exact BG `A046005` / `A046006` field/building
`R'w`, `Dn,w`, and `DnT,w` pins while keeping missing context, legacy
`auto` side-count exact promotion, A-weighted `Dn,A` / `DnT,A`,
field/building `Rw` / `STC` / `C` / `Ctr` lab aliases, impact aliases,
nearby British Gypsum rows, and lab-to-field/building copying closed.
Counters: `coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
...
Selected candidate:
`wall.british_gypsum_exact_lab_field_building_adapter_owner`.
The British Gypsum `A046005` / `A046006` exact lab rows now feed
...
returns direct `Rw 55`, `R'w 49`, `Dn,w 49`, and `DnT,w 50`; `A046006`
returns direct `Rw 58`, `R'w 52`, `Dn,w 52`, and `DnT,w 53`. Missing
field/building context, legacy `auto` side-count, A-weighted outputs,
lab aliases, impact aliases, nearby British Gypsum rows, and
lab-to-field/building copying remain closed. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 4`,
`newCalculableTargetOutputs: 12`, `runtimeBasisPromotions: 4`,
`runtimeValuesMoved 12`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, `frontendImplementationFilesTouched: 0`, and
...
Selected candidate:
`wall.british_gypsum_exact_lab_field_building_adapter_owner`.
The rerank selected the British Gypsum `A046005` / `A046006` exact lab
field/building adapter owner as the next runtime move. The rerank itself
did not move runtime values. Counters: `candidateCount: 8`,
...
It keeps the British Gypsum `A046005` / `A046006` explicit exact and
legacy auto-side-count element-lab `Rw`, `STC`, `C`, and `Ctr` outputs
live in mixed lab requests while field/building companions remain
explicit `needs_input` behind grouped topology/context prompts.
...
also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The runtime-first rerank after leaf surface-mass coverage file
...
also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The single-leaf explicit surface-mass unknown-material building-context
adapter owner file
...
also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The single-leaf explicit surface-mass unknown-material building
`DnT,A,k` characteristic adapter owner file
...
also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The single-leaf explicit surface-mass unknown-material building
`DnT,A,k` characteristic adapter coverage-refresh file
...
also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The latest selected double-leaf/framed context-owned porous-cavity
building `DnT,A,k` characteristic adapter owner plan is
...
the selected double-leaf/framed building `DnT,A,k` owner contract file
is
...
It also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The selected double-leaf/framed context-owned porous-cavity building
`DnT,A,k` characteristic adapter coverage refresh plan is
...
the compatible anchor-delta building `DnT,A,k` coverage-refresh
contract file exists and is included in
`tools/dev/run-calculator-current-gate.ts`.
The direct-fixed double-leaf/framed context absorptive-cavity building
`DnT,A,k` characteristic adapter owner plan is
...
the direct-fixed double-leaf/framed context absorptive-cavity building
`DnT,A,k` owner contract file is
...
It also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The direct-fixed double-leaf/framed context absorptive-cavity
building `DnT,A,k` characteristic adapter coverage refresh plan is
`docs/calculator/POST_V1_WALL_DOUBLE_LEAF_FRAMED_DIRECT_FIXED_CONTEXT_ABSORPTIVE_CAVITY_BUILDING_DNTAK_CHARACTERISTIC_ADAPTER_COVERAGE_REFRESH_PLAN_2026-06-22.md`;
the direct-fixed double-leaf/framed context absorptive-cavity building
`DnT,A,k` coverage-refresh contract file exists and is included in
`tools/dev/run-calculator-current-gate.ts`.
The local-substitution building `DnT,A,k` characteristic adapter owner
plan is
`docs/calculator/POST_V1_WALL_LOCAL_SUBSTITUTION_BUILDING_DNTAK_CHARACTERISTIC_ADAPTER_OWNER_PLAN_2026-06-22.md`;
the local-substitution building `DnT,A,k` owner contract file is
`packages/engine/src/post-v1-wall-local-substitution-building-dntak-characteristic-adapter-owner-contract.test.ts`.
It also exists and is included in `tools/dev/run-calculator-current-gate.ts`.
The selected local-substitution building `DnT,A,k` characteristic
adapter coverage refresh plan is
`docs/calculator/POST_V1_WALL_LOCAL_SUBSTITUTION_BUILDING_DNTAK_CHARACTERISTIC_ADAPTER_COVERAGE_REFRESH_PLAN_2026-06-22.md`;
the selected local-substitution building `DnT,A,k` coverage-refresh
contract file does not exist yet.

## Latest Wall Double-Leaf/Framed Direct-Fixed Context Absorptive-Cavity Building DnT,A,k Characteristic Adapter Owner
...
basis, field context, lab context, and impact aliases remain closed.
...
## Latest Wall Compatible Anchor-Delta Building DnT,A,k Characteristic Adapter Coverage Refresh
...
separation, and impact unsupported boundaries.
...
## Latest Wall Compatible Anchor-Delta Building DnT,A,k Characteristic Adapter Owner
...
for the paired exterior-board variant and `DnT,A,k 47.5` for the
one-side exterior-board variant. Resolver trace carries `DnT,A,k`
value pins. Missing volume, missing building output basis,
apparent-only basis, field context, lab context, and impact aliases
remain closed.
...
## Latest Wall Double-Leaf/Framed Context-Owned Porous-Cavity Building DnT,A,k Characteristic Adapter Coverage Refresh
...
## Latest Wall Double-Leaf/Framed Context-Owned Porous-Cavity Building DnT,A,k Characteristic Adapter Owner
...
requests now calculate `DnT,A,k 40.7` from `DnT,A 43.6`. Missing
volume, missing porous-flow/cavity inputs, apparent-only basis, field
context, lab context, and impact aliases remain closed.
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Building DnT,A,k Characteristic Adapter Coverage Refresh
...
The refresh protects project single-leaf and laminated project-panel
`DnT,A,k` pins, mixed building output requests, missing mass, missing
building volume, apparent-only basis, field-context separation, and
impact aliases.
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Building DnT,A,k Characteristic Adapter Owner
...
laminated single-leaf panels with positive `surfaceMassKgM2` now expose
`DnT,A,k` from the owned Gate AR building `DnT,A`, receiving-room volume,
partition area with the 7 m2 minimum, and `T0=0.5 s`. The single project
panel returns `DnT,A 35.2` and `DnT,A,k 32.3`; the laminated project
panel returns `DnT,A 33.6` and `DnT,A,k 30.7`. Missing mass, missing
building inputs, apparent-only output basis, field-context `DnT,A,k`,
and impact aliases remain closed.
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Building-Context Adapter Coverage Refresh
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Building-Context Adapter Owner
...
corridor. The single project panel returns `R'w 30`, `Dn,w 31`,
`Dn,A 32.8`, `DnT,w 33`, and `DnT,A 35.2`; the laminated project
panel returns `R'w 29`, `Dn,w 30`, `Dn,A 31.2`, `DnT,w 32`, and
`DnT,A 33.6`. Missing mass, missing building inputs, cavity/wrong
topology, and impact aliases remain closed.
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Field-Context Adapter Coverage Refresh
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Field-Context Adapter Owner
...
prediction, ASTM aliases, and impact aliases remain closed.

Counters: `accuracyPromotedRequestShapes: 2`,
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Coverage Refresh
...
## Latest Wall Single-Leaf Explicit Surface-Mass Unknown-Material Scope Opener
...
## Latest Wall Double-Leaf/Framed Explicit Surface-Mass Leaf Coverage Refresh
...
## Latest Wall Double-Leaf/Framed Explicit Surface-Mass Leaf Scope Opener
...
catalog density rows for lab and field/building double-leaf/framed
outputs. Counters: `newCalculableLayerTemplates: 1`,
...
## Latest Wall Double-Leaf/Framed Frequency-Backbone Numeric Sensitivity Coverage Refresh
...
## Latest Runtime-First Route-Family Rerank After Wall Double-Leaf/Framed Leaf Surface-Mass Numeric Sensitivity Coverage Refresh
...
The rerank selects calculated frequency-curve backbone tightening for
lab `Rw`, `STC`, `C`, and `Ctr` over the owned Gate S / Gate I / Gate AR
double-leaf/framed route. Counters: `candidateCount: 8`,
...
## Latest Wall Double-Leaf/Framed Frequency-Backbone Numeric Sensitivity Owner
...
derive from that same curve through ISO 717-1 and ASTM E413 adapters;
runtime basis exposes `calculatedFrequencyCurveShape` and the ISO
717-1 `C`/`Ctr` spectrum adaptation adapter. Missing flow resistivity,
...
## Latest Wall Double-Leaf/Framed Leaf Surface-Mass Numeric Sensitivity Coverage Refresh
...
## Latest Wall Double-Leaf/Framed Leaf Surface-Mass Numeric Sensitivity Owner
...
and `49`; the asymmetric `12.5 + 25 mm` variant keeps `leafMassRatio 2`
active; field/building companions move from the same direct curve;
missing side-leaf grouping stays `needs_input`; and impact aliases
remain `unsupported`. Counters: `accuracyPromotedRequestShapes: 3`,
...
## Latest Runtime-First Route-Family Rerank After Wall Double-Leaf/Framed Cavity-Depth Numeric Sensitivity Coverage Refresh
...
accuracy/sensitivity owner because the owned Gate S / Gate I / Gate AR
double-leaf/framed route already uses side A and side B leaf masses,
total leaf mass, leaf mass ratio, and mass-air-mass resonance.
Counters: `candidateCount: 8`, `roiAnalysisIterations: 4`,
...
## Latest Wall Double-Leaf/Framed Cavity-Depth Numeric Sensitivity Coverage Refresh
...
missing/zero/negative depth as `needs_input`, topology-authoritative
mismatch behavior, and `IIC` / `AIIC` as `unsupported`. Counters:
...
same owned Gate S / Gate I / Gate AR double-leaf/framed formula route
when only `advancedWall.cavities[0].depthMm` is supplied. It publishes
advanced-only lab `Rw`/`STC`/`C`/`Ctr` plus field/building `R'w`,
`Dn,w`, `Dn,A`, `DnT,w`, and `DnT,A`; missing or non-positive cavity
depth stays `needs_input`. When topology and advanced depth are both
...
Impact aliases stay `unsupported`. Counters:
...
The refresh protects thickness-sensitive lab pins, field/building
adapter values, no-thickness legacy behavior, missing
...
## Latest Wall Double-Leaf/Framed Porous Absorber Thickness Numeric Sensitivity Owner
...
Previous refresh counters: `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
`frontendImplementationFilesTouched: 0`.
...
preserves legacy behavior. Counters: `accuracyPromotedRequestShapes:
3`, `accuracyPromotedTargetOutputs: 13`, `newCalculableLayerTemplates:
0`, `newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
`runtimeBasisPromotions: 0`, `runtimeValuesMoved 13`,
...
## Latest Wall Double-Leaf/Framed Bridge Support-Spacing Numeric Sensitivity Coverage Refresh
...
The refresh protects support-spacing lab pins, field/building adapter
values, missing `supportSpacingMm` as `needs_input`, impact aliases as
`unsupported`, and direct-fixed double-leaf ownership. Counters:
`coverageRefreshContractFilesTouched: 1`, `newCalculableLayerTemplates:
0`, `newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
`runtimeBasisPromotions: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
...
## Latest Wall Double-Leaf/Framed Bridge Support-Spacing Numeric Sensitivity Owner
...
## Latest Wall Double-Leaf/Framed Porous Absorber Coverage-Ratio Numeric Sensitivity Coverage Refresh
...
The refresh protects ratio `1`, `0.5`, and `0.25` lab pins,
...
aliases as `unsupported`. Counters: `coverageRefreshContractFilesTouched:
1`, `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
...
## Latest Wall Double-Leaf/Framed Porous Absorber Coverage-Ratio Numeric Sensitivity Owner
...
numerically active in the context-owned double-leaf/framed porous
damping route. Ratio `1` keeps `Rw 46`, `STC 46`, `C -1`, `Ctr -6.1`,
...
No numeric ratio preserves legacy enum behavior. Counters:
`accuracyPromotedRequestShapes: 3`, `accuracyPromotedTargetOutputs:
13`, `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes:
...
## Latest Wall Double-Leaf/Framed Porous Flow-Resistivity Numeric Sensitivity Owner
...
and field/building `R'w 39`, `Dn,w 40`, `Dn,A 38.5`, `DnT,w 42`, and
`DnT,A 40.9`. Missing user flow remains `needs_input`. Counters:
...
## Latest Wall Context-Owned Porous-Cavity Field/Building Lab-Companion Basis Integrity Coverage Refresh
...
The refresh protects the context-owned physical-input double-leaf/framed
porous-cavity route without runtime movement. It keeps `Rw 46`,
`STC 46`, `C -1`, `Ctr -6.1`, `R'w 40`, `Dn,w 41`, `Dn,A 39.5`,
`DnT,w 43`, and `DnT,A 41.9` on their owned bases, with missing
flow/topology inputs and impact aliases still outside. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
...
## Latest Wall Context-Owned Porous-Cavity Field/Building Lab-Companion Basis Integrity Owner
...
The owner promotes a context-owned physical-input double-leaf/framed
wall formula variant. With explicit side leaf groups, independent frame
topology, absorptive cavity depth/fill, and
`advancedWall.cavities[].absorberFlowResistivityPaSM2`, field/building
lab companions now come from the owned direct lab curve: `Rw 46`,
`STC 46`, `C -1`, and `Ctr -6.1`. Field/building adapter outputs stay
`R'w 40`, `Dn,w 41`, `Dn,A 39.5`, `DnT,w 43`, and `DnT,A 41.9`.
...
## Latest Project/User Measured Wall Airborne Frequency Field/Building Lab-Companion Target-Output Independence Coverage Refresh
...
building lab-only `Rw`, `STC`, `C`, and `Ctr` companions and the mixed
field/building values without runtime movement. Counters:
`coverageRefreshContractFilesTouched: 1`, `newCalculableLayerTemplates:
0`, `newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
`runtimeBasisPromotions: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
...
## Latest Project/User Measured Wall Airborne Frequency Field/Building Lab-Companion Target-Output Independence Owner
...
The owner makes exact and compatible measured-frequency field/building
lab-only requests publish `Rw`, `STC`, `C`, and `Ctr` from the owned
measured-frequency lab basis. It preserves mixed field/building values
and missing-context/impact boundaries. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 4`,
`newCalculableTargetOutputs: 16`, `runtimeBasisPromotions: 4`,
`runtimeValuesMoved 16`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, `frontendImplementationFilesTouched: 0`, and
...
## Latest Runtime-First Rerank After Project/User Measured Wall Airborne Frequency Field/Building Lab-Companion Basis Integrity Coverage Refresh
...
The rerank selected measured-frequency field/building lab-companion
target-output independence because lab-only field/building target sets
still park lab `Rw`, `STC`, `C`, and `Ctr` even though exact and
compatible measured-frequency curves own those lab ratings and the
field/building context has already been proven by the adapter chain.
Counters: `candidateCount: 8`, `roiAnalysisIterations: 4`,
`estimatedNextCalculableRequestShapes: 4`,
...
## Latest Project/User Measured Wall Airborne Frequency Field/Building Lab-Companion Basis Integrity Coverage Refresh
...
The refresh protects exact and compatible measured-frequency mixed
field/building lab companions and keeps lab-only field/building,
...
## Latest Project/User Measured Wall Airborne Frequency Field/Building Lab-Companion Basis Integrity Owner
...
## Latest Runtime-First Rerank After Wall Advanced-Wall Source-Absent Field/Building Lab-Companion Target-Output Independence
...
The rerank selected measured-frequency field/building lab-companion
basis integrity because exact and compatible project/user measured wall
airborne frequency curves already own lab `Rw`, `STC`, `C`, and `Ctr`,
and the field/building adapter already owns `R'w`, `Dn,w`, `Dn,A`,
`DnT,w`, and `DnT,A`. Mixed field/building requests already publish
`STC`, `C`, and `Ctr`, but still park lab `Rw`. Counters:
`candidateCount: 8`,
`roiAnalysisIterations: 4`, `estimatedNextCalculableRequestShapes: 4`,
`estimatedNextCalculableTargetOutputs: 4`,
...
## Latest Wall Advanced-Wall Source-Absent Field/Building Lab-Companion Target-Output Independence Coverage Refresh
...
The refresh protects target-output independent advanced-wall lab
companions, mixed field/building values, and unsupported boundaries
without runtime movement. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
...
The owner supports field lab-only and building lab-only advanced-wall
requests with Gate AY lab `Rw 65`, `STC 65`, `C -1.1`, and `Ctr -6.4`.
Mixed field `R'w 63` and building `DnT,w 66` remain unchanged.
Counters: `newCalculableLayerTemplates: 0`,
`newCalculableRequestShapes: 2`, `newCalculableTargetOutputs: 8`,
`runtimeBasisPromotions: 2`, `runtimeValuesMoved 8`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`,
...
## Latest Runtime-First Rerank After Wall Advanced-Wall Source-Absent Field/Building Lab-Companion Basis Integrity
...
The rerank selected target-output independence because complete
field/building contexts already own the Gate AY lab companions in mixed
requests, but lab-only target sets still return unsupported. Counters:
`candidateCount: 8`, `roiAnalysisIterations: 4`,
`estimatedNextCalculableRequestShapes: 2`,
`estimatedNextCalculableTargetOutputs: 8`,
...
## Latest Wall Advanced-Wall Source-Absent Field/Building Lab-Companion Basis Integrity Coverage Refresh
...
The refresh protects mixed advanced-wall field/building plus Gate AY lab
companions without runtime movement. Field mixed requests remain
`R'w 63`, `Rw 65`, `STC 65`, `C -1.1`, `Ctr -6.4`; building mixed
requests remain `DnT,w 66`, `Rw 65`, `STC 65`, `C -1.1`, `Ctr -6.4`.
Lab-only field/building requests, missing context, and Gate AY
field/building publishing remain unsupported. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
...
## Latest Wall Advanced-Wall Source-Absent Field/Building Lab-Companion Basis Integrity Owner
...
The owner publishes Gate AY lab companions with field/building
advanced-wall outputs in mixed requests. Field requests support
`R'w 63`, `Rw 65`, `STC 65`, `C -1.1`, and `Ctr -6.4`; building
requests support `DnT,w 66`, `Rw 65`, `STC 65`, `C -1.1`, and
`Ctr -6.4`. Counters: `newCalculableLayerTemplates: 0`,
`newCalculableRequestShapes: 2`, `newCalculableTargetOutputs: 8`,
`runtimeBasisPromotions: 2`, `runtimeValuesMoved 8`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`,
`frontendImplementationFilesTouched: 0`, and
...
## Latest Runtime-First Rerank After Wall Advanced-Wall Source-Absent Field/Building Adapter Coverage Refresh
...
The rerank moved no runtime values. It selected the lab-companion owner
because Gate AY already owns `Rw 65`, `STC 65`, `C -1.1`, and
`Ctr -6.4`, while mixed field/building requests currently leave those
lab companions unsupported. The owner must keep field/building outputs
on Gate I / Gate AR and lab companions on Gate AY. Counters:
`candidateCount: 8`, `roiAnalysisIterations: 4`,
`estimatedNextCalculableRequestShapes: 2`,
`estimatedNextCalculableTargetOutputs: 8`,
`estimatedNextLabCompanionTargetOutputs: 8`,
...
## Latest Wall Advanced-Wall Source-Absent Field/Building Adapter Coverage Refresh
...
The refresh protects the landed advanced-wall field/building adapter
without runtime movement. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
...
The owner moves complete Gate AY advanced-wall source-absent direct
curves through Gate I / Gate AR for `R'w`, `Dn,w`, `Dn,A`, `DnT,w`,
and `DnT,A` only when field/building context is explicit. Values are
...
## Latest Runtime-First Rerank After Project/User Measured Wall Airborne Frequency Field/Building Adapter Coverage Refresh
...
## Latest Project/User Measured Wall Airborne Frequency Field/Building Adapter Coverage Refresh
...
The refresh protects the landed measured-frequency field/building
adapter route without moving runtime values. It freezes exact full-stack
and compatible exterior-board measured-frequency field/building values,
missing-context stops, scalar `Rw` anchor separation, impact boundaries,
and non-board compatible-delta rejection. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
...
## Latest Project/User Measured Wall Airborne Frequency Field/Building Adapter Owner
...
The owner promotes exact full-stack and compatible exterior-board
project/user measured wall airborne frequency curves into Gate I /
Gate AR field-building adapters. It opens `R'w`, `Dn,w`, `Dn,A`,
`DnT,w`, and `DnT,A` for explicit field/building context without
aliasing lab `Rw`, `STC`, `C`, or `Ctr`. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 4`,
...
## Latest Project/User Measured Wall Airborne Frequency Compatible-Delta Coverage Refresh
...
The refresh protects the compatible-delta runtime without moving new
values. It freezes one-side/paired exterior-board deltas, lab
`Rw`/`STC`/`C`/`Ctr` target-output independence, exact precedence,
scalar `Rw` separation, missing standard and ambiguous-anchor
boundaries, field/building/impact outside boundaries, and resolver
trace provenance. Counters: `coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
...
## Latest Runtime-First Rerank After Project/User Measured Wall Airborne Frequency Compatible-Delta Coverage Refresh
...
The rerank selects the bounded field/building adapter owner for exact
and compatible project/user measured wall airborne frequency curves. It
is the current active implementation target because it applies the
already-owned direct TL curve to `R'w`, `Dn,w`, `Dn,A`, `DnT,w`, and
`DnT,A` through Gate I / Gate AR with explicit room, area, and flanking
context. Counters: `candidateCount: 8`, `roiAnalysisIterations: 4`,
`estimatedNextCalculableRequestShapes: 4`,
...
## Latest Project/User Measured Wall Airborne Frequency Compatible-Delta Owner
...
The owner lets an active project/user measured wall airborne frequency
curve anchor a bounded compatible exterior-board delta calculation for
element-lab wall `Rw`, `STC`, `C`, and `Ctr`. It is a calculated
compatible-delta route, not an exact measured override. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 4`,
...
## Latest Runtime-First Rerank After Project/User Measured Wall Airborne Frequency Exact Curve Bridge Coverage Refresh
...
The rerank selected the bounded project/user measured wall airborne
frequency compatible-delta owner. This is the current active
implementation target because it turns exact measured curve evidence
into calculated near-measured layer-combination support for one-side or
paired exterior board additions while preserving exact-vs-compatible
basis separation. Counters: `candidateCount: 8`,
`roiAnalysisIterations: 4`, `estimatedNextCalculableRequestShapes: 4`,
`estimatedNextCalculableTargetOutputs: 4`,
`estimatedNextRequiredPhysicalInputsCaptured: 6`,
`estimatedNextRuntimeBasisPromotions: 1`,
...
## Latest Project/User Measured Wall Airborne Frequency Exact Curve Bridge Coverage Refresh
...
The refresh freezes active `airborneMeasuredFrequencySourceAnchors`,
exact element-lab wall fingerprint matching, measured curve-rated
`Rw`, `STC`, `C`, and `Ctr`, scalar `Rw` anchor separation, missing
rating-standard and ambiguous exact-row boundaries, field/building/
impact outside boundaries, and exact measured resolver trace. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
...
## Latest Project/User Measured Wall Airborne Frequency Exact Curve Bridge Owner
...
The owner bridges active project/user/team measured wall lab airborne
frequency curves into the Dynamic Calculator through
`airborneMeasuredFrequencySourceAnchors`. Exact canonical fingerprint
matches can now publish curve-rated element-lab `Rw`, `STC`, `C`, and
`Ctr` on declared rating-standard bases. Field/building outputs,
impact outputs, scalar `Rw` aliases, compatible deltas, source imports,
formula retunes, and UI/storage remain outside.
...
## Latest Runtime-First Rerank After Project/User Measured Wall Airborne Frequency Anchor Schema + Fingerprint
...
The rerank selected the exact curve bridge owner and moved no runtime
values. Counters: `candidateCount: 8`, `roiAnalysisIterations: 4`,
...
## Latest Project/User Measured Wall Airborne Frequency Anchor Schema + Fingerprint
...
The slice adds shared schema and canonical fingerprints for measured
wall lab airborne transmission-loss curve anchors. It keeps scalar `Rw`
anchors, request intake, runtime bridging, UI/storage, source import,
formula retune, and companion publication outside. Counters:
...
## Latest Runtime-First Rerank After Project/User Measured Wall Rw Exact Bridge Coverage Refresh
...
Runtime method protected by the previous refresh:
`post_v1_project_user_measured_wall_rw_exact_bridge`

Selected candidate:
`project_user_measured_wall_lab_airborne_frequency_curve_anchor_schema_fingerprint`

The rerank selected shared measured wall lab airborne frequency-curve
anchor schema + fingerprint work. It moves no runtime values and exists
to prevent scalar measured `Rw` anchors from being stretched into
unowned companion metrics. Counters: `candidateCount: 8`,
`roiAnalysisIterations: 4`,
...
The refresh moves no runtime values and protects the exact measured
wall lab `Rw` bridge boundaries: active request intake, exact
fingerprint runtime selection, resolver trace ownership, stale/inactive
and duplicate schema rejection, no-match fallback, ambiguous direct
runtime rejection, and no aliasing into `STC`, `C`, `Ctr`,
field/building, impact, or compatible-delta outputs. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
`newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
...
bridges exact active project/user/team measured wall lab `Rw` anchors
to the Dynamic Calculator when the canonical fingerprint exactly
matches the element-lab wall request, and publishes only the measured
`Rw` value. It leaves `STC`, `C`, `Ctr`, field/building outputs, impact
outputs, compatible deltas, UI/storage, source imports, and formula
...
## Latest Runtime-First Rerank After Project/User Measured Wall Rw Anchor Schema + Fingerprint
...
The rerank selected the exact bridge owner because active wall lab `Rw`
anchor schema + fingerprint prerequisites exist, while estimate request
intake and engine runtime matching are not connected yet. Counters:
`candidateCount: 6`, `roiAnalysisIterations: 4`,
...
The slice added the shared measured wall lab `Rw` anchor schema,
active-candidate guard, and canonical fingerprint builder. It did not
change estimate request intake or engine runtime. Counters:
...
## Latest Runtime-First Rerank After Wall Opening/Leak Common-Wall Same-Basis Holdout Packet
...
The rerank kept `acceptedSameBasisHoldoutRows: 0` from the previous
opening/leak common-wall packet, so no opening/leak/common-wall runtime
owner is authorized. It selected the shared schema + canonical
fingerprint prerequisite for project/user measured wall `Rw` anchors.
Counters: `candidateCount: 9`, `roiAnalysisIterations: 4`,
...
The packet reviewed one bounded same-basis evidence packet and kept
`acceptedSameBasisHoldoutRows: 0`. Opening/leak field/building,
A-weighted, and common-wall building values remain benchmark-only
source-absent runtime pins until a rights-safe same-family, same-basis
holdout packet exists. Counters: `evidencePacketsReviewed: 1`,
`localCandidateRowsReviewed: 7`, `benchmarkOnlyRows: 4`,
`rejectedCandidateRows: 3`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
`frontendImplementationFilesTouched: 0`.
...
## Latest Runtime-First Rerank After Wall Timber-Stud + CLT Formula Building Lab-Companion Basis Integrity
...
The rerank applied the high-ROI selection supplement and selected a
no-runtime same-basis holdout packet for opening/leak common-wall
building prediction residuals. It moved no runtime values, imported no
source rows, retuned no formulas, and touched no frontend implementation
files.
...
| --- | --- |
| What landed most recently? | The opening/facade outdoor-indoor OITC spectral rating coverage refresh landed no-runtime and selected its runtime-first rerank. |
| Did runtime values move in the checkpoint? | No for the refresh: `runtimeValuesMoved 0`. The previous landed owner moved `runtimeValuesMoved 1`; complete outdoor-indoor facade/opening requests with explicit `one_third_octave_80_4000` curves calculate `OITC` through ASTM E1332 from the owned Gate S area-energy composite curve. |
| What values are protected? | Ceiling multileaf/plenum values remain pinned; OITC owner values and boundaries are protected by the refresh: complete ASTM E1332 facade/opening requests calculate `OITC`, while missing band context, legacy ISO bands, indoor partition requests, `Rw`/`STC`/`NISR`/`ISR`/indoor `DnT,w`, and scalar source-report OITC aliases remain blocked. |
| Which engine basis is used? | The latest landed runtime basis is `post_v1_opening_facade_outdoor_indoor_oitc_spectral_rating_owner_landed_runtime_basis_selected_coverage_refresh`. |
| What is next? | Implement the no-runtime `post_v1_runtime_first_route_family_rerank_after_opening_facade_outdoor_indoor_oitc_spectral_rating_coverage_refresh_plan`. |
| What should not happen next? | Do not repeat already-landed `IIC` / `AIIC` exact-band work, reopen the ceiling route boundary, broad source crawl, formula retune without same-basis evidence, UI polish pass, copied source scalar, or metric aliasing. |

## Active Route File Set

The table below is retained as historical user-material route-file
orientation from the checkpoint that created it. For the current
selected implementation file, use the top-of-file selected-next section
and `NEXT_AGENT_BRIEF.md`. For the 2026-06-23 material catalog support
files, use
`PUBLIC_SOURCE_MATERIAL_CATALOG_EXPANSION_HANDOFF_2026-06-23.md`.

| Purpose | File |
| --- | --- |
| Current checkpoint | `docs/calculator/CHECKPOINT_2026-06-30_ROUTE_REQUIRED_INPUT_QUESTION_ENGINE_HANDOFF.md` |
| Current selected implementation plan | `docs/calculator/POST_V1_ROOF_AIRBORNE_FORMULA_OWNER_AFTER_INPUT_SURFACE_V1_PLAN_2026-06-30.md` |
| Current selected implementation contract | `packages/engine/src/post-v1-roof-airborne-formula-owner-after-input-surface-v1-contract.test.ts` |
...
The low-density guardrails in this section are historical unless they
match the top-of-file selected-next chain. Current guardrails are in
`NEXT_AGENT_BRIEF.md`, `CALCULATOR_SOURCE_OF_TRUTH.md`, and the current
selected British Gypsum exact lab calculated lab companion coverage
refresh plan.
...
7. The selected slice plan doc.
   For the current checkpoint, this is
   `docs/calculator/POST_V1_WALL_BRITISH_GYPSUM_EXACT_LAB_CALCULATED_LAB_COMPANION_COVERAGE_REFRESH_PLAN_2026-06-23.md`.
...
When Workbench V2 route-input validation or needs-input presentation
hardening lands without moving engine behavior, keep these in sync:

- Workbench V2 route-input state, required-task, estimate-error, and
  assistant preview implementation files under
  `apps/web/features/workbench-rebuild/`, `apps/web/features/workbench-v2/`,
  `apps/web/app/api/estimate/`, `apps/web/app/api/impact-only/`, and
  `apps/web/lib/`;
- focused regression tests for paired `partitionAreaM2` panel-dimension
  state, raw missing-input id copy, friendly validation errors,
  route-level exception normalization, client error parsing, and public
  `calculateAssembly` tiny-geometry metadata boundaries;
- a dated support handoff such as
  `WORKBENCH_V2_ROUTE_INPUT_VALIDATION_AND_NEEDS_INPUT_BUG_HUNT_2026-06-24.md`
  plus the landed fix plan
  `WORKBENCH_V2_ROUTE_INPUT_VALIDATION_AND_NEEDS_INPUT_FIX_PLAN_2026-06-24.md`;
- this documentation map with an explicit note that formulas, runtime
  values, source rows, and selected-next ownership did not move.
...
When material catalog expansion lands without changing formula routes,
keep these in sync:

- material seed rows under `packages/catalogs/src/materials/`;
- material alias resolution under `packages/engine/src/material-catalog.ts`;
- catalog seed tests and a direct-calculation engine contract when the
  new products should be immediately calculable;
- a dated support handoff such as
  `PUBLIC_SOURCE_MATERIAL_CATALOG_EXPANSION_HANDOFF_2026-06-23.md`;
- `NEXT_AGENT_BRIEF.md`, `CURRENT_STATE.md`,
  `CALCULATOR_SOURCE_OF_TRUTH.md`, and this documentation map with an
  explicit note that source rows, formula coefficients, and active
  selected-next ownership did not move.

## Current Validation Baseline

Latest opening/leak `DnT,A,k` slice validation:

- targeted owner + previous local-substitution coverage refresh passed:
  `2 files / 10 tests`;
- opening/local neighbor chain passed: `5 files / 26 tests`;
- `git diff --check` passed;
- `pnpm calculator:gate:current` ran with the new owner and previous
  coverage refresh included; those files passed inside the gate, but the
  full current gate still exits non-zero on existing unrelated global
  failures.

Last broad green checkpoint validation before the later dirty-state
failures:
