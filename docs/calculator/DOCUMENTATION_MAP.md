...
Latest landed runtime/surface chain:
...
Selected next implementation file, intentionally still absent:
...
Do not rerank a new high-ROI candidate until this no-runtime coverage
refresh is closed.
...
| --- | --- |
| What landed most recently? | Direct-fixed double-leaf/framed A-weighted field/building surface parity. |
| Did runtime values move in the latest surface slice? | No. Runtime moved in the previous owner; surface parity only kept workbench/API/replay/cards/status/report aligned. |
| What values are pinned? | Empty `Dn,A 24.9` / `DnT,A 27`; full absorptive `Dn,A 28.9` / `DnT,A 31`; partial absorptive `Dn,A 26.9` / `DnT,A 29`. |
| Which engine basis is used? | Field: `gate_i_airborne_field_apparent_context_adapter_runtime`; building: `gate_ar_airborne_building_prediction_all_owner_runtime_corridor`; base curve: `wall.direct_fixed_double_leaf.equivalent_coupled_mass_bridge_loss_owner`. |
| What is next? | Add the direct-fixed A-weighted field/building coverage refresh contract. |
| What should not happen next? | Do not broaden sources, retune formulas, touch frontend implementation, or rerank a new candidate before the coverage refresh is closed. |
...
| Selected next plan | `docs/calculator/POST_V1_WALL_DOUBLE_LEAF_FRAMED_DIRECT_FIXED_A_WEIGHTED_FIELD_BUILDING_COVERAGE_REFRESH_PLAN_2026-06-11.md` |
| Selected next contract | `packages/engine/src/post-v1-wall-double-leaf-framed-direct-fixed-a-weighted-field-building-coverage-refresh-contract.test.ts` |
| Runtime owner evidence | `packages/engine/src/post-v1-wall-double-leaf-framed-direct-fixed-a-weighted-field-building-owner-contract.test.ts` |
...
The selected next contract is the only file in this table expected not
to exist yet.

## Next Slice Guardrails

The coverage refresh should:

- re-probe the empty, full absorptive, and partial absorptive A-weighted
  field/building values;
- assert the Gate I / Gate AR runtime bases and Gate EO direct curve;
- keep missing absorber ownership, Gate AY panelized input,
  non-direct-fixed absorptive stacks, ASTM/IIC/AIIC, impact outputs, and
  lab aliases outside the route;
- update the current gate runner after the new contract is added.

The coverage refresh should not:

- move runtime values;
- import source rows;
- retune formulas;
- touch frontend implementation;
- widen the owner beyond the direct-fixed double-leaf/framed
  A-weighted field/building route.
...
- web: 126 test files, 492 passed, 18 skipped;
