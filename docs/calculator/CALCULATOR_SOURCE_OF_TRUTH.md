...
refreshes, snapshot reconciliations, validation, tooling, source triage,
and docs updates is allowed when it is bounded, has an exit condition,
and selects, unlocks, protects, or records a calculator behavior or a
user-requested support need. Do not count support work as runtime
calculator progress unless it actually moves calculator behavior.
...
`unsupported` boundary whenever the task is behavior-moving. For
support-only work, name the blocker or unlock and the next behavior it
protects. Final handoffs after calculator work must report the progress
ledger: calculator behavior opened, new calculable request shapes, new
target outputs, required inputs captured, runtime/formula values moved,
support work done, and why stopping was necessary when no
...
This user-requested guard documents the intended opening order and the
no-go paths that prevent drift. Treat it as binding for rerank and owner
selection unless a newer source-of-truth record deliberately changes the
sequence.

That checkpoint records the docs/implementation comparison after the
opening/facade OITC owner sequence, spectral rating backbone, route/input
family first-class surface, industry-grade golden scenario matrix, and
route-required input question engine landed. The current selected next
...
`post_v1_route_required_input_question_engine_v1`. It converts typed
route/input completeness blockers into ordered minimum unblocker
question groups without moving runtime values and keeps unsupported-only
OITC/IIC/AIIC/source-proximity requests unsupported. Counters:
...
Current scope note:
the ceiling multileaf airborne plenum input-boundary owner has landed.
It is a calculator input/boundary slice for non-single-leaf ceiling
stacks, not a runtime formula publication. It captures leaf grouping,
leaf surface mass, cavity/plenum depth, absorber properties, and
support/coupling class through method
`post_v1_ceiling_multileaf_airborne_plenum_input_boundary_missing_physical_inputs`.
That owner's no-runtime coverage refresh, follow-up runtime-first
rerank, and the ceiling multileaf airborne plenum element-lab formula
owner and its coverage refresh have landed. Complete explicit ceiling
plenum element-lab context now publishes `Rw 48`, `STC 48`, `C -1.7`,
and `Ctr -6.5` through route `ceiling`. The follow-up runtime-first
rerank has also landed and selected the ceiling multileaf airborne
plenum field/building adapter owner. That owner has now landed runtime
values, its coverage refresh has landed, the runtime-first rerank after
that coverage refresh has landed, the ceiling/roof/suspended-ceiling
route split boundary owner has landed, that owner's coverage refresh
has landed, and the runtime-first rerank after that coverage refresh has
landed no-runtime. The opening/facade outdoor-indoor OITC spectral
rating owner has now landed runtime values, its coverage refresh has
landed, the post-refresh rerank has landed, the spectral rating
backbone V1 support prerequisite has landed, and the route/input family
first-class surface input-surface slice has landed.
...
already-landed ASTM `IIC` / `AIIC` exact-band repeats, source-absent
ASTM estimation, ceiling boundary repeats, calibration without
same-basis evidence, broad source crawling, confidence-label work, and
frontend polish. Counters: `candidateCount: 8`,
...
and re-probes selected candidate
`ceiling.multileaf_airborne_plenum_element_lab_formula_owner`.
Complete explicit ceiling plenum element-lab context stays pinned at
`Rw 48`, `STC 48`, `C -1.7`, and `Ctr -6.5`; incomplete plenum inputs
stay on
`post_v1_ceiling_multileaf_airborne_plenum_input_boundary_missing_physical_inputs`;
field/building, impact, `IIC`/`AIIC`, `OITC`, ASTM aliases,
source-row proximity substitution, and confidence fallback remain
blocked. Counters: `coverageRefreshContractFilesTouched: 1`,
`newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
`requiredPhysicalInputsCaptured: 0`, `runtimeBasisPromotions: 0`,
...
`requiredPhysicalInputsCaptured: 0`, `runtimeBasisPromotions: 1`,
...
`post_v1_ceiling_multileaf_airborne_plenum_input_boundary_missing_physical_inputs`
so ceiling-only board plus plenum/cavity/fill airborne requests stay
route `ceiling` `needs_input` for `ceilingLeafGrouping`,
...
and uses selected candidate
`ceiling.multileaf_airborne_plenum_input_boundary_owner`. It returns
route `ceiling` `needs_input` for ceiling-only board plus
plenum/cavity/fill airborne requests before single-leaf, floor-impact,
field/building, source-row, or confidence-label fallback can publish
values. Counters: `newCalculableRequestShapes: 0`,
...
It re-probes `ceiling.single_leaf_field_building_context_adapter_owner`,
`ceiling.single_leaf_airborne_field_context_adapter`, and
`ceiling.single_leaf_airborne_building_prediction_adapter`. Complete
field requests stay at `R'w 33`, `Dn,w 33`, `Dn,A 36.5`, `DnT,w 36`,
and `DnT,A 38.9`; complete building requests stay at `R'w 33`,
...
remain blocked. This is not a broad source crawl. Selected next:
`post_v1_runtime_first_route_family_rerank_after_ceiling_single_leaf_field_building_context_adapter_coverage_refresh_plan`
/
`packages/engine/src/post-v1-runtime-first-route-family-rerank-after-ceiling-single-leaf-field-building-context-adapter-coverage-refresh-contract.test.ts`
/
...
`Dn,w 33`, `Dn,A 36.5`, `DnT,w 36`, `DnT,A 38.9`, and `DnT,A,k 36`
on route `ceiling`; lab companions remain separate. Missing context,
impact, ASTM, and OITC boundaries remain blocked. Counters:
...
`openingElementTransmissionLossCurve` instead of scalar
`openingElementRwDb`; the area-energy opening transmission-loss curve
runtime returns `Rw 46`, `STC 46`, `C -1.3`, and `Ctr -5.9`.
...
and captures `openingElementRwDb` as the missing physical input for
otherwise complete door/window/facade opening leakage requests. Complete
opening inputs remain unchanged, and missing frequency/impact
boundaries remain separate. Counters: `requiredPhysicalInputsCaptured: 1`,
`newNeedsInputRequestShapes: 3`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
`frontendImplementationFilesTouched: 0`. This is not a broad source
crawl. Selected next:
`post_v1_opening_facade_door_window_acoustic_rating_input_boundary_coverage_refresh_plan`
...
/
`post_v1_runtime_first_route_family_rerank_after_opening_facade_door_window_c_ctr_lab_companion_target_output_independence_coverage_refresh_landed_no_runtime_selected_opening_facade_door_window_acoustic_rating_input_boundary_owner`.
It follows the C/Ctr coverage refresh, selects
`post_v1_opening_facade_door_window_acoustic_rating_input_boundary_owner_plan`,
records `estimatedNextRequiredPhysicalInputsCaptured: 1`, keeps
`runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`, and
`sourceRowsImported: 0`, and has no support-only loop.
...
keeps indoor field/building numeric values and lab companions pinned,
...
and opens complete indoor door/window/facade `field_between_rooms` and
`building_prediction` lab companions: Gate S `Rw 38.2` and Gate AH
`STC 39`. Field/building facade outputs, outdoor-indoor/OITC, missing
frequency input, and impact aliases remain blocked. Counters:
`newCalculableRequestShapes: 4`, `newCalculableTargetOutputs: 2`,
`runtimeBasisPromotions: 4`, `runtimeValuesMoved 8`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
`frontendImplementationFilesTouched: 0`. Selected next:
...
opening/leak building apparent `Dn,w` / `Dn,A` companion owner and
coverage refresh, the post-refresh runtime-first rerank, and the
current-gate stale metric/basis reconciliation, and the British Gypsum
exact lab needs-input boundary fix, and the runtime-first rerank after
the current-gate stale metric/basis reconciliation, and the British
Gypsum exact lab field/building adapter owner and coverage refresh, and
the British Gypsum exact lab A-weighted field/building adapter owner
and coverage refresh, and the British Gypsum exact lab building
`DnT,A,k` characteristic adapter owner and coverage refresh, and the
...
now landed; the wall user-material formula required input-surface owner
and coverage refresh, the runtime-first rerank after that coverage
refresh, and the wall user-material double-leaf building leaf-mass
boundary owner and coverage refresh, the runtime-first rerank after
...
have now landed. The runtime-first rerank after that coverage refresh,
the wall Gate AR direct-curve building lab companion target-output
independence owner, its coverage refresh, the runtime-first rerank
after that coverage refresh, and the advanced-wall source-absent
field/building lab-companion target-output independence owner and
coverage refresh have now landed too. The current selected next is the
...
Complete built-in gypsum/rockwool/gypsum double-leaf/framed building
requests now keep `Rw 44`, `STC 44`, `C -1.3`, and `Ctr -6.4` from
the owned direct curve while field/building values stay `R'w 38`,
...
Previous landed no-runtime coverage refresh:
...
`Dn,A 39.5`, `DnT,w 43`, `DnT,A 41.9`, and `DnT,A,k 39`, and moves
`runtimeValuesMoved 0`. Counters:
`coverageRefreshContractFilesTouched: 1`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
`frontendImplementationFilesTouched: 0`. This is not a broad source
crawl.

Historical selected next at that coverage refresh:
`post_v1_runtime_first_rerank_after_wall_user_material_double_leaf_building_lab_companion_target_output_independence_coverage_refresh_plan`
/
`packages/engine/src/post-v1-runtime-first-rerank-after-wall-user-material-double-leaf-building-lab-companion-target-output-independence-coverage-refresh-contract.test.ts`
/
...
User-requested material catalog expansion support handoff:

`docs/calculator/PUBLIC_SOURCE_MATERIAL_CATALOG_EXPANSION_HANDOFF_2026-06-23.md`

That handoff records public-source product-specific material seed rows
and aliases plus direct-calculation tests for existing dynamic airborne
and heavy floating-floor impact routes. It is valid support work because
it makes more real user-entered material IDs resolve into owned formula
routes, but it does not replace the selected next action. It did not
import measured acoustic source rows, copy manufacturer ratings, retune
formula coefficients, or move runtime values. Treat the rows as
physical material inputs only.
...
calculator gate. The opening/leak building apparent `Dn,w` / `Dn,A`
companion coverage refresh contract file also exists and is in the
current calculator gate. The runtime-first rerank after that coverage
refresh also exists and is in the current calculator gate. The
current-gate stale metric/basis reconciliation contract file also
exists and is in the current calculator gate. The British Gypsum exact
lab needs-input boundary fix contract file also exists and is in the
current calculator gate. The runtime-first rerank after the current-gate
stale metric/basis reconciliation contract file also exists and is in
the current calculator gate. The selected British Gypsum exact lab
field/building adapter owner contract file also exists and is in the
current calculator gate. The selected British Gypsum exact lab
field/building adapter coverage-refresh contract file also exists and
is in the current calculator gate. The British Gypsum exact lab
A-weighted field/building adapter owner contract file also exists and
is in the current calculator gate. The British Gypsum exact lab
A-weighted field/building adapter coverage-refresh contract file also
exists and is in the current calculator gate. The British Gypsum exact
lab building `DnT,A,k` characteristic adapter owner contract file also
exists and is in the current calculator gate. The British Gypsum exact
lab building `DnT,A,k` characteristic adapter coverage refresh file
also exists and is in the current calculator gate. The British Gypsum
exact lab calculated lab companion owner contract file also exists and
is in the current calculator gate. The British Gypsum exact lab
calculated lab companion coverage refresh file also exists and is in
the current calculator gate. The runtime-first rerank after that
coverage refresh also exists and is in the current calculator gate. The
selected wall user-material formula required input-surface owner has
now landed and is in the current calculator gate. The selected wall
user-material formula required input-surface coverage refresh has also
landed and is in the current calculator gate. The runtime-first rerank
after that coverage refresh, the wall user-material double-leaf
building leaf-mass boundary owner, its coverage refresh, the
...
independence owner and coverage refresh, the runtime-first rerank after
that coverage refresh, and the wall Gate AR direct-curve building lab
companion target-output independence owner have also landed and are in
the current calculator gate.
...
Complete source-absent user-material wall formula routes remain
calculable when the physical inputs are explicit; zero-mass
user/project leaf requests remain `needs_input` for
...
`needs_input` before Gate AR can publish false values. Complete
...
requests now keep direct-curve lab companions target-output independent:
`Rw 46`, `STC 46`, `C -1`, and `Ctr -6.1`. Field/building values
remain `R'w 40`, `Dn,w 41`, `Dn,A 39.5`, `DnT,w 43`, `DnT,A 41.9`,
and `DnT,A,k 39`; zero/missing side-leaf mass remains Gate S
`needs_input`, and impact aliases remain unsupported. Counters:
...
through owned formula routes when physical inputs are explicit; zero
surface-mass user/project leaf requests now return `needs_input` for
`layer.surfaceMassKgM2_or_materialCatalog.densityKgM3_and_thicknessMm`
instead of publishing fallback wall airborne values. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
...
measured `Rw` and calculate `STC`, `C`, and `Ctr` companions from the
selected dynamic transmission-loss curve and rating adapters. `A046005`
returns `Rw 55`, `STC 55`, `C -0.6`, and `Ctr -5.4`; `A046006`
returns `Rw 58`, `STC 58`, `C -0.6`, and `Ctr -5.4`. Single-output
`Rw` stays exact, and field/building aliases remain closed. Counters:
`newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
...
and uses selected candidate
`wall.british_gypsum_exact_lab_a_weighted_field_building_adapter_owner`.
The owner opens exact BG `A046005` / `A046006` field/building `Dn,A`
and `DnT,A` companions through the exact lab `Rw` direct curve plus
Gate I / Gate AR. `A046005` now returns `Dn,A 48.0` and `DnT,A 49.1`;
`A046006` now returns `Dn,A 51.0` and `DnT,A 52.1`. Counters:
...
field/building adapter owner as the next runtime slice. The rerank
itself moved `runtimeValuesMoved 0`. Counters: `candidateCount: 8`,
...
`estimatedNextRuntimeBasisPromotions: 4`,
`estimatedNextRuntimeValuesMoved: 12`, `runtimeFormulaRetunes: 0`,
`sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
Selected next:
`post_v1_wall_british_gypsum_exact_lab_field_building_adapter_owner_plan`
/
...
It keeps British Gypsum `A046005` / `A046006` explicit exact and legacy
auto-side-count element-lab `Rw`, `STC`, `C`, and `Ctr` live in mixed lab
requests while field/building companions remain explicit `needs_input`.
Counters:
`newCalculableRequestShapes: 3`, `newCalculableTargetOutputs: 4`,
`runtimeBasisPromotions: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`,
