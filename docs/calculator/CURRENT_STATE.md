...
Document role: short-lived snapshot of what is stable **right now** on
the calculator surface. For product goal and next-slice rules, read
...
Latest floor user-material visible floating load-basis runtime owner:
...
It derives visible floating-floor `loadBasisKgM2` from upper package
surface mass when role-tagged `floor_covering`, `floating_screed`,
optional `upper_fill`, and `resilient_layer` inputs are present. The
pinned custom visible stack derives `77.6 kg/m2`. Low-density custom
concrete now calculates without manual load basis: `Ln,w 64.3`,
`DeltaLw 24.4`, `L'n,w 66.3`, `L'nT,w 63.9`, and `L'nT,50 66.9`.
Heavy custom concrete now calculates without manual load basis:
`Ln,w 50.1`, `DeltaLw 24.4`, `L'n,w 52.1`, `L'nT,w 49.7`, and
`L'nT,50 52.7`. Counters: `newCalculableRequestShapes: 2`,
`newCalculableTargetOutputs: 10`, `runtimeValuesMoved 10`,
...
The owner opens a custom visible low-density concrete floating-floor
stack through existing lightweight-concrete routes: `Rw 53`, `Ln,w
64.3`, `DeltaLw 24.3`, `L'n,w 66.3`, `L'nT,w 63.9`, and `L'nT,50
66.9`. Generic `IIC` / `AIIC` remains unsupported. Counters:
...
with status
`post_v1_next_numeric_coverage_gap_after_floor_user_material_impact_context_field_only_adapter_landed_no_runtime_selected_floor_user_material_low_density_floating_floor_family_owner`.
It ran `roiAnalysisIterations: 4` across `candidateCount 9` and
selected
`floor.user_material_low_density_floating_floor_family_owner`.
...
The owner makes direct field-only custom heavy floating-floor impact
...
Selected candidate:
`floor.user_material_impact_context_field_only_adapter_owner`.
The rerank ran `roiAnalysisIterations: 4` and selected the field-only
adapter owner after confirming mixed custom floor field requests already
calculate from the context-owned lab anchor. Counters:
...
`post_v1_floor_user_material_impact_context_dynamic_stiffness_owner_landed_runtime_selected_coverage_refresh`.
It closes
`floor.user_material_impact_context_dynamic_stiffness_owner` without
moving values. The refresh re-probes the custom heavy floating-floor
stack at `Ln,w 50.3` and `DeltaLw 24.3`, preserves
...
Latest post-flow numeric coverage-gap rerank:
`post_v1_next_numeric_coverage_gap_after_user_material_porous_flow_resistivity_input_plan`
landed in
`packages/engine/src/post-v1-next-numeric-coverage-gap-after-user-material-porous-flow-resistivity-input-contract.test.ts`
with status
...
missing user-supplied or unknown `flowResistivityPaSM2` stays
...
without moving runtime values: explicit custom `panel_leaf /
porous_absorber / panel_leaf` double-leaf/framed stacks still calculate
lab `Rw 46 / STC 46 / C -1 / Ctr -6.1` and field/building
`R'w 40 / Dn,w 41 / Dn,A 39.5 / DnT,w 43 / DnT,A 41.9`, while missing
...
Latest user-material double-leaf route-input rerank:
...
Latest user-material coverage refresh:
...
The coverage refresh for these values is now closed. The current
selected next file is a no-runtime numeric coverage-gap rerank that
should choose the next highest-ROI calculator slice from the remaining
open rows.
...
/
`post_v1_wall_double_leaf_framed_direct_fixed_a_weighted_field_building_owner_landed_runtime_selected_surface_parity`.
Workbench/API/saved replay/server snapshot/cards/status/report now keep
direct-fixed A-only values aligned: empty `Dn,A 24.9` / `DnT,A 27`,
full absorptive `Dn,A 28.9` / `DnT,A 31`, and partial absorptive
`Dn,A 26.9` / `DnT,A 29`. Field uses
`gate_i_airborne_field_apparent_context_adapter_runtime`; building uses
`gate_ar_airborne_building_prediction_all_owner_runtime_corridor`; both
...
Direct-fixed A-only field/building now uses Gate ER: empty
`Dn,A 24.9` / `DnT,A 27`, full absorptive `Dn,A 28.9` / `DnT,A 31`,
and partial absorptive `Dn,A 26.9` / `DnT,A 29`. This is not a broad
...
Selected candidate:
`wall.direct_fixed_double_leaf.a_weighted_field_building_owner`.
It ran `roiAnalysisIterations: 3`, moved no runtime values in the
selection step, and selected
`post_v1_wall_double_leaf_framed_direct_fixed_a_weighted_field_building_owner_plan`
/
`packages/engine/src/post-v1-wall-double-leaf-framed-direct-fixed-a-weighted-field-building-owner-contract.test.ts`
/
...
The selected owner is expected to align A-only `Dn,A` / `DnT,A`
requests with the already-live Gate ER mixed base+A values and correct
empty direct-fixed A-only misrouting. This is not a broad source crawl.
...
as Gate EO base curve. The resolver coverage posture stays
...
and base `wall.direct_fixed_double_leaf.equivalent_coupled_mass_bridge_loss_owner`
aligned. Values remain full `R'w 29`, `Dn,w 30`, `DnT,w 32` and
partial `R'w 27`, `Dn,w 28`, `DnT,w 30`. Counters:
`webSurfaceParityContractFilesTouched: 1`,
`frontendImplementationFilesTouched: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, and `sourceRowsImported: 0`.
Historical selected next action:
...
The owner uses
`wall.direct_fixed_double_leaf.equivalent_coupled_mass_bridge_loss_owner`
with `gate_i_airborne_field_apparent_context_adapter_runtime` and
`gate_ar_airborne_building_prediction_all_owner_runtime_corridor`.
Full absorptive field/building requests now calculate `R'w 29`,
`Dn,w 30`, `DnT,w 32`; partial requests calculate `R'w 27`,
`Dn,w 28`, `DnT,w 30`. Lab pins remain `Rw 35`, `STC 35`, `C -1.2`,
...
`STC 31`, `C -1.2`, `Ctr -5.9`. It freezes the current field/building
gap for full/partial direct-fixed context-owned absorptive cavity
requests. This is not a broad source crawl. Counters:
`coverageRefreshContractFilesTouched: 1`,
`webSurfaceParityContractFilesTouched: 1`,
...
It promotes complete direct-fixed context-owned absorptive cavity lab
requests through
`wall.direct_fixed_double_leaf.equivalent_coupled_mass_bridge_loss_owner`
with selected candidate
`candidate_wall_direct_fixed_double_leaf_bridge_loss_equivalent_coupled_mass`:
...
Current selected value-moving follow-up:
`post_v1_wall_double_leaf_framed_direct_fixed_context_absorptive_cavity_owner_plan`
/
`packages/engine/src/post-v1-wall-double-leaf-framed-direct-fixed-context-absorptive-cavity-owner-contract.test.ts`
/
...
now keep `Rw 46`, `STC 46`, `C -1`, and `Ctr -6.1` aligned through
`layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_formula_corridor`
with selected candidate
`candidate_layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_family_solver`.
This is not a broad source crawl. Counters:
...
/
`post_v1_wall_double_leaf_framed_route_input_runtime_widening_coverage_refresh_landed_no_runtime_selected_context_absorptive_cavity_input_owner`.
The route now calculates context-owned full absorptive cavity assemblies
by mapping explicit absorber input to
`flowResistivitySource=user_supplied`: `Rw 46`, `STC 46`, `C -1`, and
`Ctr -6.1` through
`layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_formula_corridor`
with selected candidate
`candidate_layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_family_solver`.
...
label `post-V1 wall double-leaf/framed context absorptive cavity input
...
- selected-next source plan:
  [POST_V1_WALL_DOUBLE_LEAF_FRAMED_ROUTE_INPUT_RUNTIME_WIDENING_COVERAGE_REFRESH_PLAN_2026-06-11.md](./POST_V1_WALL_DOUBLE_LEAF_FRAMED_ROUTE_INPUT_RUNTIME_WIDENING_COVERAGE_REFRESH_PLAN_2026-06-11.md);
...
The current selected next action is the wall double-leaf/framed
route-input runtime widening coverage refresh:
`post_v1_wall_double_leaf_framed_route_input_runtime_widening_coverage_refresh_plan`.
...
The compatible anchor-delta STC-only lab companion owner has landed
runtime, its surface parity has landed no-runtime, and its coverage
refresh has landed no-runtime. The compatible anchor-delta C/Ctr-only
owner has now landed runtime and opens standalone C-only and Ctr-only
element-lab requests from the already owned mixed lab companion curve;
its surface parity and coverage refresh have now landed no-runtime. The
coverage refresh freezes paired `C -1.1`, paired `Ctr -6`, one-side
...
`webSurfaceParityContractFilesTouched: 1`,
`frontendImplementationFilesTouched: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, and `sourceRowsImported: 0`. High-ROI
declaration for the landed runtime widening: route family
`wall.double_leaf_framed`; target outputs `Rw`, `STC`, `C`, and `Ctr`;
required physical inputs include side leaf groups, side leaf masses,
cavity depth, absorber class/fill and flow resistivity, support topology,
stud/support spacing, resilient side count when needed, and bridge class.
The expected scope movement is more board/panel/cavity/porous-fill
double-leaf/framed walls calculating through the owned formula route
while exact-source precedence, the thick-board Auto boundary,
field/building aliases, ASTM/IIC/AIIC, direct-fixed, grouped
triple-leaf/multicavity, and floor-impact boundaries stay pinned.
The runtime widening itself has now landed:
...
It opens a context-only empty-cavity two-board double-leaf/framed wall:
explicit `cavity1DepthMm 90`, `cavity1FillCoverage empty`,
`cavity1AbsorptionClass none`, side leaf groups, independent support,
and `supportSpacingMm 600` calculate `Rw 42`, `STC 42`, `C -1`, and
`Ctr -6.1` through
`layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_formula_corridor`.
Context-only absorptive cavities without a visible porous-fill layer stay
`needs_input` for `cavity1FillCoverage` and `absorberClass`. Counters:
`newCalculableLayerTemplates: 1`, `newCalculableRequestShapes: 1`,
`newCalculableTargetOutputs: 4`, `runtimeBasisPromotions: 1`,
`frontendImplementationFilesTouched: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, and `sourceRowsImported: 0`.
The surface parity follow-up has now landed:
`post_v1_wall_double_leaf_framed_route_input_runtime_widening_surface_parity_plan`
in
...
with status
`post_v1_wall_double_leaf_framed_route_input_runtime_widening_surface_parity_landed_no_runtime_selected_coverage_refresh`.
It preserves live calculation, calculator API, saved replay, server
snapshot replay, output cards, target-output status, and report summaries
for `Rw 42`, `STC 42`, `C -1`, and `Ctr -6.1`, while missing cavity
classification stays `needs_input` for `cavity1FillCoverage` and
`absorberClass`. Counters: `webSurfaceParityContractFilesTouched: 1`,
...
owner/coverage refresh are landed records, not active replacements for
the selected route-input runtime widening coverage refresh.
...
one-side exterior-board building `Dn,A 48` without moving runtime values,
importing source rows, or retuning formulas. It keeps lab, STC-only,
missing `buildingPredictionOutputBasis`, non-selected-anchor, and
ASTM/IIC/AIIC boundaries pinned. Counters:
...
This is the current value-moving follow-up.
...
`STC 57` for Knauf `416889` compatible anchor-delta element-lab requests
through
`wall.compatible_anchor_delta.calculated_lab_companions` with runtime
basis
`post_v1_wall_compatible_anchor_delta_calculated_lab_companion_runtime`.
It is value-moving and does not claim measured STC evidence, import
source rows, or retune formulas. Direct `Rw`, mixed `Rw+STC/C/Ctr`,
field/building, A-weighted, C/Ctr-only, ASTM/IIC/AIIC, missing-input,
and non-Knauf boundaries remain pinned. Counters:
...
This selected next is value-moving.
...
measured/reference residual bar. Related broad-accuracy planning context
is
...
Latest landed no-runtime targeted evidence action:
...
from the current ledger and selected the heavy-core / lined-massive
calibration owner Gate EW:
`post_v1_wall_heavy_core_lined_massive_calibration_owner_gate_ew_plan`
in
...
after two ROI plan iterations (`roiAnalysisIterations: 2`). It selected
the current coverage/accuracy gap ledger after subtracting the closed
Gate ET boundary, the thick-board safety guard, Gate ER, Gate EL, Gate
EJ, Gate DK, blocked formula retunes, and non-goal source/confidence/
frontend work. Counters: `candidateCount 10`,
`estimatedNextGapLedgers 1`, `estimatedNextBoundaryLedgers 2`,
`estimatedNextRuntimeCandidateFamiliesToEvaluate 6`,
`newCalculableLayerTemplates 0`, `newCalculableRequestShapes 0`,
`runtimeBasisPromotions 0`, `runtimeFormulaRetunes 0`,
...
keeps `Ln,w` / `DeltaLw` parked as `needs_input` with exactly
`resilientLayerDynamicStiffnessMNm3` and `loadBasisKgM2`; explicit
partial predictor input still requires `loadBasisKgM2` and
`ceilingOrLowerAssembly`, and complete explicit input still calculates
...
EU contract file named above has since landed, and Gate EU selected the
Gate EV current coverage/accuracy gap ledger. Gate EV and Gate EW have
since landed; Gate EW landed owner rejected and selected Gate EX. Gate
EX has since landed and selected Gate EY targeted evidence acquisition.
...
runner, and the latest full current gate remains green with the counts
above.
Previous landed no-runtime numeric coverage/accuracy rerank:
...
complete bounded direct-fixed double-leaf field/building requests. The
representative `gypsum_board 12.5 / air_gap 45 / gypsum_board 12.5`
stack now calculates `R'w 23 / Dn,w 24 / DnT,w 27` for both complete
`field_between_rooms` and complete `building_prediction` contexts.
Missing `receivingRoomRt60S` and missing `supportSpacingMm` remain
`needs_input`; lab `Rw`/STC/`C`/`Ctr` remains Gate EO. Counters:
...
`wall.direct_fixed_double_leaf.building_prediction_adapter_owner`, pins
the Gate EO direct separating-element curve as the bounded field/building
base, and keeps lab `Rw`/STC/`C`/`Ctr` separate from
`R'w`/`Dn,w`/`DnT,w`. It moves no runtime values. Counters:
...
after two ROI plan iterations (`roiAnalysisIterations: 2`). Gate EK
rejects repeating the now-closed ASTM exact-band input surface, closed
floor formula-route gates, already-live open-web/open-box
...
scope gap. Explicitly ASTM-labelled exact one-third-octave user/import
impact bands now carry `standardMethod` through shared estimate,
shared impact-only, API forwarding, and the workbench exact-band import
helper. Lab `ASTM E492 / ASTM E989` bands calculate owned `IIC`; field
`ASTM E1007 / ASTM E989` bands calculate owned `AIIC`. ISO import
defaults remain ISO, missing/ambiguous methods and lab/field metric
mismatch remain stopped, and ISO `Ln,w`, `DeltaLw`, `CI`, or
`CI,50-2500` still do not alias to ASTM ratings. Counters:
...
after two ROI plan iterations (`roiAnalysisIterations: 2`). Gate EH
proved the ASTM exact-band owner, shared/API payloads already carry
...
turning ISO bands or formula-derived ISO impact values into ASTM
ratings. Gate EI rejects closed formula-route repeats, already-live
...
confidence/frontend polish. Counters: `candidateCount 10`,
...
E1007/E989 exact one-third-octave sources own `AIIC`, and ISO bands or
missing/ambiguous standard methods do not publish ASTM ratings. Gate EH
moves no runtime values, retunes no formulas, imports no source rows,
and touches no frontend implementation. Counters:
...
after two ROI plan iterations (`roiAnalysisIterations: 2`). The engine
already owns ASTM E492/E1007 exact one-third-octave `IIC` / `AIIC`
calculation through the ASTM E989 owner, and shared/API schemas already
carry `exactImpactSource` with `standardMethod`; the remaining gap is
the input-owner boundary before user/import surfaces can safely promote
ASTM bands. Gate EG rejects open-web/open-box field-building repeats as
already live, lower-treatment `DeltaLw` derivations as wrong metric,
residual holdout tightening as blocked, broad source crawling, and
frontend polish. Counters: `candidateCount 12`,
...
after two ROI plan iterations (`roiAnalysisIterations: 2`). Gate CY
already owns the composite-panel published-interaction family, and the
visible suspended-ceiling-only stack currently resolves to
...
Gate EF must reconcile that route boundary: either pin the owned
published-family route or add a guard if the current owner is
...
source crawling, and frontend polish as lower ROI now. Counters:
...
floating/topping layer, board schedule, cavity depth, or cavity fill
remains `needs_input`; adjacent `furring_channel`,
...
source rows, and touches no frontend implementation; it keeps adjacent
already-live lower-treatment corridors unchanged, missing physical
inputs as `needs_input`, the generic reinforced-concrete
low-confidence fallback closed, wrong-metric lower-treatment `DeltaLw`
subtraction rejected, and ASTM `IIC` / `AIIC` unsupported. Counters:
...
after two ROI plan iterations (`roiAnalysisIterations: 2`). The
selected gap is a visible heavy-concrete combined upper/lower stack with
...
The five accepted TUAS same-source/same-carrier upper-only CLT treated
rows now keep exact measured `Ln,w` and calculate the companion
...
after two ROI plan iterations (`roiAnalysisIterations: 2`). The live
recheck kept held-AAC without `supportTopology` as `needs_input`, kept
explicit CI / `CI,50-2500` / `Ln,w+CI` / `L'nT,50` as already
...
Gate DZ must first prove a bounded same-family mass-timber CLT
...
gap without moving numeric values. The exact source
...
output `Rw` stays exact, field/building outputs are not relabelled from
lab `Rw`, and no frontend implementation was touched. Counters:
`newCalculableLayerTemplates 1`, `newCalculableRequestShapes 1`,
`newCalculableTargetOutputs: 3`, `runtimeBasisPromotions 1`,
...
two ROI plan iterations (`roiAnalysisIterations: 2`) and touched no
...
lab request now keeps `Rw 43 / STC 43 / C -1 / Ctr -5.5` and selects
...
field/building outputs remain on their separate route instead of
aliasing lab `Rw`. Counters: `runtimeBasisPromotions: 1`,
...
Gate DS selected `wall.masonry_exact_source_mixed_metric_companion_policy_gap`
after two ROI analysis passes (`roiAnalysisIterations: 2`): first it
rejected lower-scope or surface-heavy candidates, then it rejected any
plan that would promote the full mixed answer to exact source, drop
STC/C/Ctr, or retune from source proximity. Gate DS moved no runtime
values and touched no frontend implementation. Counters:
`candidateCount 10`, `estimatedNextRuntimeBasisPromotions 1`,
`estimatedNextRuntimeCorrectedLayerTemplates 1`,
...
Gate DR closes the Gate DQ selected exact-source zero-delta runtime-basis
repair without moving numeric values. The generated `wall-masonry-brick`
single-output `Rw` request keeps `Rw 43` but now selects
`measured_exact_full_stack` through
`verified_airborne_catalog_exact_match` using exact source
...
`screening_fallback`. Mixed `Rw/STC/C/Ctr` masonry requests are now
closed by Gate DT; field/building outputs are not relabelled from lab
...
Gate DQ selects `wall.exact_source_zero_delta_single_output_basis_gap`
as the highest-ROI answer-order accuracy repair after Gate DP. The
...
basis stayed `screening_fallback`. Gate DQ moved no runtime values,
...
Gate DO selects `wall.clt_laminated_leaf_runtime_basis_gap` as the
highest-ROI engine-only formula-routing accuracy step after Gate DN.
The generated `wall-clt-local` CLT + gypsum wall stack already
calculates lab `Rw 42 / STC 43 / C -1.1 / Ctr -7.1` and field
...
laminated single-leaf topology, keep numeric pins frozen, keep field
metrics on the Gate I adapter, and leave Dataholz floor-system rows,
WoodWorks/NRC STC/FSTC/ASTC context, ordinary gypsum laminated leaves,
NLT, double-leaf CLT, timber-stud, LSF, lined massive, and grouped
multicavity routes outside this repair. Counters:
...
without retuning values. The direct wood-stud double-board lab route now
reports `bounded_prediction` through
...
`R'w` / `DnT,w`. Live pins remain `Rw 50 / STC 50 / C 0.5 /
...
without retuning values. Selected owner id:
`wall.timber_stud.bounded_rule_owner_contract`. The owner envelope is
direct `wood_stud`, line-connected, 600 mm stud spacing,
board-dominant double-board gypsum leaves, one 100 mm cavity/core with
50 mm mineral fill, and the current
`stud_surrogate_blend + framed_wall_calibration` coefficient corridor.
Exact single-board rows, resilient-bar exact rows, the secondary direct
double-board benchmark, steel-framed holdouts,
`light_steel_stud_or_resilient_bar`, split double-stud, grouped
multicavity, CLT/mass-timber, and field/building aliases remain outside
this owner. Live pins remain `Rw 50 / STC 50 / C 0.5 / Ctr -4.2` and
field `R'w 42 / Dn,w 42 / DnT,w 43 / DnT,A 43.9`. Gate DM selects
...
Gate DL selects `wall.timber_stud_formula_bounded_rule_owner_gap` as the
highest-ROI engine-only accuracy owner step after Gate DK. The live
timber-stud wall stack already calculates through
`stud_surrogate_blend+framed_wall_calibration` with `Rw 50` and field
`R'w 42 / Dn,w 42 / DnT,w 43 / DnT,A 43.9`, but existing timber source
...
the bounded timber-stud formula owner with coefficient scope, tolerance,
and negative boundaries before any runtime values move. Gate DL moves no
runtime values and selects
...
`IIC` / `AIIC`. Gate DK does not retune the steel formula, does not
add a source-row pass, and does not touch frontend UI implementation.
...
Gate DJ selects `floor.steel_visible_formula_input_surface_parity_gap`
as the next highest-ROI calculator slice after Gate DI. The formula
route is already live inside `calculateAssembly`; Gate DK must make the
same visible steel/open-web route surface-reachable with the same steel
owner inputs, not retune the steel mass-spring formula, not borrow
bound-only rows, and not touch frontend implementation in Gate DJ.
Wall-held AAC/multicavity is already calculable when `supportTopology`
is supplied, exact ASTM E492/E1007 bands already calculate `IIC` /
`AIIC`, opening/leak holdout tightening still needs holdouts, and
heavy-core direct retune still needs a source row or bounded coefficient
rule. Gate DJ moved no runtime values. Counters: `candidateCount 10`,
...
next engine-only scope slice. Current visible steel/open-web generated
rows keep `Ln,w` live but leave `DeltaLw` unsupported; Gate DI must
bridge complete steel owner inputs into the existing mass-spring formula
instead of borrowing bound-only `Ln,w` rows, family archetypes, or ASTM
aliases. Required inputs are `steelSupportForm`, `steelCarrierDepthMm`,
`steelCarrierSpacingMm`, `toppingOrFloatingLayer`,
`resilientLayerDynamicStiffnessMNm3`, `loadBasisKgM2`, and
`lowerCeilingIsolationSupportForm`. Missing owner inputs remain
`needs_input`, ISO `DeltaLw` does not alias to ASTM `IIC` / `AIIC`, and
Gate DH moved no runtime values. Counters: `candidateCount 13`,
...
Gate DB keeps visible lightweight-concrete `Rw 53 / Ln,w 64.3` and now
calculates `DeltaLw 24.9` through
`predictor_lightweight_concrete_delta_lw_dynamic_improvement_estimate`
when `loadBasisKgM2` and `resilientLayerDynamicStiffnessMNm3` are
present. Complete low-density predictor input keeps
`Rw 53 / Ln,w 64.3` and also calculates `DeltaLw 24.9`. Missing
`loadBasisKgM2` or
`resilientLayerDynamicStiffnessMNm3` remains `needs_input`, Gate M's
non-dynamic low-density predictor remains `Rw 49 / Ln,w 47`, and ASTM
`IIC` / `AIIC` remain unsupported. Gate DB selected
...
value pins, deep-hybrid guards, and floor-only concrete ceiling rows are
not calibration holdouts. Gate DC moved no runtime values and selected
...
Gate DD pins the live `wall-screening-concrete` route as source-absent
screening behavior instead of retuning it: lab `Rw 57 / STC 57 /
C -1.6 / Ctr -6.5`, and field `R'w 55 / Dn,w 55 / DnT,w 56 /
DnT,A 54.9 / C -1.6 / Ctr -6.3`. Exact verified wall source and
lab-fallback matches remain absent. Knauf CC60 concrete floor rows,
manufacturer lining context, selector pins, deep-hybrid guards,
workbench concrete presets, and ISO/Sharp/Davy framework context remain
non-promotable until a wall-specific same-stack source row or a named
bounded lined-massive wall rule exists. Gate DD moved no runtime values
and selected `post_v1_next_numeric_coverage_gap_gate_de_plan` in
...
It closes the Gate CN visible-layer `DeltaLw` routing gap: visible
timber joist upper-package floors keep exact `Ln,w 51` and now calculate
`DeltaLw 25.2`; visible CLT upper-package floors keep published-family
`Ln,w 50` and now calculate `DeltaLw 22.6`. With explicit
`impactFieldContext`, timber also calculates `L'n,w 53 / L'nT,w 50.6 /
L'nT,50 53.6`, and CLT calculates `L'n,w 52 / L'nT,w 49.6 /
L'nT,50 52.6`. Missing `loadBasisKgM2` or
`resilientLayerDynamicStiffnessMNm3` remains `needs_input`, and ASTM
`IIC` / `AIIC` remain unsupported without their own owner. Counters:
...
0`. Gate CO selected next action:
...
highest-ROI next engine slice. The current runtime gap is visible
heavy-floating reinforced-concrete floors with lower ceiling treatment:
they calculate airborne `Rw` / `Ctr`, but requested `Ln,w` and
`DeltaLw` stop behind the combined upper/lower formula's
`loadBasisKgM2` `needs_input` guard even though the published
upper-treatment family already owns an `Ln,w` anchor for elastic and
rigid gypsum-ceiling variants. Gate CQ must keep that `Ln,w` anchor and
field impact companions live for two common visible lower-treatment
templates, while `DeltaLw` still requires `loadBasisKgM2` and
`resilientLayerDynamicStiffnessMNm3`. Complete physical inputs must stay
on the heavy combined upper/lower formula corridor, and ASTM `IIC` /
`AIIC` remain unsupported. Gate CP moved no runtime values. Counters:
`candidateCount 8`, `newCalculableLayerTemplates 0`,
`newCalculableRequestShapes 0`, `runtimeValuesMoved 0`,
`estimatedNextNewCalculableLayerTemplates 2`, and
`estimatedNextNewCalculableRequestShapes 10`. Gate CP selected next
action:
...
It keeps the published-family `Ln,w` anchor live for visible
heavy-floating reinforced-concrete lower-treatment stacks. The
`acoustic_hanger_ceiling` template now calculates `Ln,w 43`; with
explicit `impactFieldContext` it also calculates `L'n,w 45 /
L'nT,w 42.6 / L'nT,50 46.6`. The `resilient_stud_ceiling` template now
...
inputs stay on the heavy combined upper/lower formula corridor, and ASTM
`IIC` / `AIIC` remain unsupported. Counters:
`newCalculableLayerTemplates 2`, `newCalculableRequestShapes 10`, and
`runtimeCorrectedRequestShapes 8`. Gate CQ selected next action:
...
highest-ROI engine-only formula-routing slice. Gate CQ closed the common
floating lower-treatment anchor gap, ASTM `IIC` / `AIIC` user-band input
still needs shared/frontend input surfaces, and opening/leak/common-wall
budget tightening remains blocked by Gate CL holdout requirements. The
selected Gate CS runtime gap is explicit `flat_layer_order` wall entry:
Gate CJ proved support-owned common flat double-leaf
`building_prediction` requests can use the Gate S double-leaf/framed
direct curve through the Gate AR building adapter, but safe explicit
flat-order requests remain blocked by the older ambiguity guard. Gate CS
must admit only safely segmentable flat-entry wall stacks with required
support/stud owner inputs, keep missing `supportTopology`, missing
`studSpacingMm`, and missing `resilientBarSideCount` as `needs_input`,
keep ambiguous multicavity flat lists blocked, and keep lab metrics
separate from field/building metrics. Gate CR moved no runtime values
and touched no frontend implementation. Counters: `candidateCount 9`,
`newCalculableLayerTemplates 0`, `newCalculableRequestShapes 0`,
`runtimeValuesMoved 0`, `estimatedNextRuntimeCorrectedLayerTemplates 5`,
and `estimatedNextRuntimeCorrectedRequestShapes 25`. Gate CR selected
next action:
...
Gate CS has landed as the previous runtime scope/correctness slice:
`post_v1_wall_common_auto_topology_second_pass_gate_cs_plan`
with status
`post_v1_wall_common_auto_topology_second_pass_gate_cs_landed_runtime_selected_next_numeric_coverage_gap_gate_ct`.
It closes the Gate CR selected wall flat-entry routing gap. Safe
...
`runtimeCorrectedRequestShapes 25`. Gate CS selected next action:
...
It selected `wall.flat_layer_order_multicavity_grouped_owner_gap` as
the highest-ROI engine-only formula-routing slice. The selected runtime
...
ambiguity guard. Gate CT moved no runtime values and touched no frontend
implementation. Counters: `candidateCount 10`,
`newCalculableLayerTemplates 0`, `newCalculableRequestShapes 0`,
`runtimeValuesMoved 0`, `estimatedNextNewCalculableLayerTemplates 1`,
and `estimatedNextNewCalculableRequestShapes 14`. Gate CT selected next
action:
...
Gate CU has now landed as a previous runtime scope/correctness slice:
`post_v1_wall_flat_layer_order_multicavity_gate_cu_plan`
with status
`post_v1_wall_flat_layer_order_multicavity_gate_cu_landed_runtime_selected_next_numeric_coverage_gap_gate_cv`.
It closes the Gate CT selected formula-routing gap. Safe explicit
...
now calculate through the existing Gate AE multicavity lab formula, Gate
I field adapter, and Gate AR building adapter. Lab requests publish
`Rw 53 / STC 57 / C -0.6 / Ctr -8`; field and building requests publish
`R'w 53 / Dn,w 53 / Dn,A 52 / DnT,w 54 / DnT,A 53.5`. Missing
`supportTopology` remains `needs_input`, contradictory explicit grouped
indices on `flat_layer_order` remain blocked, field/building requests
still require their own room/flanking context, and lab metrics are not
...
and `runtimeCorrectedRequestShapes 14`. Gate CU selected next action:
...
It selects `wall.local_substitution_building_prediction_adapter_gap` as
the highest-ROI engine-only formula/adapter-routing candidate. Lab
`Rw/STC/C/Ctr` and field `R'w/DnT,w` are already owned for the
local-substitution triple-leaf wall family, but complete
`building_prediction` context still stops as unsupported. Gate CV moved
no runtime values, touched no frontend implementation, and selects next
action:
...
It closes the Gate CV selected local-substitution building adapter gap.
Complete local Rockwool / MLV / plaster grouped triple-leaf wall requests
with explicit `building_prediction` flanking, junction, room, panel,
RT60, and output-basis inputs now publish `R'w 51 / Dn,w 51 /
Dn,A 52.4 / DnT,w 53 / DnT,A 53.9` from the local-substitution lab
curve plus the building adapter, with a `+/-11 dB` source-absent budget.
Missing building physical inputs remain `needs_input`, exact
same-stack field/building source rows still win, and lab `Rw/STC/C/Ctr`
plus field `R'w/DnT,w` remain separate owners. Counters:
...
5`. Gate CW selected next action:
...
without `DeltaLw` before Gate DB. Gate DB now owns the separate
lightweight-concrete runtime corridor. Counters:
`newCalculableLayerTemplates 0`,
...
Gate CI landed as an earlier no-runtime selection/rerank:
`post_v1_next_numeric_coverage_gap_gate_ci_plan`
with status
`post_v1_next_numeric_coverage_gap_gate_ci_landed_no_runtime_selected_wall_common_auto_topology_expansion_gate_cj`.
It preserves the exact ASTM E492/E1007 `IIC` / `AIIC` owner, proves
four representative ISO impact routes keep ASTM aliases unsupported, and
selects `wall.common_auto_topology_expansion` as the next value-moving
engine slice. Gate CI selected next action:
...
Gate CJ landed as an earlier runtime scope/correctness slice:
`post_v1_wall_common_auto_topology_expansion_gate_cj_plan`
with status
`post_v1_wall_common_auto_topology_expansion_gate_cj_landed_runtime_selected_opening_leak_composite_wall_adapters_gate_ck`.
It corrects common flat double-leaf wall `building_prediction` routing:
complete support-owned flat double-leaf, multi-board, split air/porous
cavity, asymmetric, and resilient both-sides stacks now use the Gate S
double-leaf/framed direct curve inside the Gate AR building adapter
instead of generic building fallback or unsupported explicit-topology
behavior. Missing `supportTopology`, missing `studSpacingMm`, explicit
...
25`. Gate CJ selected next action:
...
Gate CK landed as an earlier runtime scope/correctness slice:
`post_v1_opening_leak_composite_wall_adapters_gate_ck_plan`
with status
`post_v1_opening_leak_composite_wall_adapters_gate_ck_landed_runtime_selected_next_numeric_coverage_gap_gate_cl`.
It corrects explicit top-level opening/leak wall field and
building-prediction requests: when `openingLeakElements`,
`hostWallAreaM2`, and the required room/flanking terms are present, the
engine now uses the owned Gate S opening/leak lab composite anchor plus
the company-internal field/building area-energy adapters without a
hidden adapter-boundary flag. Complete field opening/leak context
publishes `R'w 36.4 / Dn,w 36.7 / DnT,w 36.9`; complete
building-prediction opening/leak context publishes `R'w 31.6 /
DnT,w 32.1`. Missing room/flanking terms remain `needs_input`,
`Dn,A` / `DnT,A` still require `frequencyBandSet`, and lab `Rw` / STC
do not alias into field/building outputs. Counters:
`newCalculableLayerTemplates 2`, `newCalculableRequestShapes 5`,
`runtimeCorrectedLayerTemplates 2`, and `runtimeCorrectedRequestShapes
5`. Gate CK selected next action:
...
It creates residual and holdout ledgers for five already-owned runtime
families: common flat double-leaf building prediction, opening/leak
field/building, open-box raw-bare lab impact, open-web raw-bare lab
impact, and heavy floating upper-treatment field companion. No runtime
values moved. All five ledgers hold the existing wider error budgets
because same-family calibration and same-basis holdout evidence is not
yet sufficient for tightening; nearby source rows are explicitly blocked
from source-proximity runtime promotion. Counters: `residualLedgers 5`,
`budgetsHeldWide 5`, `budgetsTightened 0`,
`newCalculableLayerTemplates 0`, `newCalculableRequestShapes 0`, and
`runtimePromotionsFromSourceProximity 0`. Gate CK and Gate CJ numeric
pins remain frozen. Gate CL selected next action:
...
It pins selected-route required physical input and `needs_input`
...
`DeltaLw`, and Gate CH direct/flanking field-impact `L'nT,50`. No
runtime values moved, no formulas were retuned, no frontend
implementation was touched, and no source rows were crawled. Counters:
...
Gate CN landed as the previous no-runtime numeric coverage
selection:
`post_v1_next_numeric_coverage_gap_gate_cn_plan`
with status
`post_v1_next_numeric_coverage_gap_gate_cn_landed_no_runtime_selected_floor_visible_layer_upper_package_delta_lw_gate_co`.
It selected
`floor.visible_layer_upper_package_delta_lw_formula_routing_gap` because
explicit predictor input already makes the timber/CLT `DeltaLw` formula
owners calculate, while the same physically explicit visible tagged
layer stacks still publish only `Ln,w`. Gate CN moved no runtime values,
touched no frontend implementation, selected no source crawl, and did
not select confidence wording. Counters: `candidateCount 7`,
`newCalculableLayerTemplates 0`, `newCalculableRequestShapes 0`,
`runtimeValuesMoved 0`, `estimatedNextNewCalculableLayerTemplates 2`,
and `estimatedNextNewCalculableRequestShapes 10`. Gate CN selected next
action:
`post_v1_floor_visible_layer_upper_package_delta_lw_gate_co_plan`.
...
Complete dry package-transfer and EPS/screed hybrid building/impact
requests now support already-owned single-output asks without requiring
the user to also ask for `R'w` or another building output. Dry
single-output pins include `Rw 66`, `C -3.9`, `Ln,w 50.8`,
`L'n,w 52.8`, `L'nT,w 50.4`, and `L'nT,50 53.7`; EPS/screed pins
include `Rw 72`, `C -1.3`, `Ln,w 47`, `L'n,w 49`, `L'nT,w 46.6`,
and `L'nT,50 47.6`. Missing `impactFieldContext` still leaves the
field impact outputs unsupported. `Ctr`, ASTM `IIC`, and ASTM `AIIC`
remain unsupported unless their own metric-basis owners exist.
...
Gate CB remains the previous runtime scope/correctness slice:
...
Complete EPS/screed hybrid full mixed building/impact requests publish
...
Gate BZ remains the previous runtime scope/correctness slice:
...
Dry package-transfer full mixed building/impact requests now publish
`Rw 66 / C -3.9`, `R'w 64 / DnT,w 67`,
`Ln,w 50.8 / CI 1.2 / CI,50-2500 3.3 / Ln,w+CI 52`, and
`L'n,w 52.8 / L'nT,w 50.4 / L'nT,50 53.7`. EPS/screed hybrid full
mixed requests now publish `Rw 72 / C -1.3`, `R'w 70 / DnT,w 73`, and
...
Gate BX remains the previous runtime correctness slice:
...
Dry package-transfer mixed lab-impact requests now publish visible
`Rw 66 / C -3.9`; EPS/screed hybrid mixed lab-field-impact requests
publish visible `Rw 72 / C -1.3`. Existing impact pins remain
unchanged, `Ctr` remains unsupported because the package companion is
`Rw+C`, not a true `Ctr`, and ASTM `IIC` / `AIIC` remain unsupported.
...
Gate BV remains the previous runtime slice:
...
`tuas_c11c_visible_iso_weighted_impact_tuple_guarded` with pins
`Ln,w 59`, `CI 1`, `CI,50-2500 1`, `Ln,w+CI 60`, and with K=3 /
V=60 m3, `L'n,w 62`, `L'nT,w 59.2`, `L'nT,50 60.2`. C11c exact import
remains blocked until raw spectrum or source-correction evidence exists,
and `DeltaLw`, `IIC`, and `AIIC` remain unsupported. Gate AY landed as
...
The active Gate AZ planning bridge is
...
because the implementation already had a field adapter and mixed
lab-plus-field values, but assembly field-only lower-treatment requests
were not reaching that numeric lane. The selected next file was
...
with status
`post_v1_floor_suspended_ceiling_lower_treatment_field_companion_gate_bf_landed_selected_next_numeric_coverage_gap_gate_bg`.
This is calculator-capacity runtime work: assembly field-only
lower-treatment now calculates `L'n,w`, `L'nT,w`, and `L'nT,50` from the
same owned lower-treatment lab anchor and field adapter. Acoustic hanger
pins `L'n,w 47.6 / L'nT,w 44.8 / L'nT,50 48.8`; resilient stud pins
`L'n,w 46.6 / L'nT,w 43.8 / L'nT,50 47.8`. Missing
`impactFieldContext.ci50_2500Db` stays a precise `needs_input` boundary
for `L'nT,50`, and ASTM `IIC` / `AIIC` remain unsupported. Gate BF
selects `post_v1_next_numeric_coverage_gap_gate_bg_plan` in
`packages/engine/src/post-v1-next-numeric-coverage-gap-gate-bg-contract.test.ts`.
Gate AX previously landed targeted calculator coverage for complete framed
wall `building_prediction` requests: `wall-lsf-knauf` and
`wall-timber-stud` use explicit framed metadata (`studType`,
`connectionType`, and positive `studSpacingMm`) to stay on the
stud-surrogate/framed-calibration direct-curve owner instead of asking
for unrelated grouped multileaf topology fields. Current LSF pins are
`Rw 51`, STC 51, `C -1.4`, `Ctr -6.4`, `R'w 51`, `Dn,w 51`,
`Dn,A 49.6`, `DnT,w 52`, and `DnT,A 51.1`. Current timber pins are
`Rw 42`, STC 42, `C 0.4`, `Ctr -4.3`, `R'w 42`, `Dn,w 42`,
`Dn,A 42.4`, `DnT,w 43`, and `DnT,A 43.9`. Missing framed metadata,
incomplete building context, opening/leak building requests, and grouped
...
Gate AU lab companions now calculates field companions when explicit K,
receiving-room volume, and `CI,50-2500` inputs are present. Current pins
are `DeltaLw 26`, heavy-reference derived `Ln,w 52`, explicit `CI -1`,
derived `Ln,w+CI 51`, explicit `CI,50-2500 +4`, `L'n,w 54`,
`L'nT,w 52`, and `L'nT,50 56` on
`mixed_predicted_plus_estimated_standardized_field_volume_normalization`.
ASTM aliases remain blocked unless their own owners exist, and
`lowerTreatmentReductionDb` is not treated as another `DeltaLw` term on
this lane.
Previous Gate AU landed targeted calculator coverage for floor explicit
`DeltaLw` lab companions: the explicit heavy-reference `DeltaLw` lane
now also calculates `CI`, `Ln,w+CI`, and `CI,50-2500` from explicit lab
companion inputs.
Previous Gate AT landed targeted calculator coverage for floor explicit
`CI,50-2500` lab companions: lanes with owned live `Ln,w` or
conservative `Ln,w` upper bound and explicit user `CI,50-2500` now
calculate `CI,50-2500` without requiring field K, Hd, receiving-room
volume, small-room toggle, or `CI`. Current live pins are hollow-core
vinyl `Ln,w 48`, `CI,50-2500 +4`; heavy concrete `Ln,w 50`,
`CI,50-2500 +4`; and steel fallback `Ln,w 58`, `CI,50-2500 +4`.
Current bound pins are open-web / UBIQ 300 `Ln,w <= 51`,
`CI,50-2500 +4`; UBIQ 200 `Ln,w <= 53`, `CI,50-2500 +4`; and UBIQ
250 `Ln,w <= 52`, `CI,50-2500 +4`. `CI`, `Ln,w+CI`, field outputs
`L'n,w`, `L'nT,w`, and `L'nT,50`, and ASTM aliases remain blocked unless
their own inputs/owners exist.
Previous Gate AS landed targeted calculator coverage for floor explicit
`CI` lab companions: lanes with owned live `Ln,w` or conservative
`Ln,w` upper bound and explicit user `CI` now calculate `CI` and
`Ln,w+CI` without requiring field K, Hd, receiving-room volume,
small-room toggle, or `CI,50-2500`. Current live pins are hollow-core
vinyl `Ln,w 48`, `CI -1`, `Ln,w+CI 47`; heavy concrete `Ln,w 50`,
`CI -1`, `Ln,w+CI 49`; and steel fallback `Ln,w 58`, `CI -1`,
`Ln,w+CI 57`. Current bound pins are open-web / UBIQ 300
`Ln,w <= 51`, `CI -1`, `Ln,w+CI <= 50`; UBIQ 200 `Ln,w <= 53`,
`Ln,w+CI <= 52`; and UBIQ 250 `Ln,w <= 52`, `Ln,w+CI <= 51`. Field
outputs `L'n,w`, `L'nT,w`, and `L'nT,50` still require their own
physical inputs, and ASTM aliases remain blocked.
Previous Gate AR landed targeted calculator coverage for floor
small-room low-frequency impact output: lanes with explicit Turkish
small-room `L'nT,w` and explicit `CI,50-2500` now calculate
`L'nT,50 = L'nT,w + CI,50-2500`.
Previous Gate AQ landed targeted calculator coverage for combined-bound
floor split outputs: lanes with source-owned `Ln,w+CI` upper bound and
explicit `CI` derive a split `Ln,w` upper bound as
...
local-guide `L'nT,50` upper bound.
Previous Gate AO landed focused calculator coverage for explicit
`CI,50-2500` standardized-field inputs: floor lanes with owned `Ln,w`,
K, receiving-room volume, and explicit `CI,50-2500` calculate
`CI,50-2500`, `L'n,w`, `L'nT,w`, and `L'nT,50`; missing
`CI,50-2500` names `impactFieldContext.ci50_2500Db`, while ASTM
aliases remain unsupported.
The previous landed implementation
checkpoint remains
...
the previous double-leaf coverage-refresh checkpoint remains
...
Current usable V1 status: Steps 0-5 in
[USABLE_V1_EXECUTION_PLAN.md](./USABLE_V1_EXECUTION_PLAN.md) are closed
for the current company-internal usable V1 envelope. Step 1,
candidate-owned answer hardening, is closed with the shared owner-audit
helper on assembly and impact-only paths. Step 2, the Wall V1 Acceptance
Matrix, is closed in
`packages/engine/src/acoustic-calculator-answer-engine-v1-contract.test.ts`
and web parity covers the same wall answer states in
`apps/web/features/workbench/layer-combination-resolver-candidate-surface-parity.test.ts`.
Step 3, the Floor V1 Acceptance Matrix, is closed with matching
engine-side floor acceptance and floor output-card/report/API parity.
Step 4, Surface Parity For The Same Answer, is closed: answer charts,
result summary cards, report live-stack summaries, saved replay, server
snapshot replay, and resolver candidate surfaces stay scoped to the
selected answer and keep stopped outputs visible. Step 5, the
Company-Internal Acceptance Gate, is closed in
`packages/engine/src/acoustic-calculator-company-internal-usable-v1-acceptance-gate-contract.test.ts`
and is included in `pnpm calculator:gate:current`. The current tested
envelope is ready for company-internal usable V1; the next slice must be
selected separately as post-V1 accuracy/adapters or expanded formula
coverage, not as a broad source crawl or finite scenario pack.
...
Gate I selected the existing reinforced-concrete combined upper/lower
floor formula for shared resolver ownership. The runtime already
calculates lab `Ln,w 58.1` / `DeltaLw 13.7` on
`predictor_heavy_combined_upper_lower_floor_iso12354_annexc_estimate`;
Gate J made that answer candidate-owned without aliasing ASTM, field, or
building outputs. The selected candidate is
...
`ceilingOrLowerAssembly` stops as `needs_input`. Gate K maps the
...
stop as `needs_input`, and ASTM/field aliases remain blocked.
...
Latest landed implementation file:
...
Previously landed surface file:
...
Gate 0 proved the chosen path by measurable calculator ROI. Gate A was
no-runtime and set up the generalized wall multileaf formula owner,
required physical inputs, formula terms, negative boundaries, and Gate B
scope. Gate B then increased owned element-lab `Rw`, STC, `C`, and
`Ctr` coverage for complete grouped Rockwool wall multileaf stacks by
mapping runtime basis `triple_leaf_two_cavity_frequency_solver` to
`candidate_post_v1_wall_multileaf_generalized_source_absent_family_solver`.
Gate C made that answer surface consistent across workbench/API/report
and guided inputs, including partial field requests where `R'w` and
`DnT,w` stop on exact missing physical fields while lab value pins stay
live. Gate D then added compatible measured-anchor delta behavior
through named owners: Knauf LSF exact `Rw 55` plus one compatible outer
acoustic board publishes `Rw 57` on
`wall.compatible_anchor_delta.extra_board_on_verified_lsf`, while STC,
`C`, and `Ctr` remain unsupported and unsafe cavity insertions remain
`needs_input`. Gate E selected the next runtime/formula gap by ROI:
ASTM `IIC` / `AIIC` exact-band contour rating. Gate F now calculates
complete ASTM E492 lab bands into `IIC` and complete ASTM E1007 field
bands into `AIIC`, while ISO `Ln,w` rows and incomplete ASTM curves
remain unsupported for ASTM ratings. Gate G made those ratings visible
across output cards, answer chart, Markdown report, `/api/estimate`,
`/api/impact-only`, resolver trace, and metric-basis provenance. Gate H
increased floor formula coverage by mapping existing lightweight-steel
upper/lower mass-spring and suspended-ceiling-only source-absent floor
formulas into the shared resolver and answer-engine surface. Complete
upper/lower steel now publishes `Ln,w 51.6` / `DeltaLw 22.4`;
suspended-only steel publishes `Ln,w 62.2`, while `DeltaLw` without a
complete upper package remains unsupported/needs owner inputs and
ASTM/field aliases remain blocked. Gate J maps the existing
reinforced-concrete combined upper/lower lab-impact formula through the
shared resolver surface. Gate K maps the existing timber-joist and
mass-timber CLT `DeltaLw` formula corridors through the shared resolver
surface. Gate L maps the existing composite-panel published interaction
runtime through
...
`Ln,w 48.5 / Rw 60.6`, while composite `DeltaLw`, ASTM, and field
aliases remain blocked. Gate M maps the lightweight-concrete family path
through `floor.lightweight_concrete.family_solver_owner` on
...
`source_absent_field_building_adapter_error_budget` and publishes
`L'n,w 66.3 / L'nT,w 63.9` for the lightweight-concrete lab anchor when
`impactFieldContext.fieldKDb` and
`impactFieldContext.receivingRoomVolumeM3` are present; missing context
asks for only `impactFieldContext` and `receivingRoomVolumeM3`.
...
with label post-V1 input-surface guided physical fields Gate O. Latest Gate N
validation is green: full `pnpm calculator:gate:current` passed on
2026-05-25 with engine 525 files / 2969 tests, web 95 files / 402 passed
plus 18 skipped, repo build 5 / 5, and whitespace guard passed.

Gate O was corrected to calculator coverage and landed as
...
The new runtime behavior derives grouped triple-leaf topology from safe
flat wall layer order when both cavity segments expose porous fill plus
air gap. That turns the pinned flat multicavity wall from stopped/manual
grouping into calculated `Rw 53` / STC 57 / `C -0.6` / `Ctr -8`.
Ambiguous no-air-gap flat stacks, explicit `flat_layer_order`, and
invalid duplicate groups remain blocked. Gate O status is
...
It moves flat double-leaf wall stacks with explicit support context and
`studSpacingMm` from manual topology grouping into the existing
double-leaf/framed family solver. The independent support case now
publishes `Rw 45` / STC 45 / `C -1` / `Ctr -6.1`, and the resilient
both-sides case publishes `Rw 47` / STC 47 / `C -1` / `Ctr -6.1`.
Support topology is not guessed from layers alone; resilient support
without `resilientBarSideCount` remains a precise input stop. Gate P
status is
`post_v1_wall_double_leaf_auto_topology_gate_p_landed_selected_next_numeric_coverage_gap_gate_q`.
Latest Gate P targeted validation passed 4 files / 19 tests, the Gate P
regression sentinel passed 10 files / 95 tests, and full
...
It extends the flat multicavity auto-topology path to common full-fill
wall stacks when explicit support context is present. The pinned
`gypsum / rockwool / gypsum / rockwool / gypsum` wall derives grouped
triple-leaf topology with full-fill cavities and publishes `Rw 52` /
STC 53 / `C -2.6` / `Ctr -9.4`. Legacy support hints are not guessed,
explicit `flat_layer_order` remains blocked, and the resolver count
stays at 37 declared candidates and 34 active runtime-basis mappings
because Gate Q reuses the existing grouped Rockwool/multileaf candidate.
Gate Q status is
...
Latest Gate Q targeted validation passed 5 files / 21 tests, the
regression sentinel passed 9 files / 90 tests, and full
`pnpm calculator:gate:current` passed with engine 528 files / 2979
tests, web 95 files / 402 passed + 18 skipped, repo build 5 / 5, and
whitespace guard passed.

Gate R has now landed as
...
It extends the explicit-support Gate Q full-fill flat multicavity wall
to complete `field_between_rooms` apparent/standardized outputs:
`R'w 50`, `Dn,w 50`, `Dn,A 48.5`, `DnT,w 53`, and `DnT,A 50.9`.
Missing `receivingRoomRt60S` is now the exact `needs_input` field
instead of manual leaf/cavity grouping; support topology is not guessed.
Gate R status is
...
Gate R targeted validation passed 1 file / 4 tests, the
regression sentinel passed 9 files / 56 tests, and full
`pnpm calculator:gate:current` passed with engine 529 files / 2983
tests, web 95 files / 402 passed + 18 skipped, repo build 5 / 5, and
whitespace guard passed.

Gate S has now landed as
...
It promotes the common flat `gypsum / rockwool / gypsum` double-leaf
wall with explicit support context, `studSpacingMm`, and complete
`field_between_rooms` data from screening fallback into the owned
double-leaf family physics plus field adapter. The pinned case publishes
`R'w 39`, `Dn,w 40`, `Dn,A 38.5`, `DnT,w 42`, and `DnT,A 40.9`.
Missing `receivingRoomRt60S` is the exact `needs_input` field, and
support topology is not guessed. Gate S status is
...
Latest Gate S targeted validation passed 1 file / 4 tests, the
regression sentinel passed 8 files / 52 tests, and full
...
It keeps that same complete flat double-leaf field stack fully
calculable for mixed lab-spectrum plus field requests: `Rw 39`, STC 39,
`C -1`, `Ctr -5.7`, `R'w 39`, `Dn,w 40`, `Dn,A 38.5`, `DnT,w 42`,
and `DnT,A 40.9`. The field adapter trace stays on
`wall.airborne_field_context.field_apparent_adapter` and pins only field
metrics, so lab `Rw` is not relabelled as field-candidate ownership.
Field-only requests remain field-only. Gate T status is
...
Latest Gate T targeted validation passed 1 file / 4 tests, the
regression sentinel passed 10 files / 55 tests, and full
`pnpm calculator:gate:current` passed with engine 531 files / 2991
tests, web 95 files / 402 passed + 18 skipped, repo build 5 / 5, and
whitespace guard passed.

Gate U has now landed as
...
It keeps the explicit-support full-fill flat multicavity field stack
fully calculable for mixed lab-spectrum plus field requests: `Rw 50`,
STC 51, `C -2`, `Ctr -8.5`, `R'w 50`, `Dn,w 50`, `Dn,A 48.5`,
`DnT,w 53`, and `DnT,A 50.9`. The field adapter trace stays on
`wall.airborne_field_context.field_apparent_adapter` and pins only field
metrics, so lab `Rw` is not relabelled as field-candidate ownership.
Field-only requests remain field-only. Gate U status is
...
Latest Gate U targeted validation passed 1 file / 4 tests, the
regression sentinel passed 12 files / 63 tests, and full
`pnpm calculator:gate:current` passed with engine 532 files / 2995
tests, web 95 files / 402 passed + 18 skipped, repo build 5 / 5, and
whitespace guard passed.

Gate V has now landed as
`post_v1_wall_rw_field_output_gate_v_plan` in
`packages/engine/src/post-v1-wall-rw-field-output-gate-v-contract.test.ts`.
It keeps explicitly requested `Rw` live for complete double-leaf and
full-fill multicavity field requests without requiring STC or spectrum
companions: double-leaf `Rw 39`, `R'w 39`, `DnT,w 42`; multileaf
`Rw 50`, `R'w 50`, `DnT,w 53`. The field adapter trace stays on
`wall.airborne_field_context.field_apparent_adapter` and pins only field
metrics. Field-only requests remain field-only. Gate V status is
...
Latest Gate V targeted validation passed 1 file / 4 tests, the
regression sentinel passed 13 files / 67 tests, and full
`pnpm calculator:gate:current` passed with engine 533 files / 2999
tests, web 95 files / 402 passed + 18 skipped, repo build 5 / 5, and
whitespace guard passed.

Gate W has now landed as
...
It keeps explicitly requested `Rw` live for complete heavy-composite and
local-substitution grouped triple-leaf field requests: heavy-composite
`Rw 60`, `R'w 60`, `DnT,w 61`; local-substitution `Rw 51`,
`R'w 51`, `DnT,w 53`. Field-only requests remain field-only, and
field-adapter traces still pin only field metrics. Gate W status is
...
Latest Gate W targeted validation passed 1 file / 4 tests, the
regression sentinel passed 14 files / 71 tests, and full
`pnpm calculator:gate:current` passed with engine 534 files / 3003
tests, web 95 files / 402 passed + 18 skipped, repo build 5 / 5, and
whitespace guard passed.

Gate X has now landed as
...
It keeps calculated `STC`, `C`, and `Ctr` live for complete floor
family-estimate field requests when the family basis owns a calculated
airborne curve. Heavy concrete now supports `STC 58`, `C -0.9`, and
`Ctr -5.6`; lightweight steel now supports `STC 70`, `C -0.9`, and
`Ctr -5.6`. Exact measured floor rows stay metric-scoped and do not
alias `Rw` to STC. Gate X status is
...
It keeps calculated `STC` and `C` live for complete floor
`screening_mass_law_curve_seed_v3` field requests when the screening
curve already owns live `Rw` / `Ctr` / field / impact outputs. Regupol
...
and `C -0.8`. Exact measured floor rows stay metric-scoped, and ASTM /
impact boundary rows stay parked. Gate Y status is
...
It keeps calculated `Rw` live for source-absent floor
`screening_mass_law_curve_seed_v3` requests with visible floor roles.
The pinned TUAS C11c fail-closed stack now supports `Rw 47`, STC 47,
`C -1`, `Ctr -5.7`, `R'w 47`, and `DnT,w 49` through
`floor.screening_airborne.source_absent`, while `Ln,w`, `DeltaLw`,
`L'n,w`, and `L'nT,w` remain stopped. Exact measured floor rows remain
metric-scoped, UBIQ bound rows still do not fabricate missing `C`, and
ASTM / impact boundaries stay parked. Gate Z status is
...
It keeps calculated `Rw` live for the existing Gate H lined-massive wall
family when complete wall field requests also ask unrelated floor/impact
outputs. The pinned concrete lined wall now supports `Rw 55`, STC 55,
`C -1.6`, `Ctr -6.3`, `R'w 55`, `Dn,w 55`, `Dn,A 53.4`, `DnT,w 56`,
and `DnT,A 54.9`; impact outputs remain stopped. Grouped-topology wall
...
It keeps calculated `Rw` live for complete single-leaf screening wall
field requests: masonry brick publishes `Rw 40` and laminated CLT
publishes `Rw 41` alongside their already-live field/apparent and
spectrum outputs. Framed or grouped wall cases that still require
topology fields remain `needs_input`; exact/floor metric scopes and UBIQ
bound missing-`C` guards remain unchanged. Gate AB status is
...
It opens the already-calculated floor field/building airborne outputs
`Dn,w`, `Dn,A`, `DnT,w`, and `DnT,A` through the shared resolver trace
...
`needs_input` boundaries remain unchanged. Gate AC status is
...
Latest validation: full `pnpm calculator:gate:current` passed on
2026-05-26 with engine 540 files / 3027 tests, web 96 files /
...
It opens the already-calculated `Rw` companion for framed wall field/building
requests on the stud-surrogate/framed-calibration route without moving
the defended field values. The LSF Knauf framed wall now supports
`Rw 51`, STC 51, `C -1.4`, `Ctr -6.4`, `R'w 51`, `Dn,w 51`,
`Dn,A 49.6`, `DnT,w 52`, and `DnT,A 51.1`; the timber-stud wall now
supports `Rw 42`, STC 42, `C 0.4`, `Ctr -4.3`, `R'w 42`, `Dn,w 42`,
`Dn,A 42.4`, `DnT,w 43`, and `DnT,A 43.9`. No-support framed metadata
and grouped AAC/triple-leaf cases remain `needs_input`; exact/bound
floor metric scope and ASTM/IIC aliases remain closed. Gate AD status is
...
It opens the defended framed-calibration building lane's calculated lab
spectrum, not just `Rw`: complete explicit framed building requests now
support `Rw`, STC, `C`, and `Ctr`, while `R'w`, `Dn,w`, `Dn,A`,
`DnT,w`, and `DnT,A` remain parked behind field/building owners. LSF
Knauf pins `Rw 51`, STC 51, `C -1.4`, `Ctr -6.4`; timber-stud pins
`Rw 42`, STC 42, `C 0.4`, `Ctr -4.3`. No-support metadata and grouped
AAC/triple-leaf cases remain `needs_input`; exact/floor metric scope and
ASTM/IIC aliases remain closed. Gate AE status is
...
concrete pins `Rw 55`, STC 55, `C -1.6`, `Ctr -6.3`; masonry brick
pins `Rw 40`, STC 40, `C -0.2`, `Ctr -4.7`; laminated CLT pins
`Rw 41`, STC 41, `C -1.8`, `Ctr -7.6`. Grouped AAC/multileaf,
opening/leak building, exact metric scope, floor bound, and ASTM/IIC
boundaries remain closed. Gate AF status is
...
It opens calculated lab-spectrum companions for complete heavy-composite
building wall requests on the guarded double-leaf /
...
pins `Rw 60`, STC 60, `C -1.4`, and `Ctr -6.1`. Grouped
...
It opens the calculated `Rw` companion for support-backed AAC/multileaf
field requests on the existing `multileaf_screening_blend` path: `Rw`
now stays live beside the already supported STC, `C`, `Ctr`, `R'w`,
`Dn,w`, `Dn,A`, `DnT,w`, and `DnT,A` outputs. The support-backed
AAC/multileaf field case pins `Rw 41`, STC 41, `C -1.7`, `Ctr -6.8`,
`R'w 41`, `Dn,w 41`, `Dn,A 39.3`, `DnT,w 42`, and `DnT,A 40.8`.
Missing support topology still stops as `needs_input`; floor bound `C`,
exact floor STC/`C`/`Ctr` aliases, opening/leak building, ASTM/IIC, and
field/building value aliases remain closed. Gate AH status is
...
It promotes the same support-backed AAC/multileaf field request from the
screening fallback into the Gate AE two-cavity physics solver plus the
Gate I field adapter. The support-backed AAC/multileaf field case now
pins `Rw 60`, STC 60, `C -1.9`, `Ctr -8`, `R'w 60`, `Dn,w 60`,
`Dn,A 58.1`, `DnT,w 61`, and `DnT,A 59.6`. Missing support topology
still stops as `needs_input`, missing `receivingRoomRt60S` remains that
exact `needs_input` field, and floor bound `C`, exact floor
STC/`C`/`Ctr` aliases, opening/leak building, ASTM/IIC, and
field/building value aliases remain closed. Gate AI status is
...
solver plus building-prediction runtime basis. The support-backed
AAC/multileaf building case now pins `R'w 60`, `Dn,w 60`, `Dn,A 58.1`,
`DnT,w 61`, and `DnT,A 59.6`; lab `Rw`, STC, `C`, and `Ctr` remain
unsupported on the building route instead of being aliased. Missing
support topology still stops as `needs_input`, missing
`sourceRoomVolumeM3` remains that exact `needs_input` field, and floor
bound `C`, exact floor STC/`C`/`Ctr` aliases, opening/leak building,
ASTM/IIC, and field/building value aliases remain closed. Gate AJ status
is
...
It calculates `L'nT,50` upper-bound companions for bound-only floor
impact lanes when the existing bound route owns `Ln,w`, `L'n,w`, and
`L'nT,w` upper bounds and the user supplies
`impactFieldContext.ci50_2500Db`. The UBIQ open-web bound case now pins
`Ln,w <= 51`, `L'n,w <= 54`, `L'nT,w <= 51.2`, and `L'nT,50 <= 55.2`;
selected lightweight-steel bound interpolation families pin
`L'nT,50 <= 55.2`, `<= 56.2`, or `<= 57.2`. Missing
`impactFieldContext.ci50_2500Db` still stops pure `L'nT,50` requests as
`needs_input`, mixed bound field requests leave only `L'nT,50`
...
Historical planning correction: read
[ACOUSTIC_CALCULATOR_ANSWER_ENGINE_V1_PLAN_2026-05-21.md](./ACOUSTIC_CALCULATOR_ANSWER_ENGINE_V1_PLAN_2026-05-21.md)
for the product correction that moved DynEcho away from catalog drift and
into the answer-engine V1 path. That correction is now closed for the
current tested usable V1 envelope. The previous immediate selection,
`layer_combination_resolver_post_double_leaf_framed_wall_banded_coverage_revalidation_plan`,
was superseded by `acoustic_calculator_answer_engine_v1_plan` in
`packages/engine/src/acoustic-calculator-answer-engine-v1-contract.test.ts`,
and that V1 plan has landed. It is no longer the automatic selected
next slice. Future implementation must be selected explicitly as
post-V1 accuracy/adapters or expanded formula coverage. This is not a
broad source crawl, not a tolerance retune, and not another finite
scenario pack.

The current implementation operates as one calculator answer engine for
the tested company-internal envelope. The shared registry now has 37
declared candidates and 34 active runtime-basis mappings across exact
measured overrides, similarity anchors, calibrated family solvers,
source-absent family solvers, field/building adapters, and boundary
candidates. The post-V1 trace integrations add the wall
triple-leaf local-substitution `Rw` formula, lab-spectrum
`STC`/`C`/`Ctr` adapter, field `R'w`/`DnT,w` adapter, the adjacent
flat-list Rockwool/MLV/plaster guarded double-leaf lab/field paths, and
the Gate H lined-massive plus company-internal heavy-composite lab wall
formula paths to the shared resolver candidate surface, with
scenario-specific value pins and no metric aliasing. The guarded
adjacent flat-list lab case now traces `Rw 51`, STC 51, `C -1.8`, and
`Ctr -7.3` on `wall.flat_list_adjacent_swap.double_leaf_numeric_guard`;
complete field context traces `R'w 49` and `DnT,w 50` on
`wall.flat_list_adjacent_swap.field_context_adapter`. The current tested
lined massive-wall lab stack traces `Rw 60`, STC 60, `C -0.8`, and
`Ctr -5.7` on `candidate_lined_massive_wall_family_physics_prediction`;
the heavy-composite lab stack traces `Rw 63`, STC 63, `C -1.4`, and
`Ctr -6.3` on
`candidate_company_internal_heavy_composite_wall_family_physics_prediction`.
Flat split or grouping-ambiguous multileaf cases without that guarded
adjacent-swap numeric path still stop as `needs_input`. Existing
single-leaf, double-leaf, grouped triple-leaf, raw-bare floor,
helper-only floor, direct-fixed, supported band, package-transfer,
exact, and field lanes should be preserved and fed into the answer
engine. Targeted validation for this latest trace reconciliation passed
7 engine files / 52 tests. Full `pnpm calculator:gate:current` then
passed with engine 510 files / 2912 tests, web 94 files / 397 passed +
18 skipped, repo build 5 / 5, and whitespace guard passed.

The 2026-05-22 V1 slices are closing the initial value-leakage gaps:
single gypsum and explicit double-leaf/framed wall inputs still return
traced source-absent formula answers; flat gypsum / rockwool / gypsum
without topology now asks for the missing double-leaf ownership fields;
missing `resilientBarSideCount` now parks wall answer outputs; and
roleless helper-only timber/open-web stacks that would otherwise publish
impact values now ask for floor roles. Raw floor screening and
field-airborne carrier lanes remain available when they are the selected
answer path. The follow-up slice also parks detected flat
multileaf/multicavity wall answers until grouped topology fields are
supplied, while preserving the adjacent-swap double-leaf guard and
source-like resilient-bar lanes when those remain the selected answer
path. Workbench wall lab/field output cards, answer charts, and summary
cards no longer display parked diagnostic airborne metrics as active
answers. Wall field/building answer stops now also stay visible on the
wall route: partial building-prediction missing room/context requests
carry wall `needs_input` boundaries and exact missing fields, while
opening/leak building requests without the owned adapter carry wall
`unsupported` boundaries and basis-boundary traces with no value pins.
Floor impact field-adapter/source-absent lanes remain floor traces when
they are the selected answer path. Partial `field_between_rooms` wall
requests now also stop as wall `needs_input` boundaries when partition
area is missing, or when a field-only answer is missing receiving-room
volume or RT60, so unavailable field answers are not published from a
partially defaulted field adapter. Mixed lab-plus-field requests keep
owned lab outputs separate and park only unavailable field outputs.
Complete Gate I field-apparent requests remain live.
The latest Answer Engine V1 follow-up restores exact measured wall
precedence ahead of formula missing-input prompts: a true Knauf LSF exact
match now publishes the measured full-stack `Rw 55`, traces through the
exact measured override candidate on `verified_airborne_exact_source`,
and carries no `needs_input` boundary. This exact-source path is
route-scoped to wall; floor exact, package-transfer, raw-bare,
helper-only, and field-impact lanes remain separate. Exact measured wall
answers are also metric-scoped: an Rw-only source row does not make
`STC`, `C`, or `Ctr` exact outputs. Mixed requests keep exact `Rw`
supported and move the unsupported companion metrics out of the exact
answer unless a separate metric basis owner exists. Field exact labels
are scoped the same way: `R'w` exact evidence only promotes `R'w`, and
official `DnT,A,k` evidence stays on the owned `DnT,A` proxy lane rather
than aliasing `R'w` or `DnT,w`. Reversing the full wall layer order no
longer drops a true same-stack catalog row out of exact-source
precedence. Resolver traces now mirror that exact metric scope:
`DnT,A,k` field exact rows trace as `field_apparent`, support only
`DnT,A` / `DnT,A,k`, and pin those field metrics instead of showing lab
`Rw` / STC / `C` / `Ctr` capabilities. Exact measured floor rows and
impact-only exact floor traces now mirror the same rule: the trace pins
only owned floor metrics such as `Rw`, `C`, `Ln,w`, `CI`, `CI,50-2500`,
and `Ln,w+CI`, while `STC`, `Ctr`, and ASTM/IIC aliases stay unsupported
unless a separate metric owner exists. Exact ISO 717-2 impact-band
sources now have their own floor exact resolver candidate on
`exact_source_band_curve_iso7172`: lab sources pin only owned ISO impact
metrics such as `Ln,w`, `CI`, `CI,50-2500`, and `Ln,w+CI`, field sources
trace as `field_apparent` for owned metrics such as `L'nT,w` and
`LnT,A`, and ASTM `IIC`/`AIIC` stays an unsupported boundary until an
ASTM E989 owner exists. Mixed exact impact-band plus floor airborne
...
`Ctr` cannot appear as if they were owned by the ISO 717-2 impact
source.
The latest owner-audit hardening generalizes that leak guard without
turning DynEcho back into a catalog gate. It enforces exact metric scope
where traces declare owned exact metrics, and it enforces active
answer-stop boundaries only for the stopped outputs named by the
boundary. Mixed lab/field requests can therefore keep owned lab formula
outputs live while unavailable field outputs remain parked as
`needs_input` or `unsupported`.
Pure floor ASTM `IIC` / `AIIC` requests now select the value-less
unsupported answer boundary directly on both assembly and impact-only
paths. Exact floor rows and exact ISO impact-band sources can still feed
their owned ISO metrics in mixed requests, but a request whose only
requested answer is ASTM `IIC` / `AIIC` traces to
`generic.astm_iic_aiic.unsupported_boundary`, requires
`astmRatingCurveOwner`, `astmReferenceContour`, and `testStandardBasis`,
and publishes no resolver value pins.
Pure floor field-impact `L'n,w` / `L'nT,w` requests now follow the same
answer-first rule. If lab impact evidence exists but the required field
context is missing, assembly routes return a floor `needs_input`
boundary with `contextMode`, `partitionAreaM2`,
`receivingRoomVolumeM3`, `receivingRoomRt60S`, and
`impactFieldContext`; impact-only routes return `impactFieldContext`.
Mixed requests can still publish owned lab `Ln,w` while parking the
field output.

Floor runtime resolver traces now use the current selected calculation
as their metric source. Package-transfer anchors no longer advertise
unsupported `Ctr`, field, or ASTM/IIC outputs; supported-band anchors pin
live `Ctr`; and field adapter traces pin live field values only for
outputs such as `R'w`, `DnT,w`, `L'n,w`, `L'nT,w`, and `L'nT,50`.
Floor impact missing-input stops now expose floor answer boundaries and
resolver traces on assembly routes when no impact value candidate is
selected: missing `loadBasisKgM2` parks lab `Ln,w` / `DeltaLw`, and
missing field impact context parks `L'n,w` / `L'nT,w`, with exact
missing fields and no value pins.

The Step 2 wall V1 matrix now locks complete wall field-apparent
answers into the shared resolver trace instead of losing them to
incidental floor artifacts. Complete `field_between_rooms` wall
requests on the Gate I field-apparent adapter select
`wall.airborne_field_context.field_apparent_adapter`, trace as
`field_apparent`, support owned field metrics such as `R'w` and
`DnT,w`, and pin the live field values. Partial wall field context
continues to stop as `needs_input`, and unsupported building/opening
owners remain value-less basis boundaries. The registry, runtime
candidate adapter, surface parity, coverage matrix, and company-internal
envelope now include this wall field adapter as a distinct
field/building adapter candidate rather than folding it into the floor
field-impact adapter.
Workbench parity now proves those wall states surface consistently:
exact wall `Rw` remains live while unowned STC/`C`/`Ctr` cards stay
unsupported, complete wall `R'w` / `DnT,w` field cards stay live on the
wall field adapter, partial field cards stay `needs_input`, unsupported
building/opening cards stay unsupported, Markdown reports include the
same resolver candidate/value-pin state, and `/api/estimate` returns the
same answer trace.

Step 4 surface parity is now landed. Result answer charts only draw
airborne or impact lanes when the selected answer supports those outputs;
exact `Rw`-only answers no longer show unowned STC or spectrum
companions; impact-only floor answers no longer surface incidental
airborne `Rw` / STC / `C` / `Ctr` cards, chart lanes, or report answer
lines; and stopped wall/floor answers remain visible as `needs_input`
with exact missing fields across saved replay, server snapshot replay,
cards, resolver trace, and Markdown report output. The targeted Step 4
web validation passed 5 files / 28 tests.

Step 5 company-internal usable V1 acceptance is now landed. The engine
gate covers realistic wall and floor exact, anchor, source-absent
formula, `needs_input`, unsupported, hostile layer-order,
duplicate/split-layer, missing-context, missing-role, and
metric-alias-negative cases. It also locks the floor package-transfer
metric scope so ownerless STC stays parked, keeps exact impact-band
airborne companions out of the ISO impact answer, and preserves real
floor field/building continuation values only when the selected runtime
publishes them.

The V1 acceptance bar remains answer correctness, not new formulas:
...
building values as if a value candidate had won. That
company-internal usable V1 acceptance gate is now included in the
current gate and has passed with engine 510 files / 2912 tests, web 94
files / 397 passed + 18 skipped, repo build 5 / 5, and whitespace guard
passed.
...
and candidate
`candidate_layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_family_solver`.
Runtime pins remain `Rw 45` for independent, `Rw 46` for resilient
both-sides, and `Rw 45` for resilient one-side with their
scenario-specific `C`, `Ctr`, STC, and not-measured budgets. Exact
precedence, missing `resilientBarSideCount`, overlapping leaf groups,
direct-fixed double-leaf, grouped triple-leaf/multicavity,
field/building separation, floor-impact, ASTM/IIC, tolerance retune, and
broad source crawl remain blocked or separate. This is not a broad
source crawl.

Previous planning decision: the layer-combination resolver double-leaf
framed wall banded surface parity has landed as
...
banded coverage refresh.
This is no-runtime surface parity. Cards, output cards, route posture,
confidence provenance, metric-basis rows, method dossier, local saved
replay, server snapshot replay, calculator API payloads, impact-only API
payloads, Markdown report, and candidate trace now expose the same
source-absent element-lab basis
`layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_formula_corridor`
and candidate
`candidate_layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_family_solver`.
The visible scenario-specific pins remain independent absorbed `Rw 45`,
STC 45, `C -1`, `Ctr -6.1`; resilient both-sides `Rw 46`, STC 46,
`C -1.1`, `Ctr -6.2`; and resilient one-side `Rw 45`, STC 45,
`C -1.1`, `Ctr -6.2`, with the same not-measured budgets from the
runtime corridor. Exact rows still win first, missing
`resilientBarSideCount` stays `needs_input`, explicit field overlays do
not rewrite lab trace pins, and direct-fixed, grouped
triple-leaf/multicavity, field/building aliases, floor-impact,
ASTM/IIC, tolerance retune, and broad source crawling remain blocked.
This is not a broad source crawl.

Previous planning decision: the layer-combination resolver double-leaf
framed wall banded runtime corridor has landed as
...
Selected next label: layer combination resolver double-leaf framed wall
banded surface parity.
Complete source-absent element-lab double-leaf/framed walls now promote
through
`layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_formula_corridor`
and candidate
`candidate_layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_family_solver`
without numeric value movement. Independent absorbed remains `Rw 45`,
STC 45, `C -1`, `Ctr -6.1` with `+/-7 dB` `Rw` / STC budgets;
resilient both-sides remains `Rw 46`, STC 46, `C -1.1`, `Ctr -6.2`
with `+/-8 dB`; resilient one-side remains `Rw 45`, STC 45, `C -1.1`,
`Ctr -6.2` with `+/-8 dB`. Exact rows still win first, missing
`resilientBarSideCount` stays `needs_input`, and direct-fixed, grouped
...
overlay and does not relabel lab `Rw`. This is not a broad source crawl.

Previous planning decision: the layer-combination resolver double-leaf
framed wall banded formula corridor has landed as
...
Selected next label: layer combination resolver double-leaf framed wall
banded runtime corridor.
This is no-runtime. The source-absent element-lab basis
`layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_formula_corridor`
now owns the double-leaf/framed wall mass-air-mass formula, bridge
coupling, absorber flow resistivity, one-third-octave TL curve,
ISO 717-1 Rw/C/Ctr adapter, STC display boundary, exact-source
precedence, hostile topology, and not-measured budgets. Formula design
payloads are pinned to preserve the current Gate S public values:
independent absorbed double-leaf remains `Rw 45`, STC 45, `C -1`,
`Ctr -6.1` with `+/-7 dB` `Rw` / STC budgets; resilient both-sides
remains `Rw 46`, STC 46, `C -1.1`, `Ctr -6.2` with `+/-8 dB` `Rw` /
STC budgets; resilient one-side is defined as `Rw 45` / STC 45 with the
same resilient budget. Runtime values remain null in this gate; the
existing Gate S method
...
needs-input boundary, and direct-fixed, grouped triple-leaf/multicavity,
field/building, floor-impact, ASTM/IIC, tolerance retune, and broad
source crawling remain blocked. This is not a broad source crawl.

Previous planning decision: the layer-combination resolver double-leaf
framed wall banded solver owner landed as
...
Selected next label: layer combination resolver double-leaf framed wall
banded formula corridor.
This is no-runtime. Existing Dynamic Calculator Gate S double-leaf /
framed wall values remain frozen: independent absorbed gypsum /
rockwool / gypsum stays `Rw 45`, STC 45, `C -1`, `Ctr -6.1`, and
resilient both-sides stays `Rw 46` / STC 46 on
`gate_s_double_leaf_framed_bridge_mass_air_mass_bridge_damping_runtime`
through candidate
`candidate_double_leaf_framed_bridge_family_physics_prediction`. The
owner does not add that candidate to the layer-combination resolver
registry yet; it prepares the next planned basis
`layer_combination_resolver_double_leaf_framed_wall_banded_source_absent_formula_corridor`.
The owner checklist is explicit: side leaf grouping and surface masses,
cavity depth, mass-air-mass resonance, absorber flow resistivity,
support/bridge coupling, `resilientBarSideCount`, one-third-octave TL,
ISO 717-1 Rw/C/Ctr adapter, STC boundary, exact-source/holdout
precedence, source-absent budgets, hostile topology, triple-leaf /
multicavity separation, field/building boundary, and floor-impact
boundary. Direct-fixed double-leaf, missing side count, grouped
triple-leaf reassignment, field/building promotion, ASTM/STC alias
promotion, tolerance retune, and broad source crawling remain blocked.
This is not a broad source crawl.

Earlier planning decision: the layer-combination resolver post
single-leaf mass-law banded matrix refresh landed as
...
Selected next label: layer combination resolver double-leaf framed wall
banded solver owner.
This is no-runtime. The single-leaf source-absent element-lab candidate
...
with scenario-specific candidate trace value pins. Numeric values remain
frozen: gypsum 12.5 mm `Rw 31`, laminated gypsum 25 mm `Rw 34`, and
concrete wall plus floor direct-airborne `Rw 55`, with `+/-6 dB` `Rw` /
STC-compatibility budgets. The post matrix keeps 14 coverage rows,
2 `ready`, 9 `ready_with_budget`, 1 `needs_input`, 2 `unsupported`, and
0 registered `research_only` operating rows; single-leaf is now treated
as allowed-with-budget rather than the selected research gap. It reranks
the next bounded solver-family owner to double-leaf/framed wall because
side leaf grouping, cavity, support/bridge coupling, absorber state,
one-third-octave TL, ISO 717-1 adapter, and hostile topology rules are
the next common wall gap. Exact rows still win first, and floor impact,
field/building, ASTM/IIC/AIIC, broad source crawling, tolerance retune,
and new Rw-to-STC alias promotion remain blocked. This is not a broad
source crawl.

Previous single-leaf mass-law banded surface parity closeout remains
landed as
...
It exposed the scenario-specific candidate trace for the same
single-leaf source-absent basis and selected
`layer_combination_resolver_post_single_leaf_mass_law_banded_matrix_refresh_plan`
in
...
mass-law banded matrix refresh. That selected matrix refresh is now
landed by the current planning decision above. This is not a broad
source crawl.

Research refresh on 2026-05-21 confirms the same direction: public INSUL
and ISO material support a route-owned candidate resolver with
family-specific physics, banded curves, measured rows as exact/anchor /
holdout evidence, visible uncertainty budgets, and strict lab vs field /
building basis separation. The tactical next step is now the selected
double-leaf/framed wall coverage refresh after surface parity; floating
cover floor packages, field/building adapters, glazing/openings/leaks,
calibration/holdouts, and hostile personal-use readiness gates remain
ranked after it.

Implementation closeout on 2026-05-21 registered the landed single-leaf
runtime basis in the shared resolver registry/adapter path. The selected
surface-parity contract file now exists, the current-gate runner has
registered it, and the trace builder now maps both wall single-leaf and
floor direct-airborne single-leaf cases while refusing floor-impact
transfer. The post-single-leaf matrix refresh also now exists and is
registered in the current-gate runner without changing acoustic values.
The double-leaf/framed owner contract now exists and is registered in
the current-gate runner as the no-runtime handoff into the formula
corridor.

Previous single-leaf mass-law banded runtime corridor closeout remains
landed as
...
It promoted complete source-absent element-lab wall/floor
direct-airborne single-leaf stacks through
`layer_combination_resolver_single_leaf_mass_law_banded_source_absent_formula_corridor`
on runtime candidate
`candidate_layer_combination_resolver_single_leaf_mass_law_banded_source_absent_family_solver`
without moving numeric values: gypsum `Rw 31`, laminated gypsum
`Rw 34`, and concrete/floor direct-airborne `Rw 55` with `+/-6 dB`
not-measured budgets. It selected
`layer_combination_resolver_single_leaf_mass_law_banded_surface_parity_plan`
in
...
banded surface parity. Field/building, ASTM/IIC, floor impact, broad
source crawl, and tolerance retune remained blocked.

Previous single-leaf mass-law banded formula corridor closeout remains
landed as
...
It defined
`layer_combination_resolver_single_leaf_mass_law_banded_source_absent_formula_corridor`
as the no-runtime source-absent element-lab basis for gypsum `Rw 31`,
laminated gypsum `Rw 34`, and concrete/floor direct-airborne `Rw 55`
with `+/-6 dB` budgets, then selected
`layer_combination_resolver_single_leaf_mass_law_banded_runtime_corridor_plan`
in
`packages/engine/src/layer-combination-resolver-single-leaf-mass-law-banded-runtime-corridor-contract.test.ts`.
This is not a broad source crawl.

Previous single-leaf mass-law banded solver owner closeout remains
landed as
...
Previous company-internal V0 rehearsal closeout remains landed as
...
It classified 14 current resolver rows as 2 `ready`, 9
`ready_with_budget`, 1 `needs_input`, 2 `unsupported`, and 0 registered
`research_only` rows, then selected
`layer_combination_resolver_single_leaf_mass_law_banded_solver_owner_plan`
in
...
Previous candidate coverage matrix closeout remains landed as
...
It is no-runtime: the executable candidate coverage matrix covers all 14
resolver candidate declarations, 14 surface rows, 11 active runtime
...
unsupported ASTM/IIC blockers without moving runtime values. It selected
...
Earlier helper-only owner closeout remains landed as
...
the later formula, runtime, surface, coverage, post-helper-only
revalidation, registry, and runtime candidate adapter gates consume that
owner record without
changing its meaning.

That helper-only timber/open-web formula corridor has now landed as
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_formula_corridor_plan`
with selection status
...
from the owner checklist without moving public runtime values. Tagged
timber helper-only remains airborne screening with impact unsupported;
tagged open-web helper-only remains on the weak current family estimate
until the runtime corridor; exact, direct-fixed, supported-band,
raw-bare, package-transfer, and EPS/screed lanes remain separate. Raw-bare
impact field transfer, building prediction, and ASTM/IIC aliases remain
blocked. The selected next step is
...
That helper-only timber/open-web runtime corridor has now landed as
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_runtime_corridor_plan`
with selection status
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_runtime_corridor_landed_selected_surface_parity`.
Complete source-absent element-lab helper-only stacks now promote through
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_source_absent_formula_corridor`.
The live helper-only pins are open-box timber `Rw 54.8` / `Ln,w 59.6`,
timber joist `Rw 47.3` / `Ln,w 65.4`, and open-web `Rw 46.7` /
`Ln,w 59.6`, with not-measured source-absent budgets. Exact/package/
raw-bare lanes stay ahead; partial, roleless, and missing-board
helper-like stacks stay outside this runtime; field/building and
ASTM/IIC aliases remain blocked. The selected next step is
...
That helper-only timber/open-web surface parity has now landed as
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_surface_parity_plan`
with selection status
...
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_source_absent_formula_corridor`
basis and values: open-box timber `Rw 54.8` / `Ln,w 59.6`, timber
joist `Rw 47.3` / `Ln,w 65.4`, and open-web `Rw 46.7` / `Ln,w 59.6`,
with not-measured source-absent budgets. Fully tagged impact-only
entrypoints now preserve the same helper-only lane. Exact/package/
raw-bare, direct-fixed, supported-band, field/building, and ASTM/IIC
boundaries remain blocked or ahead. The selected next step is
...
That helper-only timber/open-web coverage refresh has now landed as
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_coverage_refresh_plan`
with selection status
...
It is a no-runtime executable matrix refresh: open-box timber 370 mm,
split 185/185, 4x92.5 safe fragments, timber-joist 250 mm, open-web
250 mm, and open-web split 125/125 now sit in the helper-only coverage
ledger. Runtime pins remain open-box timber `Rw 54.8` / `Ln,w 59.6`
with `+/-8.5 dB` `Rw` and `+/-10.5 dB` `Ln,w`, timber-joist `Rw 47.3`
...
field/building, and ASTM/IIC requests stay blocked. Selected next action:
...
That post-helper-only timber/open-web coverage revalidation has now
landed as
`broad_accuracy_post_helper_only_timber_open_web_impact_stack_coverage_revalidation_plan`
with selection status
...
That layer combination resolver registry has now landed as
...
That layer combination resolver runtime candidate adapter has now landed
...
That layer combination resolver runtime candidate surface parity has now
...
now expose `layerCombinationResolverTrace`, and the calculator API,
impact-only API, workbench replay path, and Markdown report expose the
same candidate trace facts without moving runtime values. The trace names
selected candidate id, candidate kind, support bucket, requested basis,
runtime basis, value pins, error-budget metrics, boundary candidates,
and rejected candidate count. Exact, anchor, raw-bare, helper-only,
direct-fixed, supported-band, and field adapter lanes keep their current
values; `needs_input`, `unsupported`, field/building, and ASTM/IIC
boundaries remain blocked. The selected next action is
...
Latest analysis pass: this direction is still correct after comparing
the current implementation with ISO 12354-1/2, ISO 717-1/2, INSUL's
public feature/technical material, CadnaB/BASTIAN-style building
workflows, and SOAB's structured ISO 12354 input-data posture.
Market-grade tools are calculation-first: measured rows help as exact
overrides, anchors, and holdouts, but unbounded layer combinations
require family-specific solvers with explicit lab/field/building basis
separation. The latest gate proved the new input surface did not disturb
existing calculator routes; the next gate should refresh the
unsupported/needs-input matrix and select one runtime solver lane from
common source-absent floor/wall gaps.

2026-05-20 implementation comparison: the workbench input surface and
web acceptance test are landed, and the current-gate runner includes the
web test. The engine post-input revalidation contract, the matrix
...
physical fields for base support family, carrier geometry, lower ceiling
board mass, cavity depth, absorber thickness/density, suspension/support
class, package absence, impact-curve ownership, ISO impact adapters,
hostile topology, and budgets. This is not a broad source crawl.

The floor open-web raw-bare coverage refresh landed as
...
The post open-web field/building input-surface matrix refresh has now
landed as
`broad_accuracy_post_open_web_field_building_input_surface_matrix_refresh_plan`
with selection status
...
floor-impact ownership because timber helper-only impact is missing and
open-web lower-treatment-only impact needs a named owner. The selected
next action is
`broad_accuracy_floor_helper_only_timber_open_web_impact_stack_owner_plan`
in
...
The post EPS/screed hybrid matrix refresh landed gate is
...
floor open-web raw-bare carrier owner has now landed as
...
It is a no-runtime owner boundary: the current 111 UBIQ open-web rows are
...
package rows stay separate lanes, field/building plus ASTM/IIC aliases
stay blocked, and raw/split/deck screening postures are explicit
formula-owner blockers. Lower-only partial package inputs also remain
fail-closed until full INEX/firestop/package ownership exists. The
...
The floor open-web raw-bare formula corridor has now landed as
...
in
`packages/engine/src/broad-accuracy-floor-open-web-raw-bare-formula-corridor-contract.test.ts`.
It is no-runtime formula-corridor work. It defines the source-absent
element-lab basis
`broad_accuracy_floor_open_web_raw_bare_source_absent_formula_corridor`
from open-web support form, carrier geometry, bare steel reference
surface, structural mobility, package exclusion, ISO 717 metric
boundaries, and source-absent budget terms. The representative raw
open-web design row is `Rw 32`, `C -2.2`, `Ctr -7.8`, `Ln,w 96`,
`CI 1.8`, `CI,50-2500 5.2`, and `Ln,w+CI 97.8`; budgets are
not-measured source-absent values including `+/-9 dB` for `Rw` and
`+/-12 dB` for `Ln,w`. The floor open-web raw-bare runtime corridor has
now landed as
...
returns `Rw 36.6` and `Ln,w 92.8`. Exact UBIQ INEX/firestop package
rows, direct-fixed and supported-band package lanes, deck-only or
partial packages, field/building outputs, and ASTM/IIC aliases stay
blocked outside this lane. Selected next action:
`broad_accuracy_floor_open_web_raw_bare_surface_parity_plan`; selected
next file:
...
The EPS/screed surface parity landed gate is
...
field/building, and ASTM/IIC boundaries stay protected. The landed
matrix-refresh contract is
`packages/engine/src/broad-accuracy-open-box-timber-post-eps-screed-hybrid-matrix-refresh-contract.test.ts`.

The 2026-05-20 detailed INSUL competitor benchmark is recorded in
[INSUL_COMPETITOR_RESEARCH_2026-05-20.md](./INSUL_COMPETITOR_RESEARCH_2026-05-20.md);
it supports this direction and does not change the selected floor
open-web field/building post-input-surface revalidation. The deeper
INSUL read reinforces route-specific family solvers, banded curve
ownership where physics is spectral, exact/anchor/formula lane
separation, visible uncertainty, and fail-closed missing-input prompts.
It does not justify a broad measurement/source crawl before the selected
revalidation and matrix refresh.
...
The no-runtime raw-bare open-box reopening guard has now landed:
...
It proves roleless, tagged, split, upper-only, lower-only, and wrong
support-family raw-bare probes do not borrow the package-transfer
`Ln,w 50.8`, `CI,50-2500 3.3`, or `Rw 66` runtime lane. Current raw
open-box airborne `Rw` is screening-only, impact remains unsupported,
exact TUAS package rows stay first, and field/building plus ASTM/IIC
aliases stay blocked. The selected next file is the bare-carrier owner
contract:
`packages/engine/src/broad-accuracy-floor-open-box-timber-raw-bare-carrier-owner-contract.test.ts`.

The no-runtime raw-bare bare-carrier owner contract has now landed:
...
Landed contract:
`packages/engine/src/broad-accuracy-floor-open-box-timber-raw-bare-carrier-owner-contract.test.ts`.
It defines the required support geometry, airborne direct-curve, bare
impact-curve, finish absence, lower-treatment state, package-transfer
exclusion, source-or-physics basis, metric-specific uncertainty budget,
exact-precedence, basis-boundary, and hostile-topology owners before any
raw-bare formula corridor can move values. It keeps `Ln,w 50.8`,
`CI,50-2500 3.3`, and `Rw 66` frozen on the finished package-transfer
lane and keeps field/building plus ASTM/IIC aliases blocked. The
selected next file is the raw-bare formula corridor:
`packages/engine/src/broad-accuracy-floor-open-box-timber-raw-bare-formula-corridor-contract.test.ts`.
The no-runtime raw-bare formula corridor has now landed:
...
Landed contract:
`packages/engine/src/broad-accuracy-floor-open-box-timber-raw-bare-formula-corridor-contract.test.ts`.
It defines the source-absent raw-bare element-lab formula basis
`broad_accuracy_floor_open_box_timber_raw_bare_source_absent_formula_corridor`
and selected the runtime corridor. The raw-bare runtime corridor has now
landed:
...
through
`broad_accuracy_floor_open_box_timber_raw_bare_source_absent_formula_corridor`.
The canonical 370 mm open-box case returns `Rw 42.3`, `C -1.4`,
`Ctr -5.8`, `Ln,w 88.2`, `CI -1.1`, `CI,50-2500 3.1`, and
`Ln,w+CI 87.1`; the 220 mm case returns `Rw 38.1`, `C -1.6`,
`Ctr -6.2`, `Ln,w 91.1`, `CI,50-2500 3.4`, and `Ln,w+CI 90.2`.
The wide not-measured budgets are `+/-8 dB` for `Rw`, `+/-10 dB` for
`Ln,w`, `+/-4 dB` for `CI,50-2500`, and `+/-10.5 dB` for `Ln,w+CI`.
The finished package-transfer values `Ln,w 50.8`, `CI,50-2500 3.3`,
and `Rw 66` remain separate, exact TUAS source rows still win, partial
package and wrong-family cases stay out, and field/building plus
ASTM/IIC aliases stay blocked. The raw-bare surface parity gate has now
landed as `broad_accuracy_floor_open_box_timber_raw_bare_surface_parity_plan`
with selection status
...
The surface-parity selected action, now consumed by the coverage refresh,
is
`broad_accuracy_floor_open_box_timber_raw_bare_coverage_refresh_plan`.
The surface-parity selected file was the raw-bare coverage refresh
contract:
...
If you need the current Gate Y CLT / mass-timber `Ctr` research plan
and next implementation order read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_Y_RESEARCH_PLAN_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_Y_RESEARCH_PLAN_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate Y
checkpoint read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_Y_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_Y_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate Z
checkpoint read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_Z_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_Z_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate AA
checkpoint read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AA_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AA_HANDOFF.md).
If you need the post-Gate-AA Gate AB readiness analysis read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_GATE_AB_READINESS_ANALYSIS_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_GATE_AB_READINESS_ANALYSIS_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate AB
checkpoint read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AB_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AB_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate AC
checkpoint read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AC_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AC_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate AD
checkpoint read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AD_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AD_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate AE
checkpoint read
[CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AE_HANDOFF.md](./CHECKPOINT_2026-05-11_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AE_HANDOFF.md).
...
If you need the latest Gate AO building prediction junction vibration
owner and Gate AP handoff read
...
If you need the preceding Personal-Use MVP Coverage Sprint Gate E
checkpoint read
...
If you need the preceding Personal-Use MVP Coverage Sprint Gate D
checkpoint read
...
If you need the preceding Personal-Use MVP Coverage Sprint Gate C
checkpoint read
...
If you need the preceding Personal-Use MVP Coverage Sprint Gate B
checkpoint read
...
If you need the preceding Personal-Use MVP Coverage Sprint Gate A
checkpoint read
...
If you need the preceding Gate BI checkpoint read
[CHECKPOINT_2026-05-08_MODEL_FIRST_PHYSICS_PREDICTION_PIVOT_GATE_BI_HANDOFF.md](./CHECKPOINT_2026-05-08_MODEL_FIRST_PHYSICS_PREDICTION_PIVOT_GATE_BI_HANDOFF.md).
If you need the current strategic ROI replan after Gate BH read
[CHECKPOINT_2026-05-08_STRATEGIC_ROI_REPLAN_HANDOFF.md](./CHECKPOINT_2026-05-08_STRATEGIC_ROI_REPLAN_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate AZ
checkpoint read
...
If you need the current Personal-Use MVP Coverage Sprint Gate BA
checkpoint read
...
If you need the current Personal-Use MVP Coverage Sprint Gate BB
checkpoint read
...
If you need the current Personal-Use MVP Coverage Sprint Gate BC
checkpoint read
...
If you need the current Personal-Use MVP Coverage Sprint Gate BD
checkpoint read
...
If you need the current Personal-Use MVP Coverage Sprint Gate BI
checkpoint read
[CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BI_HANDOFF.md](./CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BI_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate BJ
checkpoint read
[CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BJ_HANDOFF.md](./CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BJ_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate BK
checkpoint read
[CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BK_HANDOFF.md](./CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BK_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate BL
checkpoint read
[CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BL_HANDOFF.md](./CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BL_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate BM
checkpoint read
[CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BM_HANDOFF.md](./CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BM_HANDOFF.md).
If you need the current Personal-Use MVP Coverage Sprint Gate BN
checkpoint read
[CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BN_HANDOFF.md](./CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BN_HANDOFF.md).
If you need the current heavy-composite wall solver cleanup checkpoint
read
...
If you need the current company-internal matrix refresh checkpoint read
...
If you need the current company-internal steel suspended-ceiling
`DeltaLw` owner checkpoint read
...
If you need the current company-internal airborne building-prediction
reconciliation checkpoint read
[CHECKPOINT_2026-05-14_COMPANY_INTERNAL_AIRBORNE_BUILDING_PREDICTION_RECONCILIATION_HANDOFF.md](./CHECKPOINT_2026-05-14_COMPANY_INTERNAL_AIRBORNE_BUILDING_PREDICTION_RECONCILIATION_HANDOFF.md).
If you need the current company-internal matrix v2 refresh checkpoint
read
[CHECKPOINT_2026-05-14_COMPANY_INTERNAL_MATRIX_V2_REFRESH_HANDOFF.md](./CHECKPOINT_2026-05-14_COMPANY_INTERNAL_MATRIX_V2_REFRESH_HANDOFF.md).
If you need the internet-researched remaining-road analysis and plain
company-internal acceptance direction read
...
If you need the current company-internal opening/leak field/building
input-surface checkpoint read
[CHECKPOINT_2026-05-14_COMPANY_INTERNAL_OPENING_LEAK_BUILDING_INPUT_SURFACE_HANDOFF.md](./CHECKPOINT_2026-05-14_COMPANY_INTERNAL_OPENING_LEAK_BUILDING_INPUT_SURFACE_HANDOFF.md).
If you need the latest checkpoint review and validation stopping point
read
...
If you need the current remaining-gap analysis and concrete
company-internal readiness sequence read
...
The active calculator lane is realigned to company-internal
calculation-grade work: ISO floor-impact `Ln,w` / `DeltaLw`, wall
`Rw` / `R'w` / `DnT,w`, field/building missing-input safety, and cleanup
of common `screening_fallback` / `low_confidence` answers. Read
[CHECKPOINT_2026-05-14_COMPANY_INTERNAL_MAINLINE_REALIGNMENT_HANDOFF.md](./CHECKPOINT_2026-05-14_COMPANY_INTERNAL_MAINLINE_REALIGNMENT_HANDOFF.md)
and
[SLICE_COMPANY_INTERNAL_CALCULATION_GRADE_MAINLINE_PLAN.md](./SLICE_COMPANY_INTERNAL_CALCULATION_GRADE_MAINLINE_PLAN.md)
before continuing implementation.

Parked ASTM source label:

Personal-Use MVP Coverage Sprint Gate BV has landed the no-runtime
floor-impact ASTM `IIC` / `AIIC` rating curve owner scaffold. Gate BV
owns the declared ASTM impact one-third-octave curve completeness
surface for lab `IIC` and field `AIIC`, but ASTM runtime work is now
parked as boundary history. Gate BT bridge probes and Gate BV curve
probes are not runtime evidence, not source rows, and not ASTM rating
results. ISO `Ln,w` / `DeltaLw`, field `L'n,w` / `L'nT,w`,
building-prediction metrics, and ASTM E413/STC rows must not alias to
ASTM E989 impact ratings.

Landed Gate BV action:
...
Latest company-internal runtime movement:

The heavy-composite wall cleanup has landed. Complete
`wall-heavy-composite-hint-suppression` style heavy mineral/composite
double-leaf stacks now select
`company_internal_heavy_composite_wall_mass_air_mass_capped_family_physics_runtime`
instead of `screening_fallback`, keep lab `Rw 63 / STC 63` and field
`R'w 60 / DnT,w 61`, expose lab `errorBudgetDb: 8`, and wrap field
requests through the field adapter with `errorBudgetDb: 10`.

Latest company-internal matrix refresh:

`company_internal_calculation_grade_matrix_refresh_after_heavy_composite_plan`
has landed as a no-runtime executable matrix refresh. The matrix now has
61 rows, keeps the heavy-composite lab/field values pinned, keeps wall
building-prediction missing-input requests parked, keeps ASTM `IIC` /
`AIIC` parked as boundary history, and selects the next ISO lane:
`company_internal_steel_suspended_ceiling_delta_lw_owner_contract_plan`.
The matrix refresh selected file was
`packages/engine/src/company-internal-steel-suspended-ceiling-delta-lw-owner-contract.test.ts`.

Latest company-internal input-owner movement:

`company_internal_steel_suspended_ceiling_delta_lw_owner_contract_plan`
has landed. Steel suspended-ceiling-only floor input still returns lab
`Ln,w 62.2`, but `DeltaLw` is no longer just a silent unsupported gap:
the input surface now exposes the exact upper/reference package fields
needed before calculating it:
`toppingOrFloatingLayer`, `resilientLayerDynamicStiffnessMNm3`, and
`loadBasisKgM2`. Mixed `Ln,w` / `DeltaLw` requests preserve supported
`Ln,w` while prompting only for the missing `DeltaLw` owner inputs. No
numeric `DeltaLw`, ASTM alias, field alias, or building alias was added.

Current next implementation focus:

The selected airborne building-prediction runtime terms owner contract
has landed as
...
It proved the existing Gate AR/AS/AT complete-context building runtime
inside the company-internal envelope without moving runtime values:
`R'w 58` / `DnT,w 59`, the
`gate_ar_airborne_building_prediction_all_owner_runtime_corridor` basis,
visible `+/-9 dB` budget, precise partial-context prompts, lab-alias
negatives, and unsupported opening/leak building boundary.

Latest landed contract file:
...
The matrix v2 refresh has also landed as
...
It preserves Gate AT building runtime rows, retires the stale building
missing-context row, preserves partial building `needs_input`, changes
the steel suspended-ceiling `DeltaLw` row to a precise `needs_input`
owner prompt, and normalizes the heavy-floating floor matrix origin away
from hidden `screening_fallback`.
...
Matrix V2 selected next file, now landed by that runtime corridor:
...
Missing `toppingOrFloatingLayer` blocks the formula instead of
...
calculator API payload, impact-only API payload, and Markdown report now
show the same lab `Ln,w 51.6` / `DeltaLw 22.4`,
`steel_suspended_ceiling_lower_reference`, and source-absent `+/-4.5 dB`
/ `+/-2.0 dB` budget posture. Selection status:
...
`company_internal_calculation_grade_mainline_matrix_v3_refresh_after_steel_delta_lw_surface_parity_plan`.
It records the already-landed steel suspended-ceiling runtime as
calculation-grade matrix evidence: the old
`floor.lightweight_steel_suspended_ceiling_delta_lw.needs_input` owner
prompt is retired, and
`floor.lightweight_steel_suspended_ceiling_delta_lw.runtime` now pins
lab `Ln,w 51.6` / `DeltaLw 22.4` through
`predictor_lightweight_steel_mass_spring_holdout_corridor_estimate`
with `steel_suspended_ceiling_lower_reference`, `+/-4.5 dB` `Ln,w`
budget, and `+/-2.0 dB` `DeltaLw` budget. Selection status:
...
Matrix V3 selected next file, now landed by the owner contract:
...
It keeps source-absent `L'nT,50` blocked until
`lowFrequencyImpactSpectrumOrCI50_2500Owner` exists. Owner selected next
file, now landed by the runtime corridor:
`packages/engine/src/company-internal-steel-suspended-ceiling-low-frequency-lnt50-runtime-corridor-contract.test.ts`.

The steel suspended-ceiling low-frequency `L'nT,50` surface parity gate
has now landed as
`company_internal_steel_suspended_ceiling_low_frequency_lnt50_surface_parity_plan`.
Lab `Ln,w 51.6` / `DeltaLw 22.4` remains frozen. Complete field context
...
The `L'nT,50` source-absent field adapter budget is `+/-7 dB`, and that
value/basis/budget now appears consistently in cards, corridor dossier,
local saved replay, server snapshot replay, calculator API payload,
impact-only API payload, and Markdown report. Missing `CI,50-2500` /
low-frequency spectrum ownership still blocks `L'nT,50`, exact field
band packets remain exact precedence, and ASTM `IIC` / `AIIC` remain
...
Surface parity selected next action, now landed by Matrix V4:
`company_internal_calculation_grade_mainline_matrix_v4_refresh_after_lnt50_surface_parity_plan`.
Surface parity selected next file, now landed by Matrix V4:
`packages/engine/src/company-internal-calculation-grade-mainline-matrix-v4-contract.test.ts`.
Surface parity selected next label, now landed by Matrix V4:
company-internal matrix v4 refresh after steel L'nT,50 surface parity.

Matrix V4 has now landed as
`company_internal_calculation_grade_mainline_matrix_v4_refresh_after_lnt50_surface_parity_plan`.
It records the complete steel suspended-ceiling field route as supported
`Ln,w 51.6` / `DeltaLw 22.4` / `L'n,w 54.6` / `L'nT,w 51.8` /
`L'nT,50 50.8`, keeps missing `CI,50-2500` / low-frequency spectrum
ownership blocked without an `L'nT,50` budget, and keeps exact
field-band `L'nT,50` precedence separate from the source-absent steel
adapter. ASTM `IIC` / `AIIC` remain unsupported. Selection status:
...
The preceding low-frequency runtime corridor landed as
...
The opening/leak field/building adapter owner contract has now landed as
`company_internal_opening_leak_building_adapter_owner_contract_plan`.
It is a no-runtime owner boundary after Matrix V4. Gate S lab
opening/leak `Rw 38.2` / `STC 39` remains the only supported runtime for
the opening/leak route; field/apparent and building-prediction
`R'w` / `DnT,w` are not aliases of that lab value. The field owner group
now requires explicit opening/leak physical inputs plus panel geometry,
receiving-room volume, and receiving-room RT60. The building owner group
adds source-room volume, flanking/junction class, conservative flanking
assumption, junction coupling length, and building output basis. Missing
physical fields return `needs_input`; missing runtime owners return
`runtime_owner_missing`; element-lab and ASTM requests stay outside this
adapter.
...
- Complete explicit field opening/leak context with
  `openingLeakFieldBuildingAdapterBoundary` returns `R'w 36.4` /
  `Dn,w 36.7` / `DnT,w 36.9` through
  `company_internal_opening_leak_field_area_energy_runtime_corridor`
  with a `+/-8 dB` source-absent field budget.
- Complete explicit building opening/leak context returns `R'w 31.6` /
  `DnT,w 32.1` through
  `company_internal_opening_leak_building_area_energy_runtime_corridor`
  with a `+/-10 dB` source-absent building budget.
- Gate S lab opening/leak `Rw 38.2` / `STC 39` remains unchanged.
  `Dn,A` / `DnT,A` remain unsupported until a separate spectrum adapter
  owns them.
...
Latest landed opening/leak field/building surface parity:
...
- Cards, route posture, scenario summary, method/corridor dossiers,
  saved replay, calculator API payload, and Markdown report now preserve
  the same field `R'w 36.4` / `Dn,w 36.7` / `DnT,w 36.9` with `+/-8 dB`
  and building `R'w 31.6` / `DnT,w 32.1` with `+/-10 dB`.
- The visible basis names
  `company_internal_opening_leak_field_area_energy_runtime_corridor` and
  `company_internal_opening_leak_building_area_energy_runtime_corridor`
  remain source-absent field/building predictions, not measured evidence
  and not lab `Rw` / `STC` aliases.
- Selected next action, now landed by the input surface:
  `company_internal_opening_leak_building_input_surface_plan`.
- Selected next file, now landed by the input surface:
  `apps/web/features/workbench/company-internal-opening-leak-building-input-surface.test.ts`.
- Selected next label, now landed by the input surface:
  opening/leak field/building input surface.

Latest landed opening/leak field/building input surface:
...
- The Dynamic Calculator wall input surface now owns the opening/leak
  field/building physical input package: host wall area, stable opening
  ids, opening area/count/element `Rw`/basis/seal/origin, partition
  width/height, receiving-room volume/RT60, source-room volume,
  flanking/junction class, conservative flanking assumption, junction
  coupling length, and building output basis.
- UI-derived field input reaches `R'w 36.4` / `Dn,w 36.7` /
  `DnT,w 36.9` with `+/-8 dB`; UI-derived building input reaches
  `R'w 31.6` / `DnT,w 32.1` with `+/-10 dB`.
- Local saved replay, server snapshot replay, report payload, and
  calculator API payload preserve the same basis. Missing building owner
  fields return `needs_input`; duplicate opening inputs fail closed.
- Selected next action:
...
Latest landed Matrix V5 refresh after opening/leak input surface:
...
- Matrix V5 records supported opening/leak field/building runtime rows:
  field `R'w 36.4` / `Dn,w 36.7` / `DnT,w 36.9` with `+/-8 dB`, and
  building `R'w 31.6` / `DnT,w 32.1` with `+/-10 dB`.
- The stale
  `wall.opening_leak_composite_building_boundary.unsupported` row is
  retired only after those supported rows are pinned.
- `Dn,A` and `DnT,A` remain unsupported for opening/leak field/building
  requests until a spectrum-adapter owner exists; missing building
  owners still return precise `needs_input`.
...
Do not continue ASTM runtime work, broad source crawling, or unrelated
building-prediction runtime work before the opening/leak A-weighted
formula corridor unless a later active plan explicitly re-ranks them.

Latest landed opening/leak A-weighted spectrum-adapter owner:

- `company_internal_opening_leak_a_weighted_spectrum_adapter_owner_contract_plan`
  has landed.
- Selection status:
  `company_internal_opening_leak_a_weighted_spectrum_adapter_owner_contract_landed_no_runtime_selected_formula_corridor`.
- The owner gate keeps existing runtime values frozen: field
  opening/leak `R'w 36.4` / `Dn,w 36.7` / `DnT,w 36.9` with `+/-8 dB`,
  and building opening/leak `R'w 31.6` / `DnT,w 32.1` with `+/-10 dB`.
- Field `Dn,A` / `DnT,A` and building `DnT,A` remain unsupported until
  a formula corridor owns `frequencyBandSet`, the same-route
  opening/leak spectrum curve, ISO 717 C or A-weighted adapter,
  A-weighted uncertainty budget, exact A-weighted packet precedence,
  and the lab `Rw` / `STC` alias guard.
- Building `Dn,A` remains outside this owner.
...
Latest landed opening/leak A-weighted formula corridor:

- `company_internal_opening_leak_a_weighted_spectrum_adapter_formula_corridor_plan`
  has landed.
- Selection status:
  `company_internal_opening_leak_a_weighted_spectrum_adapter_formula_corridor_landed_no_runtime_selected_runtime_corridor`.
- This is a no-runtime formula corridor. It defines field `Dn,A 35.9`,
  field `DnT,A 36.1`, and building `DnT,A 31.3` from the same-route
  base values plus the owned `-0.8 dB` A-weighted adapter.
- Field A-weighted budget is base `+/-8 dB` plus `+/-1 dB` adapter
  term, total `+/-9 dB`. Building A-weighted budget is base `+/-10 dB`
  plus `+/-1 dB` adapter term, total `+/-11 dB`.
...
Latest landed opening/leak A-weighted runtime corridor:

- `company_internal_opening_leak_a_weighted_spectrum_adapter_runtime_corridor_plan`
  has landed.
- Selection status:
  `company_internal_opening_leak_a_weighted_spectrum_adapter_runtime_corridor_landed_selected_surface_parity`.
- The runtime now supports complete A-weighted opening/leak requests
  when `frequencyBandSet` is present: field `Dn,A 35.9`, field
  `DnT,A 36.1`, and building `DnT,A 31.3`.
- Base values remain stable: field opening/leak `R'w 36.4` /
  `Dn,w 36.7` / `DnT,w 36.9`; building opening/leak `R'w 31.6` /
  `DnT,w 32.1`.
- Field A-weighted budget is `+/-9 dB`; building A-weighted budget is
  `+/-11 dB`; both remain source-absent and not measured evidence.
- Missing `frequencyBandSet` keeps A-weighted outputs unsupported with a
  precise missing-input warning. Building `Dn,A`, lab `Rw` / `STC`, and
  ASTM `IIC` / `AIIC` remain non-alias boundaries.
...
Latest landed opening/leak A-weighted surface parity:
...
- Surface parity keeps field `Dn,A 35.9`, field `DnT,A 36.1`, and
  building `DnT,A 31.3` frozen while exposing method/candidate ids,
  `+/-9 dB` / `+/-11 dB` budgets, warning copy, and the frequency band set
  across output cards, route posture, scenario summary, dossiers,
  saved/server replay, API payloads, Markdown report, and the workbench
  input surface.
- Selected next action:
...
- Matrix V6 has 71 executable rows and retires
  `wall.opening_leak_a_weighted_boundary.unsupported`.
- It keeps field `Dn,A 35.9`, field `DnT,A 36.1`, and building
  `DnT,A 31.3` frozen as source-absent family-physics values, while
  building `Dn,A`, missing `frequencyBandSet`, lab aliases, ASTM
...
- Building partial-context and opening/leak building missing-owner rows
  remain precise `needs_input` rows with named physical owners such as
  `sourceRoomVolumeM3`, no value pins, and no budget.
- Floor ASTM `IIC` / `AIIC` rows and the airborne A-weighted-to-ASTM
...
[CHECKPOINT_2026-05-14_RESEARCH_BACKED_COMPANY_INTERNAL_ROADMAP.md](./CHECKPOINT_2026-05-14_RESEARCH_BACKED_COMPANY_INTERNAL_ROADMAP.md)
records the standards/tooling research, implementation-vs-doc audit, and
remaining-road decision. The company-internal bar is executable scenario
quality: complete in-scope scenarios must resolve to exact/source,
calibrated or family physics, bounded formula, precise `needs_input`, or
true `unsupported`. They must not finish as `low_confidence`,
`screening_fallback`, live nearby-row proxy fallback, or
lab/field/building alias. Source rows remain useful as exact overrides,
calibration anchors, holdouts, and negative boundaries; broad source
crawling is not the next mainline path.

Current remaining-gap analysis:

[CHECKPOINT_2026-05-15_COMPANY_INTERNAL_REMAINING_GAP_ANALYSIS_AND_PLAN.md](./CHECKPOINT_2026-05-15_COMPANY_INTERNAL_REMAINING_GAP_ANALYSIS_AND_PLAN.md)
records the current 71-row Matrix V6 posture and the controlled
internal-use sequence. The formula, runtime, surface, and Matrix V6
steps and the building/ASTM boundary revalidation have now landed as
`company_internal_opening_leak_a_weighted_spectrum_adapter_formula_corridor_plan`
and
`company_internal_opening_leak_a_weighted_spectrum_adapter_runtime_corridor_plan`,
`company_internal_opening_leak_a_weighted_spectrum_adapter_surface_parity_plan`,
...
then
`company_internal_building_astm_boundary_revalidation_after_a_weighted_matrix_plan`;
the remaining sequence is final internal-use rehearsal. The previous
implementation caveat is now closed: `frequencyBandSet` exists in the
...
mixed staged, field/building, and ASTM/IIC aliases stay blocked.
Selected next action:
`broad_accuracy_floor_open_box_timber_similarity_surface_parity_plan`;
selected next file:
...
Latest validation after this runtime gate:
...
- complete after matrix v2: Gate AT complete building runtime is imported
  into the active company-internal matrix;
- complete after matrix v2: the latest steel suspended-ceiling
  `DeltaLw` owner prompt is represented as `needs_input`, separate from
  missing numeric `DeltaLw` runtime;
- complete after matrix v2: `floor.heavy_concrete_floating_floor.lab`
  no longer hides `screening_fallback` as the matrix origin.

Parked ASTM selected next implementation file:
...
Parked ASTM selected next action:

`gate_bw_personal_use_mvp_floor_impact_astm_iic_aiic_contour_rating_owner_plan`

Current next plain label: company-internal ISO floor / wall
calculation-grade coverage and missing-input safety.

Parked ASTM next plain label: floor-impact ASTM IIC/AIIC contour rating
owner.

Parked ASTM exact label: floor-impact ASTM IIC/AIIC contour rating owner.

Company-internal opening/leak field/building input-surface validation
on 2026-05-14:

- focused web input-surface / adjacent continuity set: 3 files / 12
  tests passed
- `@dynecho/web` typecheck passed
- `@dynecho/web` lint passed
- the package-level web test wrapper was stopped after entering the full
  long-runner expansion and replaced with direct focused Vitest
  execution for the files above

Company-internal steel suspended-ceiling `DeltaLw` runtime validation on
2026-05-14:

- focused runtime corridor contract: 1 file / 6 tests passed
- nearby company-internal continuity set: 5 files / 25 tests passed
- `@dynecho/engine` typecheck passed
- scoped engine eslint passed for the steel runtime, input-surface, and
  matrix-v2 files touched by this slice

Company-internal steel suspended-ceiling `DeltaLw` surface parity
validation on 2026-05-14:

- focused surface parity contract: 1 file / 3 tests passed
- focused web surface parity: 1 file / 3 tests passed
- nearby company-internal engine continuity set: 6 files / 28 tests
  passed
- nearby steel web continuity set: 4 files / 11 tests passed
- `@dynecho/engine` and `@dynecho/web` typecheck passed
- scoped engine/web eslint passed for the files touched by this surface
  parity slice
- `pnpm calculator:gate:current` passed with engine 419 files / 2429
  tests, web 79 files / 337 passed + 18 skipped, repo build 5/5, and
  whitespace guard clean
- known non-fatal warnings: transient `fonts.googleapis.com` DNS
  retries, optional `sharp/@img` build packages, and Zustand unavailable
  test storage

Company-internal Matrix V3 validation on 2026-05-14:

- focused Matrix V3 contract: 1 file / 6 tests passed
- nearby company-internal continuity set: 6 files / 31 tests passed
- `@dynecho/engine` typecheck passed
- scoped engine eslint passed for the Matrix V3 slice files
- `pnpm calculator:gate:current` passed with engine 420 files / 2435
  tests, web 79 files / 337 passed + 18 skipped, repo build 5/5, and
  whitespace guard clean
- known non-fatal warnings: optional `sharp/@img` build packages and
  Zustand unavailable test storage

Company-internal steel suspended-ceiling low-frequency `L'nT,50` owner
validation on 2026-05-14:

- focused owner contract: 1 file / 6 tests passed
- nearby company-internal continuity with Matrix V3 and steel
  `DeltaLw` runtime/surface parity: 4 files / 21 tests passed
- `@dynecho/engine` typecheck passed
- scoped engine eslint passed for the low-frequency owner contract files
- `pnpm calculator:gate:current` passed with engine 421 files / 2441
  tests, web 79 files / 337 passed + 18 skipped, repo build 5/5, and
  whitespace guard clean
- known non-fatal warnings: optional `sharp/@img` build packages and
  Zustand unavailable test storage

Gate BV validation on 2026-05-14:
...
Heavy-composite cleanup validation on 2026-05-14:

- focused engine cleanup: 4 files / 18 tests passed
- web mixed study-mode replay: 1 file / 5 tests passed

Preceding Gate BU action:
...
Gate BU validation on 2026-05-14:
...
Gate BT validation on 2026-05-14:
...
- `pnpm calculator:gate:current` passed with engine 413 files / 2393
  tests, web 78 files / 334 passed + 18 skipped, repo build 5/5, and
  whitespace guard clean
- known non-fatal warnings: optional `sharp/@img` build packages and
  Zustand unavailable test storage

Preceding Gate BS action:
...
Gate BR next plain label: floor-impact ASTM IIC/AIIC runtime corridor.

Personal-Use MVP Coverage Sprint Gate BP has landed the no-retune
reinforced-concrete cleanup surface parity. Gate BP keeps the Gate BO
runtime values and basis frozen while making the new posture visible on
output cards, route card, impact trace, diagnostics/method/report
surfaces, saved replay, calculator API, and impact-only API. Complete
explicit or UI-derived owner input still returns lab `Ln,w 58.1` /
`DeltaLw 13.7` through
`predictor_heavy_combined_upper_lower_floor_iso12354_annexc_estimate`
with `+/-6.5 dB` / `+/-5.5 dB` source-absent budgets. Incomplete
explicit or visible-derived owner input shows impact cards as
`needs_input` with named missing physical owners instead of
unsupported-looking low-confidence output.

Landed Gate BP action:

`gate_bp_personal_use_mvp_reinforced_concrete_cleanup_surface_parity_plan`

Gate BP selects Gate BQ because the executable coverage matrix should
now be refreshed after the reinforced-concrete cleanup changed visible
posture from unsupported/low-confidence to precise needs-input or named
formula support.

Personal-Use MVP Coverage Sprint Gate BO has landed the
reinforced-concrete low-confidence cleanup. The old incomplete
reinforced-concrete combined upper/lower route no longer returns
`predictor_floor_system_low_confidence_estimate`, `Ln,w 50`, proxy
`Rw 65.9`, or proxy `Ctr 57`. Complete explicit owner input now returns
lab `Ln,w 58.1` / `DeltaLw 13.7` through
`predictor_heavy_combined_upper_lower_floor_iso12354_annexc_estimate`.
Incomplete explicit or visible-derived owner input is parked as
`needs_input` for impact outputs with named missing physical owners.
Gate BO selects Gate BP because card/report/API surfaces must now show
the new needs-input and formula corridor posture consistently.

Preceding Gate BO action:

`gate_bo_personal_use_mvp_reinforced_concrete_low_confidence_cleanup_plan`

Gate BO selection status:
...
Personal-Use MVP Coverage Sprint Gate BN has landed the no-runtime
coverage matrix refresh after steel suspended-ceiling. Gate BN selected
Gate BO after identifying the reinforced-concrete low-confidence cleanup
candidate in the refreshed matrix.
...
Gate BN selected next action:

`gate_bo_personal_use_mvp_reinforced_concrete_low_confidence_cleanup_plan`

Gate BN selected next file:
...
Personal-Use MVP Coverage Sprint Gate BM has landed the post steel
suspended-ceiling input-surface revalidation. Gate BM is no-runtime: it
revalidates complete and safe-reordered UI-derived steel
suspended-ceiling input at Gate BK lab `Ln,w 62.2` / `+/-6 dB`, keeps
partial steel input as precise missing physical owner prompts, keeps
duplicate steel carriers refused, preserves exact-source precedence,
and keeps Gate AD steel upper/lower values frozen at `Ln,w 55.6` /
`DeltaLw 22.4`.

Personal-Use MVP Coverage Sprint Gate BL has landed the steel-floor
suspended-ceiling input surface. Complete UI-derived steel
suspended-ceiling-only input now flows through the existing Dynamic
Calculator floor input surface to
`predictor_lightweight_steel_suspended_ceiling_corridor_estimate` and
keeps the Gate BK lab `Ln,w 62.2` / `+/-6 dB` source-absent budget.
Partial UI-derived input names `steelCarrierSpacingMm` and
`lowerCeilingIsolationSupportForm` instead of ending as a final
low-confidence answer.

Personal-Use MVP Coverage Sprint Gate BK has landed the steel-floor
low-confidence fallback cleanup. Complete steel suspended-ceiling-only
predictor input now gets the named
`predictor_lightweight_steel_suspended_ceiling_corridor_estimate` lab
`Ln,w` corridor instead of the broad low-confidence Pliteq/UBIQ
fallback. Gate AD steel upper/lower values stay frozen at
`Ln,w 55.6` / `DeltaLw 22.4`; exact source rows still win first.

Personal-Use MVP Coverage Sprint Gate BQ has landed the reinforced-
concrete cleanup matrix refresh. It is no-runtime: Gate BO/BP values
stay lab `Ln,w 58.1` / `DeltaLw 13.7` through
`predictor_heavy_combined_upper_lower_floor_iso12354_annexc_estimate`
with `+/-6.5 dB` / `+/-5.5 dB` source-absent budgets. The executable
matrix now replaces the old reinforced-concrete low-confidence
coverage-gap row with complete formula, visible-derived `needs_input`,
incomplete explicit `needs_input`, exact-source precedence, bare
heavy-floor, upper-only floating-floor, field/building non-alias, and
ASTM `IIC` / `AIIC` unsupported rows. The matrix has 58 rows and zero
calculation-grade `coverage_gap` rows.

Gate BQ landed action:

`gate_bq_personal_use_mvp_coverage_matrix_refresh_after_reinforced_concrete_cleanup_plan`

Preceding Gate BP landed action:
...
Gate BP selected next action:

`gate_bq_personal_use_mvp_coverage_matrix_refresh_after_reinforced_concrete_cleanup_plan`

Gate BP selected next file:
...
Preceding Gate BQ selection status:
...
Gate BQ selected implementation file:
...
`CHECKPOINT_2026-05-14_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_BR_HANDOFF.md`

Clean Gate BR implementation plan:

`SLICE_PERSONAL_USE_MVP_GATE_BR_FLOOR_IMPACT_ASTM_IIC_AIIC_ADAPTER_CONTRACT_PLAN.md`

Preceding Gate BQ implementation plan:

`SLICE_PERSONAL_USE_MVP_GATE_BQ_COVERAGE_MATRIX_REFRESH_AFTER_REINFORCED_CONCRETE_CLEANUP_PLAN.md`

Preceding Gate BP implementation plan:

`SLICE_PERSONAL_USE_MVP_GATE_BP_REINFORCED_CONCRETE_CLEANUP_SURFACE_PARITY_PLAN.md`

Preceding Gate BO implementation plan:

`SLICE_PERSONAL_USE_MVP_GATE_BO_REINFORCED_CONCRETE_LOW_CONFIDENCE_CLEANUP_PLAN.md`

Preceding Gate BN implementation plan:

`SLICE_PERSONAL_USE_MVP_GATE_BN_COVERAGE_MATRIX_REFRESH_AFTER_STEEL_SUSPENDED_CEILING_PLAN.md`

Preceding Gate BM implementation plan:

`SLICE_PERSONAL_USE_MVP_GATE_BM_POST_STEEL_SUSPENDED_CEILING_INPUT_SURFACE_REVALIDATION_PLAN.md`
...
Preceding Gate BL implementation plan:

`SLICE_PERSONAL_USE_MVP_GATE_BL_STEEL_FLOOR_SUSPENDED_CEILING_INPUT_SURFACE_PLAN.md`
...
Preceding Gate BK continuity:

`SLICE_PERSONAL_USE_MVP_GATE_BK_STEEL_FLOOR_LOW_CONFIDENCE_FALLBACK_CLEANUP_PLAN.md`
...
Gate BJ next plain label: steel-floor low-confidence fallback cleanup.

Gate BJ budget origin and low-frequency boundary:

`source_absent_field_building_adapter_error_budget`; `L'nT,50`.

Preceding Gate BI closeout:
...
Gate BI selected action:

`gate_bj_personal_use_mvp_floor_impact_field_building_runtime_corridor_plan`

Gate BI selected file:
...
- Personal-Use MVP Coverage Sprint Gate BJ has now landed
  `gate_bj_personal_use_mvp_floor_impact_field_building_runtime_corridor_plan`
  with selection status
  `gate_bj_personal_use_mvp_floor_impact_field_building_runtime_corridor_landed_selected_steel_floor_low_confidence_cleanup_gate_bk`.
  Field-volume source-absent impact now returns `L'n,w 52.3` /
  `L'nT,w 49.9`; building direct+flanking returns `L'nT,w 52.4`;
  direct+flanking is medium confidence instead of a default low bucket;
  and the structured
  `source_absent_field_building_adapter_error_budget` stays separate
  from measured lab evidence. Gate BJ selects
  `gate_bk_personal_use_mvp_steel_floor_low_confidence_fallback_cleanup_plan`
  in
  `packages/engine/src/calculator-personal-use-mvp-coverage-sprint-gate-bk-steel-floor-low-confidence-fallback-cleanup-contract.test.ts`.
- Personal-Use MVP Coverage Sprint Gate AU has now landed the
  company-internal daily-use ready handoff. Gate AV post-release
  accuracy roadmap is now landed as no-runtime planning. Gate AU
  does not move runtime values, tolerances, source precedence, inputs,
  API shape, cards, or report copy.
- Gate AV planning detail is now captured in
  ...
  It keeps Gate AV no-runtime, confirms the implementation spine is
  calculator-first rather than source-catalog-first, and recommends
  `gate_aw_personal_use_mvp_source_absent_solver_gap_cartography_plan`
  as the next Gate AW lane before any new runtime formula promotion.
- Personal-Use MVP Coverage Sprint Gate AV has now landed
  ...
  Gate AV does not move runtime behavior; it ranks source-absent solver
  gap cartography before broad source-row crawling.
- Personal-Use MVP Coverage Sprint Gate AW has now landed
  `gate_aw_personal_use_mvp_source_absent_solver_gap_cartography_plan`
  with selection status
  ...
  Gate AW maps current runtime-owned, `needs_input`, unsupported, adapter
  gap, and solver-gap layer-combination surfaces and selects
  ...
  This keeps the next work focused on stronger source-absent wall solver
  inputs before broad source crawling.
- Personal-Use MVP Coverage Sprint Gate AX has now landed
  `gate_ax_personal_use_mvp_advanced_wall_source_absent_solver_contract_plan`
  with selection status
  `gate_ax_personal_use_mvp_advanced_wall_source_absent_solver_contract_landed_no_runtime_selected_runtime_corridor_gate_ay`.
  Gate AX converts Gate AW's wall gaps into explicit physical owner
  groups for route/basis, leaf/cavity topology, panel dynamic
  properties, cavity absorbers, frame/coupling, opening/leak
  sub-elements, direct-curve/rating adapters, hostile-input guards, and
  source-absent budget visibility. It keeps runtime values frozen and
  selects
  ...
  This keeps the next work focused on a bounded advanced wall runtime
  corridor before floor impact, adapter, calibration, or broad source
  crawling lanes.
- Personal-Use MVP Coverage Sprint Gate AY has now landed
  `gate_ay_personal_use_mvp_advanced_wall_source_absent_solver_runtime_corridor_plan`
  with selection status
  ...
  Gate AY promotes complete explicit Gate AX owner input to lab
  `Rw 65` / `STC 65` / `C -1.1` / `Ctr -6.4` through
  `gate_ay_advanced_wall_source_absent_direct_curve_runtime_corridor`.
  It carries `+/-8 dB` source-absent budgets for `Rw` / `STC` and
  `+/-3 dB` budgets for `C` / `Ctr`, keeps weak opening/leak
  sub-elements in the energy balance, keeps exact source rows and
  existing grouped triple-leaf delegates first, and keeps missing-input,
  hostile-topology, and field/building boundary cases budget-free. It
  selects
  ...
  Validation passed on 2026-05-13: focused Gate AY 1 file / 8 tests,
  Gate AX + Gate AY continuity 2 files / 14 tests, Gate AW + Gate AX +
  Gate AY continuity 3 files / 20 tests,
  ...
  clean.
- Personal-Use MVP Coverage Sprint Gate AZ has now landed
  `gate_az_personal_use_mvp_advanced_wall_source_absent_solver_input_surface_plan`
  with selection status
  ...
  Gate AZ makes the Gate AY physical owner fields first-class on the
  Dynamic Calculator wall input surface and shared `AirborneContext`.
  Complete UI-derived advanced wall input preserves lab `Rw 65` /
  `STC 65` / `C -1.1` / `Ctr -6.4` with Gate AY method, candidate id,
  `family_physics_prediction`, and the visible source-absent budgets.
  Partial inputs stay `needs_input`, field/building targets stay
  unsupported, duplicate/split ownership is refused, saved/API replay
  keeps the same basis, and exact-source plus existing delegate
  precedence remains first. Gate AZ selects
...
- Personal-Use MVP Coverage Sprint Gate BA has now landed
  `gate_ba_personal_use_mvp_floor_impact_source_absent_solver_gap_cartography_plan`
  with selection status
  ...
  Gate BA is no-runtime and maps the current floor-impact
  layer-combination surface:
  exact official floor-system rows stay first, heavy concrete
  bare/floating formula paths remain stable, published-family concrete
  anchors stay labeled, steel/timber/CLT formula corridors remain owned,
  field impact context stays separate, ASTM/building outputs remain
  unsupported boundaries, and remaining source-absent gaps are
  concentrated around dynamic stiffness, load basis, carrier family,
  lower treatment coupling, mixed support families, and hostile
  floor-role topology. Runtime probes stay unchanged: exact `Ln,w 51`,
  heavy concrete `Ln,w 50.3` / `DeltaLw 24.3`, missing-load `DeltaLw`
  unsupported, field `L'n,w 52.3` / `L'nT,w 50.3`, and ASTM
  unsupported. Gate BA selects
  ...
  In plain terms, the next work is the Gate BB floor-impact
  source-absent input contract.
  Next plain label: floor-impact source-absent input contract.
- Personal-Use MVP Coverage Sprint Gate BB has now landed
  `gate_bb_personal_use_mvp_floor_impact_source_absent_input_contract_plan`
  with selection status
  `gate_bb_personal_use_mvp_floor_impact_source_absent_input_contract_landed_no_runtime_selected_formula_corridor_gate_bc`.
  Gate BB is no-runtime and turns the Gate BA floor-impact cartography
  into explicit physical owner fields: base/carrier family,
  topping/floating mass, resilient-layer dynamic stiffness or
  source-owned product curve, load basis, lower treatment coupling,
  steel carrier geometry, basis boundaries, and duplicate/split/mixed
  support guards. It keeps exact `Ln,w 51`, heavy concrete `Ln,w 50.3`
  / `DeltaLw 24.3`, missing-load published-family `Ln,w 47` with
  `DeltaLw` unsupported, and ASTM unsupported behavior frozen. Gate BB
  selects
  ...
  In plain terms, the next work is the Gate BC bounded
  floor-impact source-absent formula corridor.
  Next plain label: floor-impact source-absent formula corridor.
- Personal-Use MVP Coverage Sprint Gate BC has now landed
  `gate_bc_personal_use_mvp_floor_impact_source_absent_formula_corridor_plan`
  with selection status
  ...
  upper/lower floor-impact formula corridor: upper `DeltaLw` from loaded
  mass and dynamic stiffness, lower ceiling coupling as a separate term,
  bounded upper/lower interaction penalty, and source-absent not-measured
  budgets of `+/-6.5 dB` for lab `Ln,w` and `+/-5.5 dB` for lab
  `DeltaLw`. It keeps exact `Ln,w 51`, heavy concrete `Ln,w 50.3` /
  `DeltaLw 24.3`, missing-load `Ln,w 47` with `DeltaLw` unsupported,
  and ASTM unsupported behavior frozen. Gate BC selected the
  floor-impact source-absent runtime corridor as Gate BD:
  ...
  Gate BB + Gate BC continuity 3 files / 23 tests, engine build
  including DTS, `pnpm calculator:gate:current` with engine 396 files /
  2295 tests, web 76 files / 325 passed + 18 skipped, repo build 5/5,
  and `git diff --check` clean.
- Personal-Use MVP Coverage Sprint Gate BD has now landed
  `gate_bd_personal_use_mvp_floor_impact_source_absent_runtime_corridor_plan`
  with selection status
  ...
  `predictor_heavy_combined_upper_lower_floor_iso12354_annexc_estimate`.
  The values carry source-absent not-measured budgets of `+/-6.5 dB`
  for `Ln,w` and `+/-5.5 dB` for `DeltaLw`. Exact UBIQ remains
  `Ln,w 51` without invented `DeltaLw`; existing heavy floating remains
  `Ln,w 50.3` / `DeltaLw 24.3`; missing-load published anchor remains
  `Ln,w 47` with `DeltaLw` unsupported; missing lower treatment or
  missing dynamic stiffness blocks fallback; ASTM, field, and building
  outputs remain non-alias boundaries. Gate BD selects
  ...
  In plain terms, the next work is Gate BE floor-impact source-absent
  surface parity.
  ...
  regression guard 3 files / 109 tests, Gate AZ fixture type-safety
  guard 1 file / 5 tests, engine typecheck, engine build,
  `pnpm calculator:gate:current` with engine 397 files / 2301 tests,
  web 76 files / 325 passed + 18 skipped, repo build 5/5, whitespace
  guard clean, and `git diff --check` clean. Known non-fatal build
  warnings remain the optional `sharp/@img` resolution warnings from
  `@turbodocx/html-to-docx`.
- Post-Gate BD full revalidation passed on 2026-05-13. The first
  `pnpm check` attempt caught two lint-only issues: an unused Gate BA
  import and redundant `Boolean(...)` checks in the Gate AY advanced-wall
  adapter. After fixing them without runtime movement, `pnpm check`
  passed with lint, typecheck, engine 522 files / 3103 tests, web 182
  files / 985 passed + 18 skipped, and build 5/5. The revalidation
...
- Personal-Use MVP Coverage Sprint Gate BG has now landed
  `gate_bg_personal_use_mvp_floor_impact_source_absent_post_input_surface_revalidation_plan`
  with selection status
  ...
  `Heavy concrete combined input surface`. It keeps complete and safe
  reordered heavy-concrete combined input at lab `Ln,w 44.4` /
  `DeltaLw 30.1` through
  `predictor_heavy_combined_upper_lower_floor_iso12354_annexc_estimate`
  with unchanged `+/-6.5 dB` / `+/-5.5 dB` source-absent
  not-measured budgets. Missing load basis, duplicate or ambiguous
  heavy-concrete base ownership, exact source precedence, and
  field/ASTM basis boundaries remain blocked or parked. Gate BG
  selects
  ...
  In plain terms, the next work is Gate BH floor-impact source-absent
  ...
  typecheck, `pnpm calculator:gate:current` with engine 400 files /
  2314 tests, web 77 files / 328 passed + 18 skipped, repo build 5/5,
...
- Historical implementation chain note: the active calculator-first
  ...
  before later gates advanced to the current Gate AU/Gate AV state.
...
- Personal-Use MVP Coverage Sprint Gate X has now landed the AAC /
  ...
  did not move.
- Personal-Use MVP Coverage Sprint Gate Y has now landed the CLT /
  mass-timber `Ctr` spectrum adapter,
  `gate_y_personal_use_mvp_clt_mass_timber_ctr_spectrum_adapter_plan`.
  Complete element-lab CLT now supports
  `Rw 42 / STC 42 / C -1.2 / Ctr -6.1` through
  `gate_y_clt_mass_timber_ctr_spectrum_adapter_runtime` /
  `family_physics_prediction` with the same `+/-6 dB` uncalibrated
  budget. Gate Y preserves exact-source precedence, missing-density
  prompts, wrong-family and duplicate-CLT negatives, STC-only
  non-promotion, and field/building/ASTM boundaries. Validation
  completed on 2026-05-11: focused Gate Y 1 file / 7 tests, Gate H/X/Y
  continuity 3 files / 21 tests, targeted Gate W/O/P expectation
  refresh 3 files / 15 tests, engine typecheck,
  `pnpm calculator:gate:current`, broad `pnpm check`, and
  `git diff --check` all passed. Gate Y selection
  status:
...
- Personal-Use MVP Coverage Sprint Gate Z has now landed the
  no-runtime post-CLT-Ctr coverage revalidation,
  `gate_z_personal_use_mvp_post_clt_ctr_coverage_revalidation_plan`.
  The 28-row matrix is gap-free after Gate X and Gate Y:
  `coverage_gap: 0`, with `none: 18`, `correct_block: 5`,
  `hostile_input_refusal: 2`, `unsupported_metric: 1`, and
  `basis_boundary: 2`. No runtime values, budgets, source precedence,
  APIs, workbench inputs, route cards, or lab/field/building basis rules
  moved. Gate Z selection status:
  ...
  Gate AA is a scenario matrix v2 expansion: broaden realistic and
  hostile wall/floor combinations before high-leakage building,
  opening-field/STC, ASTM, or broad source-crawl lanes are promoted.
  Validation passed on 2026-05-11: focused Gate Z 1 file / 6 tests,
  Gate X/Y/Z continuity 3 files / 20 tests, engine typecheck,
  `pnpm calculator:gate:current` with engine 367 files / 2122 tests,
  web 73 files / 314 passed + 18 skipped, repo build 5/5, and
  whitespace guard clean.
- Personal-Use MVP Coverage Sprint Gate AA has now landed the
  no-runtime scenario matrix v2 expansion,
  `gate_aa_personal_use_mvp_scenario_matrix_v2_expansion_plan`.
  Gate AA expands the executable personal-use matrix from 28 to 40
  rows, keeps `coverage_gap: 0`, and adds realistic/hostile evidence for
  split-board double-leaf walls, safe grouped triple-leaf reorder,
  ambiguous flat multicavity stacks, multi-opening leakage, duplicate
  opening refusal, partial building prediction, missing timber/steel
  floor formula inputs, wrong-family steel floors, CLT low-frequency
  field local-guide posture, and heavy floating-floor safe reorder
  continuity. No runtime values, budgets, source precedence, APIs,
  workbench inputs, route cards, or basis rules moved. Gate AA selection
  status:
  ...
  Gate AB should add the flat multicavity topology input surface before
  opening `STC`, ASTM, building-prediction, or broad source-crawl lanes
  promote. Gate AA validation passed on 2026-05-11: focused Gate AA 1
  file / 6 tests, Gate X/Y/Z/AA continuity 4 files / 26 tests, engine
  typecheck, and `pnpm calculator:gate:current` with engine 368 files /
  2128 tests, web 73 files / 314 passed + 18 skipped, repo build 5/5,
  and whitespace guard clean.
- The post-Gate-AA planning check confirms the route is still correct:
  Gate AB should first harden flat/many-layer multicavity topology
  ownership because current schema, route assessment, workbench
  row-group parsing, and grouped runtime paths already support that
  direction. Source crawling, opening `STC`, field/building runtime
  terms, and ASTM adapters remain lower ROI until the explicit topology
  input surface is green.
- Personal-Use MVP Coverage Sprint Gate AB has now landed the
  no-runtime flat multicavity topology input contract,
  `gate_ab_personal_use_mvp_flat_multicavity_topology_input_surface_plan`.
  Selection status:
  ...
  Gate AB keeps flat/many-layer multicavity schedules `needs_input`
  until topology ownership is explicit, proves complete grouped owner
  fields including cavity fill/absorption, blocks stale
  `flat_layer_order` groups plus duplicate/empty/invalid ownership, and
  keeps the existing grouped triple-leaf runtime pin at
  `Rw 50 / STC 55 / C 0.8 / Ctr -7.3` without aliasing field or ASTM
  outputs. Gate AB validation passed on 2026-05-11: focused Gate AB 1
  file / 6 tests, Gate G/K/L/X/Y/Z/AA/AB continuity 8 files / 57 tests,
  engine typecheck, and `pnpm calculator:gate:current` with engine 369
  files / 2134 tests, web 73 files / 314 passed + 18 skipped, repo
  build 5/5, and whitespace guard clean.
- Personal-Use MVP Coverage Sprint Gate AC has now landed
  `gate_ac_personal_use_mvp_flat_multicavity_topology_surface_parity_plan`.
  Selection status:
  ...
  Gate AC carries grouped flat/many-layer topology through live
  workbench evaluation, saved replay, calculator API payloads, output
  cards, and Markdown report lines under the visible label `Wall
  multicavity topology owner set`. No runtime formulas, source rows, or
  lab/field/building/ASTM basis rules moved: the flat grouped stack
  remains explicit source-absent screening at `Rw 38 / STC 38 / C -1 /
  Ctr -5.6`, and the existing grouped triple-leaf solver pin remains
  `Rw 50 / STC 55 / C 0.8 / Ctr -7.3`. Partial topology still creates
  no report surface and remains `needs_input`. Gate AC validation passed
  on 2026-05-11: focused Gate AC engine 1 file / 4 tests, focused Gate
  AC web 1 file / 4 tests, Gate AB/AC engine continuity 2 files / 10
  tests, grouped-wall/AC web continuity 2 files / 8 tests, engine
  typecheck, web typecheck, and `pnpm calculator:gate:current` with
  engine 370 files / 2138 tests, web 74 files / 318 passed + 18
  skipped, repo build 5/5, and whitespace guard clean.
- Personal-Use MVP Coverage Sprint Gate AD has now landed
  `gate_ad_personal_use_mvp_flat_multicavity_broad_revalidation_and_internal_pilot_rehearsal_plan`.
  Selection status:
  ...
  Gate AD is no-runtime: the 40-row matrix remains gap-free, 23 rows are
  numeric supported, 17 rows are explicitly blocked/unsupported/basis or
  hostile cases, and the complete grouped flat/many-layer wall still
  stays screening-only at `Rw 38 / STC 38 / C -1 / Ctr -5.6`. The
  existing grouped triple-leaf solver pin remains `Rw 50 / STC 55 / C
  0.8 / Ctr -7.3`. Gate AE is selected because solver broadening for
  complete source-absent flat multicavity walls has higher ROI than
  broad source crawling, opening `STC`, field/building terms, floor
  polish, or ASTM/IIC adapters. Gate AD validation passed on
  2026-05-11: focused Gate AD engine 1 file / 5 tests, Gate AA/AB/AC/AD
  engine continuity 4 files / 21 tests, Gate AC web continuity 2 files /
  8 tests, engine typecheck, web typecheck, `pnpm
  calculator:gate:current` with engine 371 files / 2143 tests, web 74
  files / 318 passed + 18 skipped, repo build 5/5, whitespace guard
  clean, and explicit `git diff --check`.
- Personal-Use MVP Coverage Sprint Gate AE has now landed
  `gate_ae_personal_use_mvp_flat_multicavity_solver_broadening_plan`.
  Selection status:
  ...
  Gate AE is an algorithmic runtime promotion, not a source crawl:
  complete explicit `element_lab` grouped flat/many-layer multicavity
  walls now use `gate_ae_flat_multicavity_two_cavity_frequency_solver`
  / `family_physics_prediction` with `+/-7 dB`, returning `Rw 53 / STC
  57 / C -0.6 / Ctr -8`. The previous screening pin was `Rw 38 / STC
  38 / C -1 / Ctr -5.6`. Gate G full mineral-wool grouped triple-leaf
  remains unchanged at `Rw 50 / STC 55 / C 0.8 / Ctr -7.3`; stale,
  duplicate, missing-topology, field/building, ASTM, and IIC routes
  remain blocked by their explicit basis/input contracts. Gate AE
  validation passed on 2026-05-11: focused Gate AE/AC/AD engine
  contracts 3 files / 14 tests, focused flat-multicavity web parity 1
  file / 4 tests, engine typecheck, web typecheck, focused
  dynamic-airborne split line-count contract 1 file / 5 tests, and
  `pnpm calculator:gate:current` with engine 372 files / 2148 tests,
  web 74 files / 318 passed + 18 skipped, repo build 5/5, and
  whitespace guard clean. The Next build kept the known optional
  `sharp/@img` warnings but completed. A post-Gate-AE checkpoint on
...
- Personal-Use MVP Coverage Sprint Gate AJ has now landed
  `gate_aj_personal_use_mvp_post_opening_leak_stc_surface_revalidation_plan`.
  Selection status:
  ...
  Gate AJ is no-runtime revalidation: it proves Gate S `Rw 38.2`,
  Gate AH `STC 39`, high-leakage `Rw 33.7 / STC 34`, the `+/-6 dB`
  not-measured-evidence budget, Gate U input surface, field-context
  routes, building-prediction boundaries, and Gate W/AA matrix
  supported/unsupported separation remain unchanged. Gate AK is the
  selected STC-aware matrix refresh, not a broad source crawl.
...
- Personal-Use MVP Coverage Sprint Gate AM has now landed
  `gate_am_personal_use_mvp_airborne_building_prediction_direct_curve_owner_contract_plan`.
  Selection status:
  ...
  Gate AM is no-runtime: it defines the direct separating-element
  frequency curve owner for building prediction without promoting
  `R'w` or `DnT,w`. The owner requires selected dynamic airborne
  frequency-curve ownership, band resolution, ISO 717-1 rating basis,
  selected candidate trace ownership, and basis-compatible metric scope.
  Lab `Rw` / `STC`, field `R'w` / `DnT,w`, opening/leak lab adapters,
  source single numbers without curves, and legacy raw dynamic
  field/building continuation snapshots stay blocked as building direct
  energy aliases. Gate AN is selected because flanking path energy is
  the next required owner before junction, room, uncertainty, or runtime
  work can become safe. Validation passed on 2026-05-12: focused Gate
  AM 1 file / 6 tests, Gate AL/Gate AM continuity 2 files / 12 tests,
  engine typecheck, `pnpm calculator:gate:current` with engine 380
  files / 2193 tests, web 74 files / 318 passed + 18 skipped, build
...
- Personal-Use MVP Coverage Sprint Gate AN has now landed
  ...
  180 files / 993 passed + 18 skipped, lint/typecheck clean, and build
  5/5.
- Personal-Use MVP Coverage Sprint Gate AO has now landed
  `gate_ao_personal_use_mvp_airborne_building_prediction_junction_vibration_owner_contract_plan`.
  Selection status:
  ...
  Gate AO is no-runtime: it defines the junction vibration reduction
  owner for building prediction without promoting `R'w` or `DnT,w`.
  The owner requires the Gate AN flanking path energy dependency,
  explicit junction class ownership, coupling length ownership,
  basis-compatible vibration reduction index ownership, path-specific
  junction coupling ownership, and basis-compatible metric scope.
  Generic junction class labels, lab `Rw` / `STC`, field `R'w` /
  `DnT,w`, opening/leak lab adapters, source single numbers without
  junction terms, and legacy raw dynamic field/building continuation
  snapshots stay blocked as building junction-vibration aliases. Gate
  AP is selected because room standardization is the next required
  owner before uncertainty or runtime work can become safe. Gate AO
  validation passed on 2026-05-12: focused Gate AO 1 file / 6 tests,
  Gate AN/Gate AO continuity 2 files / 12 tests, engine typecheck,
  `pnpm calculator:gate:current` with engine 382 files / 2205 tests,
  web 74 files / 318 passed + 18 skipped, build 5/5, whitespace guard
  clean, and full `pnpm check` with engine 507 files / 3007 tests, web
  180 files / 993 passed + 18 skipped, lint/typecheck clean, and build
  5/5. `git diff --check` passed after validation-note sync.
- Personal-Use MVP Coverage Sprint Gate AP has now landed
  `gate_ap_personal_use_mvp_airborne_building_prediction_room_standardization_owner_contract_plan`.
  Selection status:
  ...
  Gate AP is no-runtime: it defines the room absorption /
  standardization owner for building prediction without promoting
  `R'w` or `DnT,w`. The owner requires the Gate AO junction vibration
  dependency, separating element area ownership, receiving-room volume
  ownership, receiving-room RT60 ownership, building standardization
  basis ownership, basis-compatible room absorption ownership, and
  basis-compatible metric scope. Generic room labels, apparent `R'w`
  relabelled as `DnT,w`, lab `Rw` / `STC`, field `R'w` / `DnT,w`,
  opening/leak lab adapters, source single numbers without room terms,
  and legacy raw dynamic field/building continuation snapshots stay
  blocked as building room-standardization aliases. Gate AQ is selected
  because uncertainty-budget ownership is the last Gate AL owner before
  any later all-owner runtime corridor can be reconsidered. Gate AP
  validation passed on 2026-05-12: focused Gate AP 1 file / 6 tests,
  Gate AO/Gate AP continuity 2 files / 12 tests, engine typecheck,
  `pnpm calculator:gate:current` with engine 383 files / 2211 tests,
  web 74 files / 318 passed + 18 skipped, build 5/5, whitespace guard
...
- Personal-Use MVP Coverage Sprint Gate AU has now landed
  ...
  accuracy roadmap. The post-release accuracy roadmap covers calibration,
  budget tightening, opening/leak building adapter, and ASTM/IIC adapter
  ranking. The detailed Gate AV plan now selects source-absent solver
  gap cartography as the recommended next lane before new runtime
  promotion. See
  [SLICE_PERSONAL_USE_MVP_GATE_AV_POST_RELEASE_ACCURACY_AND_ADAPTER_ROADMAP_PLAN.md](./SLICE_PERSONAL_USE_MVP_GATE_AV_POST_RELEASE_ACCURACY_AND_ADAPTER_ROADMAP_PLAN.md)
  and
  [CHECKPOINT_2026-05-12_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AU_HANDOFF.md](./CHECKPOINT_2026-05-12_PERSONAL_USE_MVP_COVERAGE_SPRINT_GATE_AU_HANDOFF.md).
...
- Personal-Use MVP Coverage Sprint Gate AV has now landed
  ...
  Selected next file:
  `packages/engine/src/calculator-personal-use-mvp-coverage-sprint-gate-aw-source-absent-solver-gap-cartography-contract.test.ts`.
  Gate AV is no-runtime: it preserves Gate AU values, source precedence,
  budgets, API/card/report surfaces, and basis labels while ranking
  source-absent solver gap cartography before broad source-row crawling.
  See
...
- Personal-Use MVP Coverage Sprint Gate AW has now landed
  ...
  Selected next file:
  `packages/engine/src/calculator-personal-use-mvp-coverage-sprint-gate-ax-advanced-wall-source-absent-solver-contract.test.ts`.
  Gate AW is no-runtime executable cartography. It maps 20 wall/floor
  cells across runtime-owned, source-absent solver gap, adapter gap,
  unsupported, and `needs_input` boundaries, then selects the advanced wall source-absent solver contract
  before floor impact, adapter,
  calibration, or broad source-row crawling lanes. See
  ...
  Validation passed on 2026-05-13: focused Gate AW 1 file / 6 tests,
  Gate AV + Gate AW continuity 2 files / 13 tests,
  `pnpm calculator:gate:current` with engine 390 files / 2253 tests,
  web 75 files / 321 passed + 18 skipped, build 5/5, and whitespace
  guard clean.
- Personal-Use MVP Coverage Sprint Gate AX has now landed
  `gate_ax_personal_use_mvp_advanced_wall_source_absent_solver_contract_plan`.
  Selection status:
  ...
  Selected next file:
  `packages/engine/src/calculator-personal-use-mvp-coverage-sprint-gate-ay-advanced-wall-source-absent-solver-runtime-corridor-contract.test.ts`.
  Gate AX is no-runtime. Complete advanced wall input is only
  `ready_for_runtime_gate`; missing panel dynamics, cavity/absorber
  terms, opening/leak inputs, duplicate/split topology, field/building
  outputs, and exact-source same-stack cases fail closed or delegate
  before any generalized solver can return a number. See
  ...
  Validation passed on 2026-05-13: focused Gate AX 1 file / 6 tests,
  Gate AW + Gate AX continuity 2 files / 12 tests,
  `pnpm calculator:gate:current` with engine 391 files / 2259 tests,
  web 75 files / 321 passed + 18 skipped, build 5/5, and whitespace
  guard clean.
...
- Company-internal daily-use status: company-internal daily-use ready
  ...
  remaining coverage gaps or daily-use release blockers, Gate AU has
  landed the release handoff, Gate AV has landed the no-runtime
  post-release roadmap, Gate AW has landed source-absent solver gap
  cartography, and Gate AX has landed the no-runtime advanced wall
  source-absent solver contract. The next path is Gate AY advanced wall
  runtime corridor, not broad source-row crawling. See
  [CHECKPOINT_2026-05-12_INTERNAL_DAILY_USE_FINAL_PATH_AND_REVALIDATION_HANDOFF.md](./CHECKPOINT_2026-05-12_INTERNAL_DAILY_USE_FINAL_PATH_AND_REVALIDATION_HANDOFF.md).
...
- Personal-Use MVP Coverage Sprint Gate C has now landed the
  timber/CLT `DeltaLw` formula-corridor contract without runtime
  movement.
- Personal-Use MVP Coverage Sprint Gate D has now landed the first
  timber/CLT `DeltaLw` runtime formula corridor for explicit complete
  predictor input.
- The matrix proves which common calculator cases produce exact,
  source-anchored, family-physics, bounded/screening, `needs_input`, or
  `unsupported` results through current public engine entry points.
- Gate B proves complete timber joist and CLT/mass-timber inputs can
  advance to a formula corridor, while missing dynamic stiffness, load
  basis, topping/floating mass, or lower assembly remains `needs_input`.
- Gate C proves separate timber joist and mass-timber CLT formula
  corridors, physical formula terms, `+/-7.5 dB` source-absent design
  budgets, and exact negative boundaries for missing inputs, ASTM,
  field/building basis, steel-family leakage, and exact `Ln,w`
  precedence.
- Gate D promotes timber `DeltaLw 25.2` and CLT `DeltaLw 22.6` as
  source-absent lab formula metrics while keeping timber exact
  `Ln,w 51` and CLT family `Ln,w 50` on their own basis.
- Gate E has now landed visible/API/report parity for those runtime
  values: output cards, dynamic trace, support notes, corridor/method
  dossiers, Markdown report, calculator API, and impact-only API all
  preserve the same `DeltaLw` basis and `+/-7.5 dB`
  `source_absent_formula_error_budget`.
- Gate F has now landed first-class Dynamic Calculator input-surface
...
- Gate M has now landed the airborne building-prediction input
  contract. Complete owner sets require source-room volume,
  receiving-room volume/RT60, flanking/junction class, conservative
  flanking assumption, junction coupling length, and building output
  basis, plus the existing separating element area. Complete physical
  inputs remain `unsupported` until Gate N owns the ISO 12354-1
  flanking/runtime adapter; partial owner sets remain `needs_input` with
...
- The shared airborne context now names source-room volume,
  flanking/junction class, conservative flanking assumption, junction
  coupling length, and building output basis as first-class building
  prediction fields.
- Route input topology now classifies airborne building prediction as
  `building_prediction_airborne_context` and uses the Gate M complete
...
- Complete `field_between_rooms` Gate I/J/K values remain live and
  numeric-stable, and exact-source precedence remains first.
- Gate M selected Gate N:
  `gate_n_personal_use_mvp_airborne_building_prediction_runtime_adapter_plan`.

Gate M validation result:

Validation completed on 2026-05-10: focused Gate M engine input
contract passed 1 file / 6 tests; Gate L continuity passed 1 file / 5
tests; Gate I/J/K airborne field-context continuity passed 3 files / 15
tests; focused Gate K route-input topology continuity passed 1 file / 8
tests; focused workbench building/field input surfaces passed 2 files /
8 tests; engine and web typechecks passed; final
`pnpm calculator:gate:current` passed with engine 354 files / 2050
tests, web 71 files / 306 passed + 18 skipped, repo build 5/5
successful, and whitespace guard clean; final `git diff --check`
passed. Known non-fatal warnings remain the existing Zustand unavailable
test-storage warnings and optional `sharp/@img` package resolution
warnings during the Next build.

Previous Gate L result:
...
later calculator slices moved the current composer size to 1969 physical
lines after Gate AZ; final
...
- complete UI-derived timber input returns exact `Ln,w 51` plus formula
  `DeltaLw 25.2` with the timber joist `DeltaLw` formula basis;
- complete UI-derived CLT input returns family `Ln,w 50` plus formula
  `DeltaLw 22.6` with the mass-timber CLT `DeltaLw` formula basis;
- partial/invalid physical fields remain parked with precise missing
  field warnings and no `DeltaLw` error budget;
...
Previous Gate E result:

- Gate E adds `timber-clt-delta-lw-corridor-view.ts`, the focused Gate E
  engine contract, and the focused web surface/API parity test;
- timber exact `Ln,w 51` stays exact while timber `DeltaLw 25.2` is
  visibly labelled as the timber joist formula corridor;
- CLT family `Ln,w 50` stays family-estimated while CLT `DeltaLw 22.6`
  is visibly labelled as the mass-timber CLT formula corridor;
- cards, posture, metric-basis copy, dynamic trace panel, support notes,
  Markdown report, calculator API, and impact-only API preserve the same
  metric basis and `+/-7.5 dB` not-measured error budget;
- missing physical inputs, ASTM, field requests, `Ln,w`-only requests,
  and wrong-family steel requests remain budget-free;
- Gate F is selected for first-class timber/CLT input-surface parity.
...
Gate E selected next file:

`packages/engine/src/calculator-personal-use-mvp-coverage-sprint-gate-f-timber-clt-floor-impact-delta-lw-input-surface-contract.test.ts`

Gate E selected next action:
...
Previous Gate D result:

- Gate D adds
  `timber-clt-floor-impact-delta-lw-runtime-corridor.ts` and the focused
  Gate D runtime contract test;
- explicit complete timber joist input returns exact `Ln,w 51` plus
  source-absent formula `DeltaLw 25.2`;
- explicit complete CLT/mass-timber input returns family `Ln,w 50` plus
  source-absent formula `DeltaLw 22.6`;
- `DeltaLw` uses its own metric basis:
  `predictor_timber_joist_delta_lw_formula_corridor_estimate` or
  `predictor_mass_timber_clt_delta_lw_formula_corridor_estimate`;
- both formula values carry `+/-7.5 dB`
  `source_absent_formula_error_budget` payloads marked not measured
  evidence;
- missing physical inputs, `Ln,w`-only, ASTM, field, and wrong-family
  steel requests remain blocked from timber/CLT `DeltaLw` promotion;
- Gate E is selected for output-card, dynamic-trace/support-note,
  Markdown report, calculator API, and impact-only API parity.
...
Gate D selected next file:

`packages/engine/src/calculator-personal-use-mvp-coverage-sprint-gate-e-timber-clt-floor-impact-delta-lw-surface-parity-contract.test.ts`

Gate D selected next action:

`gate_e_personal_use_mvp_timber_clt_floor_impact_delta_lw_surface_parity_plan`

Gate D validation result:

Validation completed on 2026-05-08: focused Gate D runtime contract
passed 1 file / 7 tests, focused Gate B/C/D continuity passed 3 files /
22 tests, focused Gate BI/A/B/C/D continuity passed 5 files / 36 tests,
engine typecheck passed, and final `pnpm calculator:gate:current`
...
Previous Gate C result:

- Gate C adds
  `timber-clt-floor-impact-delta-lw-formula-corridor.ts` and the focused
  Gate C contract test;
- timber joist and CLT/mass-timber routes now have named source-absent
  formula corridors with terms for reference-floor `Ln,w`, loaded upper
  mass, resilient dynamic stiffness transfer, lower-assembly coupling,
  and structural-family correction;
- current runtime is unchanged: timber exact `Ln,w 51` and CLT family
  `Ln,w 50` still leave `DeltaLw` unsupported;
- both source-absent `DeltaLw` design budgets are `+/-7.5 dB` and are
  labelled as not measured evidence;
- Gate B missing-input negatives and ASTM/field/building/exact-source
  boundaries remain blocked;
- Gate D consumed this selection and promoted the runtime corridor.
...
Previous Gate B result:

- Gate B adds `timber-clt-floor-impact-delta-lw-input-contract.ts` and
  the focused Gate B contract test;
- required physical inputs are `baseSlabOrFloor`,
  `toppingOrFloatingLayer`, `resilientLayerDynamicStiffnessMNm3`,
  `loadBasisKgM2`, and `ceilingOrLowerAssembly`;
- complete timber joist and CLT predictor inputs are
  `ready_for_formula_corridor_gate`, but current runtime still exposes
  timber exact `Ln,w 51` and CLT family `Ln,w 50` with `DeltaLw`
  unsupported;
...
- ASTM `IIC` / `AIIC`, field `L'n,w` / `L'nT,w`, steel floors, and
  exact `Ln,w` source precedence are guarded from `DeltaLw` aliasing or
  promotion;
- Gate C consumed this input contract and selected Gate D runtime
  corridor promotion.
...
skipped, repo build 5/5 successful, and whitespace guard clean. Known
warnings were existing Zustand unavailable test-storage warnings and
optional `sharp/@img` package resolution warnings during web build.

Current Gate A result:

- Gate A adds `calculator-personal-use-mvp-coverage-sprint.ts` and the
  focused Gate A scenario-matrix contract test;
- 24 executable rows cover single/masonry/AAC walls, laminated boards,
  double/resilient framed walls, grouped and ambiguous triple-leaf
  walls, lined massive and CLT walls, field airborne, heavy concrete
  floating floors, lightweight-steel formula/exact/ambiguous cases,
  timber/CLT impact, field impact, ASTM/IIC boundaries, many-layer
  stress, and invalid thickness refusal;
- critical values are pinned, including `Rw 58`, grouped triple-leaf
  `Rw 50`, non-50/50 grouped triple-leaf `Rw 55`, heavy floating floor
  `Ln,w 44.9` / `DeltaLw 26.9`, steel formula `Ln,w 55.6` /
  `DeltaLw 22.4`, exact steel `Ln,w 51`, and exact timber field impact
  `L'n,w 53` / `L'nT,w 50.6`;
- field, lab, and ASTM basis boundaries remain separate;
- the selected Gate B lane is
  `gate_b_personal_use_mvp_timber_clt_floor_impact_delta_lw_input_contract_plan`.
...
Current Gate BI result:

- Gate BI froze runtime values, tolerances, exact-source precedence,
  source ingestion, formula coefficients, and lab/field/building basis
  boundaries.
- The `tighten` label is governance-only; future corridor movement still
  requires independent measured residual ownership, topology diversity,
  rights/citation posture, exact-source boundaries, basis owners,
  tolerance-delta runtime proposal, and card/report/API parity.
- The first post-Gate-BI artifact is not another source ledger. It is an
  executable scenario matrix over current wall/floor engine behavior.
- That matrix must expose what is usable today, what correctly returns
  `needs_input` or `unsupported`, and what common scenario needs the
  next family solver.
- The next solver lane is selected from matrix failures by coverage ROI
  and accuracy risk. Likely candidates remain generalized wall
  multi-cavity/triple-leaf routing, lined-massive/masonry upgrades,
  timber/CLT floor impact, and basis-owned field/building continuations,
  but the matrix decides.

Post-Gate-BI selected lane:

`personal_use_mvp_coverage_sprint_after_gate_bi`

Post-Gate-BI first file:

`packages/engine/src/calculator-personal-use-mvp-coverage-sprint-gate-a-scenario-matrix-contract.test.ts`

Post-Gate-BI first action:
...
- Gate BH landed a no-runtime residual-policy closed-owner
  revalidation for the steel-floor same-stack ISO lab `DeltaLw` path;
- Gate BG's selected `residual_policy_closed_owner_revalidation` lane is
  the only input;
- the closed owner map preserves Gate BD holdout-count closure, Gate BE
  paired-negative closure, Gate BF open-web input ownership, and Gate BG
  field/building basis ownership as evidence-only requirements;
- policy-only residual evidence now has three same-stack ISO `DeltaLw`
  residual cases and four paired negative boundaries, with open-web
  formula inputs and field/building basis owners marked source-owned;
- the closed owner map revalidates as a policy-only `tighten` candidate
  (`max 0.6 dB`, `mean 0.6 dB`) because all owner blockers are closed;
- that candidate is not a runtime action: no tolerance change, formula
  retune, exact-source promotion, source text/document ingestion, or
  field/building alias is allowed at Gate BH;
...
- tighten-candidate governance is selected as Gate BI before any
  tolerance movement can be considered.
...
- Gate BG landed a no-runtime field/building basis owner closure for the
  steel-floor same-stack ISO lab `DeltaLw` path;
- only Gate BF's selected field/building basis owner closure lane can
  feed field/building basis owner evidence;
- field apparent impact and building-prediction owners are separate:
  field packets must own `L'n,w` / `L'nT,w` context and building packets
  must own building-prediction context;
- both context families must own receiving room geometry/volume,
  separating element area, junction/flanking context,
  reverberation/normalization basis, and rights-safe locator or
  project-context metadata;
- complete future field/building basis owner packets count only as
  residual-policy readiness evidence, not runtime field/building outputs,
  exact rows, formula retune, or tolerance movement;
- missing owner fields, missing physical context values, missing
  locator/project metadata, wrong basis, lab-corridor alias attempts,
  wrong metric family, product/inferred claims, and rights-blocked
...
- Gate BF landed a no-runtime source-owned open-web formula input
  ownership closure for the steel-floor same-stack ISO lab `DeltaLw`
  path;
- only Gate BE's selected open-web input ownership closure lane can feed
  open-web input ownership evidence;
- the closure packet must own open-web support form, carrier depth,
  carrier spacing, load basis, dynamic stiffness, lower support class,
  upper resilient topology, and rights-safe locator metadata;
- complete future source-owned open-web formula input packets count only
  as residual-policy readiness evidence, not exact rows, runtime
  ingestion, formula retune, or tolerance movement;
...
- field/building basis owner closure is selected as Gate BG because
  Gate BF can close open-web input ownership while field/building basis
  ownership remains the last ranked blocker;
- broad source crawl, source text/document ingestion, exact-source
  promotion, tolerance movement, formula retune, field/building alias,
  and runtime movement remain blocked;
- runtime values remain `Ln,w 55.6` / `DeltaLw 22.4`, tolerances remain
...
unavailable test-storage warnings and optional sharp package resolution
warnings during web build. Broad `pnpm check` was not rerun because Gate
BF has no runtime/API/UI surface change.

Gate BE result:

- Gate BE landed a no-runtime paired negative-boundary closure for the
  steel-floor same-stack ISO lab `DeltaLw` path;
- only Gate BD's selected paired-negative closure lane can feed
  paired-negative closure evidence;
- the three additional boundaries required to close the paired-negative
  shortfall must own the target metric family, ISO lab basis, explicit
  wrong-support or wrong-reference boundary, boundary identity,
  same-stack steel exclusion reason, not-holdout scope, and rights-safe
  locator metadata;
- complete future wrong-support and wrong-reference boundary packets
  count only as residual-policy readiness evidence, not exact rows,
  runtime ingestion, formula retune, or tolerance movement;
...
- source-owned open-web formula input ownership is selected as Gate BF
  because paired-negative closure can close the three-boundary shortfall
  while open-web input ownership remains a ranked blocker;
- broad source crawl, source text/document ingestion, exact-source
  promotion, tolerance movement, formula retune, field/building alias,
  and runtime movement remain blocked;
- runtime values remain `Ln,w 55.6` / `DeltaLw 22.4`, tolerances remain
...
unavailable test-storage warnings and optional sharp package resolution
warnings during web build. Broad `pnpm check` was not rerun because Gate
BE has no runtime/API/UI surface change.

Gate BD result:

- Gate BD landed a no-runtime holdout-count closure boundary for the
  steel-floor same-stack ISO lab `DeltaLw` path;
- only Gate BC's selected source-owned same-stack ISO `DeltaLw`
  holdout-count closure lane can feed holdout-closure evidence;
- the two additional holdouts required to close the Gate BC shortfall
  must own the measured `DeltaLw` value, same-stack steel reference, ISO
  lab basis, all Gate AT/AK owner fields, paired negative-boundary
  ownership, and rights-safe locator metadata;
- complete future source-owned holdout packets count only as
  residual-readiness evidence, not exact rows, runtime ingestion, formula
  retune, or tolerance movement;
...
- paired negative-boundary closure is selected as Gate BE because the
  future holdout-count closure probe can close the two-holdout
  shortfall while paired negative boundaries still have a shortfall of 3;
- broad source crawl, source text/document ingestion, exact-source
  promotion, tolerance movement, formula retune, field/building alias,
  and runtime movement remain blocked;
- runtime values remain `Ln,w 55.6` / `DeltaLw 22.4`, tolerances remain
...
- Gate BC landed a no-runtime residual blocker-closure ranking for the
  steel-floor same-stack ISO lab `DeltaLw` path;
- only Gate BB accepted policy-decision rows can feed closure-lane
  ranking;
- current blocked policy rows and rejected probes remain blocked before
  closure ranking;
- the source-owned same-stack ISO `DeltaLw` holdout-count lane is
...
- Gate BB landed a no-runtime residual-policy decision surface for the
  steel-floor same-stack ISO lab `DeltaLw` path;
- only Gate BA residual-admitted rows can reach policy decision;
- current blocked rows and rejected probes remain blocked before policy
  decision;
- the admitted future same-stack ISO `DeltaLw` row remains
  `hold_current_corridor_policy_decision`;
- blocker closure requirements stay explicit: 2 additional source-owned
...
- Gate BA landed a no-runtime residual-admission boundary for the
  steel-floor same-stack ISO lab `DeltaLw` packet path;
- only Gate AZ accepted calibration evidence candidates can enter
  residual-policy evaluation;
- current request-status rows, rejected Gate AY probes, and blocked Gate
  AZ candidate rows remain blocked;
- residual admission preserves source-owned measured `DeltaLw`,
...
- the accepted future same-stack ISO `DeltaLw` calibration candidate can
  enter residual evaluation, but its residual policy remains `hold`
  because holdout-count, paired-negative-boundary, open-web-input, and
  field/building owner thresholds are still not complete;
...
- Gate AZ landed a no-runtime calibration-candidate boundary for the
  steel-floor same-stack ISO lab `DeltaLw` packet path;
- only Gate AY accepted packet boundary rows can become calibration
  candidate inputs;
...
- rejected Gate AY probes remain blocked before calibration candidate
  use;
- the complete future same-stack ISO `DeltaLw` packet boundary probe can
  become a calibration evidence candidate only when rights-safe
...
- Gate AY landed a no-runtime packet acceptance boundary for the
  steel-floor same-stack ISO lab `DeltaLw` packet path;
- only Gate AX request-ledger rows are current packet acceptance
  boundary rows;
- current request-ledger entries remain in request status because no
  source-owned packet is present yet;
- the complete future same-stack ISO `DeltaLw` packet probe can leave
  request status only when measured `DeltaLw`, same-stack steel
  reference, ISO 10140 / 717-2 lab basis, all Gate AT/AK owner fields,
  and paired negative-boundary ownership are source-owned;
...
- Gate AX landed a no-runtime rights-safe packet request ledger for the
  steel-floor same-stack ISO lab `DeltaLw` target;
- only Gate AW ready packet request rows become request-ledger entries;
  blocked readiness rows remain blocked;
...
- Gate AW landed a no-runtime packet-acquisition readiness contract for
  the steel-floor same-stack ISO lab `DeltaLw` target;
- only Gate AV accepted intake rows can become packet request
  candidates; rejected intake rows remain blocked;
- ready packet requests must carry same-stack steel ISO `DeltaLw` scope,
...
- Gate AV landed a no-runtime metadata-only source-lead intake ledger for
  the steel-floor same-stack ISO lab `DeltaLw` target;
- Gate AU remains the only intake surface: manufacturer lab-report
  index, accredited lab-report index, and internal measurement packet
  leads can become acquisition request targets only when they preserve
  the same-stack steel, lab ISO 10140 / 717-2 `DeltaLw`, owner-field,
  and rights-safe constraints;
...
- Gate AU landed a no-runtime narrow rights-safe source-lead plan for
  the steel-floor same-stack ISO lab `DeltaLw` target;
- Gate AT packet acceptance is the only source-lead scope: same-stack
  steel, lab ISO 10140 / 717-2 `DeltaLw`, all source-owned owner fields,
  and rights-safe metadata-only handling;
- manufacturer lab-report index leads, accredited lab-report index
  leads, and internal measurement packet leads can proceed to a later
  packet acquisition/intake gate when they name every required owner
  field;
- accepted leads are metadata-only packet acquisition targets, not
  calibration evidence yet, not exact overrides, and not retune inputs;
...
- Gate AT landed a no-runtime same-stack ISO lab `DeltaLw` packet target
  fixture contract for the steel-floor formula corridor;
- Gate AS selected target is the only acceptance surface: lab ISO 10140 /
  717-2 `DeltaLw`, same-stack steel reference, and all Gate AK
  source-owned physical owner fields;
- the future same-stack source-owned ISO `DeltaLw` fixture is accepted
  as calibration evidence only, not as an exact override and not as a
  runtime retune input;
- accepted fixture residual policy still holds because holdout count,
  paired negative boundaries, open-web source-owned formula inputs, and
  field/building basis owners are incomplete;
...
- Gate AS landed a no-runtime owner-evidence targeting decision for the
  steel-floor formula corridor;
- all seven Gate AQ owners are ranked by calculator impact, acquisition
  feasibility, current ledger gap, and rejection-boundary risk;
- same-stack ISO lab `DeltaLw` packet ownership is selected as the next
  narrow target because it covers both `Ln,w` and `DeltaLw`, current
  accepted local source-owned packet count remains zero, and Gate AK/AQ/AR
  already define the packet owner fields;
...
- Gate AR landed a no-runtime calibration-evidence intake ledger for the
  steel-floor formula corridor;
- Gate AQ term owners are now the only acceptance surface for
  calibration evidence;
- current local Gate AK/AM evidence is classified into accepted,
  missing-owner-field, wrong-basis, wrong-reference-floor,
  product/inferred, and boundary-only buckets;
- current local accepted source-owned calibration packet count remains
  zero;
- product-only, inferred, STC/IIC, ASTM/field/building,
  concrete-reference, and boundary-only evidence remains unable to
  tighten the lab formula corridor;
- a future source-owned same-stack ISO `DeltaLw` packet satisfies only
  the `source_owned_delta_lw_holdout_absence` owner and still cannot
  move runtime values while residual thresholds are incomplete;
- Gate AS is selected as an owner-evidence targeting gate, not as a
  broad crawl or formula retune.
...
- Gate AQ landed a no-runtime calibration-readiness contract for the
  steel-floor formula error budget;
- every current runtime error-budget term is mapped to a source-owned
  evidence owner and current blocker, and the mapping is exhaustive
  against the actual `ImpactErrorBudget.terms`;
- current evidence still resolves to `hold` for both `Ln,w` and
  `DeltaLw`, with runtime values pinned at `Ln,w 55.6` /
  `DeltaLw 22.4` and tolerances pinned at `+/-4.5 dB` / `+/-2.0 dB`;
- product-only, inferred, STC/IIC, field/building,
  wrong-reference-floor, and concrete-reference evidence remains unable
  to shrink budget terms or tighten the corridor;
- future `hold`, `tighten`, `widen`, and `retune_candidate` cases are
  executable policy branches, but all keep `runtimeValueMovement:
  false`;
- Gate AR is selected as an evidence-intake / decision-ledger gate, not
...
2026-05-08 model-first physics prediction pivot Gate AX steel-floor
formula same-stack ISO DeltaLw packet request ledger plan.
...
- complete source-absent steel formula cases still calculate
  `Ln,w 55.6` and `DeltaLw 22.4`, but expose explicit budget terms:
...
- Gate AJ landed paired negative-boundary proof for the steel-floor
  formula lane without moving runtime values;
- the four protected negatives are wrong support family, exact measured
  row precedence, lab-to-field/building basis leakage, and source-absent
  design references that must not count as measured residuals;
- the Gate AI paired-negative blocker is now removed from the executable
  `Ln,w` and `DeltaLw` residual policies, but both policies still hold;
- measured `DeltaLw` holdout intake is now separate from product catalog
  improvements, Annex-C or companion inferences, and field/ASTM/building
  basis values;
- UBIQ open-web exact rows stay exact rows or calibration anchors until
  carrier spacing, load basis, dynamic stiffness, lower support class,
...
- preserve exact-source precedence and runtime value freeze unless the
  residual policy explicitly promotes a later correction.
...
- `Ln,w` policy currently resolves to `hold`: Gate AH residuals are low
  ...
  Pliteq holdouts and lacks source-owned open-web formula inputs and
...
- Dynamic Calculator floor route now has a first-class steel-floor
  formula input surface for `steelSupportForm`, `steelCarrierDepthMm`,
  `steelCarrierSpacingMm`,
  `resilientLayerDynamicStiffnessMNm3`, `loadBasisKgM2`, and
  `lowerCeilingIsolationSupportForm`;
- workbench route controls, scenario analysis, local/server snapshots,
  and the engine helper all bridge these user-facing fields into the
  same `ImpactPredictorInput` used by the Gate AD formula corridor;
- complete construction-image style steel rows plus the UI surface return
  lab `LnW 55.6`, `DeltaLw 22.4`, and
  `predictor_lightweight_steel_mass_spring_holdout_corridor_estimate`;
- partial steel fields stay parked and do not fall into a fake formula
  answer;
- unsafe duplicate steel carrier topology is refused instead of being
  collapsed;
- exact measured full-stack source rows remain above the formula
  corridor.
...
Gate AG first-action plan:

- add acceptance/revalidation ownership for the landed steel input
  surface across workbench UI, local saved replay, server snapshot replay,
  output cards, reports, and calculator API payloads;
- make missing/invalid steel formula fields visibly precise in route
  unlock prompts;
- broaden hostile UI coverage for invalid values, comma decimals, many
  layers, safe reorders, unsafe duplicate carriers, and steel/non-steel
  base toggles;
- preserve exact-source precedence and keep lab impact outputs separate
  from field/building-prediction outputs.

Gate AE result:

- Gate AD numeric values remain unchanged for the complete open-web
  steel example: lab `LnW 55.6`, `DeltaLw 22.4`;
- workbench cards, output posture, dynamic impact trace, impact-support
  formula notes, validation mode, proposal dossier, and Markdown report
  now all expose the same `Lightweight-steel formula corridor` basis;
- `Ln,w` card copy keeps the source-absent lab formula and `+/-4.5 dB`
  corridor tolerance visible; `DeltaLw` card copy keeps the mass-spring
  relation and `+/-2.0 dB` corridor tolerance visible;
- the validation ladder now includes
  `steel_formula_corridor_estimate`, so the formula no longer hides
  inside generic scoped/heavy-reference language;
- exact measured full-stack rows still outrank the formula corridor, and
  holdouts remain calibration evidence rather than the whole product.
...
No runtime fix was required by that pass. That pass selected Gate AF;
the Gate AF plan below is now consumed by the landed Gate AF input
surface.

Consumed Gate AF first-action plan:
...
- coverage and accuracy are both required. A broader calculator that
  gives unlabelled low-quality answers is not ready, and a narrow lookup
  table is not enough.
- exact lab/source rows override or anchor calculation when present.
- source absence blocks measured-exact/source-validated/calibration
  promotion only; it is not formula absence.
- unknown assemblies must still produce labelled, family-specific
  physics predictions when required topology/material inputs are
  sufficient.
- source data is for exact override, calibration, benchmark validation,
  tolerance ownership, and regression tests; it must not replace the
  solver engine.
- lab, field, and building-prediction bases must remain explicit; tests
  should assert numeric correctness, basis/origin, support/card parity,
  tolerance, nearby negatives, and hostile layer/reorder behavior.

Current implementation facts:

- `impact-lane.ts` already shows the desired candidate precedence model.
- shared schema now has optional airborne `airborneBasis` and
  `airborneCandidateSet` fields for result-basis metadata.
- shared schema now has optional `ratingAdapterBasisSet` metadata for
  per-output rating adapter ownership. Gate C marks ISO 717-1,
  ISO 717-2, ASTM E413, and ASTM E989 lanes explicitly, and records
  ASTM E989/IIC as not runtime-implemented instead of aliasing `Ln,w`.
- shared schema now has optional `inputCompletenessSet` metadata for
  per-family physical input completeness. Gate D defines the minimum
  input matrix for single leaf, double/framed, triple-leaf/multicavity,
  porous fill, floating floor, and field/apparent output contexts.
- shared schema now has optional `airborneCandidateResolution` metadata
  for the resolver trace. Gate E added the no-runtime resolver contract;
  Gate G now populates it at runtime for the explicit grouped Rockwool
  triple-leaf family-physics prediction.
- `airborne-source-promotion.ts` now defines the Gate H readiness policy
  for source-owned exact full-stack, calibrated-family, and
  exact-subassembly-plus-delta promotion. It requires rights-safe source
  evidence, topology/material/metric/tolerance ownership, paired
  positive and negative tests, and calibration holdout metadata before a
  source candidate can win.
- `airborne-verified-catalog.ts` has full-stack exact anchoring but no
  exact subassembly plus calculated-delta candidate.
- `dynamic-airborne.ts` detects `multileaf_multicavity`. For the narrow
...
- keep Gate G grouped Rockwool runtime values unchanged unless a future
  ...
  promotion policy.
- use exact/source rows only as exact override, calibration, benchmark,
...
Gate H selected the Gate I action
`gate_i_expand_family_material_properties_and_benchmark_scenarios`
at
...
Gate B had selected the Gate C action
`gate_c_inventory_rating_adapter_integrity_without_value_movement`
at
...
Gate A had selected the Gate B file/action:
`packages/engine/src/calculator-model-first-physics-prediction-pivot-gate-b-basis-contract.test.ts`
and
`gate_b_define_shared_airborne_basis_candidate_schema_without_value_movement`.
...
- grouped explicit-topology Rockwool now uses
  `triple_leaf_two_cavity_frequency_solver_family_physics_prediction`
  at lab `Rw 50 / STC 55 / C 0.8 / Ctr -7.3`; field/building context
  with the current reference room gives `R'w 50 / DnT,w 51 /
  DnT,A 51.3`. It is uncalibrated `family_physics_prediction`, not
  measured exact, source-validated, or design-grade.
- direct exact Rockwool promotion remains blocked by
  `rights_safe_source_owned_curve_payload_absent`, but formula-backed
  prediction must not stay blocked on that source packet.

Everything below this line is chronological history unless this top
section explicitly promotes it again. Older blocks may still contain
words such as "current", "active", or "selected next action"; read
those as current only for that historical slice.

## Superseded Selected Slice - Rockwool Source-Packet Refresh V2
...
This plan is retained for later measured-exact/source-calibration work.
It is no longer the active next step because missing source packets must
not block the calculator from producing labelled physics predictions.

## Prior Selected Slice - Before V28 Gate A
...
Source/accuracy gap revalidation after UBIQ packaged-finish
current-gate closeout.

Historical selected status:

`closed_ubiq_open_web_packaged_finish_current_gate_guard_selected_source_gap_revalidation_v28`

Historical selected implementation file:
...
Historical V28 Gate A landed selection:
...
Current-gate protected guard pack:
...
- create the V28 source-gap revalidation Gate A contract.
- re-rank the remaining source-backed accuracy gaps after UBIQ
  packaged-finish current-gate closeout.
- keep runtime values, support semantics, confidence, evidence, API,
  route-card values, output-card statuses, proposal/report copy, and
  workbench-input behavior frozen unless V28 identifies a fully
  source-owned runtime candidate.
- keep direct Rockwool exact runtime blocked by
  `rights_safe_source_owned_curve_payload_absent`.
- keep generic/raw open-web widening blocked by
...
Current protected Rockwool boundaries remain unchanged:

- adjacent Rockwool remains corrected and supported at
  `Rw 51 / R'w 49 / DnT,w 51`.
- flat-list split/internal gypsum-leaf Rockwool remains withheld from
  supported outputs with diagnostic `Rw 41 / R'w 39 / DnT,w 40`.
- grouped Rockwool remains live screening-only/source-blocked at
  `Rw 41` on `multileaf_screening_blend`, not exact, not
  source-validated, and not design-grade.
- direct exact Rockwool runtime remains blocked by
  `rights_safe_source_owned_curve_payload_absent`.

## Prior Selected Slice - Before Packaged-Finish Gate C
...
UBIQ open-web packaged-finish current-gate guard after Gate A promotion.
...
UBIQ packaged-finish Gate A implementation file:

`packages/engine/src/ubiq-open-web-packaged-finish-current-gate-guard-gate-a-contract.test.ts`

UBIQ packaged-finish Gate A checkpoint:
...
Current-gate promoted guard pack:
...
- close out the packaged-finish current-gate guard and select the next
  bounded accuracy slice.
- keep the promoted UBIQ packaged-finish current-gate pack in
...
skipped, and repo build 5 / 5 tasks. The broad result confirms there is
no immediate test failure to repair before the selected packaged-finish
current-gate guard. Known non-fatal `sharp/@img` warnings remain through
`@turbodocx/html-to-docx`.

Pre-Gate A analysis refresh completed on 2026-05-05: implementation and
docs still agree that the only active gap is current-gate ownership for
the existing UBIQ packaged-finish guard pack. The selected target file
does not exist yet, the seven existing UBIQ guard files do exist, and
focused continuity passed with engine 4 files / 10 tests and web
4 files / 5 tests. The official UBIQ PDF source is accessible. No
runtime repair is required before Gate A, and no Rockwool exact runtime
promotion is allowed without the missing rights-safe source-owned packet.

## Prior Selected Slice - Before Packaged-Finish Gate A
...
Prior selected implementation slice:

`ubiq_open_web_packaged_finish_current_gate_guard_v1`

Prior selected implementation file:

`packages/engine/src/ubiq-open-web-packaged-finish-current-gate-guard-gate-a-contract.test.ts`

Prior selected action:
...
Prior V27 selected coverage:

- `90 exact` UBIQ open-web rows.
- `21 bound` UBIQ open-web rows.

## Prior Selected Slice - Before V27 Gate A
...
Source/accuracy gap revalidation after UBIQ supported-band current-gate
closeout.
...
UBIQ supported-band current-gate guard Gate C implementation file:

`packages/engine/src/post-ubiq-open-web-supported-band-current-gate-guard-v1-next-slice-selection-contract.test.ts`

UBIQ supported-band current-gate guard Gate C checkpoint:
...
Current selected V27 scope:

- V27 must re-rank remaining source-backed runtime or guard work after
  UBIQ FL-23/25/27 weak-band and FL-24/26/28 supported-band surfaces
  became current-gate owned.
- generic/raw open-web widening remains blocked until V27 can name
  source-owned topology, metric, tolerance, and negative-boundary
  ownership.
- direct Rockwool exact runtime remains blocked until the missing
  rights-safe source-owned curve packet and companion owners exist.
- confidence/copy/productization cleanup remains lower priority than
  correct numbers and source-backed coverage.
...
V27 Gate A must run before any new runtime widening or exact Rockwool
...
## Prior Selected Slice - Before Supported-Band Closeout
...
Prior supported-band Gate A artifacts:
...
Prior supported-band protected values:

- FL-24 bare exact: `Rw 61`, `Ln,w 62`, `Ln,w+CI 60`.
- FL-26 bare exact: `Rw 60`, `Ln,w 62`, `Ln,w+CI 61`.
- FL-28 bare exact: `Rw 64`, `Ln,w 58`, `Ln,w+CI 56`.
- FL-28 carpet bound: `Rw 64`, `Ln,w+CI <= 45`, no exact `Ln,w`.

## Prior Selected Slice - Before Weak-Band Closeout
...
Prior weak-band closeout artifacts:
...
Prior weak-band protected scope:

- `FL-23/25/27` exact lower-board UBIQ rows remain source-backed and
  current-gate owned.
- upper-only FL-23/25/27 open-web weak-band stacks keep impact outputs
  fail-closed.

## Prior Selected Slice - Before UBIQ Gate A
...
## Prior Selected Slice - Before V26 Gate A
...
Source/accuracy gap revalidation after Rockwool split numeric closure.
...
Prior V26 selected status:

`selected_ubiq_open_web_weak_band_current_gate_guard_after_v26_rerank_preserved_rockwool_blockers_and_found_source_backed_floor_exact_guard_gap`

Prior V26 selected implementation slice:

`ubiq_open_web_weak_band_current_gate_guard_v1`

Prior V26 selected next action:

`gate_a_promote_source_backed_ubiq_weak_band_exact_and_fail_closed_guards_into_current_gate`

Prior V26 target first gate file:

`packages/engine/src/ubiq-open-web-weak-band-current-gate-guard-gate-a-contract.test.ts`

## Prior Selected Slice - Before Gate C
...
Split/internal Rockwool triple-leaf numeric source closure after V25
adjacent flat-list numeric recovery.
...
`rockwool_split_triple_leaf_numeric_source_closure_v1` Gate B

Rockwool split numeric closure Gate B implementation file:

`packages/engine/src/rockwool-split-triple-leaf-numeric-source-closure-gate-b-runtime.test.ts`

Rockwool split numeric closure Gate B checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_ROCKWOOL_SPLIT_TRIPLE_LEAF_NUMERIC_SOURCE_CLOSURE_GATE_B_HANDOFF.md`

Rockwool split numeric closure Gate B status:

`gate_b_withheld_flat_list_split_internal_leaf_supported_outputs_selected_closeout`

Rockwool split numeric closure Gate B landed gate:

`gate_b_withheld_flat_list_split_internal_leaf_exact_outputs_until_grouped_topology_or_source_owned_model`

Rockwool split numeric closure Gate B action:

`gate_b_withhold_flat_list_split_internal_leaf_exact_outputs_until_grouped_topology_or_source_owned_model`

Gate B changed runtime support/output behavior for the remaining
flat-list split/internal gypsum-leaf Rockwool case. The diagnostic
numbers remain calculated internally as `Rw 41 / R'w 39 / DnT,w 40` and
`STC 41`, but requested wall airborne outputs for that flat-list split
stack are now withheld from `supportedTargetOutputs` and placed in
...
Grouped Rockwool remains the existing source-blocked screening path:
`Rw 41`, live as screening, not exact, not source-validated, and not
design-grade.
...
28 tests; focused Rockwool engine continuity passed 7 files / 38 tests;
...
5 / 5 tasks, and final `git diff --check` passed. The first monolithic
...
`rockwool_split_triple_leaf_numeric_source_closure_v1` Gate A

Rockwool split numeric closure Gate A implementation file:

`packages/engine/src/rockwool-split-triple-leaf-numeric-source-closure-gate-a-contract.test.ts`

Rockwool split numeric closure Gate A checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_ROCKWOOL_SPLIT_TRIPLE_LEAF_NUMERIC_SOURCE_CLOSURE_GATE_A_HANDOFF.md`

Rockwool split numeric closure Gate A status:
...
Rockwool split numeric closure Gate A landed action:

`gate_a_decided_split_internal_leaf_requires_source_owned_topology_before_exact_numeric_closure`

Rockwool split numeric closure Gate A action:
...
Adjacent Rockwool remains corrected at `Rw 51 / R'w 49 / DnT,w 51`
and `STC 51` on
`double_leaf_porous_fill_delegate+flat_list_adjacent_swap_numeric_hold_until_grouped_topology`.
The split/internal gypsum-leaf stack still returns a finite screening
diagnostic of `Rw 41 / R'w 39 / DnT,w 40` and `STC 41` through
`multileaf_screening_blend`, but Gate A rejects treating that as a
defended physical triple-leaf penalty. NRC 2024, Uris 2006, and Ballagh
2013 remain useful evidence/model context, not runtime ownership for a
fixed Rockwool penalty or exact Rw. Exact closure still needs a
rights-safe source-owned band curve, calibrated local model,
topology/coupling owner, local Rockwool/material mapping, metric and
tolerance owners, negative boundaries, calibration holdout, and paired
visible/API tests.

Gate A selected the runtime-withhold next file:
`packages/engine/src/rockwool-split-triple-leaf-numeric-source-closure-gate-b-runtime.test.ts`
with action
`gate_b_withhold_flat_list_split_internal_leaf_exact_outputs_until_grouped_topology_or_source_owned_model`.

Rockwool split numeric closure Gate A validation completed on
2026-05-05: focused Gate A passed 1 file / 4 tests; Rockwool
triple-leaf engine continuity passed 9 files / 49 tests; `pnpm
calculator:gate:current` passed with engine 262 files / 1513 tests, web
...
made no runtime, shared-schema, API, workbench-input, report/proposal,
or visible-output behavior movement. Known non-fatal `sharp/@img`
warnings remain through `@turbodocx/html-to-docx`; `apps/web/next-env.d.ts`
...
`gate_a_rockwool_flat_list_numeric_recovery_after_user_correctness_clarification`

V25 Gate A landed `rockwool_adjacent_flat_list_numeric_recovery`: the
PDF-like adjacent Rockwool flat-list stack now stays in the double-leaf
numeric lane instead of being forced into the flat-list multileaf
screening penalty. Current recovered values are `Rw 51`, `R'w 49`,
`DnT,w 51`, and `STC 51` with strategy
`double_leaf_porous_fill_delegate+flat_list_adjacent_swap_numeric_hold_until_grouped_topology`.

V25 did not close `rockwool_split_internal_leaf_remains_numeric_open`:
the split/internal gypsum-leaf stack still returns
`Rw 41 / R'w 39 / DnT,w 40`, `STC 41` through
`multileaf_screening_blend`; it selected
the now-landed Gate A numeric source-closure decision.

V25 Gate A validation completed on 2026-05-05: focused V25 passed 1
file / 4 tests; expanded engine continuity passed 21 files / 156 tests;
`pnpm calculator:gate:current` passed with engine 261 files / 1509
...
typecheck clean, engine 394 files / 2329 tests, web 165 files / 933
passed + 18 skipped, and repo build 5 / 5 tasks. Final `git diff
--check` passed after confirming `apps/web/next-env.d.ts` stayed on
`.next-typecheck`. Known non-fatal `sharp/@img` warnings remain through
`@turbodocx/html-to-docx`.

Prior landed gate:

`rockwool_triple_leaf_support_posture_v1` Gate A

Rockwool support posture Gate A implementation file:

`packages/engine/src/rockwool-triple-leaf-support-posture-gate-a-contract.test.ts`

Rockwool support posture Gate A checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_ROCKWOOL_TRIPLE_LEAF_SUPPORT_POSTURE_GATE_A_HANDOFF.md`

Rockwool support posture Gate A status:

`gate_a_kept_rockwool_source_required_values_screening_supported_no_runtime_selected_source_gap_revalidation_v25`

Rockwool support posture Gate A action:

`gate_a_decide_supported_vs_unsupported_output_posture_for_source_required_rockwool`

Rockwool support posture Gate A landed:
...
Rockwool support posture Gate A result:

Gate A kept source-required Rockwool values in
`supportedTargetOutputs` as finite screening metrics, not
source-backed exact or design-grade results. Moving the values to
`unsupportedTargetOutputs` is rejected until a separate screening
preview value channel exists, because the current workbench would render
`Not ready` and hide the useful `Rw 41` diagnostic.

Runtime values remain grouped `Rw 41` / `STC 41`, flat-list adjacent
swap `Rw 42` / `STC 42`, and field `R'w 34` / `DnT,w 36`; they are
...
Source-required runtime warning:
...
V24 Gate A landed `controlled_use_handoff_gate_c_closeout_consumed`,
`rockwool_uris_exact_runtime_still_blocked_after_controlled_use`,
`source_promotion_owner_set_guard_selected`,
`hostile_api_import_and_frequent_combination_green_carry_forward`, and
`field_outputs_non_design_grade_carry_forward`. It kept runtime values,
...
V24 Gate A validation completed on 2026-05-05: focused V24 Gate A passed
1 file / 7 tests; engine continuity passed 8 files / 47 tests; web
...
Company-internal controlled-use Gate C closeout file:

`packages/engine/src/post-company-internal-controlled-use-handoff-v1-next-slice-selection-contract.test.ts`

Company-internal controlled-use Gate C checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_COMPANY_INTERNAL_CONTROLLED_USE_HANDOFF_GATE_C_CLOSEOUT_HANDOFF.md`

Company-internal controlled-use Gate C status:

`closed_company_internal_controlled_use_handoff_no_runtime_and_selected_source_gap_revalidation_v24`

Company-internal controlled-use Gate C action:

`gate_c_closeout_company_internal_controlled_use_handoff_and_next_slice_selection`

Company-internal controlled-use Gate C landed
`company_internal_controlled_use_handoff_closed`,
`controlled_use_pack_is_current_operator_handoff`, and
`calculator_source_gap_revalidation_v24_selected`. It kept runtime
values, support, confidence, evidence, API shape, route-card values,
output-card status, proposal/report values, and workbench-input behavior
frozen.

Company-internal controlled-use Gate C validation completed on
2026-05-05: focused Gate C passed 1 file / 5 tests; continuity passed
8 files / 43 tests; `pnpm calculator:gate:current` passed with engine
256 files / 1481 tests, web 53 files / 260 passed + 18 skipped, repo
build 5 / 5 tasks, and whitespace guard green; broad `pnpm check`
passed with lint/typecheck clean, engine 389 files / 2301 tests, web
165 files / 933 passed + 18 skipped, repo build 5 / 5 tasks, and final
`git diff --check` green. Known non-fatal `sharp/@img` warnings remain
through `@turbodocx/html-to-docx`; `apps/web/next-env.d.ts` stayed
restored to `.next-typecheck`.

Prior landed gate:

`company_internal_controlled_use_handoff_v1` Gate A

Company-internal controlled-use Gate A implementation file:

`packages/engine/src/company-internal-controlled-use-handoff-gate-a-contract.test.ts`

Company-internal controlled-use Gate A checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_COMPANY_INTERNAL_CONTROLLED_USE_HANDOFF_GATE_A_HANDOFF.md`

Company-internal controlled-use handoff document:

`docs/calculator/COMPANY_INTERNAL_CONTROLLED_USE_HANDOFF.md`

Company-internal controlled-use Gate A status:

`gate_a_prepared_company_internal_controlled_use_handoff_no_runtime_selected_closeout`

Company-internal controlled-use Gate A action:

`gate_a_prepare_company_internal_controlled_use_handoff_pack_no_runtime`

Company-internal controlled-use Gate A artifacts:
...
Gate A kept runtime values, support, confidence, evidence, API shape,
route-card values, output-card status, proposal/report values, and
workbench-input behavior frozen. It prepared the current controlled-use
handoff and selected closeout:

`packages/engine/src/post-company-internal-controlled-use-handoff-v1-next-slice-selection-contract.test.ts`

Ready controlled-use values are pinned in the handoff: LSF exact
`Rw=55`, `R'w=48`, building `DnT,w=50`; AAC benchmark `Rw=47`,
`R'w=45`, building `DnT,w=47`; masonry benchmark `Rw=43`, `R'w=41`,
building `DnT,w=43`; Pliteq floor `Rw=60`, `Ln,w=58`, `L'n,w=61`,
`L'nT,w=58.2`; Ubiq bound floor `Rw=62`, `Ln,w=52`, `L'n,w=55`,
`L'nT,w=52.2`.

The high-accuracy label remains forbidden. Rockwool remains
screening-only: grouped `Rw 41`, flat-list `Rw 42`, field `R'w 34` and
`DnT,w 36`, not exact/source-validated, with Uris 2006 still
`paused_waiting_rights_safe_source_packet`.

Company-internal controlled-use Gate A validation completed on
2026-05-05: focused Gate A passed 1 file / 6 tests; continuity passed
6 files / 33 tests; `pnpm calculator:gate:current` passed with engine
255 files / 1476 tests, web 53 files / 260 passed + 18 skipped, repo
build 5 / 5 tasks, and whitespace guard green; final `git diff --check`
passed after restoring `apps/web/next-env.d.ts` to `.next-typecheck`.
Broad `pnpm check` is reserved for the selected Gate C closeout or later
runtime/user-visible movement.

Prior landed gate:

`company_internal_high_accuracy_opening_rehearsal_v1` Gate C closeout

Company-internal opening Gate C closeout file:

`packages/engine/src/post-company-internal-high-accuracy-opening-rehearsal-v1-next-slice-selection-contract.test.ts`

Company-internal opening Gate C checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_COMPANY_INTERNAL_HIGH_ACCURACY_OPENING_REHEARSAL_GATE_C_CLOSEOUT_HANDOFF.md`

Company-internal opening Gate C status:

`closed_company_internal_high_accuracy_opening_rehearsal_no_runtime_and_selected_controlled_use_handoff`

Company-internal opening Gate C action:

`gate_c_closeout_company_internal_opening_rehearsal_and_next_slice_selection`

Company-internal opening Gate C consumed
...
`selected_opening_handoff_or_backlog_followup`. It landed
`company_internal_controlled_use_handoff_selected`.

Gate C kept runtime values, support, confidence, evidence, API shape,
route-card values, output-card status, proposal/report values, and
workbench-input behavior frozen. It did not open a direct
high-accuracy label. It selected a controlled-use handoff slice so the
latest acceptance matrix, current values, Rockwool screening-only
notice, caveats, and stop rules can be packaged before any use label is
refreshed.

Gate C validation passed on 2026-05-05: focused Gate C 1 file / 5
tests, continuity 9 files / 47 tests, `pnpm calculator:gate:current`
with engine 254 files / 1470 tests, web 53 files / 260 passed + 18
skipped, repo build 5 / 5 tasks, and whitespace guard green, broad
`pnpm check` with lint/typecheck clean, engine 387 files / 2290 tests,
web 165 files / 933 passed + 18 skipped, repo build 5 / 5 tasks, and
final `git diff --check` green.

Prior landed gate:

`company_internal_high_accuracy_opening_rehearsal_v1` Gate A

Company-internal opening Gate A implementation file:

`packages/engine/src/company-internal-high-accuracy-opening-rehearsal-gate-a-contract.test.ts`

Company-internal opening Gate A checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_COMPANY_INTERNAL_HIGH_ACCURACY_OPENING_REHEARSAL_GATE_A_HANDOFF.md`

Company-internal opening Gate A status:

`gate_a_rehearsed_company_internal_high_accuracy_opening_no_runtime_selected_closeout`

Company-internal opening Gate A action:

`gate_a_rehearse_company_internal_high_accuracy_opening_evidence_no_runtime`

Company-internal opening Gate A artifacts:
...
Gate A kept runtime values, support, confidence, evidence, API shape,
route-card values, output-card status, proposal/report values, and
workbench-input behavior frozen. It pinned current source/benchmark
ready corridors and kept Rockwool, generated/screening lanes, field
continuations, near-source rows, Uris 2006, hostile API/import payloads,
many-layer, and reorder edges caveated, blocked, or fail-closed.

High-accuracy opening remained blocked until the selected closeout
consumed the Gate A evidence and final validation evidence. Gate C has
now selected controlled-use handoff, not high-accuracy opening copy.

Company-internal opening Gate A validation completed on 2026-05-05:
focused Gate A passed 1 file / 6 tests; continuity passed 8 files / 42
tests; `pnpm calculator:gate:current` passed with engine 253 files /
1465 tests, web 53 files / 260 passed + 18 skipped, repo build 5 / 5
tasks, and whitespace guard green; broad `pnpm check` passed with lint
and typecheck clean, engine 386 files / 2285 tests, web 165 files / 933
passed + 18 skipped, and repo build 5 / 5 tasks. Known non-fatal
`sharp/@img` warnings remain through `@turbodocx/html-to-docx`;
`apps/web/next-env.d.ts` was restored to `.next-typecheck` after Next
rewrote it.

Prior source/hostile landed gate:

`source_promotion_hostile_input_readiness_guard_v1` Gate C closeout

Source/hostile Gate C closeout implementation file:

`packages/engine/src/post-source-promotion-hostile-input-readiness-guard-v1-next-slice-selection-contract.test.ts`

Source/hostile Gate C action:

`gate_c_no_runtime_closeout_and_next_slice_selection`

Source/hostile Gate C checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_SOURCE_PROMOTION_HOSTILE_INPUT_READINESS_GUARD_GATE_C_CLOSEOUT_HANDOFF.md`

Source/hostile Gate C status:
...
Source/hostile Gate C consumed `source_promotion_surface_inventory`,
...
It landed `company_internal_high_accuracy_opening_rehearsal_selected`.

Gate C kept Gate A's shared API validation tightening active:
`thicknessMm: z.number().finite().positive()`. A hostile JSON body such
as `1e309` remains rejected by estimate and impact-only routes before
calculation. Calculator numeric runtime values, output support,
confidence, evidence, route-card values, output-card status,
proposal/report values, and workbench-input behavior stayed frozen.
Unknown material IDs still reach the engine fail-closed guard and
return no supported outputs.

Exact source controls and near-source rows remain separated. Server
project import remains snapshot persistence only, not calculator
promotion; restore reads parsed `calculatorInput.payload`, not stored
`calculatorOutput` values.

Current next work is to create
`packages/engine/src/post-company-internal-high-accuracy-opening-rehearsal-v1-next-slice-selection-contract.test.ts`.
Company-internal high-accuracy opening remains blocked until this
closeout consumes the current acceptance matrix, current-gate plus broad
`pnpm check` evidence, and an explicit final opening handoff or backlog
follow-up decision.

The landed rehearsal produced
`company_internal_opening_acceptance_matrix`,
`final_validation_evidence_map`,
`rockwool_screening_and_source_blocker_registry`,
`source_promotion_no_runtime_boundary_register`,
`hostile_api_import_fail_closed_evidence`, and
`operator_caveat_and_usage_handoff_pack`, with
`selected_opening_handoff_or_backlog_followup` selecting closeout.

Source/hostile Gate C validation completed on 2026-05-05: focused Gate
C passed 1 file / 5 tests; engine continuity passed 7 files / 36 tests;
`pnpm calculator:gate:current` passed with engine 252 files / 1459
tests, web 53 files / 260 passed + 18 skipped, repo build 5 / 5 tasks,
and whitespace guard green. Broad `pnpm check` passed with lint/typecheck
...
Rockwool Gate C landed `rockwool_policy_gate_c_closeout_summary`,
...
The Rockwool company-internal criterion is now closed as explicit
screening-only, not exact runtime. Grouped split-rockwool still emits
`Rw 41` from `multileaf_screening_blend`, low confidence,
screening-only, not exact, and not source-validated. Flat-list adjacent
swaps remain `Rw 42` on
`multileaf_screening_blend_fail_closed_until_grouped_topology`. Field
`R'w 34` and `DnT,w 36` remain continuations from the Rockwool
screening lane, not independent measured field or design-grade results.
Uris 2006 remains `paused_waiting_rights_safe_source_packet`.

That handoff selected
`packages/engine/src/source-promotion-hostile-input-readiness-guard-gate-a-contract.test.ts`,
with planning surface
`docs/calculator/SLICE_SOURCE_PROMOTION_HOSTILE_INPUT_READINESS_GUARD_V1_PLAN.md`,
which has now landed through Gate C as described above.

Rockwool Gate C validation completed on 2026-05-05: focused Gate C 1
file / 6 tests; engine continuity 6 files / 37 tests; focused web Gate
B compatibility 1 file / 7 tests; `pnpm calculator:gate:current` passed
with engine 250 files / 1448 tests, web 53 files / 259 passed + 18
skipped, repo build 5 / 5 tasks, and whitespace guard green. Final
`git diff --check` was green after restoring `apps/web/next-env.d.ts` to
`.next-typecheck`. Broad `pnpm check` was not run because Gate C made no
runtime, API, shared-schema, route/report, or workbench-input behavior
change. Known non-fatal `sharp/@img` warnings remain through
`@turbodocx/html-to-docx`.

Prior Rockwool Gate B carry-forward:

Rockwool Gate B implementation file:

`apps/web/features/workbench/rockwool-triple-leaf-explicit-screening-only-policy-gate-b-visible.test.ts`

Rockwool Gate B action:

`gate_b_pin_visible_rockwool_triple_leaf_screening_only_policy`

Rockwool Gate B checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_ROCKWOOL_TRIPLE_LEAF_EXPLICIT_SCREENING_ONLY_POLICY_GATE_B_HANDOFF.md`

Rockwool Gate B status:

`gate_b_pinned_visible_rockwool_triple_leaf_screening_only_policy_no_runtime_selected_gate_c_closeout`

Rockwool Gate B landed `visible_rockwool_screening_only_policy_guard`,
...
Grouped split-rockwool still emits `Rw 41` from
`multileaf_screening_blend`, low confidence, screening-only, not exact,
and not source-validated. The output card and proposal/report coverage
now carry the `Rockwool screening-only` label and explicitly say it is
not exact, not source-validated, and not design-grade. Flat-list
adjacent swaps remain `Rw 42` on
...
source-validated, or design-grade use. Field `R'w 34` and `DnT,w 36`
remain continuations from the Rockwool screening lane, not independent
measured field or design-grade results.

Gate B selected
`packages/engine/src/post-rockwool-triple-leaf-explicit-screening-only-policy-v1-next-slice-selection-contract.test.ts`.
That Gate C closeout is now landed above.

Rockwool Gate B validation completed on 2026-05-05: focused Gate B 1
file / 7 tests; web visible continuity 5 files / 31 tests; engine
continuity 5 files / 29 tests; `pnpm calculator:gate:current` passed
with engine 249 files / 1442 tests, web 53 files / 259 passed + 18
skipped, repo build 5 / 5 tasks, and whitespace guard green. Broad
`pnpm check` passed with lint/typecheck clean, engine 382 files / 2262
tests, web 165 files / 932 passed + 18 skipped, and repo build 5 / 5
tasks. Final `git diff --check` was green after restoring
`apps/web/next-env.d.ts` to `.next-typecheck`. Known non-fatal
`sharp/@img` warnings remain through `@turbodocx/html-to-docx`.

Prior Rockwool Gate A carry-forward:

Rockwool Gate A implementation file:

`packages/engine/src/rockwool-triple-leaf-explicit-screening-only-policy-gate-a-contract.test.ts`

Rockwool Gate A action:

`gate_a_inventory_rockwool_triple_leaf_screening_only_policy_after_v23_rerank`

Rockwool Gate A checkpoint:

`docs/calculator/CHECKPOINT_2026-05-05_ROCKWOOL_TRIPLE_LEAF_EXPLICIT_SCREENING_ONLY_POLICY_GATE_A_HANDOFF.md`

Rockwool Gate A status:

`gate_a_inventoried_rockwool_triple_leaf_screening_only_policy_no_runtime_selected_visible_gate_b`

Rockwool Gate A landed
...
Field-output owner policy is now consumed as a closed visibility/honesty
prerequisite, not Rockwool/Uris runtime evidence. Finite field outputs
remain non-design-grade.

Rockwool triple-leaf remains not fixed. The grouped split-rockwool route
still emits `Rw 41` from `multileaf_screening_blend`, low confidence,
screening only, not exact, and not source-validated. Flat-list adjacent
swaps remain `Rw 42` on
`multileaf_screening_blend_fail_closed_until_grouped_topology`. Field
`R'w 34` and `DnT,w 36` remain continuations from the screening lane,
not design-grade field results. Uris 2006 remains
`paused_waiting_rights_safe_source_packet`; DOI/ScienceDirect/OpenDevEd/
Elsevier TDM signals are not a rights-safe runtime packet.

`pre_company_internal_use_exit_criteria` remains active. The
field-output criterion is closed, and the Rockwool
exact-or-explicit-screening criterion now has visible screening-only
coverage. Company-internal high-accuracy opening remains blocked until
source-promotion
ownership closes, hostile-input proof stays green, and final validation
passes at opening handoff.

Rockwool Gate A validation completed on 2026-05-05: focused Gate A 1
file / 6 tests; engine continuity 5 files / 29 tests; web visible
continuity 3 files / 16 tests; `pnpm calculator:gate:current` passed
with engine 249 files / 1442 tests, web 52 files / 252 passed + 18
skipped, repo build 5 / 5 tasks, and whitespace guard green. Final
`git diff --check` was green after restoring `apps/web/next-env.d.ts` to
`.next-typecheck`. Broad `pnpm check` was not run because Gate A made no
...
`apps/web/features/workbench/rockwool-triple-leaf-explicit-screening-only-policy-gate-b-visible.test.ts`.
It carried `rockwool_triple_leaf_screening_surface_inventory`,
`grouped_rw41_and_flat_rw42_runtime_freeze`,
`visible_route_output_report_policy_gap`,
`rockwool_visible_gate_b_selected`,
`source_promotion_hostile_input_carry_forward`, and
...
Prior V23 source-gap revalidation:
...
files / 16 tests; `pnpm calculator:gate:current` passed with engine 248
files / 1436 tests, web 52 files / 252 passed + 18 skipped, repo build
5 / 5 tasks, whitespace guard green, and final `git diff --check`
green after restoring `apps/web/next-env.d.ts` to `.next-typecheck`.
...
Prior V23 Gate A carry-forward:

`calculator_source_gap_revalidation_v23` landed
...
It selected next planning surface
`docs/calculator/SLICE_ROCKWOOL_TRIPLE_LEAF_EXPLICIT_SCREENING_ONLY_POLICY_V1_PLAN.md`.
It carried `rockwool_uris_exact_runtime_still_blocked`,
...
`pre_company_internal_use_exit_criteria` forward.

Prior field-output owner Gate C validation completed on 2026-05-05:
`closed_field_output_owner_design_grade_policy_no_runtime_and_selected_source_gap_revalidation_v23`
selected `calculator_source_gap_revalidation_v23` with first file
`packages/engine/src/calculator-source-gap-revalidation-v23-gate-a-contract.test.ts`
and planning surface
`docs/calculator/SLICE_CALCULATOR_SOURCE_GAP_REVALIDATION_V23_PLAN.md`.
focused Gate C passed 1 file / 6 tests; engine continuity passed 5
files / 30 tests; web continuity passed 7 files / 73 passed + 18
skipped; `pnpm calculator:gate:current` passed with engine 247 files /
1429 tests, web 52 files / 252 passed + 18 skipped, repo build 5 / 5
tasks, and whitespace guard green. Broad `pnpm check` passed with
lint/typecheck clean, engine full suite 380 files / 2249 tests, web full
suite 164 files / 925 passed + 18 skipped, and repo build 5 / 5 tasks.
Final `git diff --check` was green. Known non-fatal `sharp/@img`
warnings remain through `@turbodocx/html-to-docx`.

Prior field-output owner Gate C carry-forward remains active for
continuity: `field_output_owner_policy_gate_c_closeout_summary`,
`field_output_owner_and_design_grade_policy_closed_carry_forward`,
`rockwool_rw41_screening_and_uris_packet_status`,
`paused_waiting_rights_safe_source_packet`, and
`pre_company_internal_use_exit_criteria`.

Prior Gate B landed
`gate_b_pinned_visible_field_output_design_grade_owner_policy_no_runtime_selected_gate_c_closeout`
no-runtime and selected
`gate_c_no_runtime_closeout_and_next_slice_selection` at
`packages/engine/src/post-field-output-owner-and-design-grade-policy-v1-next-slice-selection-contract.test.ts`.
It produced `visible_field_output_design_grade_policy_guard`,
...
Prior Gate B validation completed on 2026-05-05: focused Gate B passed
1 file / 6 tests; engine continuity passed 4 files / 24 tests; web
continuity passed 7 files / 73 passed + 18 skipped; `pnpm
calculator:gate:current` passed with engine 246 files / 1423 tests, web
52 files / 252 passed + 18 skipped, repo build 5 / 5 tasks, and
whitespace guard green. Broad `pnpm check` passed with lint/typecheck
clean, engine full suite 379 files / 2243 tests, web full suite 164
files / 925 passed + 18 skipped, and repo build 5 / 5 tasks. Known
non-fatal `sharp/@img` warnings remain through
`@turbodocx/html-to-docx`.
...
It produced `field_output_owner_design_grade_policy_inventory`,
`field_metric_owner_matrix`,
`source_basis_and_tolerance_requirement_matrix`,
`missing_geometry_and_missing_field_input_negative_boundaries`, and
`rockwool_screening_field_output_carry_forward`.

Prior Gate A validation completed on 2026-05-05: focused Gate A passed
1 file / 6 tests; engine continuity passed 4 files / 24 tests; web
continuity passed 4 files / 50 passed + 18 skipped; `pnpm
calculator:gate:current` passed with engine 246 files / 1423 tests, web
51 files / 246 passed + 18 skipped, repo build 5 / 5 tasks, and
whitespace guard green. Broad `pnpm check` passed with lint/typecheck
clean, engine full suite 379 files / 2243 tests, web full suite 163
files / 919 passed + 18 skipped, and repo build 5 / 5 tasks.

Prior V22 carry-forward:

`calculator_source_gap_revalidation_v22` landed
...
It carried `rockwool_rw41_screening_and_uris_packet_status`,
`repeat_uris_acquisition_blocked_without_new_packet`,
`field_output_owner_and_design_grade_policy`, and
`pre_company_internal_use_exit_criteria` forward while Uris stayed
`paused_waiting_rights_safe_source_packet`.

Prior company Gate C carry-forward:

`company_internal_frequent_combination_lane_snapshot_guard_v1` closed
no-runtime with
`closed_company_internal_frequent_combination_lane_snapshot_guard_no_runtime_and_selected_source_gap_revalidation_v22`
and selected `calculator_source_gap_revalidation_v22` with first file
`packages/engine/src/calculator-source-gap-revalidation-v22-gate-a-contract.test.ts`.
That closeout remains the source of
...
Gate B strengthened visible card/report copy for finite field-style
outputs without moving runtime values. Airborne field cards such as
`R'w`, `Dn,w`, `DnT,w`, and `DnT,A` now say they are field
continuations from the active lab, screening, or apparent lane, not
independent exact field measurements. Impact field cards such as
`L'n,w`, `L'nT,w`, and `L'nT,50` now say they are field-impact
continuations, not independent exact field measurements. The shared
field-continuation posture also says these outputs are not independent
exact source rows or measured field results.

The rockwool triple-leaf defect remains unresolved. The grouped
split-rockwool building route still emits finite apparent field
continuations such as `R'w 34` and `DnT,w 36`, but the wall strategy is
still `multileaf_screening_blend`, low confidence, not fixed, and not
source-validated.
...
passed 4 files / 24 tests. The Gate B web test is included in
`tools/dev/run-calculator-current-gate.ts`. Current-gate validation
also passed: engine 241 files / 1388 tests, web 50 files / 238 passed +
18 skipped, repo build 5 / 5 tasks, and whitespace guard green. The
first build attempt saw a transient missing `next-font-manifest.json`
artifact; an immediate web build rerun and the full current-gate rerun
passed without calculator/runtime changes.
...
The old internal-use envelope remains controlled / caveated pilot
context only. It is not a high-accuracy company rollout approval while
rockwool triple-leaf, field-output basis wording, frequent-combination
lane snapshots, source-promotion ownership, and hostile API/import
guards still have active blocker rows. The executable planning contract
is
...
exact/source-owned or visibly screening-only; field outputs must not
look design-grade without an owner; frequent wall/floor lane snapshots
must stay green; source promotion requires topology, material mapping,
metric context, tolerance, negative boundaries, and paired visible
tests; hostile API/import payloads must fail closed; both
...
values or visible copy. It selected Gate B to harden visible basis
wording with paired web-card and report-copy tests.
...
Gate B rechecked exact/bound `+2 mm` inside and `+2.1 mm` outside
tolerance edges, raw role prompts, duplicate-role negatives,
visible wording separation, and direct `officialFloorSystemId` bypass
notes. It found no bounded support/confidence/evidence/field-output/
visible wording fix and selected `calculator_source_gap_revalidation_v19`.
Runtime, support, confidence, evidence, API, route-card, output-card,
proposal/report, and workbench-input behavior stayed frozen.

Next selected planning surface:
...
Prior planning-pass status, now resolved by Gate B:
...
The selected Gate B closeout file was absent during planning and has
now landed no-runtime. It closed the floor tolerance-edge guard and
selected v19 source-gap revalidation without runtime import,
...
movement, or rockwool exact promotion.

No external source research is needed for the immediate v19 Gate A
unless that gate selects source acquisition or a rights-safe source
packet/source locator becomes available.

Prior follow-up planning pass, now consumed by Gate B:
...
The Gate B file was written against the inspected floor
...
contract pins exact/bound just-inside and just-outside snapshots,
...
The implementation cross-check confirmed the selected Gate B file is
still absent and the current-gate runner correctly omits it until the
file exists. The next action is to create that closeout contract, add it
to `tools/dev/run-calculator-current-gate.ts`, run the current gate, and
then either close no-runtime into a rerank or select one bounded fix with
paired tests. No internet/source research is selected before a source
acquisition gate or rights-safe source packet exists.

Targeted planning validation passed on 2026-05-04:
...
Gate A selected next action, now historical because Gate B has landed:
...
No runtime movement was selected. At that moment Gate B closeout
remained the first implementation step; that closeout has since landed
and selected v19. Rockwool/Uris remains blocked with `Rw 41` as
unfixed screening output.

Historical note: that broad audit happened before the Gate B closeout.
Gate B has since landed no-runtime. The current first implementation
step is now
`packages/engine/src/calculator-source-gap-revalidation-v19-gate-a-contract.test.ts`
with
`gate_a_revalidate_source_accuracy_gap_order_after_floor_tolerance_edge_closeout`.
Do not start a runtime import, support promotion, field-output copy, or
rockwool exact fix unless v19 selects a bounded next slice or formal
source-acquisition path with the required source/topology/material/
tolerance/negative-boundary/visible-test proof.
...
leaf masses, computes two cavity spring resonance bands plus an
interacting resonance pair, applies fill damping and coupling/support
parameters, returns one-third-octave TL curves, and computes ISO 717
ratings through existing curve helpers. Gate G adds
`packages/engine/src/wall-triple-leaf-calibration-regime.ts` as the
source-owned calibration / holdout tolerance owner: at least two
source-owned calibration rows, at least one source-owned holdout row,
MAE <= 2 dB, max error <= 4 dB, and dip-band placement within one
neighboring one-third-octave band. Gate G2 adds
`packages/engine/src/wall-triple-leaf-source-curve-digitization-intake.ts`
as the typed NRC 2024 graph-row intake: Type C base wall plus
assemblies A-D, reported STC values, Figure 4 / Figure 5 ownership,
source topology, and blocked material mapping. Gate G2B adds
`packages/engine/src/wall-triple-leaf-source-curve-digitization-qc.ts`
as reproducible curve digitization QC: fixed render provenance, locked
Figure 4 / Figure 5 axes, Type C plus assemblies A-D TL vectors,
derived STC/Rw values, Figure 5 delta checks, and calibration/holdout
role assignment. Gate G3 adds
`packages/engine/src/wall-triple-leaf-calibration-fit.ts` as the
NRC-like source-family calibration fit and negative-boundary proof:
calibration rows A/B and holdout row D pass Gate G tolerance, ordinary
...
`wall_timber_double_board_source_research_v1` Gate C closed no-runtime
and selected `wall_clt_wall_source_research_v1`;
`wall_clt_wall_source_research_v1` Gate A landed no-runtime and
...
no-runtime and Gate C closed no-runtime and selected
...
field board-split value/warning drift and Gate C closed the slice,
selecting `calculator_source_gap_revalidation_v3`;
...
and selected `calculator_source_gap_revalidation_v4`;
`calculator_source_gap_revalidation_v4` Gate A landed no-runtime and
selected `knauf_wall_systems_source_pack_extraction_v1`;
...
`calculator_post_knauf_source_acquisition_v1` Gate A landed
no-runtime and selected
...
`B226010`, but runtime remains blocked until exact live topology,
local material mapping, metric policy, tolerance owner, protected
negative boundaries, and paired visible tests are extracted.
That prior decision selected
`packages/engine/src/british-gypsum-white-book-source-pack-extraction-gate-a-contract.test.ts`
as the first-gate file before Gate A landed and handed off to Gate B.
Prior post-Knauf selection status:
...
The prior EN-PC closeout remains active context: `EN-PC-50-055-6-2-12.5-WB-25`
is not runtime-ready because Wallboard, 25 mm Acoustic Roll, 50 mm /
0.55 gauge stud details, field-output policy, spectrum-term policy,
...
with `gate_c_no_runtime_closeout_and_next_slice_selection`; its Gate C
then selected `calculator_source_gap_revalidation_v7` with
...
and the executable scenario matrix. Gate B then landed the regular
...
source-row and metric-context extraction. CLT / mass-timber extraction
Gate A then classified WoodWorks Table 7 Single CLT Wall and Table 9
Double CLT Wall as later row-mapping candidates only; WoodWorks Table 8
...
Gate A then landed that acceptance matrix no-runtime: ready wall
benchmarks, exact/bound floor source corridors, caveated generated
routes, many-layer/reorder edges, invalid thickness fail-closed
...
closeout / next-slice selection. Gate C then closed the pilot handoff
no-runtime, kept it as controlled-use evidence rather than source-gated
promotion, and selected `calculator_source_intake_backlog_cleanup_v1`
because no concrete pilot defect or source-ready accuracy pack exists.
Source intake backlog Gate A then landed
[SOURCE_READY_INTAKE_BACKLOG.md](./SOURCE_READY_INTAKE_BACKLOG.md):
CLT / mass-timber wall, timber double-board stud wall, no-stud
double-leaf wall, generated floor fallback, lined-massive /
heavy-core wall, and historical blocked families now have one current
...
be delta-mapped without importing values. The selected Gate A file was
`packages/engine/src/generated-floor-fallback-topology-delta-gate-a-contract.test.ts`.
Generated floor topology-delta Gate A then landed no-runtime: the live
fallback stayed low-confidence, Pliteq exact and UBIQ FL-32 bound
precedence stayed source-topology gated, lab `Ln,w+CI`, lab `DeltaLw`,
and field `L'nT,50` stayed unsupported, and split live-stack variants
remained finite without exact/bound promotion. Gate B is skipped
because no runtime candidate is source-ready. Gate C then closed the
topology-delta slice no-runtime and selected
`calculator_source_gap_revalidation_v4`; the selected v4 Gate A file
was
`packages/engine/src/calculator-source-gap-revalidation-v4-gate-a-contract.test.ts`.
v4 Gate A then landed no-runtime: it re-ranked the current
source/accuracy backlog, kept every runtime and visible surface frozen,
recorded official Knauf UK/AU source locators as extraction inputs only,
and selected `knauf_wall_systems_source_pack_extraction_v1`. Knauf Gate
A then extracted official source table locators only, and Knauf Gate B
compared those locator rows against the live implementation. Gate B
...
- **Focused calculator gate** (`pnpm calculator:gate:current`):
  ...
  C closeout selection, plus
  `post-wall-double-leaf-sharp-davy-scoping-v1-next-slice-selection-contract.test.ts`
  ...
  for the no-runtime bounded framed-wall reconciliation, plus
  `post-wall-double-leaf-source-evidence-acquisition-v1-next-slice-selection-contract.test.ts`
  for the no-runtime Gate C closeout and wall source catalog selection,
  plus
  `wall-source-catalog-acquisition-gate-a-contract.test.ts` for the
  ...
  research selection, plus
  `wall-no-stud-double-leaf-source-research-gate-a-contract.test.ts`
  ...
  research selection, plus
  `wall-timber-double-board-source-research-gate-a-contract.test.ts`
  for the no-runtime timber double-board source/tolerance inventory and
  Gate C closeout decision, plus
  `post-wall-timber-double-board-source-research-v1-next-slice-selection-contract.test.ts`
  for the no-runtime Gate C closeout and CLT wall source research
  selection, plus
  `wall-clt-wall-source-research-gate-a-contract.test.ts` for the
  no-runtime CLT wall source/tolerance inventory and Gate C closeout
  decision, plus
  `post-wall-clt-wall-source-research-v1-next-slice-selection-contract.test.ts`
  for the no-runtime Gate C closeout and lined-massive / heavy-core
  source research selection, plus
  `wall-lined-massive-heavy-core-source-research-gate-a-contract.test.ts`
  for the no-runtime lined-massive / heavy-core source and bounded
  lining-rule inventory, plus
  `post-wall-lined-massive-heavy-core-source-research-v1-next-slice-selection-contract.test.ts`
  for the no-runtime Gate C closeout and source-gap revalidation v2
  selection, plus
  `calculator-source-gap-revalidation-v2-gate-a-contract.test.ts` for
  the no-runtime source/accuracy rerank and floor layer-order
  invariance expansion selection, plus
  `floor-layer-order-invariance-expansion-gate-a-contract.test.ts` for
  the no-runtime role-defined exact/raw order-sensitive/many-layer floor
  order audit and Gate C closeout selection, plus
  `post-floor-layer-order-invariance-expansion-v1-next-slice-selection-contract.test.ts`
  for the no-runtime Gate C closeout and framed-wall split warning
  stability selection, plus
  `wall-framed-facing-split-warning-stability-gate-a-contract.test.ts`
  for the no-runtime LSF/timber board-split value/warning inventory and
  Gate B stability-fix selection, plus
  ...
  for the bounded split fix and Gate C closeout, plus
  `calculator-source-gap-revalidation-v3-gate-a-contract.test.ts` for
  the no-runtime source/readiness rerank and internal-use operating
  envelope selection, plus
  `internal-use-operating-envelope-v1-gate-a-contract.test.ts` for the
  short company pilot usage note, scenario matrix, and no-promotion
  boundaries, plus
  ...
  Gate C closeout selection, plus
  ...
  no-runtime company-internal handoff pack, plus
  ...
  for the no-runtime Pliteq / UBIQ topology-delta matrix, plus
  ...
  for no-runtime Knauf Gate C closeout and v5 selection, plus
  `calculator-source-gap-revalidation-v5-gate-a-contract.test.ts`
  for no-runtime v5 rerank and `TB.5A` selection, plus
  `timber-double-board-knauf-tb5a-mapping-tolerance-gate-a-contract.test.ts`
  for no-runtime `TB.5A` mapping/tolerance Gate A, plus
  `post-timber-double-board-knauf-tb5a-mapping-tolerance-v1-next-slice-selection-contract.test.ts`
  for no-runtime `TB.5A` closeout and `MWI.2A` selection, plus
  `lined-masonry-knauf-mwi2a-mapping-tolerance-gate-a-contract.test.ts`
  for no-runtime `MWI.2A` mapping/tolerance Gate A, plus
  `post-lined-masonry-knauf-mwi2a-mapping-tolerance-v1-next-slice-selection-contract.test.ts`
  for no-runtime `MWI.2A` closeout and `TTF30.2A` selection, plus
  `twin-timber-knauf-ttf302a-mapping-tolerance-gate-a-contract.test.ts`
  for no-runtime `TTF30.2A` mapping/tolerance Gate A, plus
  `post-twin-timber-knauf-ttf302a-mapping-tolerance-v1-next-slice-selection-contract.test.ts`
  for no-runtime `TTF30.2A` closeout and v6 selection, plus
  `calculator-source-gap-revalidation-v6-gate-a-contract.test.ts` for
  no-runtime v6 rerank and EN-PC selection, plus
  `steel-stud-knauf-enpc-mapping-tolerance-gate-a-contract.test.ts`
  for no-runtime `EN-PC-50-055-6-2-12.5-WB-25` mapping/tolerance Gate
  A, plus
  `post-steel-stud-knauf-enpc-mapping-tolerance-v1-next-slice-selection-contract.test.ts`
  for no-runtime EN-PC Gate C closeout and v7 selection, plus
  `calculator-source-gap-revalidation-v7-gate-a-contract.test.ts` for
  no-runtime v7 rerank and post-Knauf source acquisition selection,
  plus `calculator-post-knauf-source-acquisition-gate-a-contract.test.ts`
  for no-runtime British Gypsum White Book source-pack selection, plus
  `british-gypsum-white-book-source-pack-extraction-gate-a-contract.test.ts`
  for no-runtime British Gypsum row extraction and Gate B selection,
  plus `wall-triple-leaf-accuracy-recovery-gate-a-contract.test.ts`,
  `wall-triple-leaf-accuracy-recovery-gate-b-contract.test.ts`,
  `wall-triple-leaf-accuracy-recovery-gate-c-contract.test.ts`,
  `wall-triple-leaf-source-pack-extraction-gate-d-contract.test.ts`,
  `wall-triple-leaf-source-corpus-contract.test.ts`,
...
[SLICE_CALCULATOR_POST_KNAUF_SOURCE_ACQUISITION_V1_PLAN.md](./SLICE_CALCULATOR_POST_KNAUF_SOURCE_ACQUISITION_V1_PLAN.md),
[SLICE_CALCULATOR_SOURCE_GAP_REVALIDATION_V7_PLAN.md](./SLICE_CALCULATOR_SOURCE_GAP_REVALIDATION_V7_PLAN.md),
[SLICE_STEEL_STUD_KNAUF_ENPC_MAPPING_TOLERANCE_PLAN.md](./SLICE_STEEL_STUD_KNAUF_ENPC_MAPPING_TOLERANCE_PLAN.md),
[SLICE_CALCULATOR_SOURCE_GAP_REVALIDATION_V6_PLAN.md](./SLICE_CALCULATOR_SOURCE_GAP_REVALIDATION_V6_PLAN.md),
...
Prior urgent checkpoint:
[CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_G2B_HANDOFF.md](./CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_G2B_HANDOFF.md).
It adds reproducible NRC 2024 Figure 4 / Figure 5 digitization QC,
records Type C and assemblies A-D TL vectors, derives STC/Rw as 64/63,
64/58, 60/49, 57/51, and 65/55, keeps numeric/runtime behavior frozen,
and selects
...
Prior urgent checkpoint:
[CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_G2_HANDOFF.md](./CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_G2_HANDOFF.md).
It adds the typed NRC 2024 source-curve intake rows, records Type C and
assemblies A-D reported STC values 64 / 64 / 60 / 57 / 65, keeps graph
TL vectors and derived `Rw` null until reproducible digitization exists,
keeps numeric/runtime behavior frozen, and selects digitization QC.

Prior urgent checkpoint:
[CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_G_HANDOFF.md](./CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_G_HANDOFF.md).
It defines the calibration / holdout acceptance regime and keeps runtime
frozen while selecting source-curve digitization intake.

Earlier urgent checkpoint:
[CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_A_HANDOFF.md](./CHECKPOINT_2026-04-30_WALL_TRIPLE_LEAF_ACCURACY_RECOVERY_GATE_A_HANDOFF.md).
It keeps runtime and visible behavior frozen, records the adjacent
rockwool stack as `Rw 51` medium-confidence double leaf, records the
split rockwool stack as `Rw 41` low-confidence multi-leaf blend, and
selects topology-input recovery with
...
It did not move runtime or visible behavior; it extracts British Gypsum
White Book rows, keeps `A046006` as an already-landed exact anchor, and
selects
`packages/engine/src/british-gypsum-white-book-source-pack-extraction-gate-b-contract.test.ts`
...
It kept every post-Knauf candidate out of runtime and selected
...
Just closed context:
Gate A landed `calculator_source_gap_revalidation_v6` no-runtime in
`packages/engine/src/calculator-source-gap-revalidation-v6-gate-a-contract.test.ts`.
It selects `steel_stud_knauf_enpc_mapping_tolerance_v1` because
`EN-PC-50-055-6-2-12.5-WB-25` is the highest-value remaining concrete
Knauf locator for a common steel-stud lane, but Wallboard / Acoustic
Roll mapping, stud-gauge equivalence, lab/field policy, tolerance
owner, existing steel-anchor precedence, and paired visible tests
remain incomplete. Selection status:
`selected_no_runtime_knauf_enpc_steel_stud_mapping_tolerance_after_v6_rerank_found_no_runtime_ready_candidate`.

Prior closeout context:
Gate C closed `twin_timber_knauf_ttf302a_mapping_tolerance_v1`
no-runtime in
`packages/engine/src/post-twin-timber-knauf-ttf302a-mapping-tolerance-v1-next-slice-selection-contract.test.ts`.
It carries forward the official `TTF30.2A` context pinned by Gate A:
asymmetric `1x13 mm` side 1 / `2x13 mm` side 2 `FIBEROCK
AQUA-TOUGH`, twin timber studs separated by a 20 mm gap, 70 / 90 mm
stud columns, 199 / 239 mm minimum wall widths, `Nil` / `KI 50G11` /
`KI 75G11` / `KI 90G11` variants, and source ratings spanning `Rw
49-64` / `Rw+Ctr 41-54`. Runtime remains blocked because live timber
is a single-frame formula-owned generic gypsum / rockwool / air-gap
route without twin-frame gap, side asymmetry, exact column,
field-output policy, or tolerance owner.
...
Prior just closed context:
Gate C closed `lined_masonry_knauf_mwi2a_mapping_tolerance_v1`
no-runtime in
`packages/engine/src/post-lined-masonry-knauf-mwi2a-mapping-tolerance-v1-next-slice-selection-contract.test.ts`.
It selects `twin_timber_knauf_ttf302a_mapping_tolerance_v1` because
`TTF30.2A` is the remaining concrete Knauf double-leaf twin timber
locator and can map the no-stud / raw open-box / simple timber
negative boundaries. It selected
...
Gate A landed `lined_masonry_knauf_mwi2a_mapping_tolerance_v1`
no-runtime in
`packages/engine/src/lined-masonry-knauf-mwi2a-mapping-tolerance-gate-a-contract.test.ts`.
It records official `MWI.2A` context: `1x13 mm SHEETROCK ONE` each
side, side 2 on 28 mm furring channels, 30 / 50 mm cavity variants,
`Nil` / `KI 25G24` / `KI 50G11` insulation variants, concrete panel
and core-filled block substrate variants, and source ratings spanning
`Rw 52-61` / `Rw+Ctr 44-51`. Runtime remains blocked because the live
route does not exact-match the board, substrate mass, furring/cavity
coupling, insulation, field-output policy, or tolerance owner. It
selected `gate_c_no_runtime_closeout_and_next_slice_selection`.

Just closed context:
Gate C closed `timber_double_board_knauf_tb5a_mapping_tolerance_v1`
no-runtime in
`packages/engine/src/post-timber-double-board-knauf-tb5a-mapping-tolerance-v1-next-slice-selection-contract.test.ts`.
It selects `lined_masonry_knauf_mwi2a_mapping_tolerance_v1` because
`MWI.2A` is the next ranked concrete Knauf locator and attacks the
lined masonry / heavy-core screening gap. Selection status:
...
Prior just landed context:
Gate A landed `timber_double_board_knauf_tb5a_mapping_tolerance_v1`
no-runtime in
`packages/engine/src/timber-double-board-knauf-tb5a-mapping-tolerance-gate-a-contract.test.ts`.
It records official `TB.5A` context: 70 mm / 122 mm column at
`Rw 46 (Rw+Ctr 39)` for `KI 75G11`, and 90 mm / 142 mm column at
`Rw 47 (Rw+Ctr 40)` for `KI 75G11`. Runtime remains blocked because
the live route does not exact-match `2x13 mm SHEETROCK ONE`,
`75 mm KI 75G11`, or a selected stud-depth column; field outputs,
`STC`, `C`, `Ctr`, and tolerance ownership remain unresolved. Gate A
selected `gate_c_no_runtime_closeout_and_next_slice_selection` with
`packages/engine/src/post-timber-double-board-knauf-tb5a-mapping-tolerance-v1-next-slice-selection-contract.test.ts`.

Prior just landed context:
Gate A landed `calculator_source_gap_revalidation_v5` no-runtime in
`packages/engine/src/calculator-source-gap-revalidation-v5-gate-a-contract.test.ts`.
It re-ranked Knauf `TB.5A`, Knauf `MWI.2A`, CLT / mass timber,
generated floor fallback, no-stud double-leaf, historical blocked
families, internal-use promotion, and productization-only work. Every
candidate remains `runtimeImportReadyNow: false`; `TB.5A` is selected
only for no-runtime mapping / tolerance because exact column, local
material mapping, lab / field output policy, tolerance ownership, and
paired visible tests remain incomplete. Selection status:
...
It kept `EN-PC-50-055-6-2-12.5-WB-25`, `TB.5A`, `TTF30.2A`, and
...
and `proposal/report/workbench-input` behavior frozen, treated Knauf
UK/AU source locators as extraction inputs rather than import approval,
and selected `knauf_wall_systems_source_pack_extraction_v1`.

Just closed context:
Gate C closed `generated_floor_fallback_topology_delta_v1` no-runtime
in
`packages/engine/src/post-generated-floor-fallback-topology-delta-v1-next-slice-selection-contract.test.ts`.
It kept the live generated floor fallback low-confidence/screening,
skipped Gate B, preserved every runtime and visible surface, and
selected `calculator_source_gap_revalidation_v4`.

Prior generated-floor context:
Gate A landed no-runtime in
`packages/engine/src/generated-floor-fallback-topology-delta-gate-a-contract.test.ts`.
It built the executable topology-delta matrix, kept live
`floor-steel-fallback` low-confidence, proved Pliteq exact and UBIQ
FL-32 bound source precedence, kept unsupported outputs explicit, and
selected Gate C closeout because no runtime candidate is source-ready.
...
It created [INTERNAL_USE_PILOT_HANDOFF.md](./INTERNAL_USE_PILOT_HANDOFF.md)
and kept `runtime/support/confidence/evidence/API/route-card/output-card`
...
/ next-slice selection without runtime/support/confidence/evidence
movement.

Prior context:
CLT / mass-timber Gate C closed no-runtime in
`packages/engine/src/post-clt-mass-timber-wall-source-pack-extraction-v1-next-slice-selection-contract.test.ts`.
It kept every Gate B roadmap track out of source-ready runtime import,
preserved current `wall-clt-local` values and Dataholz floor-only
boundaries, and selected internal-use acceptance rehearsal.
...
source group remains `runtimeImportReadyNow: false`.
...
Wall as later row-mapping candidates only; WoodWorks Table 8 Single NLT
Wall, NRC RR-335, and the NRC NLT addendum as formula/tolerance
context; and the WoodWorks database plus Dataholz CLT floor rows as
rejection-only context. It selected Gate B metric-mapping /
formula-tolerance decision, with all runtime and visible surfaces
frozen.

Earlier source-readiness context:
...
as a no-runtime role-defined vs raw order audit. It pinned UBIQ FL-28
and Dataholz GDMTXN01 exact floor rows under expanded UI order edits,
kept raw terminal concrete helper order sensitivity explicit, kept raw
open-web impact fail-closed, and kept many-layer split raw concrete
stacks finite without exact/bound promotion. It selected Gate C
no-runtime closeout / next-slice selection. Gate C then closed the slice
no-runtime in
`packages/engine/src/post-floor-layer-order-invariance-expansion-v1-next-slice-selection-contract.test.ts`
and selected the current framed-wall split warning-stability slice.

`calculator_source_gap_revalidation_v2` was selected by
`packages/engine/src/post-wall-lined-massive-heavy-core-source-research-v1-next-slice-selection-contract.test.ts`.
Gate A landed in
`packages/engine/src/calculator-source-gap-revalidation-v2-gate-a-contract.test.ts`
as a no-runtime source/accuracy rerank. It kept wall source-chain
holdouts, floor fallback, and historical blocked-source families closed,
recognized floor field-continuation as already audited, deferred
optional architecture and productization-only work, and selected the
current floor layer-order invariance expansion slice.

`wall_lined_massive_heavy_core_source_research_v1` was selected by
...
as a no-runtime source/bounded-lining-rule inventory. It kept Knauf
CC60 rows floor-only, found no wall-specific lined concrete / heavy
masonry source row, found no bounded lined-massive formula tolerance
owner, kept selector/deep-hybrid rows as stability boundaries, and
selected Gate C closeout. Gate C then closed no-runtime and selected
the current source-gap revalidation v2 slice.

`wall_clt_wall_source_research_v1` was selected by
...
as a no-runtime source/tolerance inventory. It kept Dataholz CLT rows
floor-only, found no wall-specific CLT row, found no bounded
laminated-leaf tolerance owner, and selected Gate C closeout. Gate C
then closed no-runtime and selected the current lined-massive /
heavy-core source research slice.

Earlier closed context:
...
owner. Gate A landed in
`packages/engine/src/wall-no-stud-double-leaf-source-research-gate-a-contract.test.ts`
as a no-runtime source/tolerance inventory. It found no import-ready
direct row: gypsum-block double-wall evidence is direct-family but
adjacent material, Davy/Sharp is a formula/tolerance candidate, NRC is
a row archive that still needs no-stud row extraction, and stud/framed
evidence remains adjacent context only. Gate B has now landed in
...
formula tolerance owner, or bounded family rule exists. Gate C has now
closed no-runtime in
`packages/engine/src/post-wall-double-leaf-sharp-davy-scoping-v1-next-slice-selection-contract.test.ts`
and selected `wall_double_leaf_source_evidence_acquisition_v1`. Gate A
and Gate B of that source-evidence slice landed no-runtime, and Gate C
has now closed it no-runtime in
...
Gate C. Timber double-board source research Gate A has landed
no-runtime and Gate C has closed no-runtime. CLT wall source research
Gate A has landed no-runtime and Gate C has closed no-runtime.
Lined-massive / heavy-core source research Gate A has landed
no-runtime and Gate C has closed no-runtime. Framed-wall split warning
stability Gate A landed no-runtime, Gate B fixed the LSF field
value/warning drift, and Gate C closed the slice. Calculator source-gap
revalidation v3 Gate A landed no-runtime and selected
...
handoff pack and selected Gate C closeout / next-slice selection. Gate
...
| `calculator_source_gap_revalidation_v3` Gate A | calculator source/readiness rerank | 2026-04-28 | `calculator-source-gap-revalidation-v3-gate-a-contract.test.ts` |
| `wall_framed_facing_split_warning_stability_v1` Gate C | calculator wall split stability | 2026-04-28 | `post-wall-framed-facing-split-warning-stability-v1-next-slice-selection-contract.test.ts` |
| `wall_framed_facing_split_warning_stability_v1` Gate B | calculator wall split stability | 2026-04-28 | `wall-framed-facing-split-warning-stability-gate-b-contract.test.ts` |
| `wall_framed_facing_split_warning_stability_v1` Gate A | calculator wall split stability | 2026-04-28 | `wall-framed-facing-split-warning-stability-gate-a-contract.test.ts` |
| `floor_layer_order_invariance_expansion_v1` Gate C | calculator floor order accuracy | 2026-04-28 | `post-floor-layer-order-invariance-expansion-v1-next-slice-selection-contract.test.ts` |
| `floor_layer_order_invariance_expansion_v1` Gate A | calculator floor order accuracy | 2026-04-28 | `floor-layer-order-invariance-expansion-gate-a-contract.test.ts` |
| `calculator_source_gap_revalidation_v2` Gate A | calculator revalidation | 2026-04-28 | `calculator-source-gap-revalidation-v2-gate-a-contract.test.ts` |
| `wall_lined_massive_heavy_core_source_research_v1` Gate C | calculator wall source research | 2026-04-28 | `post-wall-lined-massive-heavy-core-source-research-v1-next-slice-selection-contract.test.ts` |
| `wall_lined_massive_heavy_core_source_research_v1` Gate A | calculator wall source research | 2026-04-28 | `wall-lined-massive-heavy-core-source-research-gate-a-contract.test.ts` |
| `wall_clt_wall_source_research_v1` Gate C | calculator wall source research | 2026-04-28 | `post-wall-clt-wall-source-research-v1-next-slice-selection-contract.test.ts` |
| `wall_clt_wall_source_research_v1` Gate A | calculator wall source research | 2026-04-28 | `wall-clt-wall-source-research-gate-a-contract.test.ts` |
| `wall_timber_double_board_source_research_v1` Gate C | calculator wall source research | 2026-04-28 | `post-wall-timber-double-board-source-research-v1-next-slice-selection-contract.test.ts` |
| `wall_timber_double_board_source_research_v1` Gate A | calculator wall source research | 2026-04-28 | `wall-timber-double-board-source-research-gate-a-contract.test.ts` |
...
| C6 | Architectural hygiene (≤2000 line files) | ✅ `dynamic_airborne_split_refactor_v2` Gate C closed; `dynamic-airborne.ts` is 1969 lines after Gate AZ and still below threshold | `dynamic-airborne-*.ts` module family + `DYNAMIC_AIRBORNE_CARTOGRAPHY.md` + `dynamic-airborne-split-v2-gate-b-eleventh-carve-contract.test.ts` + `post-dynamic-airborne-split-refactor-v2-gate-c-next-slice-selection-contract.test.ts` + `coverage-grid-consistency.test.ts` |
...
| F3 | Framed-wall monotonic-floor guard drifted when a board-layer was split; Gate A revalidation found LSF field splits were value+warning drift (+1 dB on apparent field outputs), while LSF lab and timber lab/field stayed stable | 2026-04-28 Gate B fixed / Gate C closed | `wall-framed-facing-split-warning-stability-gate-b-contract.test.ts` fixes the LSF field split drift locally; `wall-framed-facing-split-warning-stability-route-card-matrix.test.ts` pins visible cards/acoustic warnings; global board coalescing remains disallowed |
...
`packages/engine/src/dynamic-airborne.ts` is 1969 lines (down
...
Remaining `dynamic-airborne.ts` (1969 lines) hosts 3 in-file
...
4. **F3 framed-wall monotonic-floor split drift** — closed as
   `wall_framed_facing_split_warning_stability_v1`; Gate B fixed the
   LSF field value+warning drift without global board coalescing, and
   Gate C selected `calculator_source_gap_revalidation_v3`.
...
1. Read [CHECKPOINT_2026-04-30_BRITISH_GYPSUM_WHITE_BOOK_SOURCE_PACK_EXTRACTION_GATE_A_HANDOFF.md](./CHECKPOINT_2026-04-30_BRITISH_GYPSUM_WHITE_BOOK_SOURCE_PACK_EXTRACTION_GATE_A_HANDOFF.md)
   and [SLICE_BRITISH_GYPSUM_WHITE_BOOK_SOURCE_PACK_EXTRACTION_PLAN.md](./SLICE_BRITISH_GYPSUM_WHITE_BOOK_SOURCE_PACK_EXTRACTION_PLAN.md)
...
2. Read [NEXT_IMPLEMENTATION_PLAN.md](./NEXT_IMPLEMENTATION_PLAN.md),
   ...
   Continue `british_gypsum_white_book_source_pack_extraction_v1` at
   Gate B by adding
   `packages/engine/src/british-gypsum-white-book-source-pack-extraction-gate-b-contract.test.ts`.
...
7. Do not change source posture, output support, evidence tiers,
   ...
   Knauf source-pack extraction unless it explicitly selects a later
   bounded runtime slice. Frozen surface shorthand:
