# Checkpoint - 2026-04-24 - Dynamic Airborne Split v2 Gate B Fifth Carve Handoff

Status: Gate B fifth carve landed. Continue
`dynamic_airborne_split_refactor_v2` with the next bounded
non-recursive field-lift carve.

## What Changed

- Moved `applyMixedBoardEmptyCavityFieldMidbandLift` out of
  `packages/engine/src/dynamic-airborne.ts` and into
  `packages/engine/src/dynamic-airborne-correction-guards.ts`.
- Kept the moved midband lift non-recursive; it does not take
  `DynamicAirborneComposer`.
- Updated `dynamic-airborne.ts` to import all five carved correction
  guards from `dynamic-airborne-correction-guards.ts`.
- Replaced the fourth-carve static contract with
  `packages/engine/src/dynamic-airborne-split-v2-gate-b-fifth-carve-contract.test.ts`.
- Updated the current gate script to include the new fifth-carve
  contract.
...
- `packages/engine/src/dynamic-airborne-correction-guards.ts`: 471
  physical lines.
- Remaining top-level `apply*` guards in `dynamic-airborne.ts`: 9.
...
Move `applyMixedPremiumSplitFieldLift` into
`dynamic-airborne-correction-guards.ts`.

Why this is next:

- it is the next remaining non-recursive field-lift correction;
...
Keep this sixth carve isolated. Do not mix it with
...
Completed after this fifth-carve patch:

- targeted Gate B fifth-carve contract + coverage-grid contract:
