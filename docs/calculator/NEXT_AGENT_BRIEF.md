...
Use it before selecting the next owner. The intended order has advanced
through the ceiling/roof/suspended-ceiling route boundary, the
already-landed floor `IIC`/`AIIC` exact-band work, and the landed
opening/facade `OITC` spectral rating owner, coverage refresh, and
post-refresh rerank, and the landed bounded spectral rating backbone V1
support prerequisite, the route/input family first-class surface
input-surface slice, the bounded industry-grade golden scenario matrix,
and the route-required input question engine. The current selected next
is the roof airborne formula owner after input surface V1.
Reject broad source crawling, confidence-label work, metric aliasing,
cosmetic UI work, and support loops that do not protect or unlock that
sequence.

Current checkpoint:
`docs/calculator/CHECKPOINT_2026-06-30_ROUTE_REQUIRED_INPUT_QUESTION_ENGINE_HANDOFF.md`.
It records the docs/implementation comparison after the OITC owner
sequence, spectral backbone, route/input surface, golden matrix, and
route-required input question engine landed. Treat older selected-next
prose as historical unless the source-of-truth, current-state,
documentation-map, and this brief name it as current.
...
1. Implement the roof airborne formula owner selected after the question
   engine.
2. Use the now-owned `roof_airborne` route/input surface and question
   payload: route intent, roof/facade mounting context, frequency band
   set, surface mass, and cavity depth must be complete before values
   publish.
3. Open only roof element-lab `Rw`, `STC`, `C`, and `Ctr` on the owned
   roof route.
4. Keep missing roof physics as `needs_input`; keep OITC, field/building,
   indoor partition, ceiling/plenum, impact, and source-row proximity
   aliases blocked.
5. Do not import source rows, make a cosmetic UI pass, or borrow indoor
   wall/ceiling values as roof outputs.
...
`post_v1_route_required_input_question_engine_v1`. It turns typed
`needs_input` route/input rows into ordered minimum unblocker question
groups and keeps unsupported-only rows unsupported. Counters:
...
and uses selected candidate
`post_v1_route_input_family_first_class_surface_v1`. The slice makes
ceiling/plenum, roof, opening/facade indoor, opening/facade
outdoor-indoor OITC, ASTM IIC/AIIC impact rating, and field/building
flanking context first-class route/input families. Counters:
...
`post_v1_spectral_rating_backbone_v1`. The support slice adds the
bounded `ratingsFromOwnedCurve` backbone, centralizes exact frequency
coverage validation for ASTM E1332 OITC and ASTM E989 impact bands,
keeps ISO 717-1 / ASTM E413 airborne curve math numerically stable, and
moves the landed OITC owner onto the backbone without publishing new
runtime values. Counters: `newCalculableRequestShapes: 0`,
...
and selects candidate `post_v1_spectral_rating_backbone_v1`. The
selection rejects the already-landed OITC owner repeat, OITC
calibration without same-family same-basis holdout evidence,
route/input-family work until the rating backbone closes, triple-leaf
runtime promotion before holdout blockers close, and broad
source/frontend/support drift. Counters: `candidateCount: 6`,
`roiAnalysisIterations: 4`, `estimatedNextCalculableRequestShapes: 0`,
...
/
`post_v1_ceiling_multileaf_airborne_plenum_element_lab_formula_owner_landed_runtime_basis_selected_coverage_refresh`.
Complete ceiling-only gypsum-board plus plenum/fill element-lab requests
with explicit `ceilingPlenum` context now publish `Rw 48`, `STC 48`,
`C -1.7`, and `Ctr -6.5` through route `ceiling`. Missing plenum
inputs, field/building outputs, impact outputs, `IIC`/`AIIC`, `OITC`,
ASTM aliases, source-row proximity substitution, and confidence fallback
remain blocked. Selected next:
`post_v1_ceiling_multileaf_airborne_plenum_element_lab_formula_owner_coverage_refresh_plan`
/
`packages/engine/src/post-v1-ceiling-multileaf-airborne-plenum-element-lab-formula-owner-coverage-refresh-contract.test.ts`
/
...
`post_v1_ceiling_multileaf_airborne_plenum_input_boundary_owner_landed_input_boundary_selected_coverage_refresh`
and re-probes selected candidate
`ceiling.multileaf_airborne_plenum_input_boundary_owner` with method
`post_v1_ceiling_multileaf_airborne_plenum_input_boundary_missing_physical_inputs`.
Ceiling-only board plus plenum/cavity/fill airborne requests stay route
`ceiling` `needs_input` for `ceilingLeafGrouping`,
`ceilingLeafSurfaceMassKgM2`, `ceilingCavityOrPlenumDepthMm`,
...
and uses selected candidate
`ceiling.multileaf_airborne_plenum_input_boundary_owner`. It captures
`ceilingLeafGrouping`, `ceilingLeafSurfaceMassKgM2`,
`ceilingCavityOrPlenumDepthMm`,
`ceilingAbsorberThicknessAndFlowResistivity`, and
`ceilingSupportCouplingOrHangerClass`, with counters
`newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
...
`post_v1_runtime_first_route_family_rerank_after_ceiling_single_leaf_field_building_context_adapter_coverage_refresh_landed_no_runtime_selected_ceiling_multileaf_airborne_plenum_input_boundary_owner`.
It selects candidate
`ceiling.multileaf_airborne_plenum_input_boundary_owner`. Counters:
...
- preserve field values `R'w 33`, `Dn,w 33`, `Dn,A 36.5`,
  `DnT,w 36`, and `DnT,A 38.9`;
- preserve building values `R'w 33`, `Dn,w 33`, `Dn,A 36.5`,
  `DnT,w 36`, `DnT,A 38.9`, and `DnT,A,k 36`;
- keep missing context, impact, ASTM/IIC/AIIC, OITC, source-row
  fallback, and confidence-label fallback blocked;
- move no runtime values, retune no formulas, import no source rows, and
  touch no frontend files;
- counters include `coverageRefreshContractFilesTouched: 1`,
  `runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- this is not a broad source crawl.
...
It selected candidate
`ceiling.single_leaf_field_building_context_adapter_owner` and kept
`runtimeValuesMoved 0`. It is not a broad source crawl.
...
Complete ceiling-only field requests now publish `R'w 33`, `Dn,w 33`,
`Dn,A 36.5`, `DnT,w 36`, and `DnT,A 38.9` on route `ceiling`.
Complete ceiling-only building requests now publish `R'w 33`,
`Dn,w 33`, `Dn,A 36.5`, `DnT,w 36`, `DnT,A 38.9`, and `DnT,A,k 36`
on route `ceiling`; lab companions remain separate. Counters:
...
It selected candidate `ceiling.single_leaf_airborne_route_owner` and
kept `runtimeValuesMoved 0`. It is not a broad source crawl.
...
It uses selected candidate
`ceiling.single_leaf_airborne_mass_law.source_absent` and opens
ceiling-only gypsum-board single-leaf `Rw 34`, `STC 34`, `C 3.7`, and
`Ctr 8` on route `ceiling`. Counters:
...
keeps indoor field/building values pinned, keeps mixed lab companions
basis-separated, and keeps missing frequency input, outdoor-indoor/OITC,
and impact aliases blocked. Counters:
`coverageRefreshContractFilesTouched: 1`,
`newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
`runtimeBasisPromotions: 0`, `runtimeValuesMoved 0`,
`runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
...
requests now calculate `R'w 36.4`, `Dn,w 36.7`, `Dn,A 35.9`,
`DnT,w 36.9`, and `DnT,A 36.1`; complete `building_prediction`
requests calculate `R'w 31.6`, `Dn,w 31.9`, `Dn,A 31.1`,
...
Outdoor-indoor/OITC, missing frequency input, scalar STC opening
ratings, broad facade prediction, source imports, and impact aliases
remain blocked. Counters: `newCalculableRequestShapes: 4`,
...
lab companions `Rw 38.2` and `STC 39` remain live, missing frequency
input stays `needs_input`, and outdoor-indoor/OITC plus impact aliases
...
Complete indoor door/window/facade `field_between_rooms` and
`building_prediction` requests now keep lab companions `Rw 38.2` and
`STC 39` from Gate S/Gate AH while field/building facade outputs,
outdoor-indoor/OITC, missing frequency input, and impact aliases remain
...
Recent user-requested non-runtime support work:
...
- `docs/calculator/WORKBENCH_V2_ROUTE_INPUT_VALIDATION_AND_NEEDS_INPUT_BUG_HUNT_2026-06-24.md`
  ...
  record the user-requested Workbench V2 route-input validation and
  needs-input support fix. It hardens paired panel-area highlighting,
  raw route-input copy for `toppingOrFloatingLayer` /
  `impactFieldContext`, raw `ratings.field.partitionAreaM2` error
  leakage, safe estimate/impact-only exception envelopes, stale ready
  result suppression, report-assistant task copy, and positive
  field-rating metadata boundaries. Final targeted validation passed web
  `7 files / 68 tests`, engine `3 files / 20 tests`, `apps/web`
  typecheck, and `git diff --check`. Treat this as Workbench/API/metadata
  boundary support only: no formula retune, source row import, route
  ownership movement, runtime value movement, selected-next replacement,
  or acoustic numeric value movement landed.
- `docs/calculator/PUBLIC_SOURCE_MATERIAL_CATALOG_EXPANSION_HANDOFF_2026-06-23.md`
  ...
  British Gypsum/Gyproc, Knauf, QuietRock, Tecsound, ROCKWOOL, Stora
  Enso CLT, H+H Celcon, and REGUPOL products, then proved direct use in
  existing dynamic airborne and heavy floating-floor impact routes.
  Treat this as catalog-input support only: no acoustic source rows,
  formula retunes, copied manufacturer ratings, runtime value movement,
  frontend work, or selected-next replacement landed.
- `docs/calculator/EXCEL_IMPACT_PRODUCT_CANDIDATE_RESCREEN_HANDOFF_2026-06-24.md`
  records the follow-up Excel impact-product candidate rescreen after
  matcher hardening. It opens only two Getzner AFM29 exact-system rows
  for `160 mm` reinforced concrete plus `172 kg/m2` and `197 kg/m2`
  concrete screed stacks. Generic Getzner AFM product-delta rows,
  DAMTEC screed-dependent rows, Isolgomma headline rows, DANOSA
  `DeltaLn`, Forbo/Tarkett unbounded floor-covering rows, generic
  ranges, and damping/barrier product references remain parked unless a
  future owner encodes exact stack/basis criteria.
- Treat the workbench bug-hunt record as UI/report/assistant copy and
  validation context only. It did not move formula ownership, numeric
  values, route selection, `needs_input` / `unsupported` semantics,
  calculator counters, or the selected calculator slice. The current
  selected slice is the runtime-first rerank after the advanced-wall
...
- After that reconciliation, a bounded British Gypsum exact lab
  ...
  `Ctr` live in mixed lab requests while field/building companions stay
  `needs_input`; it did not retune values or alias lab metrics into
  field/building outputs.
- The runtime-first rerank after the current-gate stale metric/basis
  reconciliation has landed and selected the British Gypsum exact lab
  field/building adapter owner as the next runtime slice.
- The British Gypsum exact lab field/building adapter owner has now
  landed. Complete explicit `A046005` / `A046006` field/building
  requests use the exact lab `Rw` direct curve plus Gate I / Gate AR;
  A-weighted outputs, lab aliases, impact aliases, legacy `auto`
  side-count, and nearby rows remain closed.
- The British Gypsum exact lab field/building adapter coverage refresh
  has also landed. It re-probes the exact BG field/building pins and
  keeps missing context, legacy `auto` side-count exact promotion,
  A-weighted outputs, lab aliases, impact aliases, nearby rows, and
  lab-to-field/building copying closed.
- The British Gypsum exact lab A-weighted field/building adapter owner
  has now landed. Complete explicit `A046005` / `A046006`
  field/building requests now calculate `Dn,A` and `DnT,A` companions
  from the exact lab `Rw` direct curve through Gate I / Gate AR:
  `A046005` carries `Dn,A 48.0` and `DnT,A 49.1`; `A046006` carries
  `Dn,A 51.0` and `DnT,A 52.1`. A-only `Dn,A` / `DnT,A` requests are
  calculable when field/building context is complete.
- The British Gypsum exact lab A-weighted field/building adapter
  coverage refresh has landed no-runtime with status
  ...
  and moved `runtimeValuesMoved 0`.
- The British Gypsum exact lab building `DnT,A,k` characteristic
  adapter owner has landed. Complete explicit `A046005` / `A046006`
  building requests now calculate `DnT,A,k` from the exact lab `Rw`
  direct curve, Gate AR `DnT,A`, receiving-room volume, partition area
  with the 7 m2 minimum, and `T0=0.5 s`: `A046005` carries
  `DnT,A 49.1` and `DnT,A,k 48.0`; `A046006` carries `DnT,A 52.1`
  and `DnT,A,k 51.0`. Apparent-only, field, lab, impact, missing-input,
  nearby BG row, legacy `auto`, and lab-copy boundaries remain closed.
- The British Gypsum exact lab building `DnT,A,k` coverage refresh has
  ...
  and moved `runtimeValuesMoved 0`.
- The British Gypsum exact lab calculated lab companion owner has
  landed. Mixed `Rw + STC + C + Ctr` requests for explicit exact
  `A046005` / `A046006` now keep measured exact `Rw` while calculating
  `STC`, `C`, and `Ctr` from the selected dynamic transmission-loss
  curve and rating adapters. `A046005` carries `Rw 55`, `STC 55`,
  `C -0.6`, and `Ctr -5.4`; `A046006` carries `Rw 58`, `STC 58`,
  `C -0.6`, and `Ctr -5.4`. Single-output `Rw`, field/building aliases,
  impact aliases, nearby rows, legacy `auto`, and lab-copy boundaries
  remain closed.
- The British Gypsum exact lab calculated lab companion coverage refresh
  has landed no-runtime:
  ...
  keeps `A046005` at `Rw 55`, `STC 55`, `C -0.6`, `Ctr -5.4`; and
  `A046006` at `Rw 58`, `STC 58`, `C -0.6`, `Ctr -5.4`. Counters:
  `coverageRefreshContractFilesTouched: 1`,
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 0`,
  `newCalculableTargetOutputs: 0`, `runtimeBasisPromotions: 0`,
  `runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
  This is not a broad source crawl.
- The runtime-first rerank after the BG exact lab calculated lab
  companion coverage refresh has landed no-runtime:
  ...
  `packages/engine/src/post-v1-wall-british-gypsum-exact-lab-calculated-lab-companion-coverage-refresh-contract.test.ts`,
  selects candidate
  `wall.user_material_formula_required_input_surface_owner`, and keeps
  nearby British Gypsum same-family deltas blocked until primary-source
  same-family, same-basis evidence and a bounded delta rule exist.
  ...
  `frontendImplementationFilesTouched: 0`. This is not a broad source
  crawl.
  The wall user-material formula required input-surface owner and
  coverage refresh have now landed. The runtime-first rerank after that
  coverage refresh and the wall user-material double-leaf building
  leaf-mass boundary owner and coverage refresh have now landed too.
  The runtime-first rerank after that coverage refresh and the wall
  ...
  runtime-first rerank after that coverage refresh has landed
  no-runtime:
  ...
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`.
  The wall Gate AR direct-curve building lab companion target-output
  independence owner has now landed runtime/accuracy behavior:
  ...
  building requests now keep lab companions `Rw 44`, `STC 44`,
  `C -1.3`, and `Ctr -6.4` from the owned direct curve while
  field/building values stay `R'w 38`, `Dn,w 39`, `Dn,A 37.8`,
  `DnT,w 41`, `DnT,A 39.8`, and `DnT,A,k 36.7`. Counters:
  `runtimeValuesMoved 40`, `newCalculableRequestShapes: 3`,
  `newCalculableTargetOutputs: 3`, `runtimeBasisPromotions: 3`,
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`.
  The wall Gate AR direct-curve building lab companion target-output
  independence coverage refresh has now landed no-runtime:
  ...
  It re-probes selected candidate
  `wall.gate_ar_direct_curve_building_lab_companion_target_output_independence_owner`
  and keeps `Rw 44`, `STC 44`, `C -1.3`, `Ctr -6.4`, `R'w 38`,
  ...
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
  This is not a broad source crawl.
  The runtime-first rerank after the wall Gate AR direct-curve building
  lab companion target-output independence coverage refresh has now
  landed no-runtime:
  ...
  target-output independence owner has now landed runtime behavior:
  ...
  It publishes Gate AY lab-only companions `Rw 65`, `STC 65`,
  `C -1.1`, and `Ctr -6.4` for complete source-absent advanced-wall
  field/building contexts while keeping field `R'w 63`, building
  `DnT,w 66`, and lab companions basis-separated. Counters:
  `newCalculableRequestShapes: 2`,
  `newCalculableTargetOutputs: 8`, `runtimeBasisPromotions: 2`,
  `runtimeValuesMoved 8`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
  The advanced-wall coverage refresh has also landed and is protected by
  the current gate. The fresh runtime-first rerank after the
  advanced-wall current-gate checkpoint has also landed and is now
  historical selection evidence.
  Historical selected next action at that checkpoint, now landed:
  `post_v1_runtime_first_route_family_rerank_after_advanced_wall_current_gate_checkpoint_plan`.
...
- The wall user-material formula required input-surface coverage refresh
  has landed no-runtime:
  ...
  It re-probes selected candidate
  `wall.user_material_formula_required_input_surface_owner`, keeps
  complete source-absent user-material wall formula routes calculable
  with explicit physical inputs, and keeps zero-mass user/project leaf
  requests parked behind `needs_input` for
  ...
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`.
  This is not a broad source crawl.
- The runtime-first rerank after the wall user-material formula
  required input-surface coverage refresh has landed no-runtime:
  ...
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`.
- The wall user-material double-leaf building leaf-mass boundary owner
  has landed runtime-boundary behavior:
  ...
  requests with missing or zero side-leaf mass now park behind Gate S
  ...
  requests remain calculable. Counters:
  ...
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`.
- The wall user-material double-leaf building leaf-mass boundary
  coverage refresh has landed no-runtime:
  ...
  It re-probes selected candidate
  `wall.user_material_double_leaf_building_leaf_mass_boundary_owner` and
  moved `runtimeValuesMoved 0`.
- The runtime-first rerank after the wall user-material double-leaf
  building leaf-mass boundary coverage refresh has landed no-runtime:
  ...
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`.
- The wall user-material double-leaf building lab companion
  target-output independence owner has landed runtime accuracy:
  ...
  target-output independent while field/building values remain
  `R'w 40`, `Dn,w 41`, `Dn,A 39.5`, `DnT,w 43`, `DnT,A 41.9`, and
  `DnT,A,k 39`. Single-output requests remain request-scoped; zero-mass
  side leaves stay `needs_input`, and impact aliases remain unsupported.
  Counters: `accuracyPromotedRequestShapes: 10`,
  `targetOutputIndependentMetricSets: 10`, `runtimeValuesMoved 40`,
  `newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`.
- Validation after that owner: targeted British Gypsum suite passed
  `4 files / 24 tests`; `pnpm calculator:gate:current` passed with
  engine `827 files / 4529 tests`, web
  `127 files / 506 passed / 18 skipped`, repo build `5/5` tasks, and
  `git diff --check` clean.
- The wall user-material double-leaf building lab companion
  target-output independence coverage refresh has landed no-runtime:
...
- latest landed runtime boundary owner:
  `post_v1_wall_user_material_double_leaf_building_leaf_mass_boundary_owner_plan`;
- latest landed runtime boundary owner file:
  `packages/engine/src/post-v1-wall-user-material-double-leaf-building-leaf-mass-boundary-owner-contract.test.ts`;
- latest landed runtime boundary owner plan:
  `docs/calculator/POST_V1_WALL_USER_MATERIAL_DOUBLE_LEAF_BUILDING_LEAF_MASS_BOUNDARY_OWNER_PLAN_2026-06-24.md`;
- latest landed runtime boundary owner status:
  `post_v1_wall_user_material_double_leaf_building_leaf_mass_boundary_owner_landed_runtime_boundary_selected_coverage_refresh`;
- latest landed runtime boundary owner selected candidate:
  `wall.user_material_double_leaf_building_leaf_mass_boundary_owner`;
- latest landed runtime boundary owner selected next action:
  `post_v1_wall_user_material_double_leaf_building_leaf_mass_boundary_coverage_refresh_plan`;
- latest landed runtime boundary owner selected next file:
  `packages/engine/src/post-v1-wall-user-material-double-leaf-building-leaf-mass-boundary-coverage-refresh-contract.test.ts`;
- latest landed runtime boundary owner selected next plan:
  `docs/calculator/POST_V1_WALL_USER_MATERIAL_DOUBLE_LEAF_BUILDING_LEAF_MASS_BOUNDARY_COVERAGE_REFRESH_PLAN_2026-06-24.md`;
- latest landed runtime boundary owner selected next label:
...
- latest landed runtime/input-surface owner counters:
  ...
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`;
- latest landed runtime/input-surface owner note:
  complete source-absent user-material wall stacks stay on owned
  formula routes when physical inputs are explicit; zero-mass
  user/project leaf requests now return `needs_input` for
  `layer.surfaceMassKgM2_or_materialCatalog.densityKgM3_and_thicknessMm`;
- latest landed runtime/accuracy owner:
...
- latest landed runtime/accuracy owner note:
  this is not a broad source crawl;
- latest landed runtime/accuracy owner selected next action:
  `post_v1_wall_british_gypsum_exact_lab_calculated_lab_companion_coverage_refresh_plan`;
- latest landed runtime/accuracy owner selected next file:
...
- latest landed runtime/accuracy owner counters:
  ...
  `sourceRowsImported: 0`, `frontendImplementationFilesTouched: 0`,
  and `unsupportedBoundariesProtected: 5`;
- latest landed runtime/accuracy owner note:
  this is not a broad source crawl;
- latest landed runtime/accuracy owner selected next action:
...
- latest landed no-runtime coverage refresh counters:
  ...
  `runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- latest landed no-runtime coverage refresh note:
  this is not a broad source crawl;
...
- latest landed runtime boundary fix counters:
  `newCalculableRequestShapes: 3`, `newCalculableTargetOutputs: 4`,
  `runtimeBasisPromotions: 0`, `runtimeValuesMoved 0`,
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`,
  `engineImplementationFilesTouched: 1`, `engineContractFilesTouched: 1`,
  and `frontendImplementationFilesTouched: 0`;
- latest landed no-runtime coverage refresh:
...
- latest landed no-runtime rerank counters:
  `candidateCount: 7`, `knownCurrentGateFailureAssertions: 34`,
  `roiAnalysisIterations: 3`, `runtimeValuesMoved 0`,
  `runtimeFormulaRetunes: 0`, `sourceRowsImported: 0`, and
  `frontendImplementationFilesTouched: 0`;
...
- latest landed no-runtime current-gate reconciliation:
  `post_v1_current_gate_stale_metric_basis_reconciliation_after_opening_leak_apparent_dna_plan`;
- latest landed no-runtime current-gate reconciliation file:
  `packages/engine/src/post-v1-current-gate-stale-metric-basis-reconciliation-after-opening-leak-apparent-dna-contract.test.ts`;
- latest landed no-runtime current-gate reconciliation plan:
  `docs/calculator/POST_V1_CURRENT_GATE_STALE_METRIC_BASIS_RECONCILIATION_AFTER_OPENING_LEAK_APPARENT_DNA_PLAN_2026-06-23.md`;
- latest landed no-runtime current-gate reconciliation status:
...
- latest landed no-runtime current-gate reconciliation behavior:
  reconciled 34 stale engine current-gate metric/basis assertions
  ...
  assertions were also reconciled, and one web surface helper was
  updated so building A-weighted opening/leak results are not mislabeled
  as field results;
- latest landed no-runtime current-gate reconciliation counters:
...
- latest landed no-runtime current-gate reconciliation selected next
  action:
  `post_v1_runtime_first_route_family_rerank_after_current_gate_stale_metric_basis_reconciliation_plan`;
- latest landed no-runtime current-gate reconciliation selected next
  file:
...
- implementation sync:
  ...
  post-refresh rerank file also exists and is included; the leaf
  surface-mass owner and coverage-refresh files also exist and are
  included; the runtime-first rerank after leaf surface-mass coverage
  also exists and is included; the frequency-backbone owner file also
  exists and is included; the frequency-backbone coverage refresh file
  also exists and is included; the explicit surface-mass leaf scope
  opener file also exists and is included; the explicit surface-mass
  leaf coverage refresh file also exists and is included; the
  single-leaf explicit surface-mass unknown-material scope-opener file
  also exists and is included; the single-leaf explicit surface-mass
  unknown-material coverage-refresh file also exists and is included;
  the field-context adapter owner file also exists and is included; the
  field-context adapter coverage-refresh file also exists and is
  included; the building-context adapter owner file also exists and is
  included; the building-context adapter coverage-refresh file also
  exists and is included; the building `DnT,A,k` characteristic adapter
  owner file also exists and is included; the single-leaf characteristic
  adapter coverage refresh file also exists and is included; the
  double-leaf/framed context-owned porous-cavity building `DnT,A,k`
  characteristic adapter owner file also exists and is included; the
  double-leaf/framed characteristic adapter coverage refresh file also
  exists and is included; the compatible anchor-delta building
  `DnT,A,k` characteristic adapter owner file also exists and is
  included; the compatible anchor-delta coverage refresh file also
  exists and is included; the direct-fixed double-leaf/framed context
  absorptive-cavity building `DnT,A,k` characteristic adapter owner
  file also exists and is included; the direct-fixed coverage refresh
  file also exists and is included; the local-substitution building
  ...
  opening/leak apparent companion coverage refresh file also exists and
  is included; the runtime-first rerank after that coverage refresh also
  exists and is included; the selected current-gate stale metric/basis
  reconciliation file above does not exist yet and is the next
  no-runtime guard to create;
- latest landed runtime/accuracy owner:
...
- latest landed runtime/accuracy owner status:
  `post_v1_wall_opening_leak_building_dntak_characteristic_adapter_owner_landed_runtime_selected_coverage_refresh`;
- latest landed runtime/accuracy owner follows:
...
- latest landed runtime/accuracy owner behavior:
  opening/leak `building_prediction` requests now calculate
  `DnT,A,k 30.4` from owned `DnT,A 31.3`; mixed building requests carry
  `R'w 31.6`, `DnT,w 32.1`, `DnT,A 31.3`, and `DnT,A,k 30.4`;
  missing frequency band, missing volume, missing building output
  basis, apparent-only basis, field context, lab context, building
  `Dn,A`, and impact aliases remain closed;
- latest landed runtime/accuracy owner counters:
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 1`,
  `newCalculableTargetOutputs: 1`, `runtimeBasisPromotions: 0`,
  `runtimeValuesMoved 1`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- latest landed runtime/accuracy owner validation:
  targeted owner + previous local-substitution coverage refresh passed
  `2 files / 10 tests`; the opening/local neighbor chain passed
  `5 files / 26 tests`; `git diff --check` passed; the owner and
  previous coverage refresh pass inside `pnpm calculator:gate:current`,
...
- latest landed runtime/accuracy owner selected candidate:
  `wall.local_substitution_building_dntak_characteristic_adapter_owner`;
- latest landed runtime/accuracy owner behavior:
  grouped triple-leaf local-substitution `building_prediction` requests
  now calculate `DnT,A,k 52.4` from owned `DnT,A 53.9`; mixed building
  requests carry `R'w 51`, `Dn,w 51`, `Dn,A 52.4`, `DnT,w 53`,
  `DnT,A 53.9`, and `DnT,A,k 52.4`; missing volume, missing building
  output basis, apparent-only basis, field context, lab context, and
  impact aliases remain closed;
- latest landed runtime/accuracy owner counters:
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 1`,
  `newCalculableTargetOutputs: 1`, `runtimeBasisPromotions: 0`,
  `runtimeValuesMoved 1`, `runtimeFormulaRetunes: 0`,
...
- this is not a broad source crawl;
- historical direct-fixed runtime/accuracy owner:
...
- latest landed runtime/accuracy owner behavior:
  ...
  requests calculate `DnT,A,k` from the Gate EO direct-fixed bridge-loss
  curve, owned Gate AR `DnT,A`, receiving-room volume, partition area
  with the 7 m2 minimum, and `T0=0.5 s`; full absorptive cavity returns
  `DnT,A 31.0` and `DnT,A,k 27.7`, and partial absorptive cavity
  returns `DnT,A 29.0` and `DnT,A,k 25.7`; missing volume, missing
  building output basis, missing absorber flow, apparent-only basis,
  field context, lab context, and impact aliases remain closed;
- latest landed runtime/accuracy owner counters:
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
  `newCalculableTargetOutputs: 1`, `runtimeBasisPromotions: 0`,
  `runtimeValuesMoved 2`, `runtimeFormulaRetunes: 0`,
...
- historical compatible anchor-delta `DnT,A,k` runtime owner selected candidate:
  `wall.compatible_anchor_delta.building_dntak_characteristic_adapter_owner`;
- historical compatible anchor-delta `DnT,A,k` runtime owner selected next action:
  `post_v1_wall_compatible_anchor_delta_building_dntak_characteristic_adapter_coverage_refresh_plan`;
- historical compatible anchor-delta `DnT,A,k` runtime owner selected next file:
  `packages/engine/src/post-v1-wall-compatible-anchor-delta-building-dntak-characteristic-adapter-coverage-refresh-contract.test.ts`;
- historical compatible anchor-delta `DnT,A,k` runtime owner selected next plan:
  `docs/calculator/POST_V1_WALL_COMPATIBLE_ANCHOR_DELTA_BUILDING_DNTAK_CHARACTERISTIC_ADAPTER_COVERAGE_REFRESH_PLAN_2026-06-22.md`;
- historical compatible anchor-delta `DnT,A,k` runtime owner selected next label:
  `post-V1 wall compatible anchor-delta building DnT,A,k characteristic adapter coverage refresh`;
- historical compatible anchor-delta `DnT,A,k` runtime owner counters:
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
  `newCalculableTargetOutputs: 1`, `runtimeBasisPromotions: 0`,
  `runtimeValuesMoved 2`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- this is not a broad source crawl;
- historical double-leaf/framed `DnT,A,k` runtime owner action:
  `post_v1_wall_double_leaf_framed_context_owned_porous_cavity_building_dntak_characteristic_adapter_owner_plan`;
- historical double-leaf/framed `DnT,A,k` runtime owner file:
...
- historical double-leaf/framed `DnT,A,k` runtime owner plan:
...
- historical double-leaf/framed `DnT,A,k` runtime owner status:
...
- historical double-leaf/framed `DnT,A,k` runtime owner follows:
...
- historical double-leaf/framed `DnT,A,k` runtime owner selected next plan:
...
- historical double-leaf/framed `DnT,A,k` runtime owner selected next label:
  `post-V1 wall double-leaf/framed context-owned porous-cavity building DnT,A,k characteristic adapter coverage refresh`;
- historical double-leaf/framed `DnT,A,k` runtime owner counters:
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 1`,
  `newCalculableTargetOutputs: 1`, `runtimeBasisPromotions: 0`,
  `runtimeValuesMoved 1`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
...
- latest landed runtime/accuracy owner behavior:
  ...
  calculate `DnT,A,k` from the owned Gate AR `DnT,A`, receiving-room
  volume, partition area with the 7 m2 minimum, and `T0=0.5 s`; the
  single project panel returns `DnT,A 35.2` and `DnT,A,k 32.3`, and the
  laminated project panel returns `DnT,A 33.6` and `DnT,A,k 30.7`;
  missing mass, missing building inputs, apparent-only basis, field
  context, and impact aliases remain closed;
- latest landed runtime/accuracy owner counters:
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
...
- latest landed runtime/accuracy owner behavior:
  ...
  building corridor; the single project panel returns `R'w 30`,
  `Dn,w 31`, `Dn,A 32.8`, `DnT,w 33`, and `DnT,A 35.2`; the laminated
  project panel returns `R'w 29`, `Dn,w 30`, `Dn,A 31.2`, `DnT,w 32`,
  and `DnT,A 33.6`; missing mass, missing building inputs,
  cavity/wrong topology, and impact aliases remain closed;
- latest landed runtime/accuracy owner counters:
  `newCalculableLayerTemplates: 0`, `newCalculableRequestShapes: 2`,
...
- latest landed runtime/accuracy owner selected candidate:
  `wall.single_leaf.explicit_surface_mass_unknown_material_field_context_adapter_owner`;
- latest landed runtime/accuracy owner behavior:
  complete `field_between_rooms` requests for project/user single-leaf
  and laminated single-leaf panels with positive `surfaceMassKgM2` now
  use the owned Gate I field-context adapter on top of the single-leaf
  mass-law / banded lab curve instead of diagnostic
  `screening_fallback`; the single project panel returns `R'w 30`,
  `Dn,w 31`, `Dn,A 32.8`, `DnT,w 33`, and `DnT,A 35.2`; the laminated
  project panel returns `R'w 29`, `Dn,w 30`, `Dn,A 31.2`, `DnT,w 32`,
  and `DnT,A 33.6`; missing mass, missing field inputs, cavity/wrong
  topology, building prediction, ASTM aliases, and impact aliases remain
  closed;
- latest landed runtime/accuracy owner counters:
...
- latest landed runtime scope opener selected candidate:
  `wall.single_leaf.explicit_surface_mass_unknown_material_scope_opener`;
- latest landed runtime scope opener behavior:
  ...
  basis instead of diagnostic `screening_fallback`; missing/zero mass,
  cavity/wrong topology, building promotion, and impact aliases remain
  blocked;
- latest landed runtime scope opener counters:
  `newCalculableLayerTemplates: 1`, `newCalculableRequestShapes: 2`,
  `newCalculableTargetOutputs: 8`, `runtimeBasisPromotions: 2`,
  `runtimeValuesMoved 8`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- latest landed runtime scope opener selected next action:
  `post_v1_wall_single_leaf_explicit_surface_mass_unknown_material_coverage_refresh_plan`;
- latest landed runtime scope opener selected next file:
  `packages/engine/src/post-v1-wall-single-leaf-explicit-surface-mass-unknown-material-coverage-refresh-contract.test.ts`;
- latest landed runtime scope opener selected next plan:
  `docs/calculator/POST_V1_WALL_SINGLE_LEAF_EXPLICIT_SURFACE_MASS_UNKNOWN_MATERIAL_COVERAGE_REFRESH_PLAN_2026-06-22.md`;
- latest landed runtime scope opener selected next label:
...
- latest landed runtime scope opener selected candidate:
  `wall.double_leaf_framed.explicit_surface_mass_leaf_scope_opener`;
- latest landed runtime scope opener behavior:
  explicit side-leaf layers with `surfaceMassKgM2` now calculate lab
  `Rw/STC/C/Ctr` plus field/building `R'w`, `Dn,w`, `Dn,A`, `DnT,w`,
  and `DnT,A` through the owned double-leaf/framed route even when their
  project material ids have no catalog density row; missing
  `surfaceMassKgM2` remains `needs_input`, known gap/insulation materials
  stay outside side-leaf mass ownership, and impact aliases remain
  `unsupported`;
- latest landed runtime scope opener counters:
  `newCalculableLayerTemplates: 1`, `newCalculableRequestShapes: 3`,
  `newCalculableTargetOutputs: 14`, `runtimeBasisPromotions: 3`,
  `runtimeValuesMoved 14`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- latest landed runtime scope opener selected next action:
  `post_v1_wall_double_leaf_framed_explicit_surface_mass_leaf_coverage_refresh_plan`;
- latest landed runtime scope opener selected next file:
...
- latest landed frequency-backbone coverage refresh file:
...
- latest landed frequency-backbone coverage refresh status:
  `post_v1_wall_double_leaf_framed_frequency_backbone_numeric_sensitivity_coverage_refresh_landed_no_runtime_selected_explicit_surface_mass_leaf_scope_opener`;
- latest landed frequency-backbone coverage refresh re-probed:
  `wall.double_leaf_framed.frequency_backbone_numeric_sensitivity_owner`;
- latest landed frequency-backbone coverage refresh counters:
  `coverageRefreshContractFilesTouched: 1`,
  `newCalculableRequestShapes: 0`, `newCalculableTargetOutputs: 0`,
  `runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
...
- latest landed runtime/basis owner selected candidate:
  `wall.double_leaf_framed.frequency_backbone_numeric_sensitivity_owner`;
- latest landed runtime/basis owner behavior:
  it pins the calculated Gate S transmission-loss curve as the runtime
  frequency backbone for light, baseline, heavy, and asymmetric
  double-leaf/framed gypsum leaf stacks; lab `Rw`, `STC`, `C`, and
  `Ctr` derive from that same curve through ISO 717-1 and ASTM E413
  adapters; runtime basis exposes `calculatedFrequencyCurveShape` and
  the ISO 717-1 `C`/`Ctr` spectrum adaptation adapter; missing flow
  resistivity, missing support spacing, and impact aliases remain
  `needs_input` or `unsupported`; scalar rating pins stayed numerically
  stable, so no hidden formula retune was made;
- latest landed runtime/basis owner counters:
  ...
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- latest landed runtime/basis owner selected next action:
  `post_v1_wall_double_leaf_framed_frequency_backbone_numeric_sensitivity_coverage_refresh_plan`;
- latest landed runtime/basis owner selected next file:
  `packages/engine/src/post-v1-wall-double-leaf-framed-frequency-backbone-numeric-sensitivity-coverage-refresh-contract.test.ts`;
- latest landed runtime/basis owner selected next plan:
...
- latest landed runtime/basis owner selected next label:
...
- latest landed no-runtime rerank selected candidate:
  `wall.double_leaf_framed.frequency_backbone_numeric_sensitivity_owner`;
- latest landed no-runtime rerank behavior:
  it selected the Gate S / Gate I / Gate AR calculated frequency-curve
  backbone as the next bounded runtime/accuracy owner for lab `Rw`,
  `STC`, `C`, and `Ctr`, ahead of broader building/flanking,
  source-row, impact, and UI/process work;
- latest landed no-runtime rerank counters:
  `candidateCount: 8`, `roiAnalysisIterations: 4`,
...
- latest landed input-boundary coverage refresh:
  `post_v1_wall_double_leaf_framed_leaf_surface_mass_numeric_sensitivity_coverage_refresh_plan`;
- latest landed input-boundary coverage refresh file:
  `packages/engine/src/post-v1-wall-double-leaf-framed-leaf-surface-mass-numeric-sensitivity-coverage-refresh-contract.test.ts`;
- latest landed input-boundary coverage refresh plan:
...
- latest landed input-boundary coverage refresh status:
  `post_v1_wall_double_leaf_framed_leaf_surface_mass_numeric_sensitivity_coverage_refresh_landed_input_boundary_selected_runtime_first_route_family_rerank`;
- latest landed input-boundary coverage refresh follows:
  `post_v1_wall_double_leaf_framed_leaf_surface_mass_numeric_sensitivity_owner_landed_runtime_selected_coverage_refresh`;
- latest landed input-boundary coverage refresh selected candidate:
  `wall.double_leaf_framed.leaf_surface_mass_numeric_sensitivity_owner`;
- latest landed input-boundary coverage refresh behavior:
  it re-probes light, baseline, and heavy symmetric side-leaf mass pins,
  asymmetric side-leaf mass ratio, field/building companions, missing
  side-leaf grouping, unresolved side-leaf surface mass as
  `needs_input`, and `IIC` / `AIIC` as unsupported impact aliases;
- latest landed input-boundary coverage refresh counters:
  `coverageRefreshContractFilesTouched: 1`,
  ...
  `runtimeValuesMoved 0`, `runtimeFormulaRetunes: 0`,
  `sourceRowsImported: 0`, and `frontendImplementationFilesTouched: 0`;
- latest landed input-boundary coverage refresh historical selected next action:
  `post_v1_runtime_first_route_family_rerank_after_wall_double_leaf_framed_leaf_surface_mass_numeric_sensitivity_coverage_refresh_plan`;
- latest landed input-boundary coverage refresh historical selected next file:
  `packages/engine/src/post-v1-runtime-first-route-family-rerank-after-wall-double-leaf-framed-leaf-surface-mass-numeric-sensitivity-coverage-refresh-contract.test.ts`;
- latest landed input-boundary coverage refresh historical selected next plan:
  `docs/calculator/POST_V1_RUNTIME_FIRST_ROUTE_FAMILY_RERANK_AFTER_WALL_DOUBLE_LEAF_FRAMED_LEAF_SURFACE_MASS_NUMERIC_SENSITIVITY_COVERAGE_REFRESH_PLAN_2026-06-22.md`;
- latest landed input-boundary coverage refresh historical selected next label:
...
- latest landed runtime owner behavior:
  it protects side-leaf surface mass and mass-ratio sensitivity in the
  owned Gate S / Gate I / Gate AR double-leaf/framed route; light,
  baseline, and heavy gypsum leaf variants return lab `Rw/STC 43`,
  `46`, and `49`; asymmetric `12.5 + 25 mm` leaves keep `leafMassRatio
  2` active; field/building companions move from the same direct curve;
  missing side-leaf grouping stays `needs_input`; impact aliases stay
  `unsupported`;
...
- latest landed no-runtime rerank selected candidate:
  `wall.double_leaf_framed.leaf_surface_mass_numeric_sensitivity_owner`;
- latest landed no-runtime rerank behavior:
  it selected side-leaf surface mass as the next bounded
  accuracy/sensitivity owner because the owned Gate S / Gate I / Gate AR
  double-leaf/framed route already uses `sideALeafMassKgM2`,
  `sideBLeafMassKgM2`, total leaf mass, leaf mass ratio, and
  mass-air-mass resonance; broad source crawling, UI polish, and process
...
- latest landed runtime owner behavior:
  topology `cavity1DepthMm` remains numerically active for `60`, `90`,
  and `140 mm`, and advanced-only
  `advancedWall.cavities[0].depthMm` now enters the owned Gate S /
  Gate I / Gate AR double-leaf/framed route for lab `Rw`/`STC`/`C`/
  `Ctr` and field/building `R'w`, `Dn,w`, `Dn,A`, `DnT,w`, and
  `DnT,A`; missing or non-positive cavity depth stays `needs_input`;
  when topology and advanced depth are both supplied, topology depth is
  authoritative and mismatch withholds porous-thickness precision credit;
  impact aliases stay `unsupported`;
- latest landed runtime owner counters:
...
- latest landed no-runtime coverage refresh behavior:
  re-probes topology `cavity1DepthMm = 60`, `90`, and `140 mm`,
  advanced-only `advancedWall.cavities[0].depthMm` lab/field/building
  outputs, missing/zero/negative cavity depth as `needs_input`,
