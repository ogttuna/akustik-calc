...
It corrects finished open-box impact field companions: dry
package-transfer field-only requests now calculate `L'n,w 52.8 /
L'nT,w 50.4 / L'nT,50 53.7`, and EPS/screed hybrid requests with
explicit `impactFieldContext` now calculate `L'n,w 49 /
L'nT,w 46.6 / L'nT,50 47.6`. Missing `impactFieldContext`, airborne
building outputs, and ASTM `IIC` / `AIIC` remain separate owners.
...
Gate BU is calculator scope/correctness work: complete finished open-box
package `building_prediction` `R'w` / `DnT,w` requests must use the
owned package `Rw` anchors instead of a generic predictor or
`screening_mass_law_curve_seed_v3`. This is calculator coverage work,
not catalog/source crawling.
...
- Current product state:
  ...
  resolver surface has 40 declared candidates and 37 active runtime-basis mappings across exact measured overrides, compatible
