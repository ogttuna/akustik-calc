...
function calculateLowDensityFloor(input: {
  ...
  return calculateAssembly(CUSTOM_LOW_DENSITY_STACK, {
    ...
    floorImpactContext: input.floorImpactContext ?? FLOOR_IMPACT_CONTEXT,
...
describe("post-V1 floor user-material low-density exact ASTM direct-flanking companion coverage refresh", () => {
  ...
  it("re-probes exact ASTM IIC with direct+flanking field impact companions", () => {
  ...
  it("re-probes exact ASTM direct+flanking companions in complete building context", () => {
    const result = calculateLowDensityFloor({
  ...
  it("keeps lab-airborne building aliases, simple-K fallback, generic ASTM, and missing inputs bounded", () => {
    ...
    expect(genericAstm.unsupportedTargetOutputs).toEqual(["IIC"]);
