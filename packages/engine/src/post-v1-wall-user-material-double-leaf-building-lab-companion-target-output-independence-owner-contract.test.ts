...
describe("post-V1 wall user-material double-leaf building lab-companion target-output independence owner", () => {
  ...
  it("keeps single-output and mixed-output user-material building lab companions on the same direct curve", () => {
    ...
    for (const output of MIXED_OUTPUTS) {
      ...
      expect(single.airborneBasis).toMatchObject({
        method: GATE_AR_BUILDING_METHOD,
        missingPhysicalInputs: [],
        origin: "family_physics_prediction"
      });
  ...
  it("does not widen supported outputs while normalizing visible companion metrics", () => {
    ...
    expect(fieldOnly.metrics.estimatedCtrDb).toBe(EXPECTED_LAB_COMPANIONS.Ctr);
  ...
  it("keeps missing leaf mass and impact aliases outside the runtime accuracy owner", () => {
  ...
  it("keeps docs and current-gate runner aligned with the landed owner", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(content, path).toContain("accuracyPromotedRequestShapes: 10");
