...
function ownerSummary() {
  return {
    ...
    selectedCandidateId: POST_V1_CEILING_ROOF_SUSPENDED_CEILING_ROUTE_SPLIT_BOUNDARY_SELECTED_CANDIDATE_ID,
...
describe("post-V1 ceiling roof/suspended-ceiling route split boundary owner", () => {
  ...
  it("stops ambiguous roof/ceiling/suspended-ceiling stacks with precise route-context needs_input", () => {
  ...
  it("keeps explicitly routed ceiling airborne field/building values pinned", () => {
    ...
    expect(field.airborneBasis?.requiredInputs).toEqual(
      expect.arrayContaining([...POST_V1_CEILING_ROOF_SUSPENDED_CEILING_ROUTE_SPLIT_REQUIRED_INPUTS])
    );
  ...
  it("blocks explicit wrong-family roof, impact, and airborne aliases without borrowing values", () => {
  ...
  it("keeps docs and current-gate runner aligned with the landed owner", () => {
    ...
    expect(runner).toContain("post-v1-runtime-first-route-family-rerank-after-ceiling-multileaf-airborne-plenum-field-building-adapter-owner-coverage-refresh-contract.test.ts");
