...
const POST_MIXED_FLOOR_WALL_REQUESTED_OUTPUT_CARD_HISTORY_RESTORE_GRID_NEXT_SLICE_SELECTION = {
  sliceId: "post_mixed_floor_wall_requested_output_card_history_restore_grid_next_slice_selection_v1",
  selectedImplementationSlice: "mixed_floor_wall_requested_output_card_edit_history_restore_grid_expansion_v1",
  ...
  nextExecutionAction: "mixed_floor_wall_requested_output_card_edit_history_restore_grid_expansion_v1",
  followUpPlanningAction: "post_mixed_floor_wall_requested_output_card_edit_history_restore_grid_next_slice_selection_v1",
} as const;

const BROAD_REQUESTED_OUTPUT_EDIT_HISTORY_RESTORE_SCOPE = {
  ...
  requestedOutputStressors: [
    ...
    "broad_custom_requested_output_edit_history_variant_restore",
    "broad_custom_requested_output_edit_history_opposite_mode_detour_restore",
    "broad_custom_requested_output_edit_history_save_load_restore",
    ...
    "broad_requested_output_replay_restore_regression_guard",
    "selected_requested_output_generated_restore_regression_guard",
    "selected_requested_output_edit_history_restore_regression_guard",
    "selected_requested_output_partial_restore_regression_guard",
    "representative_requested_output_generated_restore_regression_guard",
...
const DEFERRED_IMMEDIATE_RUNTIME_CANDIDATES = [
  ...
  },
  {
    ...
    reason: "requested_output_edit_history_restore_breadth_should_land_before_formula_scope_expansion",
    runtimeWideningEligible: false,
  },
  {
    id: "wall_selector_behavior_widening",
    reason: "requested_output_edit_history_restore_breadth_should_land_before_wall_selector_scope_expansion",
    runtimeWideningEligible: false,
  },
] as const;

describe("post mixed floor/wall requested-output card history restore grid next slice selection contract", () => {
  it("selects broad requested-output edit-history restore expansion before any runtime widening", () => {
    expect(POST_MIXED_FLOOR_WALL_REQUESTED_OUTPUT_CARD_HISTORY_RESTORE_GRID_NEXT_SLICE_SELECTION).toEqual({
      sliceId: "post_mixed_floor_wall_requested_output_card_history_restore_grid_next_slice_selection_v1",
      selectedImplementationSlice: "mixed_floor_wall_requested_output_card_edit_history_restore_grid_expansion_v1",
      selectedOutputSurface: "workbench_requested_output_card_edit_history_restore_breadth",
      selectedRouteFamily: "mixed_floor_wall_route_matrix",
  ...
  it("keeps the next scope on broad visible requested-output edit-history restore breadth while preserving replay and representative restore regressions", () => {
    expect(BROAD_REQUESTED_OUTPUT_EDIT_HISTORY_RESTORE_SCOPE).toEqual({
      currentBroadRequestedOutputRestoreGridGreen: true,
      currentBroadRequestedOutputReplayGridGreen: true,
      currentSelectedRequestedOutputGeneratedRestoreGridGreen: true,
      currentSelectedRequestedOutputEditHistoryRestoreGridGreen: true,
