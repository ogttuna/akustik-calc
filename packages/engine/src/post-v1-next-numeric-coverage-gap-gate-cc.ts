...
export const POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_LANDED_GATE =
  "post_v1_next_numeric_coverage_gap_gate_cc_plan" as const;

export const POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTION_STATUS =
  "post_v1_next_numeric_coverage_gap_gate_cc_landed_no_runtime_selected_floor_open_box_target_output_independence_gate_cd" as const;

export const POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_ACTION =
  "post_v1_floor_open_box_target_output_independence_gate_cd_plan" as const;

export const POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_FILE =
  "packages/engine/src/post-v1-floor-open-box-target-output-independence-gate-cd-contract.test.ts" as const;

export const POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_LABEL =
  "post-V1 floor open-box target-output independence Gate CD" as const;

const PREVIOUS_GATE_CB_LANDED_GATE =
  "post_v1_floor_open_box_eps_screed_full_mixed_field_building_gate_cb_plan" as const;
const PREVIOUS_GATE_CB_SELECTION_STATUS =
  "post_v1_floor_open_box_eps_screed_full_mixed_field_building_gate_cb_landed_selected_next_numeric_coverage_gap_gate_cc" as const;

export type PostV1GateCCCandidateId =
...
export type PostV1GateCCCandidate = {
  ...
  readonly sliceKind: PostV1GateCCSliceKind;
  readonly sourceRowsRequiredForSelection: boolean;
  readonly targetMetrics: readonly RequestedOutputId[];
  readonly wrongNumberRisk: number;
};

export type PostV1GateCCSummary = {
  readonly blockedNonGoalIds: readonly PostV1GateCCCandidateId[];
  readonly candidateCount: number;
  readonly landedGate: typeof POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_LANDED_GATE;
  readonly noRuntimeValueMovement: true;
  readonly previousGateCB: {
    readonly landedGate: typeof PREVIOUS_GATE_CB_LANDED_GATE;
    readonly selectionStatus: typeof PREVIOUS_GATE_CB_SELECTION_STATUS;
  };
  readonly selectedCandidateId: PostV1GateCCCandidateId;
  readonly selectedNextAction: typeof POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_ACTION;
  readonly selectedNextFile: typeof POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_FILE;
  readonly selectedNextLabel: typeof POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_LABEL;
  readonly selectionCandidates: readonly PostV1GateCCCandidate[];
  readonly selectionStatus: typeof POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTION_STATUS;
};

const TARGET_INDEPENDENCE_METRICS = [
  "Rw",
  "C",
  "Ln,w",
  "L'n,w",
...
export function rankPostV1GateCCNumericCoverageCandidates(): readonly PostV1GateCCCandidate[] {
  return [
    {
      accuracyImpact: 0.84,
      coverageImpact: 0.94,
      ...
      expectedBeforeAfter: [
        "finished open-box building/impact routes already calculate finite Rw, C, Ln,w, and field impact values internally",
        "before Gate CD the same complete route can mark those values unsupported when the user requests only that single owned metric",
        "Gate CD must make target output support independent of also asking for R'w/DnT,w, so Rw, C, Ln,w, L'n,w, L'nT,w, and L'nT,50 publish when their owned values and required inputs are present",
        "missing impactFieldContext must still keep field impact outputs unsupported",
        "Ctr, IIC, and AIIC remain unsupported because their metric-basis owners are separate"
      ],
      id: "floor.open_box_timber_finished_package.target_output_independence_gap",
      implementationReadiness: 0.94,
      reason:
        "This is the highest-ROI calculator step because it removes an order/target-output gating bug from already-owned open-box package routes, increasing real calculable output coverage without new source rows or unsafe aliases.",
      score: 2.72,
      selected: true,
      selectedNextActionIfSelected: POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_ACTION,
      selectedNextFileIfSelected: POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_CC_SELECTED_NEXT_FILE,
      sliceKind: "scope_accuracy_bridge",
      sourceRowsRequiredForSelection: false,
      targetMetrics: TARGET_INDEPENDENCE_METRICS,
    ...
    },
    {
      ...
      reason:
        "Useful accuracy work, but lower immediate ROI than opening already-computed outputs that are currently hidden by target-output gating.",
      score: 1.43,
      selected: false,
...
export function summarizePostV1GateCCNumericCoverageGap(): PostV1GateCCSummary {
  const selectionCandidates = rankPostV1GateCCNumericCoverageCandidates();
  const selected = selectionCandidates.find((candidate) => candidate.selected);
  if (!selected) {
    throw new Error("Gate CC requires exactly one selected calculator coverage/correctness candidate.");
  }
  ...
  return {
    ...
    previousGateCB: {
      landedGate: PREVIOUS_GATE_CB_LANDED_GATE,
      selectionStatus: PREVIOUS_GATE_CB_SELECTION_STATUS
    },
