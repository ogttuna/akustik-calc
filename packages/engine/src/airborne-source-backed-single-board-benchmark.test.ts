...
const FAMILIES = [
  ["acoustic_gypsum_board", "acoustic_gypsum_board", 15, 15],
  ["diamond_board", "diamond_board", 18, 18],
  ["silentboard", "silentboard", 12.5, 12.5],
  ["fire_board", "fire_board", 15, 15],
  ["firestop_board", "firestop_board", 15, 15],
  ["fire_board", "firestop_board", 15, 15]
] as const;
...
describe.skipIf(!core)("airborne source-backed single-board benchmark", () => {
  it(
    "keeps symmetric enhanced and fire-rated single-board framed walls inside the upstream lab and field corridor",
    () => {
      ...
      for (const [leftId, rightId, leftThicknessMm, rightThicknessMm] of FAMILIES) {
        for (const fillThicknessMm of FILLS) {
          ...
          for (const contextKey of Object.keys(CONTEXTS) as ContextKey[]) {
            ...
            const upstream = core.calculateAssembly(layers, {
              calculator: "dynamic",
              meta: context
            });
