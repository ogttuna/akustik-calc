...
const FIELD_DIRECT_FLANKING_OUTPUTS = [
  "Rw",
  "R'w",
  "Dn,w",
  "Dn,A",
...
const BUILDING_DIRECT_FLANKING_OUTPUTS = [
  "R'w",
  "Dn,w",
  "Dn,A",
  "DnT,w",
...
const BUILDING_WITH_LAB_AIRBORNE_OUTPUTS = [
  "Rw",
  "STC",
  "C",
  "Ctr",
  ...BUILDING_DIRECT_FLANKING_OUTPUTS
] as const satisfies readonly RequestedOutputId[];
...
const DIRECT_FLANKING_IMPACT_CONTEXT = {
  ...
  flankingPaths: [
    {
      id: "low_density_direct_flanking_edge_path",
...
describe("post-V1 floor user-material low-density exact ASTM direct-flanking companion owner", () => {
  it("lands the runtime owner and selects a bounded coverage refresh", () => {
  ...
  it("calculates exact ASTM IIC with direct+flanking field impact companions without losing ASTM basis", () => {
    const result = calculateLowDensityFloor({
      airborneContext: AIRBORNE_FIELD_CONTEXT,
      impactFieldContext: DIRECT_FLANKING_IMPACT_CONTEXT,
      targetOutputs: FIELD_DIRECT_FLANKING_OUTPUTS
    });

    expect(result.supportedTargetOutputs).toEqual([...FIELD_DIRECT_FLANKING_OUTPUTS]);
  ...
  it("calculates the same exact ASTM direct+flanking companion in building context when room and junction inputs are complete", () => {
    const result = calculateLowDensityFloor({
      airborneContext: AIRBORNE_BUILDING_CONTEXT,
      impactFieldContext: DIRECT_FLANKING_IMPACT_CONTEXT,
      targetOutputs: BUILDING_DIRECT_FLANKING_OUTPUTS
    });

    expect(result.supportedTargetOutputs).toEqual([...BUILDING_DIRECT_FLANKING_OUTPUTS]);
    expect(result.unsupportedTargetOutputs).toEqual([]);
  ...
  it("keeps later building lab-airborne Rw/C ownership and generic ASTM aliases bounded", () => {
    const buildingWithLabAirborne = calculateLowDensityFloor({
      airborneContext: AIRBORNE_BUILDING_CONTEXT,
      impactFieldContext: DIRECT_FLANKING_IMPACT_CONTEXT,
      targetOutputs: BUILDING_WITH_LAB_AIRBORNE_OUTPUTS
    });
    const simpleKField = calculateLowDensityFloor({
      airborneContext: AIRBORNE_FIELD_CONTEXT,
      impactFieldContext: SIMPLE_K_IMPACT_CONTEXT,
      targetOutputs: FIELD_DIRECT_FLANKING_OUTPUTS
    });
    const missingFieldInputs = calculateLowDensityFloor({
      airborneContext: AIRBORNE_BUILDING_CONTEXT,
      targetOutputs: BUILDING_DIRECT_FLANKING_OUTPUTS
    });

    expect(buildingWithLabAirborne.supportedTargetOutputs).toEqual([
      "Rw",
      "C",
      ...BUILDING_DIRECT_FLANKING_OUTPUTS
    ]);
    expect(buildingWithLabAirborne.unsupportedTargetOutputs).toEqual(["STC", "Ctr"]);
    expect(buildingWithLabAirborne.metrics).toMatchObject({
      estimatedCDb: -5.5,
      estimatedRwDb: 53
    });
    expect(buildingWithLabAirborne.impact?.basis).toBe(ASTM_E989_IMPACT_RATING_BASIS);

    expect(simpleKField.supportedTargetOutputs).toEqual([...FIELD_DIRECT_FLANKING_OUTPUTS]);
    expect(simpleKField.impact).toMatchObject({
      IIC: 50,
      LPrimeNT50: 66.9,
      LPrimeNTw: 63.9,
      LPrimeNW: 66.3,
    ...
    expect(missingFieldInputs.supportedTargetOutputs).toEqual([
      "R'w",
      "Dn,w",
      "Dn,A",
      "DnT,w",
    ...
    expect(missingFieldInputs.unsupportedTargetOutputs).toEqual(["L'n,w", "L'nT,w", "L'nT,50"]);
  });
  ...
  it("keeps docs and current-gate runner aligned with the runtime owner", () => {
    ...
    for (const path of requiredDocs) {
      ...
      expect(content, path).toContain("newCalculableTargetOutputs: 17");
      expect(content, path).toContain("runtimeBasisPromotions: 2");
      expect(content, path).toContain("runtimeValuesMoved 17");
