...
const OWNER_COUNTERS = {
  ...
  newCalculableTargetOutputs: 7,
  runtimeBasisPromotions: 4,
  runtimeFormulaRetunes: 0,
  runtimeValuesMoved: 7,
...
function summarizeOwnerCloseout() {
  return {
    ...
    previousGapAction: PREVIOUS_GAP_ACTION,
    previousGapFile: PREVIOUS_GAP_FILE,
    previousGapStatus: PREVIOUS_GAP_STATUS,
...
describe("post-V1 floor user-material low-density exact ASTM lab-airborne impact target-output independence owner", () => {
  ...
  it("publishes Rw and C from the same-stack lightweight companion when requested without DeltaLw", () => {
    ...
    const rwAndC = calculateLowDensityFloor({
      exactImpactSource: ASTM_FIELD_AIIC_SOURCE,
      targetOutputs: ["Rw", "C"]
    });
    ...
    expect(rwAndC.supportedTargetOutputs).toEqual(["Rw", "C"]);
    expect(rwAndC.unsupportedTargetOutputs).toEqual([]);
    expect(rwAndC.metrics).toMatchObject({
      estimatedCDb: -5.5,
      estimatedRwDb: 53
    });
  });

  it("publishes the ISO impact companion when Ln,w is requested without DeltaLw", () => {
  ...
  it("keeps mixed building requests target-output independent across lab airborne and field impact companions", () => {
    const result = calculateLowDensityFloor({
  ...
  it("keeps STC/Ctr, missing physical inputs, and generic ASTM aliases outside this target-output owner", () => {
  ...
  it("keeps docs and current-gate runner aligned with the runtime closeout", () => {
    const requiredDocs = [
      "AGENTS.md",
      "README.md",
    ...
    for (const path of requiredDocs) {
      ...
      expect(content, path).toContain(PREVIOUS_GAP_ACTION);
      expect(content, path).toContain(PREVIOUS_GAP_FILE);
      expect(content, path).toContain(PREVIOUS_GAP_STATUS);
      ...
      expect(content, path).toContain("newCalculableTargetOutputs: 7");
      expect(content, path).toContain("runtimeValuesMoved 7");
    ...
    expect(runner).toContain(PREVIOUS_GAP_FILE.replace("packages/engine/", ""));
