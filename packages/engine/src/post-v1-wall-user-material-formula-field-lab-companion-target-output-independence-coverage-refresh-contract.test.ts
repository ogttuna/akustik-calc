...
describe("post-V1 wall user-material formula field lab-companion target-output independence coverage refresh", () => {
  ...
  it("re-probes full lab-only field requests on direct lab formula values", () => {
    ...
    expect(result.layerCombinationResolverTrace?.supportedMetrics).toEqual(
      expect.arrayContaining([...FIELD_OUTPUTS])
    );
  ...
  it("re-probes mixed field values and preserves field-only, lab-context-only, built-in, ASTM, and impact boundaries", () => {
