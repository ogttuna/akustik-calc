...
describe("post-V1 wall double-leaf/framed cavity-depth numeric sensitivity coverage refresh", () => {
  it("lands the no-runtime refresh and selects the runtime-first rerank next", () => {
  ...
  it("re-probes topology cavity-depth-sensitive lab values without formula retuning", () => {
  ...
  it("re-probes advanced-only cavity depth across lab, field, and building outputs", () => {
  ...
  it("keeps missing, zero, and negative cavity depth as precise needs_input boundaries", () => {
    ...
    const zero = buildGateRDoubleLeafFramedBridgeSolverContract({
      airborneContext: contextOwnedPorousCavity({
        advancedDepthMm: 0,
        depthMm: 90,
        topologyDepthMm: null
    ...
    const negative = buildGateRDoubleLeafFramedBridgeSolverContract({
      airborneContext: contextOwnedPorousCavity({
        advancedDepthMm: -25,
        depthMm: 90,
        topologyDepthMm: null
    ...
    for (const contract of [missing, zero, negative]) {
  ...
  it("keeps topology cavity depth authoritative when both depth sources differ", () => {
  ...
  it("keeps docs and current-gate aligned with the refresh closeout and rerank handoff", () => {
    ...
    expect(currentGateRunner).toContain(
      "src/post-v1-wall-double-leaf-framed-cavity-depth-numeric-sensitivity-coverage-refresh-contract.test.ts"
    );
  });
});
