...
describe("post-V1 wall advanced-wall source-absent field/building lab-companion basis integrity coverage refresh", () => {
  it("lands a no-runtime refresh over the lab-companion basis-integrity owner", () => {
  ...
  it("re-probes field and building mixed lab companions without moving runtime", () => {
  ...
  it("records target-output independence closure while missing context and Gate AY field/building publishing stay outside", () => {
  ...
  it("keeps active docs aligned with the coverage refresh and next rerank handoff", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(contents, path).toContain("newCalculableRequestShapes: 0");
      expect(contents, path).toContain("newCalculableTargetOutputs: 0");
