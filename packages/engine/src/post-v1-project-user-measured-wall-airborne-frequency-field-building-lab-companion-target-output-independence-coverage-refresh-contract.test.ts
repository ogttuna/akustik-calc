...
function buildActiveFrequencyAnchor(input: {
  ...
  return {
    ...
    createdAtIso: "2026-06-18T21:00:00.000Z",
    createdBy: "project-user-measured-wall-airborne-frequency-target-output-coverage-refresh",
    createdFromProjectId: "project-user-measured-wall-airborne-frequency-target-output-coverage-refresh",
    ...
    sourceStatus: "active",
    toleranceDb: 1,
    updatedAtIso: "2026-06-18T21:00:00.000Z"
  };
}

function buildExactAddedBoardAnchor() {
  return buildActiveFrequencyAnchor({
    id: "project-user-measured-wall-airborne-frequency-target-output-coverage-exact",
...
function buildCompatibleBaseAnchor() {
  return buildActiveFrequencyAnchor({
    id: "project-user-measured-wall-airborne-frequency-target-output-coverage-compatible",
    snapshot: buildRuntimeSnapshot({
...
describe("post-V1 project/user measured wall airborne frequency field/building lab-companion target-output independence coverage refresh", () => {
  ...
  it("re-probes exact and compatible lab-only field/building companions without moving runtime", () => {
    for (const anchor of [buildExactAddedBoardAnchor(), buildCompatibleBaseAnchor()]) {
      ...
      expect(lab.airborneBasis?.method).toSatisfy((method: string | undefined) =>
  ...
  it("keeps mixed field/building values and lab companions stable", () => {
    ...
    expect(exactField.metrics.estimatedRwDb).toBe(exactLab.metrics.estimatedRwDb);
    ...
    expect(exactBuilding.metrics.estimatedRwDb).toBe(exactLab.metrics.estimatedRwDb);
    ...
    expect(compatibleField.metrics.estimatedRwDb).toBe(compatibleLab.metrics.estimatedRwDb);
    ...
    expect(compatibleBuilding.metrics.estimatedRwDb).toBe(compatibleLab.metrics.estimatedRwDb);
  });

  it("keeps missing context and impact boundaries protected", () => {
