...
const SELECTED_NEXT_LABEL =
  "post-V1 wall user-material formula field lab-companion target-output independence owner";
...
const CANDIDATES = [
  {
    ...
    reason:
      "Complete custom double-leaf/framed user-material wall field requests already own the direct lab formula curve and Gate I field adapter, but lab-only target outputs still inherit field-adapter Rw/STC/Ctr values unless a field output is also requested.",
  ...
  },
  {
    ...
    id: "wall.user_material_formula_field_lab_companion_basis_integrity_owner_closed",
    reason:
      "The previous owner already corrects mixed field requests where lab outputs and field outputs are requested together.",
    routeFamily: "wall.double_leaf_framed.user_material.field_between_rooms",
    selected: false,
    sourceRowsRequiredForRuntimeSelection: false
  },
  {
    ...
    reason:
      "The building-prediction user-material companion owner is already closed and protected by its coverage refresh.",
    routeFamily: "wall.double_leaf_framed.user_material.building_prediction",
    selected: false,
    sourceRowsRequiredForRuntimeSelection: false
  },
  {
    blockedBy: ["new same-family wall source rows still required"],
    estimatedAccuracyPromotedTargetOutputs: 0,
    estimatedRuntimeValuesMoved: 0,
  ...
  {
    blockedBy: ["screening or bounded-rule evidence remains weaker than the selected owned-route bug"],
    estimatedAccuracyPromotedTargetOutputs: 0,
    estimatedRuntimeValuesMoved: 0,
...
describe("post-V1 runtime-first route-family rerank after wall user-material formula field lab-companion basis integrity", () => {
  ...
  it("ranks an owned route-family accuracy fix above source crawling and stale formula lanes", () => {
  ...
  it("keeps the selected owner bounded to complete field context plus owned direct lab formula", () => {
    ...
    expect(selected?.reason).toContain("lab-only target outputs");
    expect(selected?.reason).toContain("field output is also requested");
