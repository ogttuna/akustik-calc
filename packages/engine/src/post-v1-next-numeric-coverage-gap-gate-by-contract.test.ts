...
import {
  POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BY_LANDED_GATE,
  ...
  POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BY_SELECTED_NEXT_LABEL,
  POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BY_SELECTION_STATUS,
  rankPostV1GateBYNumericCoverageCandidates,
  summarizePostV1GateBYNumericCoverageGap
...
describe("post-V1 next numeric coverage gap Gate BY", () => {
  it("lands a no-runtime rerank after Gate BX and selects full mixed building/impact Gate BZ", () => {
    const summary = summarizePostV1GateBYNumericCoverageGap();
    ...
    expect(summary).toMatchObject({
      ...
      selectedCandidateId: "floor.open_box_timber_finished_package.full_mixed_building_impact_gap",
  ...
  it("prioritizes already-computed mixed building/impact output coverage over residual, ASTM alias, and non-goals", () => {
    const candidates = rankPostV1GateBYNumericCoverageCandidates();
    const selected = candidates.find((candidate) => candidate.selected);
    const byId = new Map(candidates.map((candidate) => [candidate.id, candidate]));

    expect(selected?.id).toBe("floor.open_box_timber_finished_package.full_mixed_building_impact_gap");
  ...
  it("keeps the selected Gate BZ lane executable for complete full mixed building/impact requests", () => {
    ...
    expect(dry.supportedTargetOutputs).toEqual([
      "Rw",
      "C",
    ...
    expect(dry.unsupportedTargetOutputs).toEqual(["Ctr", "IIC", "AIIC"]);
    ...
    expect(eps.supportedTargetOutputs).toEqual([
      "Rw",
      "C",
      ...BUILDING_AIRBORNE_OUTPUTS,
      "Ln,w",
    ...
    expect(eps.unsupportedTargetOutputs).toEqual(["Ctr", "IIC", "AIIC"]);
  ...
  it("keeps docs and current-gate runner aligned with Gate BY selection", () => {
    for (const path of CURRENT_SELECTION_DOCS) {
      const contents = readRepoFile(path);

      expect(contents, path).toContain(POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BY_LANDED_GATE);
      expect(contents, path).toContain(POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BY_SELECTION_STATUS);
      expect(contents, path).toContain("floor.open_box_timber_finished_package.full_mixed_building_impact_gap");
    ...
    expect(runner).toContain("src/post-v1-next-numeric-coverage-gap-gate-by-contract.test.ts");
  });
});
