...
import {
  ...
  POST_V1_OPENING_FACADE_DOOR_WINDOW_INDOOR_FIELD_BUILDING_ADAPTER_SELECTED_NEXT_ACTION,
  POST_V1_OPENING_FACADE_DOOR_WINDOW_INDOOR_FIELD_BUILDING_ADAPTER_SELECTED_NEXT_FILE
...
const OPENING = {
  ...
  id: "rated-door-indoor-field-building-01",
  origin: "catalogued",
  ...
  openingLeakElements: [
    {
      ...OPENING,
      elementType: "window",
      id: "facade-window-indoor-field-building-01"
    }
  ]
} as const satisfies AirborneContext;
...
describe("post-V1 opening/facade door/window indoor field/building adapter owner", () => {
  ...
  it("promotes complete indoor partition field and building outputs through the owned opening/leak runtime", () => {
  ...
  it("keeps active docs and current gate synchronized with the owner handoff", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(content, path).toContain("runtimeValuesMoved 22");
