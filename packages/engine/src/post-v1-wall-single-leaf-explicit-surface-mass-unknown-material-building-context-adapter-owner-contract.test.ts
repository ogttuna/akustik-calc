...
function expectOwnedBuildingAdapter(
  ...
  expect(result.airborneBasis?.assumptions).toEqual(
    expect.arrayContaining([
      ...
      "direct separating-element curve comes from the owned single-leaf mass-law / banded calculated lab curve",
      ...
      "base direct-curve method remains layer_combination_resolver_single_leaf_mass_law_banded_source_absent_formula_corridor",
...
describe("post-V1 wall single-leaf explicit surface-mass unknown-material building-context adapter owner", () => {
  it("lands the runtime owner after the no-runtime field-context adapter coverage refresh", () => {
  ...
  it("opens complete building outputs for project single-leaf and laminated project panels through Gate AR", () => {
  ...
  it("keeps missing mass, missing building inputs, and cavity stacks outside the building owner", () => {
  ...
  it("keeps field context and impact aliases outside this building-context owner", () => {
