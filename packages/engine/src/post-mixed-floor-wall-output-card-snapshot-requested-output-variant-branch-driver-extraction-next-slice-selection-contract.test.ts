...
const POST_MIXED_FLOOR_WALL_OUTPUT_CARD_SNAPSHOT_REQUESTED_OUTPUT_VARIANT_BRANCH_DRIVER_EXTRACTION_NEXT_SLICE_SELECTION = {
  sliceId:
  ...
  selectedOutputSurface: "workbench_output_card_snapshot_requested_output_surface_runner_loop_harness",
...
const OUTPUT_CARD_SNAPSHOT_REQUESTED_OUTPUT_SURFACE_RUNNER_LOOP_SCOPE = {
  currentRequestedOutputVariantBranchDriverExtractionGreen: true,
  ...
  requestedOutputRunnerFileLineCountAtSelection: 979,
  ...
  requestedOutputSurfaceRunnerExtractionTargets: [
...
const DEFERRED_IMMEDIATE_RUNTIME_CANDIDATES = [
  {
    ...
    reason: "requested_output_surface_runner_loop_extraction_should_land_before_new_runtime_scope",
...
describe("post mixed floor/wall output-card snapshot requested-output variant branch driver extraction next slice selection contract", () => {
  it("selects surface runner-loop extraction before any runtime widening", () => {
    expect(
      POST_MIXED_FLOOR_WALL_OUTPUT_CARD_SNAPSHOT_REQUESTED_OUTPUT_VARIANT_BRANCH_DRIVER_EXTRACTION_NEXT_SLICE_SELECTION,
    ).toEqual({
      sliceId:
        "post_mixed_floor_wall_output_card_snapshot_requested_output_variant_branch_driver_extraction_next_slice_selection_v1",
      selectedImplementationSlice:
  ...
  it("keeps the next scope on surface runner-loop extraction while preserving the shared variant driver win", () => {
    expect(OUTPUT_CARD_SNAPSHOT_REQUESTED_OUTPUT_SURFACE_RUNNER_LOOP_SCOPE).toEqual({
      currentRequestedOutputVariantBranchDriverExtractionGreen: true,
      currentBroadRequestedOutputPartialRestoreGridGreen: true,
      currentBroadRequestedOutputEditHistoryRestoreGridGreen: true,
      currentBroadRequestedOutputRestoreGridGreen: true,
