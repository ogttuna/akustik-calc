...
import { REQUESTED_OUTPUT_IDS, type AirborneContext, type LayerInput, type RequestedOutputId } from "@dynecho/shared";
...
const SPECTRAL_OPENING = {
  ...
  id: "spectral-door-field-building-refresh-01",
  origin: "catalogued",
  ...
  openingLeakElements: [
    {
      ...SPECTRAL_OPENING,
      elementType: "window",
      id: "spectral-window-outdoor-facade-refresh-01"
    }
  ]
} as const satisfies AirborneContext;
...
describe("post-V1 opening/facade door/window spectral field/building adapter coverage refresh", () => {
  ...
  it("re-probes curve-only indoor partition field and building outputs without moving values", () => {
  ...
  it("keeps missing building context as needs_input instead of reopening unsupported or alias paths", () => {
  ...
  it("keeps outdoor-indoor facade/OITC and impact aliases outside this refresh", () => {
    ...
    expect(impact.airborneBasis?.origin).not.toBe("family_physics_prediction");
  });

  it("keeps active handoff docs synchronized with the landed refresh and selected runtime-first rerank", () => {
    ...
    expect(readRepoFile(SELECTED_NEXT_PLAN_DOC)).toContain(SELECTED_NEXT_ACTION);
  });
});
