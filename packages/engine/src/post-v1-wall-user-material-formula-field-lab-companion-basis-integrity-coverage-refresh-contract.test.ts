...
describe("post-V1 wall user-material formula field lab-companion basis integrity coverage refresh", () => {
  ...
  it("re-probes the corrected mixed field request with separate lab and field bases", () => {
  ...
  it("re-probes partial mixed field requests without target-output order dependence", () => {
    const result = calculateCustomWall(FIELD_CONTEXT, ["DnT,A", "Ctr", "R'w", "Rw", "C"]);
  ...
  it("keeps field-only, lab-only, built-in, ASTM, and impact boundaries unchanged", () => {
    const fieldOnly = calculateCustomWall(FIELD_CONTEXT, FIELD_OUTPUTS);
