...
const DEFAULT_LAMINATE_FLOATING_STACK = [
...
describe("post-V1 floor user-material visible floating heavy airborne companion coverage refresh", () => {
  ...
  it("re-probes the custom heavy floating-floor lab airborne companion values", () => {
  ...
  it("re-probes complete field impact companions without changing the lab airborne basis", () => {
    const result = calculateCustomFloor({
  ...
  it("keeps low-density and missing-dynamic-stiffness stacks outside the heavy airborne owner", () => {
  ...
  it("keeps default-catalog laminate floating floors on the existing screening airborne companion", () => {
    const result = calculateAssembly(DEFAULT_LAMINATE_FLOATING_STACK, {
  ...
  it("keeps generic ASTM impact ratings outside the ISO heavy floating owner", () => {
    const result = calculateCustomFloor({
      impactFieldContext: IMPACT_FIELD_CONTEXT,
