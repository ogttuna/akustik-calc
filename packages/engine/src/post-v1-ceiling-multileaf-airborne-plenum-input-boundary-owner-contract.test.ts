...
describe("post-V1 ceiling multileaf airborne plenum input-boundary owner", () => {
  ...
  it("stops ceiling plenum lab outputs before the single-leaf source-absent route can publish values", () => {
  ...
  it("keeps incomplete ceiling field requests blocked by the later multileaf/plenum field input surface", () => {
  ...
  it("does not disturb the already owned ceiling board-only single-leaf route", () => {
  ...
  it("keeps impact and OITC outside this ceiling airborne input-boundary owner", () => {
  ...
  it("keeps docs and current-gate runner aligned with the landed owner", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(content, path).toContain("requiredPhysicalInputsCaptured: 5");
    ...
    expect(runner).toContain("post-v1-runtime-first-route-family-rerank-after-ceiling-single-leaf-field-building-context-adapter-coverage-refresh-contract.test.ts");
    expect(runner).toContain("post-v1-ceiling-multileaf-airborne-plenum-input-boundary-owner-contract.test.ts");
  });
});
