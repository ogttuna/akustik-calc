...
const POST_MIXED_FLOOR_WALL_SEEDED_CROSS_MODE_CHAIN_NEXT_SLICE_SELECTION = {
  sliceId: "post_mixed_floor_wall_seeded_cross_mode_chain_next_slice_selection_v1",
  selectedImplementationSlice: "mixed_floor_wall_duplicate_swap_grid_expansion_v1",
  ...
  nextExecutionAction: "mixed_floor_wall_duplicate_swap_grid_expansion_v1",
  followUpPlanningAction: "post_mixed_floor_wall_duplicate_swap_grid_next_slice_selection_v1"
} as const;
...
const SELECTED_SHARED_TORTURE_SCOPE = {
  ...
  historyStressors: [
    "duplicate_then_adjacent_swap_grids",
    "rebuild_order_permutations",
    "opposite_mode_detours",
    "save_load_replay_after_duplicate_swap_pressure",
    "output_card_projection_path_expansion"
  ],
...
const DEFERRED_IMMEDIATE_RUNTIME_CANDIDATES = [
  ...
  },
  {
    ...
    reason: "shared_duplicate_swap_grid_should_land_before_formula_scope_expansion",
    runtimeWideningEligible: false
  },
  {
    id: "wall_selector_behavior_widening",
    reason: "shared_torture_pass_still_needs_broader_duplicate_swap_grid_evidence",
    runtimeWideningEligible: false
  }
] as const;

describe("post mixed floor/wall seeded cross-mode chain next slice selection contract", () => {
  it("selects duplicate-swap grid expansion before any runtime widening", () => {
  ...
  it("keeps the next scope on the selected seeded route ledger and broader duplicate-swap history pressure", () => {
