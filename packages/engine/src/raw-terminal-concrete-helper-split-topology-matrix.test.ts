...
type SplitTopologyCase = {
  expected: SplitTopologySnapshot;
  id: string;
...
const CASES: readonly SplitTopologyCase[] = [
  {
    id: "split full helper package stays inside the defended concrete lane",
    layers: [
  ...
  {
    id: "split board and fill helper over thicker terminal concrete stays defended",
    layers: [
  ...
  {
    id: "split board and cavity helper over thinner terminal concrete stays defended",
    layers: [
      { materialId: "gypsum_board", thicknessMm: 12.5 },
  ...
  {
    id: "top finish after split helper lane keeps field companion rw closed",
    layers: [
      { materialId: "gypsum_board", thicknessMm: 12.5 },
  ...
  {
    id: "wall-like concrete hybrid stays inside the same fail-closed boundary",
    layers: [
      { materialId: "gypsum_board", thicknessMm: 12.5 },
  ...
  {
    id: "steel joist helper-heavy raw carrier stays outside the concrete widening lane",
    layers: [
...
function snapshot(layers: readonly LayerInput[]): SplitTopologySnapshot {
...
describe("raw terminal concrete helper split topology matrix", () => {
  it("pins the second defended widening cut across split helper topologies and nearby negatives", () => {
