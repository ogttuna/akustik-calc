...
describe("post-V1 floor user-material impact lower-treatment depth owner", () => {
  it("lands the runtime owner and selects the lower-treatment coverage refresh", () => {
  ...
  it("calculates lab Ln,w and DeltaLw for a custom tagged lower-treatment support stack", () => {
    const result = calculateCustomFloor({ targetOutputs: LAB_OUTPUTS });

    expect(result.supportedTargetOutputs).toEqual([...LAB_OUTPUTS]);
    expect(result.unsupportedTargetOutputs).toEqual([]);
    expectCombinedLabAnchor(result);
  });

  it("uses the same combined upper/lower formula anchor for field-only requests", () => {
  ...
  it("keeps mixed lab and field requests target-output independent", () => {
    ...
    expect(result.unsupportedTargetOutputs).toEqual([]);
    expectCombinedFieldCompanions(result);
  });

  it("keeps missing field context, dynamic stiffness, and lower support parked as needs_input", () => {
    ...
    expect(missingLowerSupport.warnings).toEqual(
      expect.arrayContaining([
        "Dynamic Calculator reinforced-concrete combined upper/lower impact runtime is waiting for ceilingOrLowerAssembly before promoting Ln,w / DeltaLw from the heavy-concrete combined formula corridor."
      ])
    );
  ...
  it("keeps ASTM IIC and AIIC aliases unsupported on the ISO lower-treatment route", () => {
    const result = calculateCustomFloor({
      impactFieldContext: FIELD_CONTEXT,
  ...
  it("keeps active docs and current-gate runner aligned with the landed owner", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(content, path).toContain("requiredPhysicalInputsCaptured: 2");
