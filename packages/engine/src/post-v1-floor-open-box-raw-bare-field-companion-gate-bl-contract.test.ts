...
const LANDED_GATE = "post_v1_floor_open_box_raw_bare_field_companion_gate_bl_plan";
...
const SELECTED_NEXT_ACTION = "post_v1_next_numeric_coverage_gap_gate_bm_plan";
...
const GATE_BL_CONTRACT = {
  astmAliasesRemainUnsupported: true,
  beforeRuntimeGap: "raw-bare open-box lab Ln,w existed but L'n,w / L'nT,w / L'nT,50 stayed unsupported",
  buildingPredictionPromotion: false,
  landedGate: LANDED_GATE,
  previousGateBLSelectedNextAction: POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BL_SELECTED_NEXT_ACTION,
  previousGateBLSelectedNextFile: POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BL_SELECTED_NEXT_FILE,
  previousGateBLSelectionStatus: POST_V1_NEXT_NUMERIC_COVERAGE_GAP_GATE_BL_SELECTION_STATUS,
...
describe("post-V1 floor open-box raw-bare field companion Gate BL", () => {
  it("opens field impact outputs from the 220 mm raw-bare open-box lab anchor with explicit field context", () => {
    expect(GATE_BL_CONTRACT).toEqual({
      astmAliasesRemainUnsupported: true,
      beforeRuntimeGap: "raw-bare open-box lab Ln,w existed but L'n,w / L'nT,w / L'nT,50 stayed unsupported",
      buildingPredictionPromotion: false,
      landedGate: "post_v1_floor_open_box_raw_bare_field_companion_gate_bl_plan",
      previousGateBLSelectedNextAction: "post_v1_floor_open_box_raw_bare_field_companion_gate_bl_plan",
      previousGateBLSelectedNextFile:
        "packages/engine/src/post-v1-floor-open-box-raw-bare-field-companion-gate-bl-contract.test.ts",
      previousGateBLSelectionStatus:
        "post_v1_next_numeric_coverage_gap_gate_bl_landed_no_runtime_selected_floor_open_box_raw_bare_field_companion_gate_bl",
      runtimeMovementThisGate: true,
      selectedNextAction: "post_v1_next_numeric_coverage_gap_gate_bm_plan",
      selectionStatus:
        "post_v1_floor_open_box_raw_bare_field_companion_gate_bl_landed_selected_next_numeric_coverage_gap_gate_bm"
    });

    const result = calculateAssembly(RAW_OPEN_BOX_220, {
  ...
  it("opens the 370 mm carrier and computes the lab anchor internally for field-only requests", () => {
    const result = calculateAssembly(RAW_OPEN_BOX_370, {
      airborneContext: AIRBORNE_FIELD_CONTEXT,
      calculator: "dynamic",
      impactFieldContext: IMPACT_FIELD_CONTEXT,
  ...
  it("keeps missing field inputs and ASTM aliases outside this runtime while later building-airborne coverage stays live", () => {
    const missingFieldContext = calculateAssembly(RAW_OPEN_BOX_220, {
      airborneContext: AIRBORNE_FIELD_CONTEXT,
      calculator: "dynamic",
      targetOutputs: FIELD_ONLY_NO_ALIAS_OUTPUTS
    });
    const building = calculateAssembly(RAW_OPEN_BOX_220, {
  ...
  it("preserves Gate BK open-web runtime pins while opening the open-box companion lane", () => {
  ...
  it("keeps docs and current-gate runner aligned with Gate BL runtime closeout", () => {
    for (const path of CURRENT_SELECTION_DOCS) {
      ...
      expect(contents, path).toContain("building prediction");
