...
const COMMON_BASELINES: readonly BaselineCase[] = [
  ...
  },
  {
    ...
    source: INSUL_SEMINAR_URL,
    layers: [{ floorRole: "base_structure", materialId: "concrete", thicknessMm: 200 }],
    expected: {
      basis: "predictor_heavy_bare_floor_iso12354_annexc_estimate",
      lnw: { min: 69, max: 72 },
      rw: { min: 59, max: 59 },
...
function measureLayers(layers: readonly LayerInput[]) {
  return calculateAssembly([...layers], {
    targetOutputs: ["Ln,w", "DeltaLw", "Rw"]
  });
}

describe("impact common floor combinations", () => {
  ...
  it.each(COMMON_BASELINES)("keeps baseline $id inside its sane corridor", (entry) => {
    const result = measureLayers(entry.layers);
    const lnw = numberOrNull(result.impact?.LnW);
    const rw = numberOrNull(result.floorSystemRatings?.Rw);

    expect(result.ok).toBe(true);
    expect(result.supportedTargetOutputs).toEqual(entry.expected.supportedTargetOutputs);
    expect(result.impact?.basis).toBe(entry.expected.basis);
    expect(lnw).not.toBeNull();
    expect(lnw!).toBeGreaterThanOrEqual(entry.expected.lnw.min);
    expect(lnw!).toBeLessThanOrEqual(entry.expected.lnw.max);
    expect(rw).not.toBeNull();
    expect(rw!).toBeGreaterThanOrEqual(entry.expected.rw.min);
    expect(rw!).toBeLessThanOrEqual(entry.expected.rw.max);
    expect(numberOrNull(result.impact?.DeltaLw ?? result.lowerBoundImpact?.DeltaLwLowerBound)).toBeNull();

    if (entry.expected.floorSystemMatchId) {
      expect(result.floorSystemMatch?.system.id).toBe(entry.expected.floorSystemMatchId);
    }
  });

  it.each(COMMON_FLOOR_CASES)("keeps common floor stack $id on its expected lane and corridor", (entry) => {
    const result = measureLayers(entry.layers);
    const lnw = numberOrNull(result.impact?.LnW);
    const deltaLw = numberOrNull(result.impact?.DeltaLw ?? result.lowerBoundImpact?.DeltaLwLowerBound);
    const rw = numberOrNull(result.floorSystemRatings?.Rw);

    expect(result.ok).toBe(true);
    expect(result.supportedTargetOutputs).toEqual(entry.expected.supportedTargetOutputs);
    expect(result.impact?.basis ?? result.lowerBoundImpact?.basis).toBe(entry.expected.basis);
    expect(lnw).not.toBeNull();
    expect(lnw!).toBeGreaterThanOrEqual(entry.expected.lnw.min);
    expect(lnw!).toBeLessThanOrEqual(entry.expected.lnw.max);
    expect(rw).not.toBeNull();
    ...
    if (entry.expected.deltaLw) {
      expect(deltaLw).not.toBeNull();
      expect(deltaLw!).toBeGreaterThanOrEqual(entry.expected.deltaLw.min);
      expect(deltaLw!).toBeLessThanOrEqual(entry.expected.deltaLw.max);
    } else {
  ...
  const baselineById = new Map(
    COMMON_BASELINES.map((entry) => {
      const result = measureLayers(entry.layers);
      return [entry.id, numberOrNull(result.impact?.LnW)];
    })
  );
  ...
  )("keeps direct-adhered/common bare-like stack $id close to baseline", (entry) => {
  ...
  )("keeps resilient/exact stack $id better than its baseline", (entry) => {
    const baselineLnw = baselineById.get(entry.compareToBaselineId);
    const result = measureLayers(entry.layers);
    const lnw = numberOrNull(result.impact?.LnW);
