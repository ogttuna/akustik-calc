...
describe("post-V1 wall advanced-wall source-absent field/building lab-companion target-output independence owner", () => {
  ...
  it("publishes Gate AY lab companions for lab-only field-between-rooms requests", () => {
    const field = calculateAdvancedWall(buildFieldContext(), LAB_OUTPUTS);

    expect(field.airborneBasis?.method).toBe(GATE_I_AIRBORNE_FIELD_CONTEXT_RUNTIME_METHOD);
    expect(field.supportedTargetOutputs).toEqual([...LAB_OUTPUTS]);
    expect(field.unsupportedTargetOutputs).toEqual([]);
    expectLabCompanionValues(field);
  }, 30000);

  it("publishes Gate AY lab companions for lab-only building-prediction requests", () => {
    const building = calculateAdvancedWall(buildBuildingContext(), LAB_OUTPUTS);

    expect(building.airborneBasis?.method).toBe(GATE_AR_AIRBORNE_BUILDING_PREDICTION_RUNTIME_METHOD);
    expect(building.supportedTargetOutputs).toEqual([...LAB_OUTPUTS]);
    expect(building.unsupportedTargetOutputs).toEqual([]);
    expectLabCompanionValues(building);
  }, 30000);

  it("keeps mixed field/building outputs unchanged and basis-separated", () => {
  ...
  it("keeps missing context and Gate AY field/building publishing outside this owner", () => {
