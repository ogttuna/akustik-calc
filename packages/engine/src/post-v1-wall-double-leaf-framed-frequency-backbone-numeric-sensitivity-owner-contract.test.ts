...
type OwnerResult = ReturnType<typeof calculateContextOwnedWall>;

function requireCurve(result: OwnerResult): TransmissionLossCurve {
...
function expectMetricsFromCurve(result: OwnerResult) {
...
describe("post-V1 wall double-leaf/framed frequency-backbone numeric sensitivity owner", () => {
  it("lands the frequency-backbone owner and selects the coverage refresh next", () => {
  ...
  it("owns one calculated frequency backbone for light, baseline, heavy, and asymmetric leaf cases", () => {
    const ownerCases = [
      {
        expectedCurveDb: {
          100: 22.8,
  ...
  it("makes the frequency-curve and rating-adapter inputs explicit without aliasing STC to Rw", () => {
  ...
  it("keeps missing route inputs and impact aliases outside the frequency-backbone owner", () => {
  ...
  it("keeps docs and current-gate aligned with the frequency-backbone owner closeout", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(content, path).toContain("accuracyPromotedTargetOutputs: 16");
      expect(content, path).toContain("frequencyCurveBandsOwned: 76");
    ...
    expect(currentGateRunner).toContain(
      "src/post-v1-wall-double-leaf-framed-frequency-backbone-numeric-sensitivity-owner-contract.test.ts"
    );
  });
});
