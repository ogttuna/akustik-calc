...
describe("post-V1 wall user-material formula field lab-companion basis integrity owner", () => {
  ...
  it("keeps custom field outputs on Gate I while lab companions use direct formula values", () => {
    const result = calculateCustomWall(FIELD_CONTEXT, MIXED_OUTPUTS);
    ...
    expect(result.layerCombinationResolverTrace).toMatchObject({
      ...
      valuePins: [
        ...
        { metric: "Dn,w", value: 41 },
        { metric: "Dn,A", value: 39.5 }
      ]
    });
  ...
  it("corrects partial mixed field requests without target-output order dependence", () => {
    const result = calculateCustomWall(FIELD_CONTEXT, ["Ctr", "Rw", "R'w", "DnT,w"]);

    expect(result.supportedTargetOutputs).toEqual(["Ctr", "Rw", "R'w", "DnT,w"]);
    expect(result.unsupportedTargetOutputs).toEqual([]);
    expect(result.metrics).toMatchObject({
      estimatedCtrDb: -6.1,
      estimatedDnTwDb: 43,
  ...
  it("keeps field-only, lab-context-only, built-in, ASTM, and impact boundaries unchanged", () => {
