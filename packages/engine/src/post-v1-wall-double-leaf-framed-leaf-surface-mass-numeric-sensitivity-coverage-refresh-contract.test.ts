...
const UNRESOLVED_SIDE_B_MASS_STACK = [
  { materialId: "gypsum_board", thicknessMm: 12.5 },
  { materialId: "unknown_user_leaf_without_surface_mass", thicknessMm: 12.5 }
] as const satisfies readonly LayerInput[];
...
describe("post-V1 wall double-leaf/framed leaf surface-mass numeric sensitivity coverage refresh", () => {
  it("lands the refresh and selects the runtime-first rerank next", () => {
  ...
  it("re-probes symmetric side-leaf surface mass lab pins without formula retuning", () => {
  ...
  it("re-probes mass-sensitive field and building adapters from the same owned direct curve", () => {
  ...
  it("re-probes asymmetric side-leaf mass ratio and resonance pins", () => {
  ...
  it("keeps missing side-leaf ownership and unresolved side-leaf mass as precise input boundaries", () => {
    ...
    const unresolvedMassSolver = buildGateRDoubleLeafFramedBridgeSolverContract({
      airborneContext: contextOwnedPorousCavity(),
      layers: UNRESOLVED_SIDE_B_MASS_STACK,
      targetOutputs: LAB_OUTPUTS
    });
    ...
    expect(unresolvedMassSolver.readinessStatus).toBe("needs_input");
    expect(unresolvedMassSolver.missingPhysicalInputs).toContain("surfaceMassKgM2");
    expect(unresolvedMassSolver.physicalInputs.sideALeafMassKgM2).toBe(10.6);
    expect(unresolvedMassSolver.physicalInputs.sideBLeafMassKgM2).toBeNull();
    expect(unresolvedMassSolver.benchmarkRange).toBeNull();
  });

  it("keeps unowned impact aliases outside this airborne wall route", () => {
  ...
  it("keeps docs and current-gate aligned with the refresh closeout and rerank handoff", () => {
    for (const path of REQUIRED_DOCS) {
      ...
      expect(content, path).toContain("requiredInputBoundariesMoved: 1");
    ...
    expect(currentGateRunner).toContain(
      "src/post-v1-wall-double-leaf-framed-leaf-surface-mass-numeric-sensitivity-coverage-refresh-contract.test.ts"
    );
  });
});
