...
describe("post-V1 floor user-material visible floating ASTM exact-band field impact companion owner", () => {
  ...
  it("adds field impact companions to exact lab ASTM IIC without moving the ASTM rating basis", () => {
  ...
  it("adds field impact companions to exact field ASTM AIIC without aliasing ISO and ASTM metrics", () => {
    const result = calculateCustomHeavyFloor({
  ...
  it("keeps generic ASTM aliases and missing physical inputs outside the owner", () => {
    const genericAstm = calculateCustomHeavyFloor({
      impactFieldContext: COMPLETE_IMPACT_FIELD_CONTEXT,
      targetOutputs: [...FIELD_IMPACT_OUTPUTS, "IIC", "AIIC"]
    });
    ...
    expect(genericAstm.supportedTargetOutputs).toEqual([...FIELD_IMPACT_OUTPUTS]);
